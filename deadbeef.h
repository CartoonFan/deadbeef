...
// that means that the plugins which use the API 1.0 will work without
// recompiling until API 2.0.
//
...
#define PLUG_TEST_COMPAT(plug, x, y)                                           \
  ((plug)->version_major == (x) && (plug)->version_minor >= (y))

#define DDB_REQUIRE_API_VERSION(x, y)                                          \
  .plugin.api_vmajor = x, .plugin.api_vminor = y,

////////////////////////////
// default values for some common config variables should go here

// network.ctmapping : content-type to plugin mapping
#define DDB_DEFAULT_CTMAPPING                                                  \
  "audio/mpeg {stdmpg ffmpeg} audio/x-mpeg {stdmpg ffmpeg} application/ogg "   \
  "{stdogg opus ffmpeg} audio/ogg {stdogg opus ffmpeg} audio/aac {aac "        \
  "ffmpeg} audio/aacp {aac ffmpeg} audio/x-m4a {aac ffmpeg} audio/wma {wma "   \
  "ffmpeg}"
...
enum {
  DDB_IS_SUBTRACK =
      (1
       << 0), // file is not single-track, might have metainfo in external file
  DDB_IS_READONLY =
      (1 << 1), // check this flag to block tag writing (e.g. in iso.wv)
...
enum {
  ...
  DB_EV_PLAY_CURRENT =
      3, // play current track (will start/unpause if stopped or paused)
  ...
  DB_EV_PLAYLIST_REFRESH DEPRECATED_18 =
      9, // [use DB_EV_PLAYLISTCHANGED instead]
  DB_EV_REINIT_SOUND =
      10, // reinitialize sound output with current output_plugin config value
  ...
  DB_EV_PLAYLISTCHANGED =
      15, // playlist contents were changed (e.g. metadata in any track)
  ...
  // p1 is one of ddb_playlist_change_t enum values, detailing what exactly has
  // been changed.
  ...
  DB_EV_ACTIONSCHANGED =
      20, // plugin actions were changed, e.g. for reinitializing gui
  DB_EV_DSPCHAINCHANGED =
      21, // emitted when any parameter of the main dsp chain has been changed

// since 1.5
#if (DDB_API_LEVEL >= 5)
  // DB_EV_SELCHANGED is obsolete and isn't emitted; DB_EV_PLAYLISTCHANGED with
  // DDB_PLAYLIST_CHANGE_SELECTION should be used instead.
  DB_EV_SELCHANGED = 22,
  DB_EV_PLUGINSLOADED = 23, // after all plugins have been loaded and connected
#endif

#if (DDB_API_LEVEL >= 8)
  // A caller sends this event, to ask playlist viewer(s) to focus on selected
  // track.
...
// since 1.5
#if (DDB_API_LEVEL >= 5)
  // NOTE: this is not a structured event, but too late to fix, needs to stay
  // here for backwards compat
  DB_EV_TRACKFOCUSCURRENT =
      1006, // user wants to highlight/find the current playing track
#endif

#if (DDB_API_LEVEL >= 10)
  DB_EV_CURSOR_MOVED =
      1007, // used for syncing cursor position between playlist views, p1 =
            // PL_MAIN or PL_SEARCH, ctx is a ddb_event_track_t, containing the
            // new track under cursor
#endif
...
#if (DDB_API_LEVEL >= 10)
  // Return value, is set to non-zero if text was <<<dimmed>>> or
  // >>>brightened<<< It's used to determine whether the text needs to be
  // searched for the corresponding esc sequences
  int dimmed;
#endif
} ddb_tf_context_t;
#endif
...
typedef struct {
  ...
  const char *(*get_config_dir)(void)
      DEPRECATED_18; // installdir/config | $XDG_CONFIG_HOME/.config/deadbeef
  ...
  const char *(*get_plugin_dir)(void)
      DEPRECATED_18; // installdir/plugins | LIBDIR/deadbeef
  const char *(*get_pixmap_dir)(void)
      DEPRECATED_18; // installdir/pixmaps | PREFIX "/share/deadbeef/pixmaps"
  ...
  DB_playItem_t *(*plt_load)(ddb_playlist_t *plt, DB_playItem_t *after,
                             const char *fname, int *pabort,
                             int (*cb)(DB_playItem_t *it, void *data),
                             void *user_data)DEPRECATED_15;
  int (*plt_save)(ddb_playlist_t *plt, DB_playItem_t *first,
                  DB_playItem_t *last, const char *fname, int *pabort,
                  int (*cb)(DB_playItem_t *it, void *data), void *user_data);
  ...
  // This would mark playlist as "dirty" -- meaning it needs to be saved when
  // `pl_save_all` is called. The flag is reset as soon as playlist is saved.
  // This is called automatically when playlists are created / cleared /
  // removed, and when items are added/removed to them. However, other changes
  // -- like changing track metadata -- would not call this function. You need
  // to call it yourself, to make sure the playlist gets saved on exit. It
  // doesn't need to be called if you save the playlist via direct call to
  // `plt_save_*`, or `pl_save_current`
  ...
  void (*plt_add_meta)(ddb_playlist_t *handle, const char *key,
                       const char *value);

  // replace (or add) existing meta
  void (*plt_replace_meta)(ddb_playlist_t *handle, const char *key,
                           const char *value);

  // append meta to existing one, or add if doesn't exist
  void (*plt_append_meta)(ddb_playlist_t *handle, const char *key,
                          const char *value);
  ...
  void (*plt_set_meta_float)(ddb_playlist_t *handle, const char *key,
                             float value);
  ...
  // returns integer value of requested meta, def is the default value if not
  // found
  int (*plt_find_meta_int)(ddb_playlist_t *handle, const char *key, int def);

  // returns float value of requested meta, def is the default value if not
  // found
  float (*plt_find_meta_float)(ddb_playlist_t *handle, const char *key,
                               float def);
  ...
  DB_playItem_t *(*plt_insert_item)(ddb_playlist_t *playlist,
                                    DB_playItem_t *after, DB_playItem_t *it);
  DB_playItem_t *(*plt_insert_file)(ddb_playlist_t *playlist,
                                    DB_playItem_t *after, const char *fname,
                                    int *pabort,
                                    int (*cb)(DB_playItem_t *it, void *data),
                                    void *user_data)DEPRECATED_15;
  DB_playItem_t *(*plt_insert_dir)(ddb_playlist_t *plt, DB_playItem_t *after,
                                   const char *dirname, int *pabort,
                                   int (*cb)(DB_playItem_t *it, void *data),
                                   void *user_data)DEPRECATED_15;
  void (*plt_set_item_duration)(ddb_playlist_t *plt, DB_playItem_t *it,
                                float duration);
  ...
  DB_playItem_t *(*plt_get_item_for_idx)(ddb_playlist_t *playlist, int idx,
                                         int iter);
  void (*plt_move_items)(ddb_playlist_t *to, int iter, ddb_playlist_t *from,
                         DB_playItem_t *drop_before, uint32_t *indexes,
                         int count);
  void (*plt_copy_items)(ddb_playlist_t *to, int iter, ddb_playlist_t *from,
                         DB_playItem_t *before, uint32_t *indices, int cnt);
  ...
  void (*plt_sort)(ddb_playlist_t *plt, int iter, int id, const char *format,
                   int order) DEPRECATED_18;

  // add files and folders to current playlist
  int (*plt_add_file)(ddb_playlist_t *plt, const char *fname,
                      int (*cb)(DB_playItem_t *it, void *data),
                      void *user_data) DEPRECATED_15;
  int (*plt_add_dir)(ddb_playlist_t *plt, const char *dirname,
                     int (*cb)(DB_playItem_t *it, void *data),
                     void *user_data) DEPRECATED_15;

  // cuesheet support
  DB_playItem_t *(*plt_insert_cue_from_buffer)(ddb_playlist_t *plt,
                                               DB_playItem_t *after,
                                               DB_playItem_t *origin,
                                               const uint8_t *buffer,
                                               int buffersize, int numsamples,
                                               int samplerate)DEPRECATED_110;
  DB_playItem_t *(*plt_insert_cue)(ddb_playlist_t *plt, DB_playItem_t *after,
                                   DB_playItem_t *origin, int numsamples,
                                   int samplerate)DEPRECATED_110;
  ...
  DB_playItem_t *(*pl_item_alloc_init)(const char *fname,
  ...
  /*
     ...
     more to come
  */
  int (*pl_format_title)(DB_playItem_t *it, int idx, char *s, int size, int id,
                         const char *fmt) DEPRECATED_18;

  // _escaped version wraps all conversions with '' and replaces every ' in
  // conversions with \'
  int (*pl_format_title_escaped)(DB_playItem_t *it, int idx, char *s, int size,
                                 int id, const char *fmt) DEPRECATED_18;

  // format duration 't' (fractional seconds) into string, for display in
  // playlist
  ...
  DB_metaInfo_t *(*pl_get_metadata_head)(
      DB_playItem_t *it); // returns head of metadata linked list
  ...
  void (*pl_replace_meta)(DB_playItem_t *it, const char *key,
                          const char *value);
  ...
  void (*pl_items_copy_junk)(DB_playItem_t *from, DB_playItem_t *first,
                             DB_playItem_t *last);
  ...
  int (*junk_id3v2_read_full)(DB_playItem_t *it, DB_id3v2_tag_t *tag,
                              DB_FILE *fp);
  int (*junk_id3v2_convert_24_to_23)(DB_id3v2_tag_t *tag24,
                                     DB_id3v2_tag_t *tag23);
  int (*junk_id3v2_convert_23_to_24)(DB_id3v2_tag_t *tag23,
                                     DB_id3v2_tag_t *tag24);
  int (*junk_id3v2_convert_22_to_24)(DB_id3v2_tag_t *tag22,
                                     DB_id3v2_tag_t *tag24);
  ...
  int (*junk_apev2_read_full)(DB_playItem_t *it, DB_apev2_tag_t *tag_store,
                              DB_FILE *fp);
  int (*junk_apev2_read_full_mem)(DB_playItem_t *it, DB_apev2_tag_t *tag_store,
  ...
  // The function was accidentally removed from the codebase, so this pointer is
  // always NULL.
  ...
  int (*junk_rewrite_tags)(DB_playItem_t *it, uint32_t flags, int id3v2_version,
  ...
  void (*conf_get_str)(const char *key, const char *def, char *buffer,
                       int buffer_size);
  ...
  int (*pcm_convert)(const ddb_waveformat_t *inputfmt, const char *input,
                     const ddb_waveformat_t *outputfmt, char *output,
                     int inputsize);
  ...
  // this function must return original un-overridden value (ignoring the keys
  // prefixed with '!') it's not thread-safe, and must be used under the same
  // conditions as the pl_find_meta
  ...
  int (*pl_get_meta_raw)(DB_playItem_t *it, const char *key, char *val,
                         int size);
  int (*plt_get_meta)(ddb_playlist_t *handle, const char *key, char *val,
                      int size);
  ...
  void (*vis_waveform_listen)(void *ctx,
  ...
  void (*vis_spectrum_listen)(void *ctx,
                              void (*callback)(void *ctx,
                                               ddb_audio_data_t *data));
  ...
  // returns ID
  int (*listen_file_added)(int (*callback)(ddb_fileadd_data_t *data,
                                           void *user_data),
                           void *user_data);
  void (*unlisten_file_added)(int id);

  int (*listen_file_add_beginend)(
  ...
  // has the visibility
  DB_playItem_t *(*plt_load2)(
      int visibility, ddb_playlist_t *plt, ddb_playItem_t *after,
      const char *fname, int *pabort,
      int (*callback)(DB_playItem_t *it, void *user_data), void *user_data);
  ...
  int (*plt_add_dir2)(int visibility, ddb_playlist_t *plt, const char *dirname,
                      int (*callback)(DB_playItem_t *it, void *user_data),
                      void *user_data);
  ddb_playItem_t *(*plt_insert_file2)(
      int visibility, ddb_playlist_t *playlist, ddb_playItem_t *after,
      const char *fname, int *pabort,
      int (*callback)(DB_playItem_t *it, void *user_data), void *user_data);
  ddb_playItem_t *(*plt_insert_dir2)(
      int visibility, ddb_playlist_t *plt, ddb_playItem_t *after,
      const char *dirname, int *pabort,
      int (*callback)(DB_playItem_t *it, void *user_data), void *user_data);
  ...
  // returns the pointer to compiled bytecode, which must be tf_free'd by the
  // caller.
  ...
  // out: buffer allocated by the caller, must be big enough to fit the output
  // string outlen: the size of out buffer returns -1 on failure, output size on
  // success
  int (*tf_eval)(ddb_tf_context_t *ctx, const char *code, char *out,
                 int outlen);

  // sort using title formatting v2
  void (*plt_sort_v2)(ddb_playlist_t *plt, int iter, int id, const char *format,
                      int order);
  ...
  void (*plt_search_process2)(ddb_playlist_t *plt, const char *text,
                              int select_results);

  // try loading external and embedded cuesheet, using the configured order
  // (cue.prefer_embedded, default=0)
  DB_playItem_t *(*plt_process_cue)(ddb_playlist_t *plt, DB_playItem_t *after,
                                    DB_playItem_t *it, uint64_t numsamples,
                                    int samplerate);
  ...
  // #define trace(...) { deadbeef->log_detailed (&plugin.plugin, 0,
  // __VA_ARGS__); } Then use trace () as you would use printf The user would be
  // able to enable/disable logging in your plugin via the standard UI features.
  ...
  // Layers is a combination of bits, which define the priority/visibility of
  // the message. See DDB_LOG_LAYER_* for details
  void (*log_detailed)(struct DB_plugin_s *plugin, uint32_t layers,
                       const char *fmt, ...);

  // Same as log_detailed but uses va_list
  void (*vlog_detailed)(struct DB_plugin_s *plugin, uint32_t layer,
                        const char *fmt, va_list ap);

  // High level easy-to-use log function, with no scope
  // These log messages cannot be disabled, and will always appear in the Log
  // Viewers
  ...
  void (*log_viewer_register)(void (*callback)(struct DB_plugin_s *plugin,
                                               uint32_t layers,
                                               const char *text, void *ctx),
                              void *ctx);
  void (*log_viewer_unregister)(void (*callback)(struct DB_plugin_s *plugin,
                                                 uint32_t layers,
                                                 const char *text, void *ctx),
                                void *ctx);
  ...
  // It works by calling the filter right after a file or folder was found, but
  // before it's open / processed Then if the filter returns a negative value --
  // the file/folder will be skipped It's designed to work with plt_insert_dir
  // and plt_insert_file. In case of plt_insert_file, the filter will only be
  // used for recursive plt_insert_dir calls (e.g. for VFS containers)

  // Registers the file add filter, and returns the filter ID, which can be used
  // to unregister the filter Calls the callback before each file is processed
  // The callback must return 0 to continue, or a negative value to skip the
  // file
  ...
  void (*replaygain_apply_with_settings)(ddb_replaygain_settings_t *settings,
                                         ddb_waveformat_t *fmt, char *bytes,
                                         int numbytes);

  // Utility function to init the replaygain settings from the current
  // player configuration, and the supplied track.
  // After initializing the settings, pass the pointer to
  // replaygain_apply_with_settings
  void (*replaygain_init_settings)(ddb_replaygain_settings_t *settings,
                                   DB_playItem_t *it);

  ////// Sort APIs available from 1.10+ //////

  // Sort a plain array of tracks, according to specified title formatting v2
  // script and column id. The sorting is applied in-place, i.e. the input array
  // is modified. `playlist` can be NULL, otherwise must point to the playlist
  // which ALL of the tracks belong to; `tracks` and `numtracks` specify the
  // array of tracks; `format` is title formatting v2 script; `order` can be one
  // of DDB_SORT_ASCENDING or DDB_SORT_DESCENDING (no random).
  void (*sort_track_array)(ddb_playlist_t *playlist, DB_playItem_t **tracks,
                           int num_tracks, const char *format, int order);

  // initialize playitem, same as plt_add_file, except do not add to any
  // playlist
  ...
  // this should be called by plugins to prevent running cuesheet code at a
  // wrong time.
...
// since 1.12
#if (DDB_API_LEVEL >= 12)
  DB_metaInfo_t *(*pl_meta_for_key_with_override)(ddb_playItem_t *it,
                                                  const char *key);
  const char *(*pl_find_meta_with_override)(DB_playItem_t *it, const char *key);
  int (*pl_get_meta_with_override)(ddb_playItem_t *it, const char *key,
...
// since 1.13
#if (DDB_API_LEVEL >= 13)
  void (*plt_item_set_selected)(ddb_playlist_t *plt, ddb_playItem_t *it,
                                int sel);
...
// if (flags & DB_ACTION_COMMON)  -> main menu, or nowhere, or where GUI plugin
// wants
//    basically, to put it into main menu, prefix the item title with the menu
//    name e.g. title = "File/MyItem" --> this will add the item under File menu
//
...
enum {
  // An menu item for this action should be added to the main menu (ex.
  // Playback/Skip to/Previous genre)
  ...
  // Action will get the track list by itself, instead of getting the list as
  // argument.
...
#if (DDB_API_LEVEL >= 5)
  // A menu item should be added to the menu(s), if the item name contains slash
  // symbol(s)
  DB_ACTION_ADD_MENU = 1 << 6,
#endif

#if (DDB_API_LEVEL >= 10)
  // Don't allow running this action in playlist context, even if it supports
  // multiple selection
...
typedef int (*DB_plugin_action_callback_t)(struct DB_plugin_action_s *action,
                                           void *userdata);

#if (DDB_API_LEVEL >= 11)
typedef int (*DB_plugin_action_callback2_t)(struct DB_plugin_action_s *action,
...
enum {
  ...
  // Tells the system that the plugin supports replaygain, and streamer should
  // not do it.
...
typedef struct DB_plugin_s {
  ...
  // opposite of connect, will be called before stop, while all plugins are
  // still in "started" state
  int (*disconnect)(void);

  // exec_cmdline may be called at any moment when user sends commandline to
  // player can be NULL if plugin doesn't support commandline processing cmdline
  // is 0-separated list of strings, guaranteed to have 0 at the end
...
#if (DDB_API_LEVEL >= 8)
  // Decoders should only call the streamer_set_bitrate from plugin.read
  // function,
...
#endif
#if (DDB_API_LEVEL >= 10)
  // Don't modify the stream (e.g. no replaygain, clipping, etc), provide the
  // maximum possible precision, preferably in float32.
...
typedef struct DB_decoder_s {
  ...
  // decoder is responsible to calculate duration, split it into subsongs, load
  // cuesheet, etc after==NULL means "prepend before 1st item in playlist"
  DB_playItem_t *(*insert)(ddb_playlist_t *plt, DB_playItem_t *after,
                           const char *fname);
...
typedef struct DB_output_s {
  ...
  int (*init)(void);
  // free is called if output plugin was changed to another, or unload is about
  // to happen
  int (*free)(void);
  ...
  void (*enum_soundcards)(void (*callback)(const char *name, const char *desc,
                                           void *),
                          void *userdata);
...
typedef struct DB_dsp_s {
  ...
  // plugins are allowed to modify channels, samplerate, channelmask in the fmt
  // structure buffer size can fit up to maxframes frames by default ratio=1,
  // and plugins don't need to touch it unless they have to
...
#if (DDB_API_LEVEL >= 1)
  // can be NULL
  // should return 1 if the DSP plugin will not touch data with the current
...
// misc plugins should be mostly event driven, so no special entry points in
// them
typedef struct {
  DB_plugin_t plugin;
} DB_misc_t;
...
typedef struct DB_vfs_s {
  ...
  // Optional method to abort any file / stream operation on a file with
...
typedef struct DB_gui_s {
  ...
  int (*run_dialog)(ddb_dialog_t *dlg, uint32_t buttons,
                    int (*callback)(int button, void *ctx), void *ctx);
} DB_gui_t;
...
typedef struct DB_playlist_s {
  ...
  DB_playItem_t *(*load)(ddb_playlist_t *plt, DB_playItem_t *after,
                         const char *fname, int *pabort,
                         int (*cb)(DB_playItem_t *it, void *data),
                         void *user_data);
  ...
  int (*save)(ddb_playlist_t *plt, const char *fname, DB_playItem_t *first,
              DB_playItem_t *last);

  const char **extensions; // NULL-terminated list of supported file extensions,
                           // e.g. {"m3u", "pls", NULL}

  // since 1.5
#if (DDB_API_LEVEL >= 5)
  DB_playItem_t *(*load2)(int visibility, ddb_playlist_t *plt,
                          DB_playItem_t *after, const char *fname, int *pabort);
#endif
} DB_playlist_t;

// NOTE: Media source API is a work in progress, and is disabled int this
// version of source code. This is to prevent plugin devs from releasing media
// source plugins, before this API is finalized. Use the appropriate development
// branch to test media source plugins.
...
typedef void (*ddb_medialib_listener_t)(ddb_mediasource_event_type_t event,
                                        void *user_data);
...
typedef struct {
  ...
  /// @param source_path: a unique name to identify the instance, this will be
  /// used to prefix individual instance configuration files, caches, etc.
  ddb_mediasource_source_t (*create_source)(const char *source_path);
  void (*free_source)(ddb_mediasource_source_t source);

  int (*add_listener)(ddb_mediasource_source_t source,
                      ddb_medialib_listener_t listener, void *user_data);
  void (*remove_listener)(ddb_mediasource_source_t source, int listener_id);

  ddb_medialib_item_t *(*create_list)(ddb_mediasource_source_t source,
