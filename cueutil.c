...
#define MAX_EXTRA_TAGS_FROM_CUE                                                \
  ((sizeof(cue_field_map) / sizeof(cue_field_map[0])))
...
typedef struct {
  ...
  playlist_t
      *temp_plt; // temporary playlist, for loading tracks before splitting
...
static const uint8_t *skipspaces(const uint8_t *p, const uint8_t *end) {
  while (p < end && *p <= ' ') {
    p++;
  }
  return p;
...
static const uint8_t *pl_cue_skipspaces(const uint8_t *p) {
  while (*p && *p <= ' ') {
    p++;
  }
  return p;
...
static void pl_get_qvalue_from_cue(const uint8_t *p, int sz, char *out,
                                   const char *charset) {
  char *str = out;
  if (*p == 0) {
    *out = 0;
  ...
  char recbuf[l * 10];
  int res =
      junk_recode(str, (int)l, recbuf, (int)(sizeof(recbuf) - 1), charset);
  if (res >= 0) {
    strcpy(str, recbuf);
  } else {
...
static void pl_get_value_from_cue(const char *p, int sz, char *out) {
  while (*p >= ' ' && sz > 1) {
    sz--;
    *out++ = *p++;
  }
...
static float pl_cue_parse_time(const char *p) {
  char *endptr;
...
static void pl_cue_get_total_tracks_and_files(const uint8_t *buffer,
                                              const uint8_t *buffer_end,
                                              int *ncuefiles, int *ncuetracks) {
...
static void pl_cue_set_track_field_values(playItem_t *it, cueparser_t *cue) {
  if (cue->cuefields[CUE_FIELD_PERFORMER][0]) {
    ...
    if (cue->cuefields[CUE_FIELD_ALBUM_PERFORMER][0] &&
        strcmp(cue->cuefields[CUE_FIELD_ALBUM_PERFORMER],
               cue->cuefields[CUE_FIELD_PERFORMER])) {
      pl_add_meta(it, "album artist",
...
static void pl_cue_reset_per_track_fields(char cuefields[CUE_MAX_FIELDS][255]) {
...
// error:
//    -1
static int pl_cue_get_field_value(cueparser_t *cue) {
  if (!strncasecmp(cue->p, "FILE ", 5)) {
    pl_get_qvalue_from_cue(cue->p + 5, CUE_FIELD_LEN,
                           cue->cuefields[CUE_FIELD_FILE], cue->charset);
    const char *ext = strrchr(cue->cuefields[CUE_FIELD_FILE], '.');
    if (ext && !strcasecmp(ext, ".cue")) {
      trace_err(
          "Cuesheet %s refers to another cuesheet, which is not allowed\n",
          cue->fname);
      return -1;
    }
    ...
    for (int m = 0; cue_field_map[m]; m += 2) {
      if (!strncasecmp(cue->p, cue_field_map[m], strlen(cue_field_map[m]))) {
        ...
        pl_get_qvalue_from_cue(cue->p + strlen(cue_field_map[m]), CUE_FIELD_LEN,
                               value, cue->charset);
...
static int _file_exists(const char *fname) {
...
static int _set_last_item_region(playlist_t *plt, playItem_t *it,
                                 playItem_t *origin, int64_t totalsamples,
                                 int samplerate) {
  pl_item_set_endsample(it, pl_item_get_startsample(origin) + totalsamples - 1);
  if ((pl_item_get_endsample(it) - pl_item_get_startsample(origin)) >=
          totalsamples ||
      (pl_item_get_startsample(it) - pl_item_get_startsample(origin)) >=
          totalsamples) {
    return -1;
  }
  plt_set_item_duration(
      plt, it,
      (float)(pl_item_get_endsample(it) - pl_item_get_startsample(it) + 1) /
          samplerate);
  return 0;
}

playItem_t *plt_load_cue_file(playlist_t *plt, playItem_t *after,
                              const char *fname, const char *dirname,
  ...
  after = plt_load_cuesheet_from_buffer(plt, after, fname, NULL, 0, 0, buffer,
                                        sz, dirname, namelist, n);
error:
  if (fp) {
    vfs_fclose(fp);
  }
...
static int _file_present_in_namelist(const char *fullpath, cueparser_t *cue) {
  int i = 0;
  for (i = 0; i < cue->n; i++) {
    if (cue->namelist[i]->d_name[0]) {
      char path[PATH_MAX];
      snprintf(path, sizeof(path), "%s/%s", cue->dirname,
...
static int cue_addfile_filter(cueparser_t *cue) {
...
static int _load_nextfile(cueparser_t *cue) {
  cue->origin = NULL;
  ...
  // full path in CUE entry
    ...
    // try relative path
    snprintf(cue->fullpath, sizeof(cue->fullpath), "%s/%s", cue->cue_file_dir,
             audio_file);
      ...
      if (cue->namelist) {
        ...
        if (cue->ncuefiles == 1) {
          ...
          for (int i = 0; i < cue->n; i++) {
            ...
            if (!strncasecmp(cue->cue_fname, cue->namelist[i]->d_name, l - 4)) {
              ...
              snprintf(cue->fullpath, sizeof(cue->fullpath), "%s/%s",
                       cue->dirname, cue->namelist[i]->d_name);
              int res = cue_addfile_filter(cue);
              if (res >= 0) {
                cue->origin = plt_insert_file2(-1, cue->temp_plt, NULL,
                                               cue->fullpath, NULL, NULL, NULL);
              }
              if (cue->origin) {
                image_found = 1;
        ...
        if (!image_found) {
          ...
          for (int i = 0; i < cue->n; i++) {
            ...
            if (!strncasecmp(audio_file, cue->namelist[i]->d_name,
                             ext - audio_file) &&
              ...
              // adding to temp playlist will fail file add filters, so need to
              // call this manually here
              int res = cue_addfile_filter(cue);
              if (res >= 0) {
                cue->origin = plt_insert_file2(-1, cue->temp_plt, NULL,
                                               cue->fullpath, NULL, NULL, NULL);
  ...
  if (cue->fullpath[0] && !cue->origin) {
    ...
    // need to check if the fullpath file is present in the list, to avoid
    // double-loading
  ...
  if (cue->fullpath[0] && !cue->origin) {
    // adding to temp playlist will fail file add filters, so need to call this
    // manually here
    int res = cue_addfile_filter(cue);
    if (res >= 0) {
      cue->origin = plt_insert_file2(-1, cue->temp_plt, NULL, cue->fullpath,
                                     NULL, NULL, NULL);
    }
    if (cue->origin) {
      ...
      if (cue->namelist) {
        ...
        // this is for files inside of VFS containers, e.g.
        // zip://file.zip:path/to/the/file
        if (fn_col) {
          fn_vfs = fn_col + 1;
        } else if (fn_slash) {
          fn_vfs = fn_slash + 1;
        ...
        for (int i = 0; i < cue->n; i++) {
          if (!strcmp(fn_vfs, cue->namelist[i]->d_name) ||
              !strcmp(fn_nonvfs, cue->namelist[i]->d_name)) {
            cue->namelist[i]->d_name[0] = 0;
            break;
          }
        }
  ...
  if (!cue->origin) {
    if (!cue->namelist) {
      ...
      // this is to prevent bogus errors when auto-scanning for cuesheets in
      // folders.
      trace_err("Invalid FILE entry %s in cuesheet %s, and could not guess any "
                "suitable file name.\n",
...
static int plt_process_cue_track(playlist_t *plt, cueparser_t *cue) {
  // fix track number
  ...
  *p = 0;
  ...
  float f_pregap = cue->cuefields[CUE_FIELD_PREGAP][0]
                       ? pl_cue_parse_time(cue->cuefields[CUE_FIELD_PREGAP])
                       : 0;

  if (cue->prev) {
    // knowing the startsample of the current track,
    // now it's possible to calculate startsample and duration of the previous
    // one.
    ...
    plt_set_item_duration(
        plt, cue->prev,
        (float)(cue->currsample - pl_item_get_startsample(cue->prev)) /
            cue->samplerate);
  }

  playItem_t *it = pl_item_alloc_init(cue->fullpath, cue->dec);
  pl_set_meta_int(it, ":TRACKNUM", atoi(cue->cuefields[CUE_FIELD_TRACK]));
  pl_item_set_startsample(it, cue->currsample +
                                  (int64_t)(f_pregap * cue->samplerate));
  ...
  if (cue->last_round) {
    int res = _set_last_item_region(plt, it, cue->origin, cue->numsamples,
                                    cue->samplerate);
    if (res < 0) {
      return res;
    }
...
static int _is_audio_track(const char *track) {
  return !strcmp(track + strlen(track) - 6, " AUDIO");
}

playItem_t *plt_load_cuesheet_from_buffer(
    playlist_t *plt, playItem_t *after, const char *fname,
    playItem_t *embedded_origin, int64_t embedded_numsamples,
    int embedded_samplerate, const uint8_t *buffer, int sz, const char *dirname,
    struct dirent **namelist, int n) {
  ...
  pl_cue_get_total_tracks_and_files(cue.p, end, &cue.ncuefiles,
                                    &cue.ncuetracks);
  if (!cue.ncuefiles) {
  ...
  if (cue.ncuefiles > 1 && embedded_origin) {
    trace_err(
        "Can't load embedded cuesheet referencing multiple audio files (%s)\n",
        fname);
    goto error;
  }

  snprintf(cue.cuefields[CUE_FIELD_TOTALTRACKS], CUE_FIELD_LEN, "%d",
           cue.ncuetracks);
  ...
  while (!cue.last_round) {
    ...
    // Next field immediately after FILE, indicates whether current TRACK
    // belongs to previous or next FILE
    if (filefield) {
      filefield = 0;
      // If FILE is immediately followed by TRACK, that next TRACK is from the
      // new FILE
      ...
      if (cue.prev) {
        _set_last_item_region(plt, cue.prev, cue.origin, cue.numsamples,
                              cue.samplerate);
        cue.prev = NULL;
      }
    ...
    if (field == CUE_FIELD_INDEX01) {
      float sec = cue.cuefields[CUE_FIELD_INDEX01][0]
                      ? pl_cue_parse_time(cue.cuefields[CUE_FIELD_INDEX01])
                      : 0;
      // that's the startsample of the current track, and endsample-1 of the
      // next one. relative to the beginning of previous file
      int64_t val =
          pl_item_get_startsample(cue.origin) + (int64_t)(sec * cue.samplerate);
      ...
      pl_get_value_from_cue(cue.p + 6, CUE_FIELD_LEN,
                            cue.cuefields[CUE_FIELD_TRACK]);
      cue.have_track = 1;
    }
