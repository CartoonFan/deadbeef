...
static void print_help(void) {
    ...
    fprintf(stdout,
            _("   --help  or  -h     Print help (this message) and exit\n"));
    ...
    fprintf(stdout, _("   --play-pause       Start playback if stopped, toggle "
                      "pause otherwise\n"));
    ...
    fprintf(stdout,
            _("   --queue            Append file(s) to existing playlist\n"));
    fprintf(stdout, _("   --gui PLUGIN       Tells which GUI plugin to use, "
                      "default is \"GTK2\"\n"));
    fprintf(stdout,
            _("   --nowplaying FMT   Print formatted track name to stdout\n"));
    fprintf(
        stdout,
        _("                      FMT %%-syntax: [a]rtist, [t]itle, al[b]um,\n"
          "                      [l]ength, track[n]umber, [y]ear, [c]omment,\n"
          "                      copy[r]ight, [e]lapsed\n"));
    fprintf(stdout, _("                      example: --nowplaying \"%%a - %%t\" "
    ...
    fprintf(stdout, _("   --nowplaying-tf FMT  Print formatted track name to "
                      "stdout, using the new title formatting\n"));
    fprintf(stdout, _("                      FMT syntax: "
                      "http://github.com/DeaDBeeF-Player/deadbeef/wiki/"
                      "Title-formatting-2.0\n"));
    fprintf(stdout,
            _("                      example: --nowplaying-tf \"%%artist%% - "
              "%%title%%\" should print \"artist - title\"\n"));
    fprintf(stdout,
            _("   --volume [NUM]     Print or set deadbeef volume level.\n"));
    fprintf(stdout, _("                      The NUM parameter can be specified "
                      "in percents (absolute value or increment/decrement)\n"));
    fprintf(stdout,
            _("                      or in dB [-50, 0] (if with suffix).\n"));
    fprintf(stdout, _("                      Examples: --volume 80, --volume "
                      "+10, --volume -5 or --volume -20dB\n"));
#ifdef ENABLE_NLS
...
// are resolved.
char *prepare_command_line(int argc, char *argv[], int *size) {
    int seen_ddash = 0;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        char *arg;
        if ((!strncmp("--", argv[i], 2) && !seen_ddash) ||
...
int server_exec_command_line(const char *cmdline, int len, char *sendback,
                             int sbsize) {
    if (sendback) {
        sendback[0] = 0;
    }
            ...
            if (parg < pend) {
                ...
                if (!strcasecmp(end, "db")) {
                    ...
                    const float new_volume =
                        is_increment ? deadbeef->volume_get_db() * 2 + 100 + pct : pct;
                    deadbeef->volume_set_db((new_volume / 100.0 * 50) - 50);
                }
            }
            if (sendback) {
                snprintf(sendback, sbsize, "\1%.0f%% (%.2f dB)",
...
int add_paths(const char *paths, int len, int queue, char *sendback,
              int sbsize) {
    ...
    if (plt_add_files_begin(curr_plt, 0) != 0) {
        ...
        snprintf(
            sendback, sbsize,
            "it's not allowed to add files to playlist right now, because another "
            "file adding operation is in progress. please try again later.");
        return 1;
    }
    // add files
    if (!queue) {
    ...
    while (parg < pend) {
        ...
        if (deadbeef->plt_add_dir2(0, (ddb_playlist_t *)curr_plt, pname, NULL,
                                   NULL) < 0) {
            if (deadbeef->plt_add_file2(0, (ddb_playlist_t *)curr_plt, pname, NULL,
                                        NULL) < 0) {
                int ab = 0;
...
int db_socket_set_unix(struct sockaddr_un *remote, int *len) {
    ...
    *len = offsetof(struct sockaddr_un, sun_path) + sizeof(server_id) - 1;
    ...
    snprintf(remote->sun_path, sizeof(remote->sun_path), "%s/socket",
             socketdirenv ? socketdirenv : dbruntimedir);
...
int server_start(void) {
    ...
    len =
        offsetof(struct sockaddr_un, sun_path) + (int)strlen(srv_local.sun_path);
#endif
...
char *read_entire_message(int sockfd, int *size) {
...
void server_loop(void *ctx) {
    ...
    while (!server_terminate) {
        ...
        if ((ret = select(srv_socket + 1, &rds, NULL, NULL, &timeout)) < 0 &&
                errno != EINTR) {
            perror("select");
            exit(-1);
        }
...
void restore_resume_state(void) {
    DB_output_t *output = plug_get_output();
    if (conf_get_int("resume_last_session", 1) &&
            output->state() == DDB_PLAYBACK_STATE_STOPPED) {
...
void main_cleanup_and_quit(void) {
    ...
    // delete legacy session file
    {
        ...
        if (snprintf(sessfile, sizeof(sessfile), "%s/deadbeef/session", confdir) <
                sizeof(sessfile)) {
            unlink(sessfile);
        }
    }
...
static void mainloop_thread(void *ctx) {
...
int main(int argc, char *argv[]) {
    ...
    // detect portable version by looking for plugins/ and deadbeef.png and
    // portable_full by config/
...
#define VERSION "devel"
    ...
    if (portable_full) {
        if (snprintf(confdir, sizeof(confdir), "%s/config", dbinstalldir) >
                sizeof(confdir)) {
        ...
        if (xdg_conf_dir) {
            if (snprintf(confdir, sizeof(confdir), "%s", xdg_conf_dir) >
                    sizeof(confdir)) {
            ...
            if (snprintf(confdir, sizeof(confdir), "%s/" SYS_CONFIG_DIR, homedir) >
                    sizeof(confdir)) {
        ...
        if (snprintf(dbconfdir, sizeof(dbconfdir), "%s/deadbeef", confdir) >
                sizeof(dbconfdir)) {
    ...
    // Get cache directory
    if (portable_full) {
        if (snprintf(dbcachedir, sizeof(dbcachedir), "%s/cache", dbconfdir) >
                sizeof(dbcachedir)) {
        ...
        if (xdg_cache) {
            if (snprintf(dbcachedir, sizeof(dbcachedir), "%s/deadbeef/", xdg_cache) >
                    sizeof(dbcachedir)) {
                trace_err("fatal: cache path is too long: %s\n", dbcachedir);
                return -1;
            }
        } else {
            if (snprintf(dbcachedir, sizeof(dbcachedir), "%s/.cache/deadbeef",
                         homedir) > sizeof(dbcachedir)) {
                trace_err("fatal: cache path is too long: %s\n", dbcachedir);
                return -1;
            }
        }
    ...
    if (xdg_runtime) {
        if (snprintf(dbruntimedir, sizeof(dbruntimedir), "%s/deadbeef/",
                     xdg_runtime) >= sizeof(dbruntimedir)) {
            trace_err("fatal: cache path is too long: %s\n", dbruntimedir);
            return -1;
        }
    ...
    // Get plugins dir from environment variable, portable directory or library
    // dir
    ...
    if (env_plugin_dir) {
...
#else
        if (snprintf(dbplugindir, sizeof(dbplugindir), "%s/plugins", dbinstalldir) >
                sizeof(dbplugindir)) {
            trace_err("fatal: install path is too long: %s\n", dbinstalldir);
            return -1;
        }
        ...
        if (snprintf(dbplugindir, sizeof(dbplugindir), "%s/deadbeef", LIBDIR) >
                sizeof(dbplugindir)) {
            trace_err("fatal: install path is too long: %s\n", dbinstalldir);
            return -1;
        }
...
#else
        if (snprintf(dbresourcedir, sizeof(dbresourcedir), "%s/plugins",
                     dbinstalldir) > sizeof(dbresourcedir)) {
            trace_err("fatal: install path is too long: %s\n", dbinstalldir);
            return -1;
        }
#endif
    ...
    if (portable) {
#ifdef OSX_APPBUNDLE
        if (snprintf(dbdocdir, sizeof(dbdocdir), "%s/doc", dbresourcedir) >
                sizeof(dbdocdir)) {
            trace_err("fatal: install path is too long: %s\n", dbresourcedir);
            return -1;
        }
        if (snprintf(dbpixmapdir, sizeof(dbpixmapdir), "%s/pixmaps",
                     dbresourcedir) > sizeof(dbpixmapdir)) {
            trace_err("fatal: install path is too long: %s\n", dbresourcedir);
            return -1;
        }
#else
        if (snprintf(dbdocdir, sizeof(dbdocdir), "%s/doc", dbinstalldir) >
                sizeof(dbdocdir)) {
            trace_err("fatal: install path is too long: %s\n", dbinstalldir);
            return -1;
        }
        if (snprintf(dbpixmapdir, sizeof(dbpixmapdir), "%s/pixmaps", dbinstalldir) >
                sizeof(dbpixmapdir)) {
            trace_err("fatal: install path is too long: %s\n", dbinstalldir);
            return -1;
        }
        ...
        if (snprintf(dbpixmapdir, sizeof(dbpixmapdir), "%s/share/deadbeef/pixmaps",
                     PREFIX) > sizeof(dbpixmapdir)) {
            trace_err("fatal: install path is too long: %s\n", dbinstalldir);
            return -1;
        }
    }
...
#ifdef __MINGW32__
    char *directories[] = {dbconfdir,   dbinstalldir, dbdocdir,      dbplugindir,
                           dbpixmapdir, dbcachedir,   dbresourcedir, NULL
                          };
    ...
    volume_set_db(
        conf_get_float("playback.volume",
                       0)); // volume need to be initialized before plugins start
    ...
    // On some platforms (cocoa), main_cleanup_and_quit is called directly before
    // quit.
