...
static int plt_loading =
    0; // disable sending event about playlist switch, config regen, etc
...
static playlist_t dummy_playlist = {.refc = 1};
...
void pl_free(void) {
  ...
  while (playlists_head) {
    ...
    for (playItem_t *it = playlists_head->head[PL_MAIN]; it;
         it = it->next[PL_MAIN]) {
      if (it->_refc > 1) {
        fprintf(stderr,
                "\033[0;31mWARNING: playitem %p %s(%s) has refc=%d at delete "
                "time\033[37;0m\n",
                it, pl_find_meta_raw(it, ":URI"), pl_find_meta_raw(it, "track"),
                it->_refc);
      }
    }
...
static void pl_item_free(playItem_t *it);

static void plt_gen_conf(void) {
  if (plt_loading) {
    return;
  }
  LOCK;
...
playlist_t *plt_get_list(void) { return playlists_head; }

playlist_t *plt_get_curr(void) {
  LOCK;
  if (!playlist) {
...
playlist_t *plt_get_for_idx(int idx) {
  LOCK;
  playlist_t *p = playlists_head;
...
void plt_ref(playlist_t *plt) {
  LOCK;
  plt->refc++;
  UNLOCK;
}

void plt_unref(playlist_t *plt) {
  LOCK;
  assert(plt->refc > 0);
  plt->refc--;
  if (plt->refc < 0) {
...
int plt_get_count(void) { return playlists_count; }
...
playItem_t *plt_get_head(int plt) {
  playlist_t *p = playlists_head;
...
playlist_t *plt_alloc(const char *title) {
  ...
  if (!playlist) {
    ...
    if (!plt_loading) {
      ...
      for (int i = playlists_count - 1; i >= before + 1; i--) {
        char path1[PATH_MAX];
        char path2[PATH_MAX];
        if (snprintf(path1, sizeof(path1), "%s/playlists/%d.dbpl", dbconfdir,
                     i) > sizeof(path1)) {
          fprintf(stderr,
                  "error: failed to make path string for playlist file\n");
          continue;
        }
        if (snprintf(path2, sizeof(path2), "%s/playlists/%d.dbpl", dbconfdir,
                     i + 1) > sizeof(path2)) {
          fprintf(stderr,
                  "error: failed to make path string for playlist file\n");
          continue;
...
playlist_t *plt_append(const char *title) {
...
void plt_remove(int plt) {
  ...
  if (!plt_loading) {
    ...
    for (int i = plt + 1; i < playlists_count; i++) {
      ...
      if (snprintf(path2, sizeof(path2), "%s/playlists/%d.dbpl", dbconfdir, i) >
          sizeof(path2)) {
        fprintf(stderr,
                "error: failed to make path string for playlist file\n");
        continue;
...
playlist_t *plt_find_by_name(const char *name) {
  playlist_t *p = playlists_head;
  for (; p; p = p->next) {
    if (!strcmp(p->title, name)) {
      return p;
...
void plt_set_curr(playlist_t *plt) {
  LOCK;
  if (plt != playlist) {
...
int plt_get_idx_of(playlist_t *plt) {
  int i;
  LOCK;
  playlist_t *p = playlists_head;
  for (i = 0; p && i < playlists_count; i++) {
...
int plt_get_title(playlist_t *p, char *buffer, int bufsize) {
  LOCK;
  if (!buffer) {
...
int plt_set_title(playlist_t *p, const char *title) {
  LOCK;
  free(p->title);
...
void plt_modified(playlist_t *plt) {
  pl_lock();
  plt->modification_idx++;
  pl_unlock();
}

int plt_get_modification_idx(playlist_t *plt) {
  pl_lock();
...
void plt_move(int from, int to) {
  ...
  char path1[PATH_MAX];
  char temp[PATH_MAX];
  if (snprintf(path1, sizeof(path1), "%s/playlists/%d.dbpl", dbconfdir, from) >
      sizeof(path1)) {
    fprintf(stderr, "error: failed to make path string for playlist file\n");
    UNLOCK;
    return;
  }
  if (snprintf(temp, sizeof(temp), "%s/playlists/temp.dbpl", dbconfdir) >
      sizeof(temp)) {
    fprintf(stderr, "error: failed to make path string for playlist file\n");
    UNLOCK;
    return;
  ...
  for (int i = from; i < playlists_count - 1; i++) {
    char path2[PATH_MAX];
    if (snprintf(path1, sizeof(path1), "%s/playlists/%d.dbpl", dbconfdir, i) >
        sizeof(path1)) {
      fprintf(stderr, "error: failed to make path string for playlist file\n");
      continue;
    }
  ...
  // move temp file
  if (snprintf(path1, sizeof(path1), "%s/playlists/%d.dbpl", dbconfdir, to) >
      sizeof(path1)) {
    fprintf(stderr, "error: failed to make path string for playlist file\n");
  } else {
...
plt_insert_cue_from_buffer(playlist_t *playlist, playItem_t *after,
                           playItem_t *origin, const uint8_t *buffer,
                           int buffersize, int numsamples, int samplerate) {
...
plt_insert_cue(playlist_t *plt, playItem_t *after, playItem_t *origin,
...
static playItem_t *plt_insert_dir_int(int visibility, playlist_t *playlist,
                                      DB_vfs_t *vfs, playItem_t *after,
                                      const char *dirname, int *pabort,
                                      int (*cb)(playItem_t *it, void *data),
                                      void *user_data);

static playItem_t *plt_load_int(int visibility, playlist_t *plt,
                                playItem_t *after, const char *fname,
                                int *pabort,
                                int (*cb)(playItem_t *it, void *data),
                                void *user_data);

int fileadd_filter_test(ddb_file_found_data_t *data) {
...
static playItem_t *plt_insert_file_int(int visibility, playlist_t *playlist,
                                       playItem_t *after, const char *fname,
                                       int *pabort,
                                       int (*cb)(playItem_t *it, void *data),
                                       void *user_data) {
    ...
    for (int i = 0; vfsplugs[i]; i++) {
      if (vfsplugs[i]->is_container) {
        if (vfsplugs[i]->is_container(fname)) {
          playItem_t *it =
              plt_insert_dir_int(visibility, playlist, vfsplugs[i], after,
                                 fname, pabort, cb, user_data);
          if (it) {
            return it;
          }
        }
  ...
  if (strncasecmp(fname, "file://", 7)) {
    ...
    if (detect_on_access && *p == ':') {
      ...
      after = plt_insert_item(addfiles_playlist ? addfiles_playlist : playlist,
                              after, it);
      pl_item_unref(it);
      return after;
    }
  ...
  // path should start with "X:/", not "/X:/", fixing to avoid file opening
  // problems
  ...
  // now that it's known we're not dealing with URL, check if it's a relative
  // path
  ...
  if (!strcasecmp(eol, "cue")) {
    playItem_t *inserted =
        plt_load_cue_file(playlist, after, fname, NULL, NULL, 0);
    return inserted;
  }
  ...
  for (int i = 0; decoders[i]; i++) {
    if (decoders[i]->exts && decoders[i]->insert) {
      ...
      for (int e = 0; exts[e]; e++) {
        if (!strcasecmp(exts[e], eol) || !strcmp(exts[e], "*")) {
          ...
          playItem_t *inserted = (playItem_t *)decoders[i]->insert(
              (ddb_playlist_t *)playlist, DB_PLAYITEM(after), fname);
          if (inserted != NULL) {
            ...
            if (file_add_listeners) {
              ...
              for (ddb_fileadd_listener_t *l = file_add_listeners; l;
                   l = l->next) {
    ...
    if (decoders[i]->prefixes && decoders[i]->insert) {
      ...
      for (int e = 0; prefixes[e]; e++) {
        if (!strncasecmp(prefixes[e], fn, strlen(prefixes[e])) &&
            *(fn + strlen(prefixes[e])) == '.') {
          if (!filter_done) {
            ddb_file_found_data_t dt;
            dt.filename = fname;
            dt.plt = (ddb_playlist_t *)playlist;
...
playItem_t *plt_insert_file(playlist_t *playlist, playItem_t *after,
...
static void _get_fullname_and_dir(char *fullname, int sz, char *dir, int dirsz,
                                  DB_vfs_t *vfs, const char *dirname,
                                  const char *d_name) {
  if (!vfs) {
...
static playItem_t *plt_insert_dir_int(int visibility, playlist_t *playlist,
...
#else
    if (namelist[i]->d_name[0] == '.' ||
        (namelist[i]->d_type != DT_REG && namelist[i]->d_type != DT_UNKNOWN)) {
#endif
      continue;
    }
  ...
  for (int c = 0; c < ncuefiles; c++) {
    int i = cuefiles[c];
    _get_fullname_and_dir(fullname, sizeof(fullname), fulldir, sizeof(fulldir),
                          vfs, dirname, namelist[i]->d_name);

    playItem_t *inserted =
        plt_load_cue_file(playlist, after, fullname, fulldir, namelist, n);
  ...
  if (!pabort || !*pabort) {
    for (int i = 0; i < n; i++) {
      ...
      _get_fullname_and_dir(fullname, sizeof(fullname), NULL, 0, vfs, dirname,
                            namelist[i]->d_name);
      playItem_t *inserted = NULL;
      if (!vfs) {
        inserted = plt_insert_dir_int(visibility, playlist, vfs, after,
                                      fullname, pabort, cb, user_data);
      }
      if (!inserted) {
        inserted = plt_insert_file_int(visibility, playlist, after, fullname,
                                       pabort, cb, user_data);
      }
...
playItem_t *plt_insert_dir(playlist_t *playlist, playItem_t *after,
                           const char *dirname, int *pabort,
                           int (*cb)(playItem_t *it, void *data),
                           void *user_data) {
  ...
  playItem_t *ret = plt_insert_dir_int(0, playlist, NULL, after, dirname,
                                       pabort, cb, user_data);
...
static int plt_add_file_int(int visibility, playlist_t *plt, const char *fname,
                            int (*cb)(playItem_t *it, void *data),
                            void *user_data) {
  int abort = 0;
  playItem_t *it = plt_insert_file_int(visibility, plt, plt->tail[PL_MAIN],
                                       fname, &abort, cb, user_data);
  if (it) {
...
int plt_add_file(playlist_t *plt, const char *fname,
                 int (*cb)(playItem_t *it, void *data), void *user_data) {
  return plt_add_file_int(0, plt, fname, cb, user_data);
}

int plt_add_dir(playlist_t *plt, const char *dirname,
                int (*cb)(playItem_t *it, void *data), void *user_data) {
  int abort = 0;
  playItem_t *it =
      plt_insert_dir(plt, plt->tail[PL_MAIN], dirname, &abort, cb, user_data);
  if (it) {
    // pl_insert_file doesn't hold reference, don't unref here
    return 0;
  }
...
int pl_add_files_begin(playlist_t *plt) { return plt_add_files_begin(plt, 0); }

void pl_add_files_end(void) { return plt_add_files_end(addfiles_playlist, 0); }

int plt_remove_item(playlist_t *playlist, playItem_t *it) {
  if (!it)
    return -1;
  ...
  for (int iter = PL_MAIN; iter <= PL_SEARCH; iter++) {
    if (it->prev[iter] || it->next[iter] || playlist->head[iter] == it ||
...
int plt_get_item_count(playlist_t *plt, int iter) { return plt->count[iter]; }
...
int plt_getselcount(playlist_t *playlist) {
  LOCK;
  int cnt = 0;
...
playItem_t *plt_get_item_for_idx(playlist_t *playlist, int idx, int iter) {
  LOCK;
...
playItem_t *pl_get_for_idx_and_iter(int idx, int iter) {
  LOCK;
...
playItem_t *pl_get_for_idx(int idx) {
  return pl_get_for_idx_and_iter(idx, PL_MAIN);
}

int plt_get_item_idx(playlist_t *playlist, playItem_t *it, int iter) {
  LOCK;
...
int pl_get_idx_of(playItem_t *it) { return pl_get_idx_of_iter(it, PL_MAIN); }

int pl_get_idx_of_iter(playItem_t *it, int iter) {
  LOCK;
...
playItem_t *plt_insert_item(playlist_t *playlist, playItem_t *after,
                            playItem_t *it) {
  LOCK;
  pl_item_ref(it);
  if (!after) {
  ...
  if (streamer_get_shuffle() == DDB_SHUFFLE_ALBUMS && prev &&
      pl_find_meta_raw(prev, "album") == pl_find_meta_raw(it, "album") &&
      ((aa && prev_aa && aa == prev_aa) ||
       pl_find_meta_raw(prev, "artist") == pl_find_meta_raw(it, "artist"))) {
...
playItem_t *pl_insert_item(playItem_t *after, playItem_t *it) {
  return plt_insert_item(addfiles_playlist ? addfiles_playlist : playlist,
                         after, it);
}

void pl_item_copy(playItem_t *out, playItem_t *it) {
  LOCK;
...
playItem_t *pl_item_alloc_init(const char *fname, const char *decoder_id) {
...
void pl_item_ref(playItem_t *it) {
  LOCK;
  it->_refc++;
  // fprintf (stderr, "\033[0;34m+it %p: refc=%d: %s\033[37;0m\n", it,
  // it->_refc, pl_find_meta_raw (it, ":URI"));
  UNLOCK;
}
...
void pl_item_unref(playItem_t *it) {
  LOCK;
  it->_refc--;
...
int plt_delete_selected(playlist_t *playlist) {
  LOCK;
  int i = 0;
  int ret = -1;
...
void plt_crop_selected(playlist_t *playlist) {
  LOCK;
  playItem_t *next = NULL;
...
int plt_save(playlist_t *plt, playItem_t *first, playItem_t *last,
             const char *fname, int *pabort,
             int (*cb)(playItem_t *it, void *data), void *user_data) {
  LOCK;
        ...
        if (exts && plug[i]->save) {
          for (int e = 0; exts[e]; e++) {
            if (!strcasecmp(exts[e], ext + 1)) {
              int res =
                  plug[i]->save((ddb_playlist_t *)plt, fname,
                                (DB_playItem_t *)playlist->head[PL_MAIN], NULL);
              UNLOCK;
              return res;
            }
          }
...
int plt_save_n(int n) {
  ...
  plt_loading = 1;
  if (snprintf(path, sizeof(path), "%s/playlists/%d.dbpl", dbconfdir, n) >
      sizeof(path)) {
    fprintf(stderr, "error: failed to make path string for playlist file\n");
    UNLOCK;
    return -1;
...
int pl_save_current(void) { return plt_save_n(plt_get_curr_idx()); }
...
int pl_save_all(void) {
  ...
  plt_loading = 1;
  for (i = 0; i < cnt; i++, p = p->next) {
    if (snprintf(path, sizeof(path), "%s/playlists/%d.dbpl", dbconfdir, i) >
        sizeof(path)) {
      fprintf(stderr, "error: failed to make path string for playlist file\n");
      err = -1;
      break;
...
static playItem_t *plt_load_int(int visibility, playlist_t *plt,
      ...
      for (e = 0; plug[p]->extensions[e]; e++) {
        if (plug[p]->load && !strcasecmp(ext, plug[p]->extensions[e])) {
          ...
          if (cb || (plug[p]->load && !plug[p]->load2)) {
            it = plug[p]->load((ddb_playlist_t *)plt, (DB_playItem_t *)after,
                               fname, pabort,
                               (int (*)(DB_playItem_t *, void *))cb, user_data);
          } else if (plug[p]->load2) {
            plug[p]->load2(visibility, (ddb_playlist_t *)plt,
                           (DB_playItem_t *)after, fname, pabort);
          }
  ...
  for (uint32_t i = 0; i < cnt; i++) {
    ...
    if (minorver >= 2) {
      ...
      if (pl_item_get_startsample(it) > 0 || pl_item_get_endsample(it) > 0 ||
          tracknum > 0) {
        flg |= DDB_IS_SUBTRACK;
      }
    }
    ...
    for (int j = 0; j < nm; j++) {
      ...
      if (l >= 20000) {
        ...
        if (res != l) {
          //                    trace ("playlist read error: requested %d, got
          //                    %d\n", l, res);
          goto load_fail;
        }
        value[l] = 0;
        if (key[0] == ':') {
          if (!strcmp(key, ":STARTSAMPLE")) {
            ...
            // here, we delete what was set from legacy, and overwrite with
            // metadata
...
playItem_t *plt_load(playlist_t *plt, playItem_t *after, const char *fname,
                     int *pabort, int (*cb)(playItem_t *it, void *data),
                     void *user_data) {
  return plt_load_int(0, plt, after, fname, pabort, cb, user_data);
}

int pl_load_all(void) {
  int i = 0;
  int err = 0;
  char path[1024];
  ...
  if (!it) {
    ...
    if (snprintf(defpl, sizeof(defpl), "%s/default.dbpl", dbconfdir) >
        sizeof(defpl)) {
  ...
  while (it) {
    ...
    if (snprintf(path, sizeof(path), "%s/playlists/%d.dbpl", dbconfdir, i) >
        sizeof(path)) {
      fprintf(stderr,
              "error: failed to make path string for playlist filename\n");
      err = -1;
    } else {
      ...
      if (!it) {
        fprintf(stderr,
                "WARNING: there were errors while loading playlist '%s' (%s)\n",
                it->value, path);
      }
    }
...
void pl_set_selected_in_playlist(playlist_t *playlist, playItem_t *it,
                                 int sel) {
  it->selected = sel;
  playlist->recalc_seltime = 1;
}

void plt_select_all(playlist_t *playlist) {
  LOCK;
  for (playItem_t *it = playlist->head[PL_MAIN]; it; it = it->next[PL_MAIN]) {
...
void plt_set_item_duration(playlist_t *playlist, playItem_t *it,
                           float duration) {
  LOCK;
...
float pl_get_item_duration(playItem_t *it) { return it->_duration; }

void pl_set_item_replaygain(playItem_t *it, int idx, float value) {
  char s[100];
  switch (idx) {
...
float pl_get_item_replaygain(playItem_t *it, int idx) {
...
int pl_format_item_queue(playItem_t *it, char *s, int size) {
  LOCK;
  *s = 0;
  int initsize = size;
...
const char *pl_format_duration(playItem_t *it, const char *ret, char *dur,
                               int size) {
  if (ret) {
    return ret;
  }
...
static const char *pl_format_elapsed(const char *ret, char *elapsed, int size) {
  if (ret) {
    return ret;
  }
...
// @escape_chars: list of escapable characters terminated with 0, or NULL if
// none
static int pl_format_title_int(const char *escape_chars, playItem_t *it,
                               int idx, char *s, int size, int id,
                               const char *fmt) {
  char tmp[50];
  char tags[200];
    ...
    if (*fmt != '%') {
      *s++ = *fmt;
      ...
      if (*fmt == 0) {
        ...
        const char *value =
            (duration = pl_format_duration(it, duration, tmp, sizeof(tmp)));
        while (n > 0 && *value) {
          *s++ = *value++;
          n--;
        }
        ...
        // what a hack..
        const char *value =
            (elapsed = pl_format_elapsed(elapsed, tmp, sizeof(tmp)));
        while (n > 0 && *value) {
          *s++ = *value++;
          n--;
        }
        ...
        *t = 0;
        ...
        float l = 0;
        for (playItem_t *it = playlist->head[PL_MAIN]; it;
             it = it->next[PL_MAIN]) {
          if (it->selected) {
            l += it->_duration;
          }
      ...
      if (meta) {
        ...
        if (escape_chars) {
          ...
          *s++ = '\'';
          ...
          if (n < 1) {
            fprintf(stderr,
                    "pl_format_title_int: got unpredicted state while "
                    "formatting escaped string. please report a bug.\n");
...
int pl_format_title(playItem_t *it, int idx, char *s, int size, int id,
                    const char *fmt) {
  return pl_format_title_int(NULL, it, idx, s, size, id, fmt);
}

int pl_format_title_escaped(playItem_t *it, int idx, char *s, int size, int id,
                            const char *fmt) {
  return pl_format_title_int("'", it, idx, s, size, id, fmt);
}

void plt_reset_cursor(playlist_t *playlist) {
  int i;
  LOCK;
...
float plt_get_totaltime(playlist_t *playlist) {
  if (!playlist) {
    return 0;
  }
...
float plt_get_selection_playback_time(playlist_t *playlist) {
  LOCK;
...
void pl_set_selected(playItem_t *it, int sel) {
  LOCK;
  // NOTE: it's not really known here, which playlist the item belongs to, but
  // we're assuming it's in the current playlist. If the item is in another
  // playlist -- this call will make selection playback time to be recalculated
  // for the current playlist, next time it's requested, while the same value
  // will be off in the playlist which the item belongs to.
  pl_set_selected_in_playlist(playlist, it, sel);
  UNLOCK;
}

int pl_is_selected(playItem_t *it) { return it->selected; }

playItem_t *plt_get_first(playlist_t *playlist, int iter) {
  if (!playlist) {
    return NULL;
  }
...
playItem_t *pl_get_first(int iter) {
  LOCK;
...
playItem_t *plt_get_last(playlist_t *playlist, int iter) {
...
playItem_t *pl_get_last(int iter) {
  LOCK;
...
playItem_t *pl_get_next(playItem_t *it, int iter) {
...
playItem_t *pl_get_prev(playItem_t *it, int iter) {
...
int plt_get_cursor(playlist_t *playlist, int iter) {
  if (!playlist) {
    return -1;
  }
...
void plt_set_cursor(playlist_t *playlist, int iter, int cursor) {
...
void plt_move_items(playlist_t *to, int iter, playlist_t *from,
                    playItem_t *drop_before, uint32_t *indexes, int count) {
  LOCK;
  ...
  for (playItem_t *it = from->head[iter]; it && processed < count;
       it = next, idx++) {
    next = it->next[iter];
...
void plt_copy_items(playlist_t *to, int iter, playlist_t *from,
                    playItem_t *before, uint32_t *indices, int cnt) {
  pl_lock();
...
static void plt_search_reset_int(playlist_t *playlist, int clear_selection) {
  LOCK;
...
void plt_search_reset(playlist_t *playlist) {
  plt_search_reset_int(playlist, 1);
}

static void _plsearch_append(playlist_t *plt, playItem_t *it,
                             int select_results) {
...
void plt_search_process2(playlist_t *playlist, const char *text,
                         int select_results) {
  LOCK;
  ...
  for (playItem_t *it = playlist->head[PL_MAIN]; it; it = it->next[PL_MAIN]) {
    ...
    if (*text) {
      ...
      for (m = it->meta; m; m = m->next) {
        ...
        if (abs(cmp) ==
            playlist
                ->search_cmpidx) { // string was already compared in this search
          ...
          do {
            ...
            if (lc_is_valid_u8 && u8_valid(value, len, NULL) &&
                utfcasestr_fast(value, lc)) {
              _plsearch_append(playlist, it, select_results);
...
void plt_search_process(playlist_t *playlist, const char *text) {
  plt_search_process2(playlist, text, 1);
}

void send_trackinfochanged(playItem_t *track) {
  ddb_event_track_t *ev =
      (ddb_event_track_t *)messagepump_event_alloc(DB_EV_TRACKINFOCHANGED);
...
void pl_items_copy_junk(playItem_t *from, playItem_t *first, playItem_t *last) {
  LOCK;
...
uint32_t pl_get_item_flags(playItem_t *it) {
  LOCK;
...
void pl_set_item_flags(playItem_t *it, uint32_t flags) {
  LOCK;
  it->_flags = flags;
  ...
  pl_replace_meta(it, ":HAS_EMBEDDED_CUESHEET",
                  (flags & DDB_HAS_EMBEDDED_CUESHEET) ? _("Yes") : _("No"));
  UNLOCK;
}

playlist_t *pl_get_playlist(playItem_t *it) {
  LOCK;
  playlist_t *p = playlists_head;
  while (p) {
...
// this function must be called when the user starts track manually in shuffle
// albums mode r is an index of current track mark previous songs in the album
// as played
void plt_init_shuffle_albums(playlist_t *plt, int r) {
  pl_lock();
...
void plt_set_fast_mode(playlist_t *plt, int fast) {
  plt->fast_mode = (unsigned)fast;
}

int plt_is_fast_mode(playlist_t *plt) { return plt->fast_mode; }

void pl_ensure_lock(void) {
#if DETECT_PL_LOCK_RC
  ...
  fprintf(stderr, "\033[0;31mnon-thread-safe playlist access function was "
                  "called outside of pl_lock. please make a backtrace and post "
                  "a bug. thank you.\033[37;0m\n");
  assert(0);
#endif
}

int plt_get_idx(playlist_t *plt) {
  int i;
  playlist_t *p;
...
int plt_save_config(playlist_t *plt) {
...
void unlisten_file_add_beginend(int id) {
  ...
  for (ddb_fileadd_beginend_listener_t *l = file_add_beginend_listeners; l;
       prev = l, l = l->next) {
    if (l->id == id) {
      if (prev) {
        prev->next = l->next;
...
playItem_t *plt_load2(int visibility, playlist_t *plt, playItem_t *after,
                      const char *fname, int *pabort,
                      int (*callback)(playItem_t *it, void *user_data),
                      void *user_data) {
  return plt_load_int(visibility, plt, after, fname, pabort, callback,
                      user_data);
}

int plt_add_file2(int visibility, playlist_t *plt, const char *fname,
                  int (*callback)(playItem_t *it, void *user_data),
                  void *user_data) {
...
playItem_t *pl_item_init(const char *fname) {
  playlist_t plt;
  memset(&plt, 0, sizeof(plt));
  return plt_insert_file_int(-1, &plt, NULL, fname, NULL, NULL, NULL);
}

int plt_add_dir2(int visibility, playlist_t *plt, const char *dirname,
                 int (*callback)(playItem_t *it, void *user_data),
                 void *user_data) {
  ...
  int abort = 0;
  playItem_t *it = plt_insert_dir_int(visibility, plt, NULL, plt->tail[PL_MAIN],
                                      dirname, &abort, callback, user_data);
...
playItem_t *plt_insert_file2(int visibility, playlist_t *playlist,
                             playItem_t *after, const char *fname, int *pabort,
                             int (*callback)(playItem_t *it, void *user_data),
                             void *user_data) {
  return plt_insert_file_int(visibility, playlist, after, fname, pabort,
                             callback, user_data);
}

playItem_t *plt_insert_dir2(int visibility, playlist_t *plt, playItem_t *after,
                            const char *dirname, int *pabort,
                            int (*callback)(playItem_t *it, void *user_data),
                            void *user_data) {
  int prev_sl = plt->follow_symlinks;
  ...
  playItem_t *ret = plt_insert_dir_int(visibility, plt, NULL, after, dirname,
                                       pabort, callback, user_data);
...
int plt_add_files_begin(playlist_t *plt, int visibility) {
  pl_lock();
...
void plt_add_files_end(playlist_t *plt, int visibility) {
  pl_lock();
  if (addfiles_playlist) {
...
void plt_deselect_all(playlist_t *playlist) {
  LOCK;
  for (playItem_t *it = playlist->head[PL_MAIN]; it; it = it->next[PL_MAIN]) {
    it->selected = 0;
  }
...
void plt_set_scroll(playlist_t *plt, int scroll) { plt->scroll = scroll; }

int plt_get_scroll(playlist_t *plt) { return plt->scroll; }

static playItem_t *plt_process_embedded_cue(playlist_t *plt, playItem_t *after,
                                            playItem_t *it,
  ...
  if (cuesheet) {
    ...
    cue = plt_load_cuesheet_from_buffer(plt, after, fname, it, totalsamples,
                                        samplerate, (const uint8_t *)cuesheet,
                                        (int)strlen(cuesheet), NULL, 0, 0);
  }
  pl_unlock();
  return cue;
}
...
plt_process_cue(playlist_t *plt, playItem_t *after, playItem_t *it,
                uint64_t totalsamples, int samplerate) {
  if (plt->loading_cue) {
    // means it was called from _load_cue
    plt->cue_numsamples = totalsamples;
    plt->cue_samplerate = samplerate;
  ...
  return plt_process_embedded_cue(plt, after, it, (int64_t)totalsamples,
                                  samplerate);
}
...
int64_t pl_item_get_startsample(playItem_t *it) {
  int64_t res;
  pl_lock();
...
int64_t pl_item_get_endsample(playItem_t *it) {
  int64_t res = 0;
  pl_lock();
...
void pl_item_set_startsample(playItem_t *it, int64_t sample) {
  pl_lock();
...
void pl_item_set_endsample(playItem_t *it, int64_t sample) {
  pl_lock();
...
int plt_is_loading_cue(playlist_t *plt) { return plt->loading_cue; }
