...
void ddb_ctmap_free(ddb_ctmap_t *ctmap) {
    while (ctmap) {
...
ddb_ctmap_t *ddb_ctmap_init_from_string(const char *mapstr) {
    const char *p = mapstr;
    char t[MAX_TOKEN];
...
char *ddb_ctmap_to_string(const ddb_ctmap_t *ctmap) {
    ...
    while (ctmap) {
        ...
        for (int i = 0; ctmap->plugins[i]; i++) {
            ...
            strncat(plugins, ctmap->plugins[i],
                    sizeof(plugins) - strlen(plugins) - 1);
        }
