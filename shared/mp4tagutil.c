...
static const char *_mp4_atom_map[] = {
    COPYRIGHT_SYM "alb", "album", COPYRIGHT_SYM "ART", "artist", "aART", "band",
    COPYRIGHT_SYM "cmt", "comment", COPYRIGHT_SYM "day", "year",
    COPYRIGHT_SYM "nam", "title", COPYRIGHT_SYM "gen", "genre", "gnre", "genre",
    "trkn", "track", "disk", "disc", COPYRIGHT_SYM "wrt", "composer",
    COPYRIGHT_SYM "too", "encoder", "tmpo", "bpm", "cprt", "copyright",
    COPYRIGHT_SYM "grp", "grouping", "cpil", "compilation", "pcst", "podcast",
    "catg", "category", "keyw", "keyword", "desc", "description",
    COPYRIGHT_SYM "lyr", "lyrics", "purd", "purchase date",
    "MusicBrainz Track Id", "musicbrainz_trackid", "DISCID", "DISCID",
    "iTunSMPB", "iTunSMPB",
    // NOTE: these replaygain fields are read/written in a special way,
    // but they need to be here to be "known" fields, so that they're cleaned up
    // on write.
    "replaygain_track_gain", "replaygain_track_gain", "replaygain_track_peak",
    "replaygain_track_peak", "replaygain_album_gain", "replaygain_album_gain",
 ...
 * Generate data block
 * If the new udta block can fit over the old one, with at least 8 bytes extra
 * for the "free" atom: Overwrite the old block Pad with "free" atom if
 * necessary If can't fit: the entire moov atom has to be relocated! Rename the
 * existing moov into "free" Append the modified moov block to the end of file,
 * after the last atom IMPORTANT: the entirety of moov atom with all sub atoms
 * needs to be loaded and saved Further work: Find if there are "free" blocks
 * between ftyp and mdat, and try to fit the moov there; If that works, truncate
 * the file.
 */

static void _remove_known_fields(mp4p_atom_t *ilst) {
  ...
  while (meta_atom) {
    ...
    for (int i = 0; _mp4_atom_map[i]; i += 2) {
      ...
      if ((!meta->custom &&
           !strcmp(type, _mp4_atom_map[i])) // all non-custom mapped fields
          || (meta->custom &&
              meta->data_version_flags == 1)) { // all custom text fields
...
static mp4p_atom_t *mp4tagutil_find_udta(mp4p_atom_t *moov, mp4p_atom_t **pmeta,
                                         mp4p_atom_t **pilst) {
...
static int _mp4tagutil_file_editable(mp4p_atom_t *mp4file) {
...
static void _mp4tagutil_add_metadata_fields(mp4p_atom_t *ilst,
                                            DB_playItem_t *it) {
  deadbeef->pl_lock();
  ...
  while (m) {
    ...
    if (!strcasecmp(m->key, "track") || !strcasecmp(m->key, "numtracks") ||
        !strcasecmp(m->key, "disc") || !strcasecmp(m->key, "numdiscs") ||
    ...
    while (value < end) {
      if (!_mp4_atom_map[i] || strlen(_mp4_atom_map[i]) != 4) {
        mp4p_atom_append(
            ilst, mp4p_ilst_create_custom(
                      _mp4_atom_map[i] ? _mp4_atom_map[i] : m->key, value));
      } else {
  ...
  if (itrack || inumtracks) {
    mp4p_atom_append(ilst,
                     mp4p_ilst_create_track_disc("trkn", itrack, inumtracks));
  }
  if (idisc || inumdiscs) {
    mp4p_atom_append(ilst,
                     mp4p_ilst_create_track_disc("disk", idisc, inumdiscs));
  }
...
mp4p_atom_t *mp4tagutil_modify_meta(mp4p_atom_t *mp4file, DB_playItem_t *it) {
  ...
  for (mp4p_atom_t *curr = padding;
       curr && !mp4p_atom_type_compare(curr, "free"); curr = curr->next) {
  ...
  if (before_moov_begin->pos + before_moov_begin->size + moov->size ==
          eop->pos ||
      before_moov_begin->pos + before_moov_begin->size + moov->size <
          eop->pos - 8) {
...
int mp4_write_metadata(DB_playItem_t *it) {
  char fname[PATH_MAX];
...
#if 0 // FIXME: skipping junk is unsupported yet with file callbacks (junklib
      // can't read using this interface -- a VFS wrapper needed?
...
void mp4_load_tags(mp4p_atom_t *mp4file, DB_playItem_t *it) {
  ...
  for (mp4p_atom_t *meta_atom = ilst->subatoms; meta_atom;
    ...
    if (!strcasecmp(name, "replaygain_track_gain")) {
      deadbeef->pl_set_item_replaygain(it, DDB_REPLAYGAIN_TRACKGAIN,
                                       atof(meta->text));
    } else if (!strcasecmp(name, "replaygain_album_gain")) {
      deadbeef->pl_set_item_replaygain(it, DDB_REPLAYGAIN_ALBUMGAIN,
                                       atof(meta->text));
    } else if (!strcasecmp(name, "replaygain_track_peak")) {
      deadbeef->pl_set_item_replaygain(it, DDB_REPLAYGAIN_TRACKPEAK,
                                       atof(meta->text));
    } else if (!strcasecmp(name, "replaygain_album_peak")) {
      deadbeef->pl_set_item_replaygain(it, DDB_REPLAYGAIN_ALBUMPEAK,
                                       atof(meta->text));
    } else {
      int i = 0;
...
int mp4_read_metadata_file(DB_playItem_t *it, mp4p_file_callbacks_t *cb) {
...
static ssize_t _file_read(mp4p_file_callbacks_t *stream, void *ptr,
                          size_t size) {
  return deadbeef->fread(ptr, 1, size, (DB_FILE *)stream->ptrhandle);
}

static off_t _file_seek(mp4p_file_callbacks_t *stream, off_t offset,
                        int whence) {
  return deadbeef->fseek((DB_FILE *)stream->ptrhandle, offset, whence);
}

static int64_t _file_tell(mp4p_file_callbacks_t *stream) {
  return deadbeef->ftell((DB_FILE *)stream->ptrhandle);
}

void mp4_init_ddb_file_callbacks(mp4p_file_callbacks_t *cb) {
  cb->read = _file_read;
  cb->seek = _file_seek;
  cb->tell = _file_tell;
}

int mp4_read_metadata(DB_playItem_t *it) {
  char fname[PATH_MAX];
  deadbeef->pl_get_meta(it, ":URI", fname, sizeof(fname));
...
mp4p_atom_t *mp4_get_cover_atom(mp4p_atom_t *mp4file) {
  mp4p_atom_t *moov = mp4p_atom_find(mp4file, "moov");
  if (!moov) {
    return NULL;
  }
