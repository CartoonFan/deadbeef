...
ddbUtilTrackList_t
ddbUtilTrackListInitWithPlaylist(ddbUtilTrackList_t trackList,
                                 ddb_playlist_t *plt,
                                 ddb_action_context_t ctx) {
    ...
    if (ctx == DDB_ACTION_CTX_SELECTION) {
        ...
        if (it) {
            const char *uri = deadbeef->pl_find_meta(it, ":URI");
            if (deadbeef->is_local_file(uri)) {
                int idx = idx_current_song =
                              deadbeef->plt_get_item_idx(plt, it, PL_MAIN);
                if (idx != -1) {
...
ddbUtilTrackList_t ddbUtilTrackListInitWithWithTracks(
    ddbUtilTrackList_t trackList, ddb_playlist_t *plt, ddb_action_context_t ctx,
    ddb_playItem_t **tracks, unsigned count) {
    ddbUtilTrackListData_t *data = trackList;
...
ddbDeleteFromDiskControllerInitWithPlaylist(ddbDeleteFromDiskController_t ctl,
    ...
    data->trackList =
        ddbUtilTrackListInitWithPlaylist(ddbUtilTrackListAlloc(), plt, ctx);
    return ctl;
}
...
static void _remove_file_from_all_playlists(const char *search_uri) {
...
static void _delete_and_remove_track_from_all_playlists(
    ddbDeleteFromDiskController_t ctl, const char *uri, ddb_playlist_t *plt,
    ddb_playItem_t *it) {
    ddbDeleteFromDiskControllerData_t *data = ctl;
...
void *
ddbDeleteFromDiskControllerGetUserData(ddbDeleteFromDiskController_t ctl) {
    ddbDeleteFromDiskControllerData_t *data = ctl;
    return data->userData;
}

static void _warningCallback(ddbDeleteFromDiskController_t ctl,
                             int shouldCancel) {
    ddbDeleteFromDiskControllerData_t *data = ctl;
    ...
    if (data->shouldSkipDeletedTracks &&
            deadbeef->plt_get_item_idx(plt, trackListData->it_current_song,
                                       PL_MAIN) == -1 &&
            deadbeef->streamer_get_current_playlist() ==
            deadbeef->plt_get_curr_idx() &&
        ...
        if (trackListData->idx_current_song != -1 &&
            ...
            deadbeef->sendmessage(DB_EV_PLAY_NUM, 0, trackListData->idx_current_song,
                                  0);
        } else {
...
void ddbDeleteFromDiskControllerRunWithDelegate(
    ...
    data->delegate.warningMessageForCtx(
        ctl, trackListData->ctx, trackListData->trackcount, _warningCallback);
}
