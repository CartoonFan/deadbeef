...
static void set_convert_errno(void) {
  ...
  /*if (GetLastError() != ERROR_NO_UNICODE_TRANSLATION) {
      // Sad days =(
      trace_err ("The scandir implementation is using a bad argument for
  encoding conversion!\n");
  }*/
  _set_errno(EILSEQ);
}
...
int scandir(const char *dirname_o, struct dirent ***namelist_to,
  ...
  int utf16_points = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS,
                                         dirname_o, -1, NULL, 0);
  if (utf16_points < 1) {
    set_convert_errno();
    return -1;
  ...
  int ret = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, dirname_o, -1,
                                dir_wide, utf16_points);
  if (ret < 1) {
    // I don't see how this could fail if the first call succeeded, but check
    // anyways.
    set_convert_errno();
    return -1;
  }
