...
#ifdef WIN_CHARSET_CONV_WIN
int win_charset_conv(const void *in, int inlen, void *out, int outlen,
  ...
  if (strncmp(cs_in, "UTF-8", 5) == 0 && strncmp(cs_out, "WCHAR_T", 7) == 0) {
    ...
    ret = MultiByteToWideChar(CP_UTF8, /*FLAGS*/ 0, (char *)in, inlen,
                              (wchar_t *)out, outlen);
  } else if (strncmp(cs_out, "UTF-8", 5) == 0 &&
             strncmp(cs_in, "WCHAR_T", 7) == 0) {
    // WideChar to MultiByte
    ret = WideCharToMultiByte(CP_UTF8, /*FLAGS*/ 0, (wchar_t *)in, inlen,
...
int path_short(const char *path_in, char *path_out, int len) {
  ...
  int iconv_ret = win_charset_conv(in_c, strlen(in_c) + 1, (char *)in_w,
                                   strlen(in_c) * 2 + 1, "UTF-8", "WCHAR_T");
  // convert path to shortpath
  wchar_t out_w[strlen(in_c)];
  int win_ret = GetShortPathNameW(in_w, out_w, 256);
  // convert to UTF-8 :D
  int iconv2_ret = win_charset_conv((char *)out_w, wcslen(out_w) * 2 + 2,
                                    (char *)path_out, len, "WCHAR_T", "UTF-8");
  return iconv2_ret;
}
...
int path_long(const char *path_in, char *path_out, int len) {
  ...
  // convert to UTF-8 :D
  int iconv2_ret =
      win_charset_conv((char *)abs_path_w, wcslen(abs_path_w) * 2 + 2,
                       (char *)path_out, len, "WCHAR_T", "UTF-8");
...
int startup_fixes(char *out_p) {
  ...
  // ensures that plugins can be dlopened even when they are located in
  // non-ASCII path uses char *out_p variable and returns ret variable (this
  // function is called from realpath implementation)
  ...
  // convert to utf8
  int ret = win_charset_conv((char *)argv_win, (wcslen(argv_win) + 1) * 2,
                             out_p, PATH_MAX, "WCHAR_T", "UTF-8");

  // Set current directory, so that windows can find libraries needed to load
  // plugins
  {
  ...
  // since gtk 3.24.12 on windows (msys) client-side decorations are not forced,
  ...
  if (getenv("CURL_CA_BUNDLE") == NULL) {
    char capath[PATH_MAX +
                strlen("CURL_CA_BUNDLE=\\share\\ssl\\certs\\ca-bundle.crt")];
...
char *realpath(const char *path, char *resolved_path) {
  ...
  // To ensure that plugins will load sucessfully, we need to return DOS path
  // (as it is ASCII-only)
  if (first_call) {
