case "$TRAVIS_OS_NAME" in
  ...
  osx)
    ...
    xcodebuild "MACOSX_DEPLOYMENT_TARGET=10.13" test -project osx/deadbeef.xcodeproj -scheme deadbeef -configuration Release -quiet | xcpretty
    test "${PIPESTATUS[0]}" -eq 0 || exit 1
    xcodebuild "MACOSX_DEPLOYMENT_TARGET=10.13" -project osx/deadbeef.xcodeproj -target DeaDBeeF -configuration Release -quiet | xcpretty
    test "${PIPESTATUS[0]}" -eq 0 || exit 1
