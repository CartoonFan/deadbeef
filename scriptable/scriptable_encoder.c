...
static scriptableItem_t *scriptableEncoderCreatePreset(scriptableItem_t *root,
                                                       const char *type);

static int scriptableEncoderPresetSave(scriptableItem_t *item);

static int scriptableEncoderUpdateItem(struct scriptableItem_s *item);

static void
scriptableEncoderPropertyValueWillChangeForKey(struct scriptableItem_s *item,
                                               const char *key);

static int scriptableEncoderDelete(scriptableItem_t *item);

static int scriptableEncoderRootRemoveSubItem(scriptableItem_t *item,
...
static scriptableCallbacks_t scriptableEncoderCallbacks = {
    ...
    .propertyValueWillChangeForKey =
        scriptableEncoderPropertyValueWillChangeForKey,
};
...
static const char *configdialog =
    ...
    "property \"Data transfer method\" select[3] method 0 \"Pipe (stdin)\" "
    "\"Temporary file\" \"Source file\";"
...
static scriptableItem_t *scriptableEncoderCreatePreset(scriptableItem_t *root,
  ...
  scriptableItemSetUniqueNameUsingPrefixAndRoot(item, "New Encoder Preset",
                                                root);
  return item;
}

static int scriptableItemLoadEncoderPreset(scriptableItem_t *preset,
...
static int scriptableEncoderPresetSaveAtPath(scriptableItem_t *item,
                                             char *path) {
...
static int getEncoderPresetFullPathName(const char *fname, char *path,
                                        size_t size) {
  int res = snprintf(path, size, "%s/presets/encoders/%s",
...
scriptableItem_t *scriptableEncoderRoot(void) {
  scriptableItem_t *encoderRoot =
      scriptableItemSubItemForName(scriptableRoot(), "EncoderPresets");
  if (!encoderRoot) {
...
void scriptableEncoderLoadPresets(void) {
  ...
  for (int di = 0; preset_dirs[di]; di++) {
    ...
    int n = scandir(presetspath, &namelist, scandir_preset_filter,
                    dirent_alphasort);
    int i;
    for (i = 0; i < n; i++) {
      char s[PATH_MAX];
      if (snprintf(s, sizeof(s), "%s/%s", presetspath, namelist[i]->d_name) >
          0) {
...
static const char *valueForKeyOrDefault(scriptableItem_t *item, const char *key,
                                        const char *def) {
  return scriptableItemPropertyValueForKey(item, key) ?: def;
}

void scriptableEncoderPresetToConverterEncoderPreset(
    scriptableItem_t *item, ddb_encoder_preset_t *encoder_preset) {
  ...
  encoder_preset->tag_id3v2 =
      atoi(valueForKeyOrDefault(item, "tag_id3v2", "0"));
  encoder_preset->tag_id3v1 =
      atoi(valueForKeyOrDefault(item, "tag_id3v1", "0"));
  encoder_preset->tag_apev2 =
      atoi(valueForKeyOrDefault(item, "tag_apev2", "0"));
  encoder_preset->tag_flac = atoi(valueForKeyOrDefault(item, "tag_flac", "0"));
  encoder_preset->tag_oggvorbis =
      atoi(valueForKeyOrDefault(item, "tag_oggvorbis", "0"));
  encoder_preset->tag_mp4 = atoi(valueForKeyOrDefault(item, "tag_mp4", "0"));
  encoder_preset->id3v2_version =
      atoi(valueForKeyOrDefault(item, "id3v2_version", "0"));
}
