...
static scriptableItem_t *scriptableDspCreateItemOfType(scriptableItem_t *root,
        const char *type);

static void scriptableDspInitContextFromItem(scriptableItem_t *c,
        ddb_dsp_context_t *ctx,
        DB_dsp_t *dsp);

static int scriptableDspPresetUpdateItem(scriptableItem_t *item);

static int scriptableDspPresetUpdateItemForSubItem(scriptableItem_t *item,
        scriptableItem_t *subItem);
...
static int scriptableDspRootRemoveSubItem(scriptableItem_t *item,
        scriptableItem_t *subItem);

static int isPresetNameAllowed(scriptableItem_t *preset, const char *name);

static char *scriptableDspPresetNodeSaveToString(scriptableItem_t *item);

static void
scriptableDspPropertyValueWillChangeForKey(struct scriptableItem_s *item,
        const char *key);

static void scriptableDspPresetNodePropertyValueDidChangeForKey(
    struct scriptableItem_s *item, const char *key);

static int scriptableDspPresetSave(scriptableItem_t *item);

static int scriptableDspPresetDelete(scriptableItem_t *subItem);

static scriptableCallbacks_t scriptableDspNodeCallbacks = {
    .saveToString = scriptableDspPresetNodeSaveToString,
    .propertyValueDidChangeForKey =
    scriptableDspPresetNodePropertyValueDidChangeForKey,
};

static scriptableCallbacks_t scriptableDspPresetCallbacks = {
    .isList = 1,
    .isReorderable = 1,
...
static int scandir_preset_filter(const struct dirent *ent) {
...
static int getDspPresetFullPathName(const char *fname, char *path,
                                    size_t size) {
    int res = snprintf(path, size, "%s/presets/dsp/%s",
                       deadbeef->get_system_dir(DDB_SYS_DIR_CONFIG), fname);
    return res < size ? 0 : -1;
}

static int scriptableItemLoadDspPreset(scriptableItem_t *preset,
                                       const char *fname) {
    char path[PATH_MAX];
...
static DB_dsp_t *dspPluginForId(const char *pluginId) {
...
static void scriptableDspPresetNodePropertyValueDidChangeForKey(
    ...
    const char *parentName =
        scriptableItemPropertyValueForKey(item->parent, "name");
    if (parentName) {
    ...
    for (scriptableItem_t *node = item->parent->children; node;
            node = node->next) {
        if (node == item) {
            int param = atoi(key);
...
static char *scriptableDspPresetNodeSaveToString(scriptableItem_t *node) {
    ...
    if (!dsp) {
        // when a plugin is missing: write out all numeric-name properties, in their
        // original order
        ...
        // when a plugin is present, create a context, and get the values from
        // plugin
...
static int scriptableDspPresetSaveAtPath(scriptableItem_t *item, char *path) {
...
scriptableItem_t *scriptableDspRoot(void) {
    scriptableItem_t *dspRoot =
        scriptableItemSubItemForName(scriptableRoot(), "DSPPresets");
    if (!dspRoot) {
        ...
        scriptableItemSetPropertyValueForKey(passThroughDspPreset, "Pass-through",
                                             "name");
...
void scriptableDspLoadPresets(void) {
...
static scriptableItem_t *
scriptableDspCreateNodeFromContext(ddb_dsp_context_t *context) {
    scriptableItem_t *node = scriptableItemAlloc();
    node->callbacks = &scriptableDspNodeCallbacks;
    scriptableItemSetPropertyValueForKey(node, context->plugin->plugin.id,
                                         "pluginId");
    scriptableItemSetPropertyValueForKey(node, context->plugin->plugin.name,
                                         "name");
...
scriptableDspNodeItemFromDspContext(ddb_dsp_context_t *context) {
    ...
    scriptableItemSetPropertyValueForKey(node, context->enabled ? "1" : "0",
                                         "enabled");
...
ddb_dsp_context_t *scriptableDspConfigToDspChain(scriptableItem_t *item) {
...
scriptableItem_t *scriptableDspPresetFromDspChain(ddb_dsp_context_t *chain) {
