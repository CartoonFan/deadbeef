...
#define STREAMER_HINTS                                                         \
  (DDB_DECODER_HINT_NEED_BITRATE | DDB_DECODER_HINT_CAN_LOOP)
...
// This counter is incremented by one for each streamer_read call, which returns
// -1, which means audio should stop, but we need to wait a bit until buffered
// data has finished playing, so we wait AUDIO_STALL_WAIT periods
...
//
// It's guaranteed that outbuffer contains only samples from the files with same
// wave format.
//
#define OUTPUT_BUFFER_SIZE                                                     \
  (512 * 1024) // FIXME: need to be able to calculate that size from DSP chain
...
#endif
void streamer_lock(void) {
#if DETECT_PL_LOCK_RC
  extern pthread_t pl_lock_tid;
  assert(
      pl_lock_tid !=
      pthread_self()); // not permitted to lock streamer from inside of pl_lock
#endif
  mutex_lock(mutex);
#if DETECT_PL_LOCK_RC
...
static void play_index(int idx, int startpaused);

static void play_current(void);

static void play_next(int dir, ddb_shuffle_t shuffle, ddb_repeat_t repeat);

static void streamer_set_current_playlist_real(int plt);

static int stream_track(playItem_t *track, int startpaused);

static void _handle_playback_stopped(void);

static void _streamer_mark_album_played_up_to(playItem_t *item);

static void streamer_abort_files(void) {
  ...
  trace("%lld %lld %lld\n", file_identifier, newfile_identifier,
        strfile_identifier);
...
static void send_songstarted(playItem_t *trk) {
  ddb_event_track_t *pev =
      (ddb_event_track_t *)messagepump_event_alloc(DB_EV_SONGSTARTED);
...
static void send_songfinished(playItem_t *trk) {
  ddb_event_track_t *pev =
      (ddb_event_track_t *)messagepump_event_alloc(DB_EV_SONGFINISHED);
  pev->track = DB_PLAYITEM(trk);
  pl_item_ref(trk);
...
static void send_trackchanged(playItem_t *from, playItem_t *to) {
  ddb_event_trackchange_t *event =
      (ddb_event_trackchange_t *)messagepump_event_alloc(DB_EV_SONGCHANGED);
...
void streamer_set_last_played(playItem_t *track) {
  if (last_played) {
...
static void streamer_start_playback(playItem_t *from, playItem_t *it) {
  if (from) {
    pl_item_ref(from);
  }
  if (it) {
  ...
  if (playing_track) {
    ...
    trace("from=%p (%s), to=%p (%s) [2]\n", from,
          from ? pl_find_meta(from, ":URI") : "null", it,
...
playItem_t *streamer_get_streaming_track(void) {
  if (streaming_track) {
...
playItem_t *streamer_get_playing_track(void) {
  playItem_t *it =
      (buffering_track && !playing_track) ? buffering_track : playing_track;
  if (it) {
    pl_item_ref(it);
  }
  return it;
...
playItem_t *streamer_get_buffering_track(void) {
...
int str_get_idx_of(playItem_t *it) {
  pl_lock();
...
playItem_t *str_get_for_idx(int idx) {
  pl_lock();
  if (!streamer_playlist) {
    playlist_t *plt = plt_get_curr();
    streamer_set_streamer_playlist(plt);
...
static int stop_after_album_check(playItem_t *cur, playItem_t *next) {
  if (!stop_after_album) {
    return 0;
  }
...
static playItem_t *get_random_track(void) {
  if (!streamer_playlist) {
    playlist_t *plt = plt_get_curr();
    streamer_set_streamer_playlist(plt);
    plt_unref(plt);
...
static playItem_t *get_next_track(playItem_t *curr, ddb_shuffle_t shuffle,
                                  ddb_repeat_t repeat) {
  pl_lock();
  if (!streamer_playlist) {
    playlist_t *plt = plt_get_curr();
  ...
  if (shuffle == DDB_SHUFFLE_TRACKS ||
      shuffle == DDB_SHUFFLE_ALBUMS) { // shuffle
    playItem_t *it = NULL;
...
static playItem_t *get_prev_track(playItem_t *curr, ddb_shuffle_t shuffle,
                                  ddb_repeat_t repeat) {
  pl_lock();
  ...
  if (shuffle == DDB_SHUFFLE_TRACKS ||
    ...
    if (!curr) {
      ...
      for (playItem_t *i = plt->head[PL_MAIN]; i; i = i->next[PL_MAIN]) {
        if (i != curr && i->played &&
            (!amax || i->shufflerating > amax->shufflerating)) {
          amax = i;
        }
      ...
      if (pmax && shuffle == DDB_SHUFFLE_ALBUMS) {
        while (pmax && pmax->next[PL_MAIN] && pmax->next[PL_MAIN]->played &&
               pmax->shufflerating == pmax->next[PL_MAIN]->shufflerating) {
...
int streamer_move_to_nextsong(int r) {
  if (r) {
...
int streamer_move_to_prevsong(int r) {
  if (r) {
    streamer_abort_files();
  }
...
int streamer_move_to_randomsong(int r) {
  if (r) {
    streamer_abort_files();
  }
...
void streamer_song_removed_notify(playItem_t *it) {
  if (!mutex) {
...
static void ctmap_init_mutex(void) { ctmap_mutex = mutex_create(); }

static void ctmap_free_mutex(void) {
  if (ctmap_mutex) {
...
static void ctmap_lock(void) { mutex_lock(ctmap_mutex); }

static void ctmap_unlock(void) { mutex_unlock(ctmap_mutex); }

static int is_remote_stream(playItem_t *it) {
  int remote = 0;
  pl_lock();
...
static DB_fileinfo_t *dec_open(DB_decoder_t *dec, uint32_t hints,
                               playItem_t *it) {
...
// and initialize dummy fileinfo for handling track switching for the tracks
// which can't be played
static void streamer_play_failed(playItem_t *failed_track) {
  streamer_lock();
  ...
  if (failed_track) {
    ...
    // but we want to send it down to streamreader for proper track switching
    // etc.
...
static void fileinfo_free(DB_fileinfo_t *fileinfo) {
  if (fileinfo->plugin) {
...
static int stream_track(playItem_t *it, int startpaused) {
  if (fileinfo_curr) {
  ...
  if (!decoder_id[0] && (!strcmp(filetype, "content") || !filetype[0])) {
    ...
    if (!plugs[0] &&
        (!strcmp(cct, "audio/x-mpegurl") || !strncmp(cct, "text/html", 9) ||
         !strncmp(cct, "audio/x-scpls", 13) ||
         !strncmp(cct, "application/octet-stream", 9))) {
      ...
      for (p = 0; plug[p]; p++) {
        if (plug[p]->load) {
          m3u = plug[p]->load((ddb_playlist_t *)plt, NULL, tempfile, NULL, NULL,
                              NULL);
          if (m3u) {
            break;
          }
      ...
      if (!m3u) {
        trace("failed to load playlist from %s using any of the installed "
              "playlist plugins\n",
              tempfile);
        plt_free(plt);
        goto m3u_error;
      ...
      // hack: need to sleep here, some servers like to reject frequent
      // connections
      usleep(conf_get_int("streamer.wait_ms_after_m3u_link", 400000));

      // for every playlist uri: override stream uri with the one from playlist,
      // and try to play it
  ...
  for (;;) {
    ...
    if (!new_fileinfo) {
      ...
      trace("bps=%d, channels=%d, samplerate=%d\n", new_fileinfo->fmt.bps,
            new_fileinfo->fmt.channels, new_fileinfo->fmt.samplerate);
      break;
    }
  }
success:
...
int streamer_get_apx_bitrate(void) { return avg_bitrate; }

void streamer_set_nextsong(int song, int startpaused) {
  streamer_abort_files();
  if (song == -1) {
...
void streamer_set_seek(float pos) {
  streamer_lock();
  last_seekpos = pos;
  streamer_unlock();
...
static void update_stop_after_current(void) {
...
static void streamer_next(ddb_shuffle_t shuffle, ddb_repeat_t repeat,
                          playItem_t *next) {
  if (playing_track) {
    if (repeat ==
        DDB_REPEAT_SINGLE) { // song finished, loop mode is "loop 1 track"
      next = playing_track;
    }
  }
  if (!next) {
...
static void streamer_shuffle_changed(ddb_shuffle_t prev, ddb_shuffle_t shuffle);

static void streamer_seek_real(float seekpos) {
  ...
  if (seek >= 0 && dur > 0) {
    ...
    ddb_event_playpos_t *ev =
        (ddb_event_playpos_t *)messagepump_event_alloc(DB_EV_SEEKED);
...
static void _update_buffering_state() {
...
static void handle_track_change(playItem_t *from, playItem_t *track) {
  // next track started
  if (from) {
...
static void get_desired_output_format(ddb_waveformat_t *in_fmt,
  ...
  // android simulation on PC: force 16 bit and 44100 or 48000 only, to force
  // format conversion
#if !defined(ANDROID) && defined(HAVE_XGUI)
  out_fmt->bps = 16;
...
static void streamer_set_output_format(ddb_waveformat_t *fmt) {
...
static void _streamer_requeue_after_current(ddb_repeat_t repeat,
...
static void _streamer_track_deleted(ddb_repeat_t repeat,
                                    ddb_shuffle_t shuffle) {
...
void streamer_thread(void *unused) {
  ...
  while (!streaming_terminate) {
    ...
    // each event can modify shuffle/repeat, so update them here, so that
    // subsequent event handlers use new values
    ...
    int res =
        streamreader_read_block(block, streaming_track, fileinfo_curr, mutex);
    int last = 0;
...
void streamer_dsp_refresh(void) {
  handler_push(handler, STR_EV_DSP_RELOAD, 0, 0, 0);
}

int streamer_init(void) {
  ...
  ctmap_init_mutex();
  conf_get_str("network.ctmapping", DDB_DEFAULT_CTMAPPING,
               conf_network_ctmapping, sizeof(conf_network_ctmapping));
...
static char *_get_output_buffer(size_t size) {
  if (_output_buffer) {
...
void streamer_reset(
    int full) { // must be called when current song changes by external reasons
  if (!mutex) {
...
static int process_output_block(streamblock_t *block, char *bytes) {
  DB_output_t *output = plug_get_output();
  ...
  // A block with 0 size is a valid block, and needs to be processed as usual
  // (code above this line). But here we do early exit, because there's no data
  // to process in it.
  if (!block->size) {
  ...
  if (block->fmt.bps != 16) {
    ...
    ddb_waveformat_t out_fmt = {.bps = 16,
    ...
    pcm_convert(&block->fmt, (char *)input, &out_fmt, (char *)temp_audio_data,
                sz);
  ...
  dsp_apply_simple_downsampler(datafmt.samplerate, datafmt.channels, input, sz,
                               output->fmt.samplerate, &dspbytes, &dspsize);
  datafmt.samplerate = output->fmt.samplerate;
  sz = dspsize;
#else
  int dsp_res = dsp_apply(&block->fmt, block->buf + block->pos, sz, &datafmt,
                          &dspbytes, &dspsize, &dspratio);
  if (dsp_res) {
    block->pos += sz;
    sz = dspsize;
  } else {
  ...
  playpos += (float)sz / output->fmt.samplerate /
             ((output->fmt.bps >> 3) * output->fmt.channels) * dspratio;
  playtime += (float)sz / output->fmt.samplerate /
              ((output->fmt.bps >> 3) * output->fmt.channels) * dspratio;
...
void streamer_set_volume_modifier(float (*modifier)(float delta_time)) {
  streamer_volume_modifier = modifier;
}

static void streamer_apply_soft_volume(char *bytes, int sz) {
  DB_output_t *output = plug_get_output();

  float mod = 1.f;

  if (streamer_volume_modifier) {
    float dt = sz * (output->fmt.bps >> 3) / output->fmt.channels /
               (float)output->fmt.samplerate;
    ...
    if (output->fmt.bps == 16) {
      ...
      if (ivolume != 1000) {
        ...
        for (int i = 0; i < third; i++) {
          int32_t sample = ((unsigned char)stream[0]) |
                           ((unsigned char)stream[1] << 8) |
                           ((signed char)stream[2] << 16);
...
static float *_get_temp_audio_buffer(size_t size) {
...
static void viz_process(char *restrict bytes, int bytes_size,
                        DB_output_t *output) {
  if (waveform_listeners || spectrum_listeners) {
    ...
    ddb_waveformat_t out_fmt = {.bps = 32,
    ...
    float *temp_audio_data =
        _get_temp_audio_buffer(in_frames * out_fmt.channels * sizeof(float));
    pcm_convert(&output->fmt, bytes, &out_fmt, (char *)temp_audio_data,
                bytes_size);
    ddb_audio_data_t waveform_data = {
        .fmt = &out_fmt, .data = temp_audio_data, .nframes = in_frames};
    ...
    if (spectrum_listeners) {
      ...
      do {
        ...
        for (int c = 0; c < audio_data_channels; c++) {
          for (int s = 0; s < sz; s++) {
            audio_data[DDB_FREQ_BANDS * 2 * c + audio_data_fill + s] =
                temp_audio_data[(in_frames - remaining + s) *
                                    audio_data_channels +
                                c];
          }
        }
        ...
        if (audio_data_fill == DDB_FREQ_BANDS * 2) {
          for (int c = 0; c < audio_data_channels; c++) {
            calc_freq(&audio_data[DDB_FREQ_BANDS * 2 * c],
                      &freq_data[DDB_FREQ_BANDS * c]);
          }
          ddb_audio_data_t spectrum_data = {
              .fmt = &out_fmt, .data = freq_data, .nframes = DDB_FREQ_BANDS};
          mutex_lock(wdl_mutex);
...
int streamer_read(char *bytes, int size) {
  ...
  if (!block) {
    ...
    if (!streaming_track) {
      ...
      // this message is printed in more cases, than if output is broken, so
      // disable it but keep for reference -- this is a good place to set
      // breakpoint
      //            fprintf (stderr, "streamer: streamer_read has starved. The
      //            current output plugin might be broken\n");
    }
    streamer_unlock();
  ...
  if (!memcmp(&block->fmt, &last_block_fmt, sizeof(ddb_waveformat_t))) {
    ...
    while (block != NULL && _outbuffer_remaining < size &&
...
int streamer_ok_to_read(int len) { return !streamer_is_buffering; }
...
void streamer_configchanged(void) {
  streamer_lock();
  ...
  char mapstr[2048];
  conf_get_str("network.ctmapping", DDB_DEFAULT_CTMAPPING, mapstr,
               sizeof(mapstr));
  ...
  int new_conf_streamer_override_samplerate =
      conf_get_int("streamer.override_samplerate", 0);
  int new_conf_streamer_use_dependent_samplerate =
      conf_get_int("streamer.use_dependent_samplerate", 0);
  int new_conf_streamer_samplerate =
      clamp_samplerate(conf_get_int("streamer.samplerate", 44100));
  int new_conf_streamer_samplerate_mult_48 =
      clamp_samplerate(conf_get_int("streamer.samplerate_mult_48", 48000));
  int new_conf_streamer_samplerate_mult_44 =
      clamp_samplerate(conf_get_int("streamer.samplerate_mult_44", 44100));

  if (conf_streamer_override_samplerate !=
          new_conf_streamer_override_samplerate ||
      conf_streamer_use_dependent_samplerate !=
          new_conf_streamer_use_dependent_samplerate ||
      conf_streamer_samplerate != new_conf_streamer_samplerate ||
      conf_streamer_samplerate_mult_48 !=
          new_conf_streamer_samplerate_mult_48 ||
      conf_streamer_samplerate_mult_44 !=
          new_conf_streamer_samplerate_mult_44 ||
      formatchanged) {
...
static void _play_track(playItem_t *it, int startpaused) {
  DB_output_t *output = plug_get_output();
  output->stop();
  streamer_reset(1);
...
static void play_index(int idx, int startpaused) {
  ...
  if (shuffle == DDB_SHUFFLE_ALBUMS) {
    ...
    // This ensures that the manually triggered item becomes first in shuffle
    // queue. It works because shufflerating is generated using rand(), which
    // gives only numbers in the [0..RAND_MAX] range.
    it->shufflerating = -1;
  }
...
playItem_t *streamer_get_current_track_to_play(playlist_t *plt) {
...
static void play_current(void) {
  ...
  if (output->state() == DDB_PLAYBACK_STATE_PAUSED && playing_track) {
    ...
    if (is_remote_stream(playing_track) &&
        pl_get_item_duration(playing_track) < 0) {
      streamer_reset(1);
...
playItem_t *streamer_get_next_track_with_direction(int dir,
                                                   ddb_shuffle_t shuffle,
                                                   ddb_repeat_t repeat) {
  ...
  if (!next && streamer_playlist->count[PL_MAIN] != 0) {
    if (repeat == DDB_REPEAT_OFF) {
      if (shuffle == DDB_SHUFFLE_ALBUMS || shuffle == DDB_SHUFFLE_TRACKS) {
        plt_reshuffle(streamer_playlist, dir > 0 ? &next : NULL,
                      dir < 0 ? &next : NULL);
        if (next && dir < 0) {
...
static void play_next(int dir, ddb_shuffle_t shuffle, ddb_repeat_t repeat) {
  ...
  playItem_t *next =
      streamer_get_next_track_with_direction(dir, shuffle, repeat);
...
void streamer_play_current_track(void) {
...
void streamer_set_current_playlist(int plt) {
...
void streamer_notify_playlist_deleted(playlist_t *plt) {
...
static void _streamer_mark_album_played_up_to(playItem_t *item) {
  ...
  while (next) {
    if (alb == pl_find_meta_raw(next, "album") &&
        art == pl_find_meta_raw(next, "artist")) {
      next->played = 1;
...
static void streamer_shuffle_changed(ddb_shuffle_t prev,
                                     ddb_shuffle_t shuffle) {
...
void streamer_set_streamer_playlist(playlist_t *plt) {
  pl_lock();
  if (streamer_playlist) {
    plt_unref(streamer_playlist);
  }
...
struct handler_s *streamer_get_handler(void) {
  return handler;
}

void streamer_set_playing_track(playItem_t *it) {
  if (it == playing_track) {
    return;
  }
...
void streamer_set_buffering_track(playItem_t *it) {
...
void streamer_yield(void) {
  while (handler_hasmessages(handler)) {
    usleep(50000);
  }
}

void streamer_set_output(DB_output_t *output) {
  if (mutex) {
    streamer_lock();
  }
...
void streamer_notify_track_deleted(void) {
