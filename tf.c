 ...
 * Integer functions: As with string functions. Returns a number in string
 * format. Boolean functions: Returns a positive value to indicate truthiness,
 *                    but with an empty output string (*out == 0).
 *
 * In any context, -1 indicates an error.
 * The output string is sometimes, but not always, set to the empty string (*out
 * == 0) in such cases. Use of TF_EVAL_CHECK will always ensure that the output
 * string is always null terminated in such cases, once reached.
 *
 * @param outlen Available bytes in `out`, not including space for any
 * terminating null.
 */
typedef int (*tf_func_ptr_t)(ddb_tf_context_t *ctx, int argc,
                             const uint16_t *arglens, const char *args,
                             char *out, int outlen, int fail_on_undef);
 ...
 * @param outlen Available bytes in the buffer `out`, not including the
 * terminating null byte.
 */
static int tf_eval_int(ddb_tf_context_t *ctx, const char *code, int size,
                       char *out, int outlen, int *bool_out, int fail_on_undef);

static const char *_tf_get_combined_value(playItem_t *it, const char *key,
                                          int *needs_free);
...
static int snprintf_clip(char *buf, size_t len, const char *fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
...
/*
 * @param outlen bytes available in the buffer `out`, including the terminating
 * null byte
 */
int tf_eval(ddb_tf_context_t *ctx, const char *code, char *out, int outlen) {
  ...
  case DB_COLUMN_FILENUMBER:
    if (ctx->flags & DDB_TF_CONTEXT_HAS_INDEX) {
      ...
      int idx = plt_get_item_idx((playlist_t *)ctx->plt, (playItem_t *)ctx->it,
                                 PL_MAIN);
...
int tf_func_greater(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                    const char *args, char *out, int outlen,
                    int fail_on_undef) {
  if (argc != 2) {
    return -1;
  }
...
int tf_func_strcmp(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                   const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 2) {
    return -1;
  }
  const char *arg = args;
...
int tf_func_stricmp(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                    const char *args, char *out, int outlen,
                    int fail_on_undef) {
  if (argc != 2) {
    return -1;
...
static int tf_prefix_helper(ddb_tf_context_t *ctx, int argc,
                            const uint16_t *arglens, const char *args,
                            char *out, int outlen, int fail_on_undef,
                            int swap) {
  if (argc == 0) {
  ...
  if (argc == 1) {
    ...
    for (i = 0; i < prefix_count; ++i) {
      prefixes[i] = ptr;
      TF_EVAL_CHECK(prefix_lengths[i], ctx, arg, arglens[i + 1], ptr,
                    (int)(buf + buffer_size - ptr - 1), fail_on_undef);
  ...
  for (i = 0; i < prefix_count; i++) {
    ...
    if (!strncasecmp(str, prefixes[i], prefix_lengths[i]) &&
        str[prefix_lengths[i]] == ' ') {
...
// $stripprefix(str,...) strips a list of prefixes from a string. If no prefixes
// are supplied, 'A' and 'The' are used.
int tf_func_stripprefix(ddb_tf_context_t *ctx, int argc,
                        const uint16_t *arglens, const char *args, char *out,
                        int outlen, int fail_on_undef) {
  return tf_prefix_helper(ctx, argc, arglens, args, out, outlen, fail_on_undef,
                          0);
}

// $swapprefix(str,...) moves a list of prefixes to the end of a string. If no
// prefixes are supplied, 'A' and 'The' are used.
int tf_func_swapprefix(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                       const char *args, char *out, int outlen,
                       int fail_on_undef) {
  return tf_prefix_helper(ctx, argc, arglens, args, out, outlen, fail_on_undef,
                          1);
...
int tf_func_upper(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                  const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1) {
    return -1;
  }
  ...
  int len;
  char temp_str[1000];
  TF_EVAL_CHECK(len, ctx, args, arglens[0], temp_str, sizeof(temp_str) - 1,
                fail_on_undef);
...
int tf_func_lower(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                  const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1) {
    return -1;
  }
...
// $num(n,len) Formats the integer number n in decimal notation with len
// characters. Pads with zeros from the left if necessary. len includes the dash
// when the number is negative. If n is not numeric, it is treated as zero.
int tf_func_num(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  const char *arg = args;
  int bool_out = 0;
  int len;
...
// substrings
int tf_func_replace(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                    const char *args, char *out, int outlen,
                    int fail_on_undef) {
  if (argc < 3 || argc % 2 == 0) {
    return -1;
  ...
  for (i = 0; i < argc; ++i) {
    lines[i] = ptr;
    TF_EVAL_CHECK(lens[i], ctx, arg, arglens[i], ptr,
                  (int)(buf + sizeof(buf) - ptr - 1), fail_on_undef);
    ptr += lens[i] + 1;
    arg += arglens[i];
  }
  ...
  for (;;) {
    int chunklen = (int)(lines[0] + lens[0] -
                         iptr); // chunk is a substring before the found needle
...
int tf_func_abbr(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                 const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1 && argc != 2) {
    return -1;
  }
  ...
  if (argc == 2) {
    ...
    arg += arglens[0];
    int l;
    TF_EVAL_CHECK(l, ctx, arg, arglens[1], num_chars_str,
                  sizeof(num_chars_str) - 1, fail_on_undef);
...
int tf_func_ansi(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                 const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1) {
    return -1;
  }
...
int tf_func_ascii(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                  const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1) {
    return -1;
  }
...
int tf_caps_impl(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                 const char *args, char *out, int outlen, int fail_on_undef,
                 int do_lowercasing) {
  if (argc != 1) {
    return -1;
...
int tf_func_caps(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                 const char *args, char *out, int outlen, int fail_on_undef) {
  return tf_caps_impl(ctx, argc, arglens, args, out, outlen, fail_on_undef, 1);
}

int tf_func_caps2(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                  const char *args, char *out, int outlen, int fail_on_undef) {
  return tf_caps_impl(ctx, argc, arglens, args, out, outlen, fail_on_undef, 0);
}

int tf_func_char(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                 const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1) {
    return -1;
  }
...
int tf_func_crc32(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                  const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1) {
    return -1;
  }
...
int tf_func_crlf(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                 const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 0 || outlen < 2) {
    return -1;
  }
...
int tf_func_left(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                 const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 2) {
    return -1;
  }
  ...
  arg += arglens[0];
  int len;
  TF_EVAL_CHECK(len, ctx, arg, arglens[1], num_chars_str,
                sizeof(num_chars_str) - 1, fail_on_undef);
  int num_chars = atoi(num_chars_str);
  ...
  char text[1000];
  arg = args;
  TF_EVAL_CHECK(len, ctx, arg, arglens[0], text, sizeof(text) - 1,
                fail_on_undef);
...
int tf_func_repeat(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                   const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 2) {
    return -1;
  }
...
int tf_func_insert(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                   const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 3) {
    return -1;
  }
  ...
  char str[1000];
  arg = args;
  TF_EVAL_CHECK(str_len, ctx, arg, arglens[0], str, sizeof(str) - 1,
                fail_on_undef);
  ...
  char insert[1000];
  arg += arglens[0];
  TF_EVAL_CHECK(insert_len, ctx, arg, arglens[1], insert, sizeof(insert) - 1,
                fail_on_undef);

  // get insertion point
  char num_chars_str[10];
  arg += arglens[1];
  TF_EVAL_CHECK(len, ctx, arg, arglens[2], num_chars_str,
                sizeof(num_chars_str) - 1, fail_on_undef);
...
int tf_func_len(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1) {
    return -1;
  }
...
int tf_func_len2(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                 const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1) {
    return -1;
  }
...
int tf_func_extremest_impl(ddb_tf_context_t *ctx, int argc,
                           const uint16_t *arglens, const char *args, char *out,
                           int outlen, int fail_on_undef, int shortest) {
  if (argc < 1) {
    return -1;
  }
  int bool_out = 0;
...
int tf_func_shortest(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                     const char *args, char *out, int outlen,
                     int fail_on_undef) {
  return tf_func_extremest_impl(ctx, argc, arglens, args, out, outlen,
                                fail_on_undef, 1);
}

int tf_func_longest(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                    const char *args, char *out, int outlen,
                    int fail_on_undef) {
  return tf_func_extremest_impl(ctx, argc, arglens, args, out, outlen,
                                fail_on_undef, 0);
...
int tf_func_longer(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                   const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 2) {
    return -1;
  }
...
// $pad(expr,len[, char]): If `expr` is shorter than len characters, the
// function adds `char` characters (if present otherwise spaces) to the right of
// `expr` to make the result `len` characters long. Otherwise the function
// returns str unchanged.
int tf_func_pad_impl(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                     const char *args, char *out, int outlen, int fail_on_undef,
                     int right, int cut) {
  if (argc < 1 || argc > 3) {
    return -1;
  ...
  if (argc == 3) {
    arg += arglens[1];
    TF_EVAL_CHECK(len, ctx, arg, arglens[2], pad_char_str,
                  sizeof(pad_char_str) - 1, fail_on_undef);
  ...
  if (str_chars >= padlen_chars) {
    if (str_chars > padlen_chars && cut) {
      // u8_strncpy has no limiter in byte units available. We rely on str being
      // of size outlen above for safety
...
int tf_func_pad(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  return tf_func_pad_impl(ctx, argc, arglens, args, out, outlen, fail_on_undef,
                          0, 0);
}

// $pad_right(expr,len[,char]): same as $pad but right aligns string
int tf_func_pad_right(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                      const char *args, char *out, int outlen,
                      int fail_on_undef) {
  return tf_func_pad_impl(ctx, argc, arglens, args, out, outlen, fail_on_undef,
                          1, 0);
...
int tf_func_padcut(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                   const char *args, char *out, int outlen, int fail_on_undef) {
  return tf_func_pad_impl(ctx, argc, arglens, args, out, outlen, fail_on_undef,
                          0, 1);
}

int tf_func_padcut_right(ddb_tf_context_t *ctx, int argc,
                         const uint16_t *arglens, const char *args, char *out,
                         int outlen, int fail_on_undef) {
  return tf_func_pad_impl(ctx, argc, arglens, args, out, outlen, fail_on_undef,
                          1, 1);
...
int tf_func_progress_impl(ddb_tf_context_t *ctx, int argc,
                          const uint16_t *arglens, const char *args, char *out,
                          int outlen, int fail_on_undef, int alt) {
  if (argc != 5) {
    return -1;
  }
  ...
  len = tf_eval_int(ctx, argpos, arglens[4], out, outlen - 1, &bool_out,
                    fail_on_undef);
  if (len < 0) {
    free(notch);
    *out = 0;
   ...
   * This adjustment sets what happens once we've reached the end, whether we
   * should:
  ...
  for (int i = 0; i != barsize; ++i) {
     ...
     * What would strictly precede the mark's normal mode position gets marked
     * in alt mode, with the exception of a full bar.
     */
...
int tf_func_progress(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                     const char *args, char *out, int outlen,
                     int fail_on_undef) {
  return tf_func_progress_impl(ctx, argc, arglens, args, out, outlen,
                               fail_on_undef, 0);
}

int tf_func_progress2(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                      const char *args, char *out, int outlen,
                      int fail_on_undef) {
  return tf_func_progress_impl(ctx, argc, arglens, args, out, outlen,
                               fail_on_undef, 1);
...
int tf_func_right(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                  const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 2) {
    return -1;
  }
  ...
  TF_EVAL_CHECK(len, ctx, args + arglens[0], arglens[1], out, outlen,
                fail_on_undef);
...
int tf_func_roman(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                  const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1) {
    return -1;
  }
...
int tf_func_rot13(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                  const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1) {
    return -1;
  }
...
int tf_func_strchr(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                   const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc < 1) {
    return -1;
  }
  ...
  TF_EVAL_CHECK(len, ctx, args + arglens[0], arglens[1], out, outlen,
                fail_on_undef);
  int dummy = 0;
  // fb2k permits strings with many characters as the needle, but uses exactly
  // the first character from them.
...
int tf_func_strrchr(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                    const char *args, char *out, int outlen,
                    int fail_on_undef) {
  if (argc < 1) {
    return -1;
  ...
  char str[10];
  TF_EVAL_CHECK(len, ctx, args + arglens[0], arglens[1], str, sizeof(str) - 1,
                fail_on_undef);
  int dummy = 0;
...
int tf_func_strstr(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                   const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 2) {
    return -1;
  }
  ...
  TF_EVAL_CHECK(len, ctx, args + arglens[0], arglens[1], needle,
                sizeof(needle) - 1, fail_on_undef);
...
// fb2k uses 1-based indexing for $substr, with both points inclusive, and all
// arguments mandatory.
int tf_func_substr(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                   const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 3) {
    return -1;
  }
  ...
  TF_EVAL_CHECK(len, ctx, args + arglens[0] + arglens[1], arglens[2], temp,
                outlen, fail_on_undef);
  int to = atoi(temp);
  free(temp);
  if (to <= 0) {
...
int tf_func_tab(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc > 1) {
    return -1;
  }
  ...
  if (argc == 1) {
    ...
    char str[10];
    TF_EVAL_CHECK(len, ctx, args, arglens[0], str, sizeof(str) - 1,
                  fail_on_undef);
    amount = atoi(str);
    if (amount < 0) {
      return -1;
...
int tf_func_trim(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                 const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc == 0) {
    return 0;
  }
...
int tf_func_directory(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                      const char *args, char *out, int outlen,
                      int fail_on_undef) {
  if (argc < 1 || argc > 2) {
    return -1;
  ...
  if (argc == 2) {
    char temp[20];
    args += arglens[0];
    TF_EVAL_CHECK(len, ctx, args, arglens[1], temp, sizeof(temp) - 1,
                  fail_on_undef);
    levels = atoi(temp);
    if (levels < 0) {
      return -1;
...
int tf_func_directory_path(ddb_tf_context_t *ctx, int argc,
                           const uint16_t *arglens, const char *args, char *out,
                           int outlen, int fail_on_undef) {
  if (argc < 1 || argc > 2) {
    return -1;
...
int tf_func_ext(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc < 1 || argc > 2) {
    return -1;
  }
...
int tf_func_filename(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                     const char *args, char *out, int outlen,
                     int fail_on_undef) {
  if (argc < 1 || argc > 2) {
    return -1;
...
int tf_func_add(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  int bool_out = 0;
...
int tf_func_div(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  int bool_out = 0;
...
int tf_func_max(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  int bool_out = 0;
...
int tf_func_min(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  int bool_out = 0;
...
int tf_func_mod(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  int bool_out = 0;
...
int tf_func_mul(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  int bool_out = 0;
...
int tf_func_muldiv(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                   const char *args, char *out, int outlen, int fail_on_undef) {
  int bool_out = 0;
...
int tf_func_rand(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                 const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 0) {
    return -1;
  }
...
int tf_func_sub(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  int bool_out = 0;
...
int tf_func_if(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
               const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc < 2 || argc > 3) {
    return -1;
  }
...
int tf_func_if2(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 2) {
    return -1;
  }
...
int tf_func_if3(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc < 2) {
    return -1;
  }
...
int tf_func_ifequal(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                    const char *args, char *out, int outlen,
                    int fail_on_undef) {
  if (argc != 4) {
    return -1;
...
int tf_func_ifgreater(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                      const char *args, char *out, int outlen,
                      int fail_on_undef) {
  if (argc != 4) {
    return -1;
...
int tf_func_iflonger(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                     const char *args, char *out, int outlen,
                     int fail_on_undef) {
  if (argc != 4) {
    return -1;
...
int tf_func_select(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                   const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc < 3) {
    return -1;
  }
...
static void tf_append_out(char **out, int *out_len, const char *in,
                          int in_len) {
...
int tf_func_meta(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                 const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1) {
    return -1;
  }
  ...
  int needs_free = 0;
  const char *meta =
      _tf_get_combined_value((playItem_t *)ctx->it, out, &needs_free);
  if (!meta) {
    return 0;
  }
...
const char *tf_get_channels_string_for_track(playItem_t *it) {
...
int tf_func_channels(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                     const char *args, char *out, int outlen,
                     int fail_on_undef) {
  if (argc != 0) {
    return -1;
...
// Boolean
int tf_func_and(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  int bool_out = 0;
...
int tf_func_or(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
               const char *args, char *out, int outlen, int fail_on_undef) {
  int bool_out = 0;
...
int tf_func_not(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1) {
    return -1;
  }
...
int tf_func_xor(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  int bool_out = 0;
  int result = 0;
...
int tf_func_fix_eol(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                    const char *args, char *out, int outlen,
                    int fail_on_undef) {
  if (argc != 1 && argc != 2) {
    return -1;
  ...
  if (argc == 2) {
    TF_EVAL_CHECK(indlen, ctx, args + arglens[0], arglens[1], ind, indlen - 1,
                  fail_on_undef);
  } else {
    strcpy(ind, " (...)");
...
int tf_func_hex(ddb_tf_context_t *ctx, int argc, const uint16_t *arglens,
                const char *args, char *out, int outlen, int fail_on_undef) {
  if (argc != 1 && argc != 2) {
    return -1;
  }
...
static int format_playback_time(char *out, int outlen, float t) {
  ...
  int len = 0;
  if (daystotal == 0) {
    len = snprintf_clip(out, outlen, "%d:%02d:%02d", hourtotal, mintotal,
                        sectotal);
  } else if (daystotal == 1) {
    len = snprintf_clip(out, outlen, _("1 day %d:%02d:%02d"), hourtotal,
                        mintotal, sectotal);
  } else {
    len = snprintf_clip(out, outlen, _("%d days %d:%02d:%02d"), daystotal,
...
static int tf_eval_int(ddb_tf_context_t *ctx, const char *code, int size,
  ...
  while (size) {
    if (*code) {
      ...
      if (*code == 1) {
        ...
        int res =
            func(ctx, numargs, arglens, code + 1 + numargs * sizeof(uint16_t),
                 out, outlen, fail_on_undef);
        if (res == -1) {
          return -1;
        }
        if (res > 0) {
        ...
        const char *val = NULL;
        int needs_free = 0;
        const char *aa_fields[] = {
            "album artist", "albumartist", "band", "artist",
            "composer",     "performer",   NULL};
        const char *a_fields[] = {"artist", "album artist", "albumartist",
                                  "band",   "composer",     "performer",
                                  NULL};
        ...
        if (!strcmp(name, aa_fields[0])) {
        ...
        } else if ((tmp_a = !strcmp(name, "playback_time")) ||
                   (tmp_b = !strcmp(name, "playback_time_seconds")) ||
            ...
            if (t >= 0) {
              ...
              if (tmp_a || tmp_c || tmp_e) {
                ...
                if (tmp_e) {
                  ...
                  if (hr) {
                    l = snprintf_clip(out, outlen, "%d:%02d:%02d.%03d", hr, mn,
                                      sc, ms);
                  } else {
        ...
        } else if ((tmp_a = !strcmp(name, "length")) ||
                   (tmp_b = !strcmp(name, "length_ex"))) {
          ...
          if (t >= 0) {
            int hr = (int)(t / 3600);
            int mn = (int)((t - hr * 3600) / 60);
            int sc = (int)(tmp_a ? t - hr * 3600 - mn * 60
                                 : t - hr * 3600 - mn * 60);
            ...
            if (tmp_a) {
              ...
              if (hr) {
                l = snprintf_clip(out, outlen, "%d:%02d:%02d.%03d", hr, mn, sc,
                                  ms);
              } else {
                l = snprintf_clip(out, outlen, "%d:%02d.%03d", mn, sc, ms);
              }
        ...
        } else if ((tmp_a = !strcmp(name, "length_seconds") ||
                            (tmp_b = !strcmp(name, "length_seconds_fp")))) {
          float t = pl_get_item_duration(it);
          if (t >= 0) {
            int l;
            if (tmp_a) {
          ...
          int l =
              snprintf_clip(out, outlen, "%lld",
                            pl_item_get_endsample((playItem_t *)ctx->it) -
                                pl_item_get_startsample((playItem_t *)ctx->it));
          out += l;
          outlen -= l;
          skip_out = 1;
        } else if ((tmp_a = !strcmp(name, "isplaying")) ||
                   (tmp_b = !strcmp(name, "ispaused"))) {
          playItem_t *playing = streamer_get_playing_track();

          if (playing && ((tmp_a && plug_get_output()->state() ==
                                        DDB_PLAYBACK_STATE_PLAYING) ||
                          (tmp_b && plug_get_output()->state() ==
        ...
        // index of track in playlist (zero-padded)
          ...
          if (it) {
            int total_tracks =
                plt_get_item_count((playlist_t *)ctx->plt, ctx->iter);
            int digits = 0;
            do {
              total_tracks /= 10;
              digits++;
        ...
        // total amount of tracks in queue
          ...
          float seltime =
              plt_get_selection_playback_time((playlist_t *)ctx->plt);
        ...
        int res =
            tf_eval_int(ctx, code, len, out, outlen, &ignored_bool_out, 1);
        if (res >= 0) {
          out += res;
          outlen -= res;
        ...
        if (ctx->flags & DDB_TF_CONTEXT_TEXT_DIM) {
          ...
          snprintf(undim, sizeof(undim), "\033%d;%dm", DDB_TF_ESC_DIM,
                   -(int)amount);
        ...
        int ignored_bool_out = 0;
        int res = tf_eval_int(ctx, code, len, out, outlen - dimlen,
                              &ignored_bool_out, fail_on_undef);
        if (res >= 0) {
          out += res;
          outlen -= res;
          count_true_conditionals++;
  ...
  *out = 0;
  ...
  if (fail_on_undef && count_false_conditionals > 0 &&
      count_true_conditionals == 0) {
    return -1;
  }
...
int tf_compile_plain(tf_compiler_t *c);

int tf_compile_func(tf_compiler_t *c) {
  c->i++;
  ...
  *(c->o++) = 0; // num args
  ...
  while (*(c->i)) {
    if (*(c->i) == ',' || *(c->i) == ')') {
      ...
      memmove(arglens + num_args + 1, arglens + num_args,
              c->o - start - num_args * sizeof(uint16_t));
      c->o += 2;
      // store arg length
      // FIXME: `arglens` comes in a byte stream without 16 bit alignment, this
      // may cause unaligned access and crash.
...
int tf_compile_field(tf_compiler_t *c) {
  c->i++;
  *(c->o++) = 0;
  *(c->o++) = 2;
...
int tf_compile_ifdef(tf_compiler_t *c) {
  c->i++;
  *(c->o++) = 0;
  *(c->o++) = 3;
...
int tf_compile_text_dim(tf_compiler_t *c) {
  *(c->o++) = 0;
  *(c->o++) = 5;
...
char *tf_compile(const char *script) {
  ...
  memset(out + 4 + size, 0,
         4); // FIXME: this is the padding for possible buffer overflow bug fix
  *((int32_t *)out) = (int32_t)(size);
  return out;
}

void tf_free(char *code) { free(code); }
