...
static void messagepump_reset(void);
...
int messagepump_push(uint32_t id, uintptr_t ctx, uint32_t p1, uint32_t p2) {
  if (!mfree) {
    // fprintf (stderr, "WARNING: message queue is full! message ignored (%d %p
    // %d %d)\n", id, (void*)ctx, p1, p2);
...
int messagepump_hasmessages(void) { return mqueue ? 1 : 0; }
...
ddb_event_t *messagepump_event_alloc(uint32_t id) {
  ...
  case DB_EV_SEEKED:
    ...
    break;
  default:
    trace("Invalid event %d to use with messagepump_event_alloc, use "
          "sendmessage instead\n",
          id);
    return NULL;
  }
