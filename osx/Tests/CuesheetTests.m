...
- (void)
    test_BogusEmbeddedImageCueInSingleTrack_ReturnsSingleTrackWithCorrectMeta {
  playlist_t *plt = plt_alloc("test");
...
- (void)test_ImageAndCue_Adds2TracksWithCorrectTitles {
  ...
  XCTAssertEqual(
      strcmp(pl_find_meta(plt->head[PL_MAIN], "title"), "Test Track 01"), 0);
  XCTAssertEqual(
      strcmp(pl_find_meta(plt->head[PL_MAIN]->next[PL_MAIN], "title"),
             "Test Track 02"),
      0);
