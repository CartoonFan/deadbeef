...
- (void)test_loadM3UFromBuffer_SimplePlaylist_Loads2Items {
    ...
    ddb_playItem_t *after =
        load_m3u_from_buffer(NULL, m3u, strlen(m3u), NULL, "", NULL, plt, NULL);
...
- (void)
    test_loadM3UFromBuffer_UnicodeCharactersNoDash_LoadsItemWithCorrectArtistTitle {
    char path[PATH_MAX];
    snprintf(path, sizeof(path), "%s/TestData/chirp-1sec.mp3", dbplugindir);
...
- (void)
    test_loadM3UFromBuffer_UnicodeCharactersWithDash_LoadsItemWithCorrectArtistTitle {
    char path[PATH_MAX];
    snprintf(path, sizeof(path), "%s/TestData/chirp-1sec.mp3", dbplugindir);
