...
// in realtime mode, simulate real time audio playback with sleeps calculated
// from bytes / samplerate
static int _realtime;

static int fakeout_callback(char *stream, int len);

static void fakeout_thread(void *context);

static int fakeout_init(void);

static int fakeout_free(void);

int fakeout_setformat(ddb_waveformat_t *fmt);

static int fakeout_play(void);

static int fakeout_stop(void);

static int fakeout_pause(void);

static int fakeout_unpause(void);

int fakeout_init(void) {
  trace("fakeout_init\n");
...
static int fakeout_callback(char *stream, int len) {
  if (!deadbeef->streamer_ok_to_read(len)) {
    memset(stream, 0, len);
    return 0; // NOTE: this is not what real output plugins should do, but
              // better for testing
  }
...
ddb_playback_state_t fakeout_get_state(void) { return state; }

int fakeout_plugin_start(void) { return 0; }

int fakeout_plugin_stop(void) { return 0; }
...
void fakeout_set_manual(int manual) { _manual = manual; }

void fakeout_consume(int nbytes) {
...
void fakeout_set_realtime(int realtime) { _realtime = realtime; }
