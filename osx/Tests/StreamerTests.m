...
- (void)test_Play2TracksNoLoop_Sends2SongChanged {
  ...
  //    DB_playItem_t *_sinewave = deadbeef->plt_insert_file2 (0,
  //    (ddb_playlist_t *)plt, NULL, "/sine.fake", NULL, NULL, NULL);
  //    DB_playItem_t *_squarewave = deadbeef->plt_insert_file2 (0,
  //    (ddb_playlist_t *)plt, _sinewave, "/square.fake", NULL, NULL, NULL);
...
// Start track A
// Start track B
// Monitor trackinfochanged events, and make sure that track A is never in
// "playing" state after track B started "buffering"
...
- (void)test_SwitchBetweenTracks_DoesNotJumpBackToPrevious {
  // for this test, we want "loop single" mode, to make sure first track is
  // playing when we start the 2nd one.
  streamer_set_repeat(DDB_REPEAT_SINGLE);

  playlist_t *plt = plt_alloc("testplt");
  // create two test fake tracks
  switchtest_tracks[0] = deadbeef->plt_insert_file2(
      0, (ddb_playlist_t *)plt, NULL, "/sine.fake", NULL, NULL, NULL);
  switchtest_tracks[1] =
      deadbeef->plt_insert_file2(0, (ddb_playlist_t *)plt, switchtest_tracks[0],
                                 "/square.fake", NULL, NULL, NULL);
