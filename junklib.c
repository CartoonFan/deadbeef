...
static const char *frame_mapping[] = {
    ...
    "artist", "TPE1", "TPE1", "TP1", "Artist", "disc", "TPOS", "TPOS", "TPA",
    "Disc", // NOTE: this is special case when writing id3v2
    "title", "TIT2", "TIT2", "TT2", "Title", "album", "TALB", "TALB", "TAL",
    "Album", "copyright", "TCOP", "TCOP", "TCO", "Copyright", "genre", "TCON",
    ...
    "TDLY", "TDY", NULL, "TEXT_WRITERS", "TEXT", "TEXT", "TXT", NULL,
    "FILE_TYPE", "TFLT", "TFLT", "TFT", NULL, "CONTENT_GROUP_DESCRIPTION",
    "TIT1", "TIT1", "TT1", NULL, "SUBTITLE", "TIT3", "TIT3", "TT3", NULL,
    "INITIAL_KEY", "TKEY", "TKEY", "TKE", NULL, "LANGUAGE", "TLAN", "TLAN",
    "TLA", NULL, "LENGTH", "TLEN", "TLEN", "TLE", NULL, "MEDIA TYPE", "TMED",
    "TMED", "TMT", NULL, "ORIGINAL_ALBUM_TITLE", "TOAL", "TOAL", "TOT", NULL,
    "ORIGINAL_FILENAME", "TOFN", "TOFN", "TOF", NULL, "ORIGINAL_TEXT_WRITERS",
    "TOLY", "TOLY", "TOL", NULL, "ORIGINAL_ARTISTS", "TOPE", "TOPE", "TOA",
    NULL, "FILE_OWNER", "TOWN", "TOWN", NULL, NULL, "PERFORMER_REFINEMENT",
    "TPE3", "TPE3", "TP3", NULL, "MODIFIED_BY", "TPE4", "TPE4", "TP4", NULL,
    "PUBLISHER", "TPUB", "TPUB", "TPB", NULL, "INTERNET_RADIO_STATION_NAME",
    "TRSN", "TRSN", NULL, NULL, "INTERNET_RADIO_STATION_OWNER", "TRSO", "TRSO",
    NULL, NULL, "ISRC", "TSRC", "TSRC", NULL, NULL,
    "ENCODING_SOFTWARE_HARDWARE", "TSSE", "TSSE", "TSS", NULL, "RECORDING_TIME",
    NULL, "TDRC", NULL, NULL, "RELEASE_TIME", NULL, "TDRL", NULL, NULL,
    "TAGGING_TIME", NULL, "TDTG", NULL, NULL, "ALBUM_SORT_ORDER", NULL, "TSOA",
    NULL, NULL, "PERFORMER_SORT_ORDER", NULL, "TSOP", NULL, NULL,
    "TITLE_SORT_ORDER", NULL, "TSOT", NULL, NULL, "SIZE", "TSIZ", NULL, "TSI",
    NULL, "RECORDING_DATES", "TRDA", NULL, "TRD", NULL, "INVOLVED_PEOPLE_LIST",
    NULL, "TIPL", NULL, NULL, "MUSICIAN_CREDITS_LIST", NULL, "TMCL", NULL, NULL,
    "ENCODING_TIME", NULL, "TDEN", NULL, NULL, "ORIGINAL_RELEASE_TIME", NULL,
    "TDOR", NULL, NULL, "ORIGINAL_RELEASE_YEAR", NULL, NULL, NULL,
    "ORIGINALYEAR", "MOOD", NULL, "TMOO", NULL, NULL, "PRODUCED_NOTICE", NULL,
    "TPRO", NULL, NULL, "musicbrainz_trackid", NULL, NULL, NULL, NULL, "rating",
...
const char *ddb_internal_rg_keys[] = {
    ":REPLAYGAIN_ALBUMGAIN", ":REPLAYGAIN_ALBUMPEAK", ":REPLAYGAIN_TRACKGAIN",
    ":REPLAYGAIN_TRACKPEAK", NULL};

static const char wmp_popm_email[] = "Windows Media Player 9 Series";

static inline uint32_t extract_i32_le(unsigned char *buf) {
  uint32_t x;
...
static inline uint16_t extract_i16(const uint8_t *buf) {
  uint16_t x;
  // big endian extract
...
void _split_multivalue(char *text, size_t text_size) {
...
static const char *_get_combined_meta_value(DB_metaInfo_t *meta,
...
static int cp1251_to_utf8(const uint8_t *in, int inlen, uint8_t *out,
                          int outlen) {
...
int junk_cp1252_to_utf8(const uint8_t *in, int inlen, uint8_t *out,
                        int outlen) {
  int len = 0;
...
int junk_utf8_to_cp1252(const uint8_t *in, int inlen, uint8_t *out,
                        int outlen) {
  uint8_t *outptr = out;
    ...
    if (*in < 0x80) {
      *out++ = *in++;
      ...
      for (i = 0; cp1252_charset[i]; i++) {
        if (strlen(cp1252_charset[i]) == idx &&
            !memcmp(in, cp1252_charset[i], idx)) {
          *out++ = i + 0x80;
          outlen--;
          break;
        }
...
int junk_utf8_to_ascii(const uint8_t *in, int inlen, uint8_t *out, int outlen) {
  uint8_t *outptr = out;
  while (inlen && outlen > 0) {
    if (*in < 0x80) {
      *out++ = *in++;
...
ConversionResult ConvertUTF16BEtoUTF8(const UTF16 **sourceStart,
  ...
  ConversionResult res =
      ConvertUTF16toUTF8(&leStart, pLE, targetStart, targetEnd, flags);
  *sourceStart += pLE - sourceLE;
  return res;
}

ConversionResult ConvertUTF8toUTF16BE(const UTF8 **sourceStart,
                                      const UTF8 *sourceEnd,
                                      UTF16 **targetStart, UTF16 *targetEnd,
                                      ConversionFlags flags) {
  UTF16 *output = *targetStart;
  ConversionResult res =
      ConvertUTF8toUTF16(sourceStart, sourceEnd, targetStart, targetEnd, flags);
  if (res == conversionOK) {
...
int ddb_iconv(const char *cs_out, const char *cs_in, char *out, int outlen,
  ...
  if (!strcasecmp(cs_out, UTF8_STR)) {
    if (!strcasecmp(cs_in, UTF8_STR)) {
      ...
      len = cp1251_to_utf8(in, inlen, out, outlen);
               ...
               !strcasecmp(cs_in, "cp1252")) {
      len = junk_cp1252_to_utf8(in, inlen, out, outlen);
    } else if (!strcasecmp(cs_in, "UTF-16LE") ||
               !strcasecmp(cs_in, "UCS-2LE")) {
      char *target = out;
      ConversionResult result = ConvertUTF16toUTF8(
          (const UTF16 **)&in, (const UTF16 *)(in + inlen), (UTF8 **)&target,
          (UTF8 *)(out + outlen), strictConversion);
      if (result == conversionOK) {
        *target = 0;
        len = target - out;
      }
    } else if (!strcasecmp(cs_in, "UTF-16BE") ||
               !strcasecmp(cs_in, "UCS-2BE")) {
      ...
      while (inlen > 0 && len < outlen) {
        if (*in > 0) {
          *out++ = *in++;
          ...
          if (sj_to_unicode[i]) {
            // slow conversion!
            char unicode_val[2] = {(sj_to_unicode[i + 1] & 0xff00) >> 8,
                                   sj_to_unicode[i + 1] & 0xff};
            char utf8_val[5];
            char *src = unicode_val, *dst = utf8_val;

            ConversionResult res = ConvertUTF16toUTF8(
                (const UTF16 **)&src, (const UTF16 *)(src + 2), (UTF8 **)&dst,
                dst + 5, strictConversion);
            if (res == conversionOK) {
    ...
    if (!strcasecmp(cs_out, "UTF-16LE") || !strcasecmp(cs_out, "UCS-2LE")) {
      ...
      ConversionResult result = ConvertUTF8toUTF16(
          (const UTF8 **)&in, (const UTF8 *)(in + inlen), (UTF16 **)&target,
          (UTF16 *)(out + outlen), strictConversion);
      if (result == conversionOK) {
        *target = 0;
        *(target + 1) = 0;
        len = target - out;
      }
    } else if (!strcasecmp(cs_out, "UTF-16BE") ||
               !strcasecmp(cs_out, "UCS-2BE")) {
      char *target = out;
    ...
    } else if (!strcasecmp(cs_out, "cp1252") ||
               !strcasecmp(cs_out, "iso8859-1")) {
      int res =
          junk_utf8_to_cp1252((uint8_t *)in, inlen, (uint8_t *)out, outlen);
      if (res >= 0) {
        len = res;
      }
    } else if (!strcasecmp(cs_out, "ascii")) {
      int res =
          junk_utf8_to_ascii((uint8_t *)in, inlen, (uint8_t *)out, outlen);
      if (res >= 0) {
        len = res;
      }
    } else {
...
// NOTE: this function must support utf8->utf8 conversion, used for validation
  ...
  // trace ("iconv -f %s -t %s '%s': returned %d, inbytes %d/%d, outbytes %d/%d,
  // errno=%d\n", cs_in, cs_out, in, (int)res, inlen, (int)inbytesleft, outlen,
  // (int)outbytesleft, err);
  if (res == -1) {
    return -1;
  }
  out[pout - out] = 0;
...
static int can_be_russian(const signed char *str, int size) {
...
static int can_be_chinese(const uint8_t *str, int sz) {
  if (!enable_cp936_detection) {
    return 0;
  }
  size_t len = strlen(str);
  for (int i = 0; i < sz; str++, i++) {
    if (i < len - 3 && (*str >= 0x81 && *str <= 0xFE) &&
...
static int can_be_shift_jis(const unsigned char *str, int size) {
  if (!size) {
    return 0;
  }
  ...
  if (s >= 2) {
    if (junk_iconv(str, size, out, (int)sizeof(out), "shift-jis", UTF8_STR) >=
        0) {
      return 1;
    }
  }
...
static char *convstr_id3v2(const char *sb_charset, int version,
                           uint8_t encoding, const uint8_t *str, int sz,
                           int *out_size) {
  const char *enc = NULL;
...
static const char *convstr_id3v1(const char *str, int sz, const char *charset,
...
static void str_trim_right(uint8_t *str, int len) {
  uint8_t *p = str + len - 1;
  while (p >= str && *p <= 0x20) {
    p--;
  }
...
int junk_id3v1_read_int(playItem_t *it, char *buffer, const char **charset) {
  const char *cs = NULL;
  ...
  if (*title) {
    pl_add_meta(it, "title",
                convstr_id3v1(title, (int)strlen(title), *charset, utf8_value,
                              sizeof(utf8_value)));
  }
  if (*artist) {
    pl_add_meta(it, "artist",
                convstr_id3v1(artist, (int)strlen(artist), *charset, utf8_value,
                              sizeof(utf8_value)));
  }
  if (*album) {
    pl_add_meta(it, "album",
                convstr_id3v1(album, (int)strlen(album), *charset, utf8_value,
                              sizeof(utf8_value)));
  }
  if (*year) {
    pl_add_meta(it, "year",
                convstr_id3v1(year, (int)strlen(year), *charset, utf8_value,
                              sizeof(utf8_value)));
  }
  if (*comment) {
    pl_add_meta(it, "comment",
                convstr_id3v1(comment, (int)strlen(comment), *charset,
                              utf8_value, sizeof(utf8_value)));
  }
  if (genre && *genre) {
    pl_add_meta(it, "genre",
                convstr_id3v1(genre, (int)strlen(genre), *charset, utf8_value,
                              sizeof(utf8_value)));
  }
  if (tracknum != 0) {
    char s[4];
...
int junk_id3v1_read(playItem_t *it, DB_FILE *fp) {
  uint8_t id3[128];
...
static void id3v2_conv(playItem_t *it, const char *name, char store[],
                       size_t store_len, const char *enc) {
  memset(store, 0x20, store_len);
  const char *meta = pl_find_meta(it, name);
  if (meta) {
    char temp[1000];
    int l = junk_iconv(meta, (int)strlen(meta), temp, (int)sizeof(temp),
                       UTF8_STR, enc);
    if (l == -1) {
...
int junk_id3v1_write2(int fd, playItem_t *it, const char *enc) {
  char title[30] = "";
  char artist[30] = "";
  char album[30] = "";
  char year[4] = "";
...
int junk_id3v1_write(FILE *fp, playItem_t *it, const char *enc) {
  char title[30] = "";
  char artist[30] = "";
  char album[30] = "";
  char year[4] = "";
...
int64_t junk_id3v1_find2(DB_FILE *fp) {
  uint8_t buffer[3];
  if (deadbeef->fseek(fp, -128, SEEK_END) == -1) {
    return -1;
  }
...
int64_t junk_apev2_find2(DB_FILE *fp, int32_t *psize, uint32_t *pflags,
                         uint32_t *pnumitems) {
  uint8_t header[32];
...
int junk_apev2_find(DB_FILE *fp, int32_t *psize, uint32_t *pflags,
                    uint32_t *pnumitems) {
...
int junk_find_id3v1(DB_FILE *fp) {
  if (deadbeef->fseek(fp, -128, SEEK_END) == -1) {
    return -1;
  }
  char buffer[3];
...
int junk_add_track_meta(playItem_t *it, const char *track) {
...
int junk_add_disc_meta(playItem_t *it, const char *disc) {
...
int junk_apev2_add_frame(playItem_t *it, DB_apev2_tag_t *tag_store,
                         DB_apev2_frame_t **tail, uint32_t itemsize,
                         uint32_t itemflags, const char *key,
                         const uint8_t *value) {
  if (tag_store) {
  ...
  if (it) {
    ...
    if (valuetype == 0 && (itemsize < MAX_TEXT_FRAME_SIZE ||
                           (!strcasecmp(key, "cuesheet") &&
                            itemsize < MAX_CUESHEET_FRAME_SIZE))) {
      ...
      for (m = 0; frame_mapping[m]; m += FRAME_MAPPINGS) {
        if (frame_mapping[m + MAP_APEV2] &&
            !strcasecmp(key, frame_mapping[m + MAP_APEV2])) {
          if (!strcmp(frame_mapping[m + MAP_DDB], "track")) {
            ...
            pl_append_meta_full(it, frame_mapping[m + MAP_DDB], value,
                                itemsize + 1);
          }
          break;
        }
...
int junk_apev2_read_full_mem(playItem_t *it, DB_apev2_tag_t *tag_store,
  ...
  for (i = 0; i < numitems; i++) {
    ...
    while (keysize <= 255 && mem < end) {
      ...
      if (key[keysize] < 0x20) {
        ...
        return -1; // non-ascii chars and chars with codes 0..0x1f not allowed
                   // in ape item keys
      }
      keysize++;
    }
    ...
    if (itemsize <= MAX_APEV2_FRAME_SIZE) // just a sanity check
    {
      ...
      junk_apev2_add_frame(it, tag_store, &tail, itemsize, itemflags, key,
                           value);
...
int junk_apev2_read(playItem_t *it, DB_FILE *fp) {
  return junk_apev2_read_full(it, NULL, fp);
}

int junk_apev2_read_mem(playItem_t *it, char *mem, int size) {
...
int junk_id3v2_find(DB_FILE *fp, int *psize) {
  ...
  // check for bad size
  if ((header[9] & 0x80) || (header[8] & 0x80) || (header[7] & 0x80) ||
      (header[6] & 0x80)) {
    return -1; // bad header
  }
  uint32_t size = (header[9] << 0) | (header[8] << 7) | (header[7] << 14) |
                  (header[6] << 21);
...
int junk_id3v2_unsync(uint8_t *out, int len, int maxlen) {
  uint8_t buf[maxlen];
  uint8_t *p = buf;
  int res = -1;
  for (int i = 0; i < len; i++) {
...
static DB_id3v2_frame_t *junk_id3v2_append_frame(DB_id3v2_tag_t *tag,
                                                 DB_id3v2_frame_t *f) {
...
DB_id3v2_frame_t *junk_id3v2_add_text_frame2(DB_id3v2_tag_t *tag,
                                             const char *frame_id,
                                             const char *value, size_t inlen) {
  ...
  if (tag->version[0] == 4) {
    ...
    out = malloc(bufsize);
    outlen =
        junk_iconv(value, (int)inlen, out, (int)bufsize, UTF8_STR, "cp1252");
    if (outlen == -1) {
      outlen = junk_iconv(value, (int)inlen, out + 2, (int)(bufsize - 2),
                          UTF8_STR, "UCS-2LE");
      if (outlen <= 0) {
        return NULL;
      }
...
DB_id3v2_frame_t *junk_id3v2_add_text_frame(DB_id3v2_tag_t *tag,
                                            const char *frame_id,
                                            const char *value) {
  return junk_id3v2_add_text_frame2(tag, frame_id, value, strlen(value));
}

static void _id3v2_append_combined_text_frame_from_meta(DB_id3v2_tag_t *id3v2,
                                                        const char *key,
                                                        DB_metaInfo_t *meta) {
  int out_size;
  ...
  if (id3v2->version[0] == 4) {
    ...
    tag_value =
        _get_combined_meta_value(meta, &out_size, " / ", 3, &needs_free);
  } else {
    assert(0);
  }
...
DB_id3v2_frame_t *junk_id3v2_add_comment_frame(DB_id3v2_tag_t *tag,
                                               const char *frame_id,
                                               const char *lang,
                                               const char *descr,
                                               const char *value) {
  ...
  if (tag->version[0] == 4) {
    ...
    l = junk_iconv(input, (int)inputsize, buffer, (int)buffersize, UTF8_STR,
                   "cp1252");
    if (l <= 0) {
      l = junk_iconv(input, (int)inputsize, buffer + 2, (int)buffersize - 2,
                     UTF8_STR, "UCS-2LE");
      if (l <= 0) {
...
int junk_id3v2_remove_txxx_frame(DB_id3v2_tag_t *tag, const char *key) {
  DB_id3v2_frame_t *prev = NULL;
  for (DB_id3v2_frame_t *f = tag->frames; f;) {
    DB_id3v2_frame_t *next = f->next;
    if (!strcmp(f->id, "TXXX")) {
      char *txx = convstr_id3v2(NULL, tag->version[0], f->data[0], f->data + 1,
                                f->size - 1, NULL);
...
int junk_id3v2_remove_all_txxx_frames(DB_id3v2_tag_t *tag) {
  DB_id3v2_frame_t *prev = NULL;
  for (DB_id3v2_frame_t *f = tag->frames; f;) {
    DB_id3v2_frame_t *next = f->next;
    if (!strcmp(f->id, "TXXX")) {
...
static DB_id3v2_frame_t *junk_id3v2_add_txxx_frame(DB_id3v2_tag_t *tag,
                                                   const char *key,
                                                   const char *value,
                                                   size_t valuelen) {
  ...
  if (tag->version[0] == 4) {
    ...
    res =
        junk_iconv(buffer, (int)len, out, (int)outsize, UTF8_STR, "iso8859-1");
    if (res == -1) {
      res = junk_iconv(buffer, (int)len, out + 2, (int)(outsize - 2), UTF8_STR,
                       "UCS-2LE");
      if (res == -1) {
        return NULL;
      }
...
// TODO: 2.4 TDTG frame (tagging time) should not be converted, but might be
// useful to create it
int junk_id3v2_convert_24_to_23(DB_id3v2_tag_t *tag24, DB_id3v2_tag_t *tag23) {
  ...
  const char *copy_frames[] = {"AENC", "APIC", "COMR", "ENCR", "ETCO", "GEOB",
                               "GRID", "LINK", "MCDI", "MLLT", "OWNE", "PRIV",
                               "POPM", "POSS", "RBUF", "RVRB", "SYLT", "SYTC",
                               "UFID", "USER", "USLT", NULL};
  ...
  const char *text_frames[] = {
      "TALB", "TBPM", "TCOM", "TCON", "TCOP", "TDLY", "TENC", "TEXT", "TFLT",
  ...
  for (f24 = tag24->frames; f24; f24 = f24->next) {
    ...
    if (!simplecopy && !text) {
      if (!strcmp(f24->id, "COMM") || !strcmp(f24->id, "USLT")) {
        ...
        int outsize = 0;
        char *descr =
            convstr_id3v2(NULL, 4, enc, f24->data + 4, f24->size - 4, &outsize);
        if (!descr) {
          ...
          if (*value != 0) {
            ...
            *value = 0;
            value++;
            f23 = junk_id3v2_add_comment_frame(tag23, f24->id, lang, descr,
                                               value);
            if (f23) {
              tail = f23;
              f23 = NULL;
    ...
    // 2.4 format is %0h00kmnp (6:grouping, 3:compression, 2:encryption,
    // 1:unsync, 0:datalen) 2.3 format is %ijk00000 (7:compression,
    // 6:encryption, 5:grouping)
    flags[1] = 0;
    if (f24->flags[1] & (1 << 6)) {
      flags[1] |= (1 << 5);
    }
    ...
    if (simplecopy) {
      ...
      int decoded_size;
      char *decoded = convstr_id3v2(NULL, 4, f24->data[0], f24->data + 1,
                                    f24->size - 1, &decoded_size);
      if (!decoded) {
      ...
      if (!strcmp(f24->id, "TDRC")) {
        ...
        if (c == 3) {
          ...
          trace("junk_id3v2_add_text_frame: 2.4 TDRC doesn't have month/day "
                "info; discarded\n");
        }
        ...
        if (c == 1) {
          ...
          trace("junk_id3v2_add_text_frame: 2.4 TDOR doesn't have month/day "
                "info; discarded\n");
        }
      } else if (f24->id[0] == 'T') {
        for (int f = 0; frame_mapping[f]; f += FRAME_MAPPINGS) {
          if (frame_mapping[f + MAP_ID3V24] &&
              !strcmp(frame_mapping[f + MAP_ID3V24], f24->id)) {
            if (!frame_mapping[f + MAP_ID3V23]) {
              f23 = junk_id3v2_add_text_frame(tag23, frame_mapping[f + MAP_DDB],
                                              decoded);
            }
            break;
          }
...
int junk_id3v2_convert_23_to_24(DB_id3v2_tag_t *tag23, DB_id3v2_tag_t *tag24) {
  ...
  const char *copy_frames[] = {"AENC", "APIC", "COMM", "COMR", "ENCR", "ETCO",
                               "GEOB", "GRID", "LINK", "MCDI", "MLLT", "OWNE",
                               "PRIV", "POPM", "POSS", "RBUF", "RVRB", "SYLT",
                               "SYTC", "UFID", "USER", "USLT", NULL};
  ...
  for (f23 = tag23->frames; f23; f23 = f23->next) {
    ...
    if (simplecopy) {
      ...
      int decoded_size;
      char *decoded = convstr_id3v2(NULL, 3, f23->data[0], f23->data + 1,
                                    f23->size - 1, &decoded_size);
      if (!decoded) {
      ...
      if (!strcmp(f23->id, "TDRC")) {
        ...
        if (!added) {
          f24 =
              junk_id3v2_add_text_frame2(tag24, f23->id, decoded, decoded_size);
          if (f24) {
            tail = f24;
            f24 = NULL;
          }
...
int junk_id3v2_convert_22_to_24(DB_id3v2_tag_t *tag22, DB_id3v2_tag_t *tag24) {
  ...
  const char *copy_frames[] = {"BUF", "COM", "CRA", "ETC", "GEO",
                               "IPL", "MCI", "MLL", "POP", "REV",
                               "SLT", "STC", "UFI", "ULT", NULL};

  const char *copy_frames_24[] = {"RBUF", "COMM", "AENC", "ETCO", "GEOB",
                                  "TIPL", "MCDI", "MLLT", "POPM", "RVRB",
                                  "SYLT", "SYTC", "UFID", "USLT", NULL};
  ...
  // NOTE: PIC is discarded (needs conversion from custom image-format field to
  // mime-type) NOTE: RVA is discarded (subjective, and difficult to convert to
  // RVA2)

  const char *text_frames[] = {"TAL", "TBP", "TCM", "TCO", "TCR", "TDY", "TEN",
                               "TFT", "TKE", "TLA", "TLE", "TMT", "TOA", "TOF",
                               "TOL", "TOT", "TP1", "TP2", "TP3", "TP4", "TPA",
                               "TPB", "TRC", "TRK", "TSS", "TT1", "TT2", "TT3",
                               "TXT", "TXX", "TOR", NULL};

  const char *text_frames_24[] = {
      "TALB", "TBPM", "TCOM", "TCON", "TCOP", "TDLY", "TENC", "TFLT",
  ...
  for (f22 = tag22->frames; f22; f22 = f22->next) {
    ...
    if (simplecopy != -1) {
      ...
      // decode text into utf8
      int decoded_size;
      char *decoded = convstr_id3v2(NULL, 2, f22->data[0], f22->data + 1,
                                    f22->size - 1, &decoded_size);
      if (!decoded) {
        trace("junk_id3v2_convert_23_to_24: failed to decode text frame %s\n",
              f22->id);
        continue; // failed, discard it
      ...
      // encode for 2.4
      f24 = junk_id3v2_add_text_frame2(tag24, text_frames_24[text], decoded,
                                       decoded_size);
      if (f24) {
        tail = f24;
        f24 = NULL;
...
int junk_apev2_remove_all_text_frames(DB_apev2_tag_t *tag) {
  DB_apev2_frame_t *prev = NULL;
  for (DB_apev2_frame_t *f = tag->frames; f;) {
    DB_apev2_frame_t *next = f->next;
...
DB_apev2_frame_t *junk_apev2_add_text_frame2(DB_apev2_tag_t *tag,
                                             const char *frame_id,
...
DB_apev2_frame_t *junk_apev2_add_text_frame(DB_apev2_tag_t *tag,
                                            const char *frame_id,
                                            const char *value) {
  return junk_apev2_add_text_frame2(tag, frame_id, value, strlen(value));
}

static void _apev2_append_combined_text_frame_from_meta(DB_apev2_tag_t *apev2,
                                                        const char *key,
                                                        DB_metaInfo_t *meta) {
  int out_size;
  int needs_free;
  const char *tag_value =
      _get_combined_meta_value(meta, &out_size, "\0", 1, &needs_free);
...
int junk_id3v2_convert_apev2_to_24(DB_apev2_tag_t *ape, DB_id3v2_tag_t *tag24) {
  ...
  const char *text_keys[] = {"Title",     "Subtitle",  "Artist", "Album",
                             "Publisher", "Conductor", "Track",  "Composer",
                             "Copyright", "Genre",     "Disc",   "ISRC",
                             "Language",  "Year",      NULL};

  const char *text_keys_24[] = {"TIT2", "TIT3", "TPE1", "TALB", "TPUB",
                                "TPE3", "TRCK", "TCOM", "TCOP", "TCON",
                                "TPOS", "TSRC", "TLAN", "TDRC"};

  const char *comm_frames[] = {"Comment",          "Unsynced lyrics",
                               "EAN/UPC",          "ISBN",
                               "Catalog",          "LC",
                               "Publicationright", "Record Location",
                               "Related",          "Abstract",
                               "Bibliography",     NULL};

  // FIXME: additional frames: File->WOAF
  // converted to COMM: Comment, Unsynced Lyrics, EAN/UPC, ISBN, Catalog, LC,
  // Publicationright, Record Location, Related, Abstract, Bibliography "Debut
  // album" is discarded "Index" is discarded "Introplay" is discarded
  ...
  for (f_ape = ape->frames; f_ape; f_ape = f_ape->next) {
    ...
    if (text_keys[i]) {
      ...
      str[f_ape->size] = 0;
      f24 =
          junk_id3v2_add_text_frame2(tag24, text_keys_24[i], str, f_ape->size);
      if (f24) {
        tail = f24;
        f24 = NULL;
      }
      ...
      for (i = 0; comm_frames[i]; i++) {
        if (!strcasecmp(f_ape->key, comm_frames[i])) {
          ...
          if (!strcasecmp(f_ape->key, "Comment")) {
            ...
            junk_id3v2_add_comment_frame(tag24, "COMM", "eng", comm_frames[i],
                                         str);
          }
          break;
        }
...
int junk_apev2_write_i32_le2(int fd, uint32_t data) {
  int shift = 0;
  for (int i = 0; i < 4; i++) {
...
int junk_apev2_write_i32_le(FILE *fp, uint32_t data) {
  int shift = 0;
  for (int i = 0; i < 4; i++) {
    uint8_t d = (data >> shift) & 0xff;
...
int junk_apev2_write2(int fd, DB_apev2_tag_t *tag, int write_header,
                      int write_footer) {
...
int junk_apev2_write(FILE *fp, DB_apev2_tag_t *tag, int write_header,
                     int write_footer) {
  // calc size and numitems
  uint32_t numframes = 0;
  uint32_t size = 0;
...
int junk_id3v2_write2(int out, DB_id3v2_tag_t *tag) {
  if (tag->version[0] < 3) {
  ...
  for (DB_id3v2_frame_t *f = tag->frames; f; f = f->next) {
    ...
    if (write(out, frame_size, 4) != 4) {
      fprintf(stderr,
              "junk_write_id3v2: failed to write frame size, id %s, size %d\n",
              f->id, f->size);
      goto error;
    }
    if (write(out, f->flags, 2) != 2) {
      fprintf(stderr,
              "junk_write_id3v2: failed to write frame header flags, id %s, "
              "size %d\n",
              f->id, f->size);
      goto error;
    }
    if (write(out, f->data, f->size) != f->size) {
      fprintf(stderr,
              "junk_write_id3v2: failed to write frame data, id %s, size %d\n",
              f->id, f->size);
      goto error;
    }
    sz += f->size;
...
int junk_id3v2_write(FILE *out, DB_id3v2_tag_t *tag) {
  if (tag->version[0] < 3) {
    fprintf(stderr, "junk_write_id3v2: writing id3v2.2 is not supported\n");
    return -1;
  }
...
static int junklib_id3v2_sync_frame(int version_major, uint8_t *data,
                                    int size) {
  char *writeptr = data;
  int written = 0;
  while (size > 0) {
...
char *junk_append_meta(const char *old, const char *new) {
...
int junk_load_comm_frame(int version_major, const char *field_name,
                         playItem_t *it, uint8_t *readptr, int synched_size) {
...
#pragma unused(lang)
  ...
  int outsize = 0;
  char *descr = convstr_id3v2(NULL, version_major, enc, readptr + 4,
                              synched_size - 4, &outsize);
  if (!descr) {
    trace("failed to decode COMM frame, probably wrong encoding (%d)\n", enc);
    return -1;
  }
...
static int junk_id3v2_load_rva2(int version_major, playItem_t *it,
                                uint8_t *readptr, int synched_size) {
  ...
  int16_t volume_adjust =
      (int16_t)(((int16_t)rva_data[1] << 8) |
                rva_data[2]); /* this is little-endian safe :) */
  ...
  if (!strcasecmp(rva_desc, "album")) {
    if (!pl_find_meta(it, ddb_internal_rg_keys[DDB_REPLAYGAIN_ALBUMGAIN])) {
      pl_set_item_replaygain(it, DDB_REPLAYGAIN_ALBUMGAIN,
                             (float)volume_adjust / 512.0);
    }
    if (!pl_find_meta(it, ddb_internal_rg_keys[DDB_REPLAYGAIN_ALBUMPEAK]) &&
        peak_val) {
    ...
    if (!pl_find_meta(it, ddb_internal_rg_keys[DDB_REPLAYGAIN_TRACKGAIN])) {
      pl_set_item_replaygain(it, DDB_REPLAYGAIN_TRACKGAIN,
                             (float)volume_adjust / 512.0);
    }
    if (!pl_find_meta(it, ddb_internal_rg_keys[DDB_REPLAYGAIN_TRACKPEAK]) &&
        peak_val) {
      pl_set_item_replaygain(it, DDB_REPLAYGAIN_TRACKPEAK,
                             (float)peak_val / 32767.0);
    }
  }
...
int junk_id3v2_load_ufid(int version_major, playItem_t *it, uint8_t *readptr,
                         unsigned synched_size) {
  char *owner = readptr;
...
unsigned junk_stars_from_popm_rating(uint8_t rating) {
  if (rating == 0) {
    return 0;
  } else if (rating < 64) {
    return 1;
...
uint8_t junk_popm_rating_from_stars(unsigned stars) {
  switch (stars) {
  case 0:
    return 0;
  case 1:
...
int junk_id3v2_load_popm(int version_major, playItem_t *it, uint8_t *readptr,
                         unsigned synched_size, int *found_wmp_popm) {
  char *email = readptr;
...
static int junk_id3v2_remove_popm_with_email(DB_id3v2_tag_t *tag,
                                             const char *email) {
  ...
  while (frame) {
    ...
    if (!strcmp(frame->id, "POPM") &&
        !strcasecmp(frame->data, wmp_popm_email)) {
      if (prev) {
...
static DB_id3v2_frame_t *junk_id3v2_add_popm_with_email(DB_id3v2_tag_t *tag,
                                                        const char *email,
                                                        uint8_t rating,
                                                        int have_playcount,
                                                        uint8_t playcount) {
...
int junk_id3v2_remove_ufid_frames(DB_id3v2_tag_t *tag, const char *frame_id,
                                  const char *owner) {
  DB_id3v2_frame_t *prev = NULL;
  for (DB_id3v2_frame_t *f = tag->frames; f;) {
    DB_id3v2_frame_t *next = f->next;
    if (!strcmp(f->id, frame_id) && f->size >= strlen(owner) &&
        !strcmp(f->data, owner)) {
      if (prev) {
        prev->next = f->next;
      } else {
...
DB_id3v2_frame_t *junk_id3v2_add_ufid_frame(DB_id3v2_tag_t *tag,
...
static int junk_id3v2_load_txx(const char *sb_charset, int version_major,
                               playItem_t *it, uint8_t *readptr,
                               int synched_size) {
  int decoded_size;
  char *txx = convstr_id3v2(sb_charset, version_major, *readptr, readptr + 1,
                            synched_size - 1, &decoded_size);
  if (!txx) {
    return -1;
  }
...
int junk_id3v2_add_genre(playItem_t *it, char *genre) {
  int numeric = 0;
  if (genre[0] == '(') {
...
static int junk_id3v2_set_metadata_from_frame(playItem_t *it,
  ...
  if (version_major == 3 || version_major == 4) {
    if (strcmp(frameid, "TXXX")) {
      for (int f = 0; frame_mapping[f]; f += FRAME_MAPPINGS) {
        const char *frm_name = version_major == 3
                                   ? frame_mapping[f + MAP_ID3V23]
                                   : frame_mapping[f + MAP_ID3V24];
        if (frm_name && !strcmp(frameid, frm_name)) {
          ...
          int text_size;
          char *text = convstr_id3v2(sb_charset, version_major, readptr[0],
                                     readptr + 1, synched_size - 1, &text_size);

          // couple of simple tests
          // char *text = convstr_id3v2 (4, 3, "текст1\0текст2", strlen
          // ("текст1")*2+2); const char ucstext[] = { 0x42, 0x04, 0x35, 0x04,
          // 0x3a, 0x04, 0x41, 0x04, 0x42, 0x04, 0x31, 0x00, 0x00, 0x00, 0x42,
          // 0x04, 0x35, 0x04, 0x3a, 0x04, 0x41, 0x04, 0x42, 0x04, 0x32, 0x00 };
          // char *text = convstr_id3v2 (4, 1, ucstext, sizeof (ucstext));

          if (text && *text) {
            if (!strcmp(frameid,
                        "TRCK")) { // special case for track/totaltracks
              junk_add_track_meta(it, text);
            } else if (!strcmp(frameid,
                               "TPOS")) { // special case for disc/totaldiscs
              junk_add_disc_meta(it, text);
            } else if (!strcmp(frameid, "TCON")) {
              junk_id3v2_add_genre(it, text);
            } else {
              if (version_major == 3 &&
                  _is_multivalue_field(frame_mapping[f + MAP_DDB])) {
                _split_multivalue(text, text_size + 1);
              }
              pl_append_meta_full(it, frame_mapping[f + MAP_DDB], text,
                                  text_size + 1);
            }
            //                                if (text) {
            //                                    trace ("%s = %s\n", frameid,
    ...
    if (!strcmp(frameid, "COMM")) {
      ...
      return junk_load_comm_frame(version_major, "comment", it, readptr,
                                  synched_size);
      ...
      return junk_load_comm_frame(version_major, "UNSYNCED LYRICS", it, readptr,
                                  synched_size);
      ...
      return junk_id3v2_load_popm(version_major, it, readptr, synched_size,
                                  found_wmp_popm);
      ...
      return junk_id3v2_load_txx(sb_charset, version_major, it, readptr,
                                 synched_size);
    }
  }
  ...
  // parse basic 2.2 text frames
    ...
    if (strcmp(frameid, "TXX")) {
      for (int f = 0; frame_mapping[f]; f++) {
        if (frame_mapping[f + MAP_ID3V22] &&
            !strcmp(frameid, frame_mapping[f + MAP_ID3V22])) {
          added = 1;
          if (synched_size > MAX_TEXT_FRAME_SIZE) {
            trace("frame %s is too big, discard\n", frameid);
            break;
          ...
          if (text && *text) {
            if (!strcmp(frameid, "TRK")) { // special case for track/totaltracks
              junk_add_track_meta(it, text);
                               ...
                               "TPA")) { // special case for disc/totaldiscs
              junk_add_disc_meta(it, text);
...
static const char *junk_id3v2_detect_charset(DB_id3v2_tag_t *id3v2_tag) {
  int sz = 1000 * 200;
...
static int junk_id3v2_set_metadata(playItem_t *it, DB_id3v2_tag_t *id3v2_tag,
                                   const char *charset, int *found_wmp_popm) {
...
int junk_id3v2_read_full(playItem_t *it, DB_id3v2_tag_t *tag_store,
                         DB_FILE *fp) {
  int err = -1;
  if (!tag_store) {
    return -1;
...
#pragma unused(footerpresent)
  ...
  if (!tag) {
    fprintf(
        stderr,
        "junklib: out of memory while reading id3v2, tried to alloc %d bytes\n",
        size);
    goto error;
  }
  ...
  if (extheader) {
    uint32_t sz = (readptr[3] << 0) | (readptr[2] << 7) | (readptr[1] << 14) |
                  (readptr[0] << 21);
    if (size < sz) {
  ...
  while (readptr - tag <= size - 4 && *readptr) {
    if (version_major == 3 || version_major == 4) {
      ...
      uint32_t sz;
      if (version_major == 4) {
        sz = (readptr[3] << 0) | (readptr[2] << 7) | (readptr[1] << 14) |
             (readptr[0] << 21);
      } else if (version_major == 3) {
        sz = (readptr[3] << 0) | (readptr[2] << 8) | (readptr[1] << 16) |
             (readptr[0] << 24);
      } else {
      ...
      if (!strcmp(frameid, "APIC")) {
        if (sz > MAX_ID3V2_APIC_FRAME_SIZE) {
          trace("junk_id3v2_read_full: frame %s size is too big (%d), "
                "discarded\n",
                frameid, sz);
          readptr += sz;
          continue;
        ...
        trace(
            "junk_id3v2_read_full: frame %s size is too big (%d), discarded\n",
            frameid, sz);
        readptr += sz;
        continue;
      }
      ...
      if (version_major == 4) {
        ...
        if (flags2 & 0x01) { // data size
          synched_size = (readptr[3] << 0) | (readptr[2] << 7) |
                         (readptr[1] << 14) | (readptr[0] << 21);
...
int junk_id3v2_read(playItem_t *it, DB_FILE *fp) {
  DB_id3v2_tag_t id3v2_tag;
...
const char *junk_detect_charset_len(const char *s, int len) {
...
int junk_recode(const char *in, int inlen, char *out, int outlen,
                const char *cs) {
  return junk_iconv(in, inlen, out, outlen, cs, UTF8_STR);
}

int junk_rewrite_tags(playItem_t *it, uint32_t junk_flags, int id3v2_version,
                      const char *id3v1_encoding) {
  ...
  out = open(tmppath, O_CREAT | O_LARGEFILE | O_WRONLY | _O_BINARY,
             stat_struct.st_mode);
  ...
  if (!strip_id3v2 && !write_id3v2 && id3v2_size > 0) {
    if (deadbeef->fseek(fp, id3v2_start, SEEK_SET) == -1) {
      trace("cmp3_write_metadata: failed to seek to original id3v2 tag "
            "position in %s\n",
            pl_find_meta(it, ":URI"));
      goto error;
    }
    ...
    if (write(out, buf, id3v2_size) != id3v2_size) {
      trace("cmp3_write_metadata: failed to copy original id3v2 tag from %s to "
            "temp file\n",
            pl_find_meta(it, ":URI"));
      free(buf);
      goto error;
    ...
    trace("writing id3v2\n");
    if (id3v2_size <= 0 || strip_id3v2 ||
        deadbeef->junk_id3v2_read_full(NULL, &id3v2, fp) != 0) {
    ...
    pl_lock();
    {
      ...
      if (val && *val) {
        junk_id3v2_add_ufid_frame(&id3v2, "http://musicbrainz.org", val,
                                  (int)strlen(val));
      }
      ...
      if (val && *val) {
        ...
        junk_id3v2_add_popm_with_email(&id3v2, wmp_popm_email,
    ...
    // remove all known normal frames (they will be refilled from track
    // metadata)
    ...
    while (meta) {
      ...
      for (int i = 0; frame_mapping[i]; i += FRAME_MAPPINGS) {
        if (!strcasecmp(meta->key, frame_mapping[i + MAP_DDB])) {
          const char *frm_name = id3v2_version == 3
                                     ? frame_mapping[i + MAP_ID3V23]
                                     : frame_mapping[i + MAP_ID3V24];
          if (frm_name) {
      ...
      static const char *txxx_no_fallbacks[] = {"comment", "unsynced lyrics",
                                                "track"
                                                "numtracks"
                                                "disc"
                                                "numdiscs",
      ...
      if (!noremap) {
        if (!frame_mapping[mapidx + MAP_ID3V24] &&
            frame_mapping[mapidx + MAP_ID3V24][0] != 'T') {
          noremap = 1;
        }
      }
  ...
  if (!write_apev2 && !strip_apev2 && apev2_start != 0) {
    ...
    if (deadbeef->fseek(fp, apev2_start, SEEK_SET) == -1) {
      trace("cmp3_write_metadata: failed to seek to original apev2 tag "
            "position in %s\n",
            pl_find_meta(it, ":URI"));
      goto error;
    }
    ...
    if (write(out, buf, apev2_size) != apev2_size) {
      trace("cmp3_write_metadata: failed to copy original apev2 tag from %s to "
            "temp file\n",
            pl_find_meta(it, ":URI"));
      free(buf);
      goto error;
    ...
    while (meta) {
      ...
      for (i = 0; frame_mapping[i]; i += FRAME_MAPPINGS) {
        if (!strcasecmp(meta->key, frame_mapping[i + MAP_DDB]) &&
            frame_mapping[i + MAP_APEV2]) {
          trace("apev2 appending known field: %s=%s\n", meta->key, meta->value);
          _apev2_append_combined_text_frame_from_meta(
              &apev2, frame_mapping[i + MAP_APEV2], meta);
          break;
        }
      }
      if (!frame_mapping[i] && strcasecmp(meta->key, "track") &&
          strcasecmp(meta->key, "numtracks") && strcasecmp(meta->key, "disc") &&
  ...
  if (!write_id3v1 && !strip_id3v1 && id3v1_start != 0) {
    trace("copying original id3v1 tag %d %d %d\n", write_id3v1, strip_id3v1,
          id3v1_start);
    if (deadbeef->fseek(fp, id3v1_start, SEEK_SET) == -1) {
      trace("cmp3_write_metadata: failed to seek to original id3v1 tag "
            "position in %s\n",
            pl_find_meta(it, ":URI"));
      goto error;
    }
    ...
    if (write(out, buf, 128) != 128) {
      trace("cmp3_write_metadata: failed to copy id3v1 tag from %s to temp "
            "file\n",
            pl_find_meta(it, ":URI"));
      goto error;
    }
...
void junk_enable_cp1251_detection(int enable) {
  enable_cp1251_detection = enable;
}

void junk_enable_cp936_detection(int enable) {
  enable_cp936_detection = enable;
}

void junk_enable_shift_jis_detection(int enable) {
  enable_shift_jis_detection = enable;
}

void junk_configchanged(void) {
  ...
  conf_get_str("junk.multivalue_fields", DEFAULT_MULTIVALUE_FIELDS,
               junk_multivalue_fields, sizeof(junk_multivalue_fields));
