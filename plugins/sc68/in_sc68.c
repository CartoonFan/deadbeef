...
static DB_fileinfo_t *in_sc68_open(uint32_t hints) {
...
// return -1 on failure
static int in_sc68_init(DB_fileinfo_t *_info, DB_playItem_t *it) {
  ...
  if (ti.trk.time_ms > 0) {
    ...
    info->totalsamples =
        deadbeef->conf_get_float("c68.songlength", 2) * 60 * samplerate;
  }

  ddb_playlist_t *plt = deadbeef->pl_get_playlist(it);
  deadbeef->plt_set_item_duration(plt, it,
                                  (float)info->totalsamples / samplerate);
...
static void in_sc68_free(DB_fileinfo_t *_info) {
  in_sc68_info_t *info = (in_sc68_info_t *)_info;
  if (info) {
    if (info->sc68) {
...
// or 0 on EOF/error
static int in_sc68_read(DB_fileinfo_t *_info, char *bytes, int size) {
  in_sc68_info_t *info = (in_sc68_info_t *)_info;
...
// return 0 on success
// return -1 on failure
static int in_sc68_seek_sample(DB_fileinfo_t *_info, int sample) {
  in_sc68_info_t *info = (in_sc68_info_t *)_info;
...
// return 0 on success
// return -1 on failure
static int in_sc68_seek(DB_fileinfo_t *_info, float time) {
  return in_sc68_seek_sample(_info, time * _info->fmt.samplerate);
}

static void in_c68_meta_from_music_info(DB_playItem_t *it,
                                        sc68_music_info_t *ti, int trk) {
...
static DB_playItem_t *in_sc68_insert(ddb_playlist_t *plt, DB_playItem_t *after,
                                     const char *fname) {
#if 0
    // open file
...
static int in_sc68_read_metadata(DB_playItem_t *it) {
...
static int in_sc68_start(void) {
  ...
  snprintf(datadir, sizeof(datadir), "%s/data68",
...
DB_plugin_t *in_sc68_load(DB_functions_t *api) {
  deadbeef = api;
