...
// a list of unique names in collection, as a list, and as a hash, with each
// item associated with list of tracks
typedef struct {
...
typedef struct {
  ...
  // this hash purpose is to quickly check whether the filename is in the
  // library already NOTE: this hash doesn't contain all of the tracks from the
  // `tracks` list, because of subtracks
  ...
  // This hash is formed from track_uri ([%:TRACKNUM%#]%:URI%), and supposed to
  // have all tracks from the `tracks` list Main purpose is to find a library
  // instance of a track for given track pointer
...
static void ml_free_list(ddb_mediasource_source_t source,
                         ddb_medialib_item_t *list);

static uint32_t hash_for_ptr(void *ptr) {
...
static ml_string_t *hash_find_for_hashkey(ml_string_t **hash, const char *val,
                                          uint32_t h) {
...
static ml_string_t *hash_find(ml_string_t **hash, const char *val) {
  uint32_t h = hash_for_ptr((void *)val);
  return hash_find_for_hashkey(hash, val, h);
}

/// When it is null, it's expected that the bucket will be added, without any
/// associated tracks
static ml_string_t *hash_add(ml_string_t **hash, const char *val,
                             DB_playItem_t /* nullable */ *it) {
  uint32_t h = hash_for_ptr((void *)val);
...
static ml_string_t *ml_reg_col(ml_collection_t *coll,
                               const char /* nonnull */ *c, DB_playItem_t *it) {
  int need_unref = 0;
...
static void ml_free_col(ml_collection_t *coll) {
...
static void ml_reg_item_in_folder(ml_tree_node_t *node, const char *path,
                                  DB_playItem_t *it) {
  if (*path == 0) {
...
static void ml_free_tree(ml_tree_node_t *node) {
  while (node->children) {
...
static void ml_notify_listeners(medialib_source_t *source, int event);

static void ml_index(medialib_source_t *source, ddb_playlist_t *plt);

static void ml_free_db(medialib_source_t *source) {
...
static json_t *_ml_get_music_paths(medialib_source_t *source) {
...
static void ml_index(medialib_source_t *source, ddb_playlist_t *plt) {
  ...
  while (it && !source->scanner_terminate) {
    ...
    deadbeef->tf_eval(&ctx, artist_album_id_bc, artistalbum,
                      sizeof(artistalbum));
    ...
    // at this point, we only have unique pointers, and don't need a duplicate
    // check
  ...
  long ms = (tm2.tv_sec * 1000 + tm2.tv_usec / 1000) -
            (tm1.tv_sec * 1000 + tm1.tv_usec / 1000);

  fprintf(stderr,
          "index build time: %f seconds (%d albums, %d artists, %d genres, %d "
          "folders)\n",
...
static void _ml_load_playlist(medialib_source_t *source, const char *plpath) {
  struct timeval tm1, tm2;
...
static char **get_medialib_paths(medialib_source_t *source,
...
static void free_medialib_paths(char **medialib_paths,
                                size_t medialib_paths_count) {
...
static void scanner_thread(void *ctx) {
  ...
  snprintf(plpath, sizeof(plpath), "%s/medialib.dbpl",
  ...
  ddb_playlist_t *plt = deadbeef->plt_alloc("medialib");
  // doesn't need to be protected by mutex -- this is only supposed to be
  // accessed on the scan thread
  ...
  for (int i = 0; i < medialib_paths_count; i++) {
    ...
    deadbeef->plt_insert_dir(plt, NULL, musicdir, &source->scanner_terminate,
                             NULL, NULL);
  }
  ...
  fprintf(stderr, "scan time: %f seconds (%d tracks)\n", ms / 1000.f,
          deadbeef->plt_get_item_count(source->ml_playlist, PL_MAIN));
...
static int ml_fileadd_filter(ddb_file_found_data_t *data, void *user_data) {
  int res = 0;
      ...
      if (str) {
        for (ml_collection_item_t *item = str->items; item; item = item->next) {
          const char *stimestamp =
              deadbeef->pl_find_meta(item->it, ":MEDIALIB_SCAN_TIME");
          if (!stimestamp) {
            // no scan time
            deadbeef->mutex_unlock(source->mutex);
            return 0;
        ...
        for (ml_collection_item_t *item = str->items; item; item = item->next) {
          ...
          if (deadbeef->plt_get_item_idx(source->ml_playlist, item->it,
                                         PL_MAIN) == -1) {
            continue;
          }
          ...
          ddb_playItem_t *tail =
              deadbeef->plt_get_tail_item(data->plt, PL_MAIN);
...
static int ml_start(void) {
  artist_album_bc = deadbeef->tf_compile("[%album artist% - ]%album%");
  title_bc = deadbeef->tf_compile("[%tracknumber%. ]%title%");
  artist_album_id_bc =
      deadbeef->tf_compile("artist=$if2(%album artist%,Unknown "
                           "Artist);album=$if2(%album%,Unknown Album)");
  return 0;
}

static int ml_stop(void) {
  if (artist_album_bc) {
...
static int ml_add_listener(ddb_mediasource_source_t _source,
...
static void ml_remove_listener(ddb_mediasource_source_t _source,
                               int listener_id) {
  medialib_source_t *source = (medialib_source_t *)_source;
...
static void get_albums_for_collection_group_by_field(
    medialib_source_t *source, ddb_medialib_item_t *root, ml_collection_t *coll,
    const char *field, int field_tf,
    const char /* nonnull */ *default_field_value, int selected) {
  ...
  for (int i = 0; i < source->db.albums.count; i++, album = album->next) {
    ...
    if (!tf) {
      ...
      track_field = mc_str_for_track_field =
          deadbeef->metacache_add_string(text);
    }
    ...
    for (int j = 0; j < album->items_count;
         j++, album_coll_item = album_coll_item->next) {
...
static void get_list_of_tracks_for_album(ddb_medialib_item_t *libitem,
                                         ml_string_t *album, int selected) {
  char text[1024];
...
static void get_subfolders_for_folder(ddb_medialib_item_t *folderitem,
                                      ml_tree_node_t *folder, int selected) {
  if (!folderitem->text) {
...
static ddb_medialib_item_t *ml_create_list(ddb_mediasource_source_t _source,
                                           const char *index,
                                           const char *filter) {
  medialib_source_t *source = (medialib_source_t *)_source;
  ...
  if (type == folder) {
    ...
    get_albums_for_collection_group_by_field(source, root, coll, "artist", 0,
                                             "Unknown Artist", selected);
  } else if (type == genre) {
    // list of albums for genre
    get_albums_for_collection_group_by_field(source, root, coll, "genre", 0,
                                             "Unknown Genre", selected);
  } else if (type == album) {
...
static int ml_message(uint32_t id, uintptr_t ctx, uint32_t p1, uint32_t p2) {
  return 0;
}

#pragma mark - folder access

static unsigned ml_folder_count(ddb_mediasource_source_t _source) {
  medialib_source_t *source = (medialib_source_t *)_source;
  deadbeef->mutex_lock(source->mutex);
...
static void ml_folder_at_index(ddb_mediasource_source_t _source, int index,
...
static void ml_set_folders(ddb_mediasource_source_t _source,
                           const char **folders, size_t count) {
  medialib_source_t *source = (medialib_source_t *)_source;
  deadbeef->mutex_lock(source->mutex);
...
static ddb_mediasource_source_t ml_create_source(const char *source_path) {
  medialib_source_t *source = calloc(1, sizeof(medialib_source_t));

  source->musicpaths_json = _ml_get_music_paths(source);

  source->mutex = deadbeef->mutex_create();
  source->filter_id = deadbeef->register_fileadd_filter(
      ml_fileadd_filter, &source->ml_filter_state);
...
static void ml_free_source(ddb_mediasource_source_t _source) {
  medialib_source_t *source = (medialib_source_t *)_source;
  if (source->tid) {
...
DB_plugin_t *medialib_load(DB_functions_t *api) {
  deadbeef = api;
