...
#define LOCK                                                                   \
  {                                                                            \
    deadbeef->mutex_lock(                                                      \
        mutex); /*fprintf (stderr, "alsa lock %s:%d\n", __FILE__, __LINE__);*/ \
  }
#define UNLOCK                                                                 \
  {                                                                            \
    deadbeef->mutex_unlock(mutex); /*fprintf (stderr, "alsa unlock %s:%d\n",   \
                                      __FILE__, __LINE__);*/                   \
  }
...
static int palsa_callback(char *stream, int len);

static void palsa_thread(void *context);

static int palsa_init(void);

static int palsa_free(void);

static int palsa_setformat(ddb_waveformat_t *fmt);

static ddb_playback_state_t palsa_get_state(void);

static int palsa_play(void);

static int palsa_stop(void);

static int palsa_pause(void);

static int palsa_unpause(void);

static int palsa_get_channels(void);

static int palsa_get_endianness(void);

static void palsa_enum_soundcards(void (*callback)(const char *name,
                                                   const char *desc, void *),
                                  void *userdata);

static int palsa_set_hw_params(ddb_waveformat_t *fmt) {
  ...
  if ((err = snd_pcm_hw_params_set_access(audio, hw_params,
  ...
  if ((err = snd_pcm_hw_params_set_format(audio, hw_params, sample_fmt)) < 0) {
    fprintf(stderr,
            "cannot set sample format to %d bps (error: %s), trying all "
            "supported formats\n",
            plugin.fmt.bps, snd_strerror(err));

    int fmt_cnt[] = {16, 24, 32, 32, 8};
#if WORDS_BIGENDIAN
    int fmt[] = {SND_PCM_FORMAT_S16_BE, SND_PCM_FORMAT_S24_3BE,
                 SND_PCM_FORMAT_S32_BE, SND_PCM_FORMAT_FLOAT_BE,
                 SND_PCM_FORMAT_S8,     -1};
#else
    int fmt[] = {SND_PCM_FORMAT_S16_LE, SND_PCM_FORMAT_S24_3LE,
  ...
  if ((err = snd_pcm_hw_params_set_rate_resample(audio, hw_params,
                                                 conf_alsa_resample)) < 0) {
    fprintf(stderr, "cannot setup resampling (%s)\n", snd_strerror(err));
    goto error;
  }

  if ((err = snd_pcm_hw_params_set_rate_near(audio, hw_params, &val, &ret)) <
      0) {
  ...
  if (nchan == 4) {
    plugin.fmt.channelmask = DDB_SPEAKER_FRONT_LEFT | DDB_SPEAKER_FRONT_RIGHT |
                             DDB_SPEAKER_BACK_LEFT | DDB_SPEAKER_BACK_RIGHT;
  }
  if (nchan == 5) {
    plugin.fmt.channelmask = DDB_SPEAKER_FRONT_LEFT | DDB_SPEAKER_FRONT_RIGHT |
                             DDB_SPEAKER_BACK_LEFT | DDB_SPEAKER_BACK_RIGHT |
  ...
  if (nchan == 7) {
    plugin.fmt.channelmask = DDB_SPEAKER_FRONT_LEFT | DDB_SPEAKER_FRONT_RIGHT |
                             DDB_SPEAKER_BACK_LEFT | DDB_SPEAKER_BACK_RIGHT |
                             DDB_SPEAKER_FRONT_CENTER | DDB_SPEAKER_SIDE_LEFT |
  ...
  if (nchan == 8) {
    plugin.fmt.channelmask = DDB_SPEAKER_FRONT_LEFT | DDB_SPEAKER_FRONT_RIGHT |
                             DDB_SPEAKER_BACK_LEFT | DDB_SPEAKER_BACK_RIGHT |
                             DDB_SPEAKER_FRONT_CENTER | DDB_SPEAKER_SIDE_LEFT |
                             DDB_SPEAKER_SIDE_RIGHT | DDB_SPEAKER_LOW_FREQUENCY;
  }
error:
  if (err < 0) {
...
static int palsa_init(void) {
  ...
  deadbeef->conf_get_str("alsa_soundcard", "default", conf_alsa_soundcard,
                         sizeof(conf_alsa_soundcard));
  trace("alsa_soundcard: %s\n", conf_alsa_soundcard);

  snd_pcm_sw_params_t *sw_params = NULL;
  state = DDB_PLAYBACK_STATE_STOPPED;
  // const char *conf_alsa_soundcard = conf_get_str ("alsa_soundcard",
  // "default");
  if ((err = snd_pcm_open(&audio, conf_alsa_soundcard, SND_PCM_STREAM_PLAYBACK,
                          0))) {
  ...
  snd_pcm_sw_params_set_start_threshold(audio, sw_params,
                                        buffer_size - period_size);

  if ((err = snd_pcm_sw_params_set_avail_min(audio, sw_params, period_size)) <
      0) {
  ...
  //    if ((err = snd_pcm_sw_params_set_start_threshold (audio, sw_params, 0U))
  //    < 0) {
...
static int _setformat_apply(void) {
  _setformat_requested = 0;

  trace("palsa_setformat %dbit %s %dch %dHz channelmask=%X\n",
        requested_fmt.bps, requested_fmt.is_float ? "float" : "int",
        requested_fmt.channels, requested_fmt.samplerate,
        requested_fmt.channelmask);
  if (!audio ||
    ...
    trace("switching format to (requsted -> actual):\n"
          ...
          "samplerate %d -> %d\n"
          "channelmask %d -> %d\n",
          requested_fmt.bps, plugin.fmt.bps, requested_fmt.is_float,
          plugin.fmt.is_float, requested_fmt.channels, plugin.fmt.channels,
  ...
  trace("new format %dbit %s %dch %dHz channelmask=%X\n", plugin.fmt.bps,
        plugin.fmt.is_float ? "float" : "int", plugin.fmt.channels,
        plugin.fmt.samplerate, plugin.fmt.channelmask);
  return 0;
}
...
static void palsa_hw_pause(int pause) {
  if (!audio) {
    return;
  }
...
static int alsa_configchanged(void) {
  deadbeef->conf_lock();
  int alsa_resample = deadbeef->conf_get_int("alsa.resample", 1);
  const char *alsa_soundcard =
      deadbeef->conf_get_str_fast("alsa_soundcard", "default");
  ...
  if (audio && (alsa_resample != conf_alsa_resample ||
                strcmp(alsa_soundcard, conf_alsa_soundcard) ||
                buffer != req_buffer_size || period != req_period_size)) {
...
static ddb_playback_state_t palsa_get_state(void) { return state; }

static int alsa_message(uint32_t id, uintptr_t ctx, uint32_t p1, uint32_t p2) {
  switch (id) {
...
DB_plugin_t *alsa_load(DB_functions_t *api) {
  deadbeef = api;
...
static const char settings_dlg[] =
    ...
    "property \"Preferred buffer size\" entry "
    "alsa.buffer " DEFAULT_BUFFER_SIZE_STR ";\n"
    "property \"Preferred period size\" entry "
    "alsa.period " DEFAULT_PERIOD_SIZE_STR ";\n";
