...
static const char *exts[] = {"psf",     "psf2",    "spu",     "ssf",
                             "qsf",     "dsf",     "minipsf", "minipsf2",
                             "minissf", "miniqsf", "minidsf", NULL};
...
static DB_fileinfo_t *psfplug_open(uint32_t hints) {
  psfplug_info_t *info = calloc(sizeof(psfplug_info_t), 1);
  return &info->info;
}

static int psfplug_init(DB_fileinfo_t *_info, DB_playItem_t *it) {
  ...
  if (deadbeef->fread(info->filebuffer, 1, info->filesize, file) !=
      info->filesize) {
    deadbeef->pl_lock();
  ...
  deadbeef->pl_lock();
  info->decoder = ao_start(info->type, deadbeef->pl_find_meta(it, ":URI"),
                           (uint8 *)info->filebuffer, (uint32)info->filesize);
  deadbeef->pl_unlock();
  if (!info->decoder) {
...
static void psfplug_free(DB_fileinfo_t *_info) {
  psfplug_info_t *info = (psfplug_info_t *)_info;
  if (info) {
    if (info->type >= 0) {
...
static int psfplug_read(DB_fileinfo_t *_info, char *bytes, int size) {
  psfplug_info_t *info = (psfplug_info_t *)_info;
  //    printf ("psfplug_read_int16 %d samples, curr %d, end %d\n", size/4,
  //    info->currentsample, (int)(info->duration * _info->samplerate));
  ...
  while (size > 0) {
    if (info->remaining > 0) {
      if (info->skipsamples > 0) {
        ...
        if (info->remaining > n) {
          memmove(info->buffer, info->buffer + n * 4,
                  (info->remaining - n) * 4);
        }
        info->remaining -= n;
        info->skipsamples -= n;
  ...
  info->currentsample +=
      (initsize - size) / (_info->fmt.channels * _info->fmt.bps / 8);
  _info->readpos = (float)info->currentsample / _info->fmt.samplerate;
  return initsize - size;
}

static int psfplug_seek_sample(DB_fileinfo_t *_info, int sample) {
  psfplug_info_t *info = (psfplug_info_t *)_info;
...
static int psfplug_seek(DB_fileinfo_t *_info, float time) {
  return psfplug_seek_sample(_info, time * _info->fmt.samplerate);
}

static void psfplug_add_meta(DB_playItem_t *it, const char *key,
                             const char *value, const char *comment_title) {
  //    const char *res = NULL;
  char tmp[200];
  // check utf8
  if (deadbeef->junk_recode(value, (int)strlen(value), tmp, sizeof(tmp),
                            "utf-8") >= 0) {
    if (key) {
...
static DB_playItem_t *psfplug_insert(ddb_playlist_t *plt, DB_playItem_t *after,
                                     const char *fname) {
  ...
  if (have_info) {
    ...
    for (i = 1; i < 9; i++) {
      ...
      } else if (!strncasecmp(info.title[i], "Name: ", 6) ||
                 !strncasecmp(info.title[i], "Song: ", 6)) {
...
static int psfplug_start(void) { return 0; }

static int psfplug_stop(void) { return 0; }
...
static DB_decoder_t plugin = {
    ...
    .plugin.copyright =
        ...
        "\n"
        "Uses modified aosdk-1.4.8 - library for playing .PSF (Sony "
        "PlayStation), .SPU (Sony PlayStation), .PSF2 (Sony PlayStation 2), "
        ".SSF (Sega Saturn), .DSF (Sega Dreamcast), and .QSF (Capcom QSound) "
        "audio file formats,\n"
...
DB_plugin_t *psf_load(DB_functions_t *api) {
  deadbeef = api;
