...
/* block buffers have to be large enough to maintain at least 20 ms of music
 * (maybe 25?), else stuttering may be heard */
#define AUDIO_BUFFER_DURATION 25l /* ms of audio data per block */
/* cannot rely on time measures below 10 ms because we are really close to the
   NT default time granularity (1/64 s), so even the sound card driver may
   behave oddly */

#define AUDIO_BUFFER_MAXIMUM_REQ 6 /* kilobytes per millisecond */
/* 1 ms of a 192 KHz 7.1 (32 bit/sample) stream require 6 kB */
/* this is the most demanding audio stream I can think of, so I'll use this as
 * reference */
/* in case of more challenging streams, then less audio blocks will be used, so
 * everything will work anyway */

/* max size reserved for audio buffers - rarely used */
#define AUDIO_BUFFER_SIZE                                                      \
  (AUDIO_BUFFER_NO * AUDIO_BUFFER_MAXIMUM_REQ * AUDIO_BUFFER_DURATION * 1024)
...
// {0x00000001,0x0000,0x0010,{0x80,0x00,0x00,0xaa,0x00,0x38,0x9b,0x71}}; static
...
static void pwaveout_thread(void *context);

static int pwaveout_stop(void);

static void remove_blanks(char *buffer, int len) {
  int idx;
...
static int waveout_message(uint32_t id, uintptr_t ctx, uint32_t p1,
                           uint32_t p2) {
  int num_devices, idx;
  WAVEOUTCAPS woc;
    ...
    if (strcmp(current_dev, "default") == 0) {
      ...
      if (num_devices != 0) {
        for (idx = 0; idx < num_devices; idx++) {
          if (waveOutGetDevCaps(idx, &woc, sizeof(WAVEOUTCAPS)) ==
              MMSYSERR_NOERROR) {
...
int pwaveout_setformat(ddb_waveformat_t *fmt) {
  int result = 0;
  MMRESULT openresult;
  ...
  trace(
      "pwaveout_setformat: new settings sr %d ch %d bps %d channelmask=%08X\n",
  ...
  wave_format.SubFormat = (fmt->is_float) ? KSDATAFORMAT_SUBTYPE_IEEE_FLOAT
  ...
  if (state == DDB_PLAYBACK_STATE_STOPPED && audio_blocks_sent == 0) {
    openresult = waveOutOpen(NULL, waveout_device, (WAVEFORMATEX *)&wave_format,
                             0, 0, WAVE_FORMAT_QUERY);
    if (openresult != MMSYSERR_NOERROR) {
      trace("pwaveout_setformat: audio format not supported by the selected "
            "device (result=%d)\n",
            openresult);
...
static void CALLBACK waveOutProc(HWAVEOUT hwo, UINT uMsg, DWORD_PTR dwInstance,
  ...
  if (hwo == device_handle && uMsg == WOM_DONE) {
    ...
    /* DON'T DO THIS! */
    /* Calling waveOutAnything functions from within this callback will cause a
     * deadlock! */
    // waveOutUnprepareHeader(device_handle, (LPWAVEHDR)dwParam1,
    // sizeof(WAVEHDR));
  }
}

static int pwaveout_play(void) {
  int result = -1;
  ...
  // if (deadbeef->thread_alive(waveout_tid))
  {
    if (wave_format.Format.wFormatTag == WAVE_FORMAT_EXTENSIBLE) {
      ...
      if (waveOutOpen(&device_handle, waveout_device,
                      (WAVEFORMATEX *)&wave_format, (DWORD_PTR)waveOutProc, 0,
                      CALLBACK_FUNCTION) == MMSYSERR_NOERROR) {
        /* let's dance! - setup buffers */
        bytes_per_block =
            wave_format.Format.nAvgBytesPerSec * AUDIO_BUFFER_DURATION / 1000;
        avail_audio_buffers = AUDIO_BUFFER_SIZE / bytes_per_block;
        if (avail_audio_buffers > AUDIO_BUFFER_NO)
          avail_audio_buffers = AUDIO_BUFFER_NO;
  ...
  if (device_handle != NULL) {
    ...
    for (idx = 0; idx < AUDIO_BUFFER_NO; idx++) {
      if (waveout_headers[idx].dwFlags & WHDR_PREPARED)
        waveOutUnprepareHeader(device_handle, &waveout_headers[idx],
                               sizeof(WAVEHDR));
    }
  }
...
static int pwaveout_pause(void) {
...
static int pwaveout_unpause(void) {
...
static void pwaveout_thread(void *context) {
  ...
  while (1) {
    ...
    if (state != DDB_PLAYBACK_STATE_PLAYING && !audio_format_change_pending) {
      nanosleep(&sleep_time, NULL);
      //__mingw_sleep(0, AUDIO_BUFFER_DURATION*1000000); /*
      //__mingw_sleep(seconds, nanoseconds) */
    } else {
      /* is the device full? */
      ...
      if (deadbeef->streamer_ok_to_read(bytes_per_block)) {
        bytesread = deadbeef->streamer_read(
            audio_data + audio_block_write_index * bytes_per_block,
            bytes_per_block);
        if (bytesread > 0) {
          /* I cannot "unprepare" into the callback, so this seems a good place
           * to do it */
          if (waveout_headers[audio_block_write_index].dwFlags & WHDR_PREPARED)
            waveOutUnprepareHeader(device_handle,
                                   &waveout_headers[audio_block_write_index],
                                   sizeof(WAVEHDR));
          waveout_headers[audio_block_write_index].dwBufferLength = bytesread;
          waveout_headers[audio_block_write_index].lpData =
              audio_data + audio_block_write_index * bytes_per_block;
          waveout_headers[audio_block_write_index].dwFlags = 0;

          mmresult = waveOutPrepareHeader(
              device_handle, &waveout_headers[audio_block_write_index],
              sizeof(WAVEHDR));
          if (mmresult == MMSYSERR_NOERROR) {
            waveOutWrite(device_handle,
                         &waveout_headers[audio_block_write_index],
                         sizeof(WAVEHDR));
            EnterCriticalSection(&waveoutCS);
            audio_blocks_sent++;
            LeaveCriticalSection(&waveoutCS);
            audio_block_write_index =
                (audio_block_write_index + 1) % avail_audio_buffers;
          }
        } else {
      ...
      if (audio_blocks_sent == 0 && audio_format_change_pending) {
        ...
        mmresult = waveOutOpen(&device_handle, waveout_device,
                               (WAVEFORMATEX *)&wave_format,
                               (DWORD_PTR)waveOutProc, 0, CALLBACK_FUNCTION);
        if (mmresult != MMSYSERR_NOERROR) {
          /* FATAL! cannot continue playback */
          trace("pwaveout_thread: FATAL audio format not supported by the "
                "selected device (result=%d)\n",
                mmresult);
          memset((void *)&wave_format, 0, sizeof(WAVEFORMATEXTENSIBLE));
          // result = -1;
          break;
...
ddb_playback_state_t pwaveout_get_state(void) {
  // trace("pwaveout_get_state\n"); /* this routine is called several times a
  // second, so too much logging */
  return state;
}
...
static void pwaveout_enumdevices(void (*callback)(const char *name,
...
DB_plugin_t *waveout_load(DB_functions_t *api) {
  trace("waveout_load\n");
  deadbeef = api;
...
static DB_output_t plugin = {
    ...
    .plugin.copyright =
        ...
        "software\n"
        "   in a product, an acknowledgement in the product documentation "
        "would be\n"
        "   appreciated but is not required.\n"
        "\n"
    ...
    /* waveOut API has the waveOutGetVolume and waveOutSetVolume interfaces: fix
       this? */
    .has_volume = 0};
#endif /* __MINGW32__ */
