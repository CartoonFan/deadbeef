...
#define DEFAULT_FILEMASK                                                       \
  "front.png;front.jpg;folder.png;folder.jpg;cover.png;cover.jpg;f.png;f.jpg;" \
  "*front*.png;*front*.jpg;*cover*.png;*cover*.jpg;*folder*.png;*folder*.jpg;" \
  "*.png;*.jpg"
...
static int make_cache_dir_path(const char *artist, char *outpath, int outsize) {
  ...
  int path_length;
  path_length =
      snprintf(outpath + strlen(outpath), size_left, "covers2/%s/", esc_artist);
...
static int make_cache_path(const char *filepath, const char *album,
                           const char *artist, char *outpath, int outsize) {
  outpath[0] = '\0';
...
static int vfs_scan_results(struct dirent *entry, const char *container_uri,
...
static int dir_scan_results(struct dirent **files, int files_count,
                            const char *container, ddb_cover_info_t *cover) {
  ...
  for (size_t i = 0; i < files_count; i++) {
    ...
    if (!stat(cover->image_filename, &stat_struct) &&
        S_ISREG(stat_struct.st_mode) && stat_struct.st_size > 0) {
      return 0;
    } else {
...
static int scan_local_path(char *mask, const char *local_path, const char *uri,
                           DB_vfs_t *vfsplug, ddb_cover_info_t *cover) {
...
static int filter_strcasecmp(const struct dirent *f) {
  return !strcasecmp(filter_strcasecmp_name, f->d_name);
}

// FIXME: this returns only one path that matches subfolder. Usually that's
// enough, but can be improved.
static char *get_case_insensitive_path(const char *local_path,
...
static int local_image_file(const char *local_path, const char *uri,
                            DB_vfs_t *vfsplug, ddb_cover_info_t *cover) {
  if (!artwork_filemask) {
    return -1;
  }
...
static const uint8_t *id3v2_artwork(const DB_id3v2_frame_t *f,
                                    int minor_version, int type) {
  if ((minor_version > 2 && strcmp(f->id, "APIC")) ||
      (minor_version == 2 && strcmp(f->id, "PIC"))) {
    return NULL;
  }
...
static int id3_extract_art(const char *outname, ddb_cover_info_t *cover) {
  int err = -1;
    ...
    for (DB_id3v2_frame_t *f = id3v2_tag.frames; f; f = f->next) {
      ...
      if (image_data) {
        ...
        if (!artwork_disable_cache) {
          ...
          cover->blob_image_offset =
              (uint64_t)((char *)image_data - (char *)cover->blob);
...
static int apev2_extract_art(const char *outname, ddb_cover_info_t *cover) {
  int err = -1;
...
static int mp4_extract_art(const char *outname, ddb_cover_info_t *cover) {
  int ret = -1;
  mp4p_atom_t *mp4file = NULL;
  DB_FILE *fp = NULL;
  uint8_t *image_blob = NULL;

  if (!strcasestr(cover->filepath, ".mp4") &&
      !strcasestr(cover->filepath, ".m4a") &&
      !strcasestr(cover->filepath, ".m4b")) {
    return -1;
  }
...
static int web_lookups(const char *cache_path, ddb_cover_info_t *cover) {
  if (!cache_path) {
    return 0;
  }
#if USE_VFS_CURL
...
// Web cover: save_to_local ? save_to_local&return_path : ( !cache_disabled ?
// save_to_cache&return_path : NOP )
static void process_query(ddb_cover_info_t *cover) {
  char cache_path_buf[PATH_MAX];
  char *cache_path = NULL;

  if (!artwork_disable_cache) {
    cache_path = cache_path_buf;
    make_cache_path(cover->filepath, cover->album, cover->artist, cache_path,
                    sizeof(cache_path_buf));
  }
...
#warning FIXME not needed during development; also this assumes that disk cache is used for everything
  ...
  if (artwork_enable_local && islocal) {
    ...
    if (fname_copy) {
      ...
      if (!local_image_file(dirname(vfs_fname ? vfs_fname : fname_copy), NULL,
                            NULL, cover)) {
        free(fname_copy);
        cover->cover_found = 1;
        return;
...
#ifdef USE_VFS_CURL
  /* Web lookups */
  if (artwork_enable_wos && strlen(cover->filepath) > 3 &&
      !strcasecmp(cover->filepath + strlen(cover->filepath) - 3, ".ay")) {
...
static void cover_info_free(ddb_cover_info_t *cover) {
  cover->refc--;
  if (cover->refc != 0) {
    return;
  }
...
static void cover_update_cache(ddb_cover_info_t *cover) {
  // any empty items?
...
static void cover_cache_free(void) {
  for (int i = 0; i < MAX_COVERS_IN_CACHE; i++) {
    if (!cover_cache[i]) {
      continue;
    }
...
static ddb_cover_info_t *cover_cache_find(ddb_cover_info_t *cover) {
  for (int i = 0; i < MAX_COVERS_IN_CACHE; i++) {
    if (!cover_cache[i]) {
      continue;
    }
    ...
    if (!strcmp(cover->album, cached_cover->album) &&
        !strcmp(cover->artist, cached_cover->artist)) {
      return cached_cover;
    }
  }
...
static void cover_get(ddb_cover_query_t *query, ddb_cover_callback_t callback) {
  ...
  dispatch_async(fetch_queue, ^{
    ...
    deadbeef->pl_lock();
    strncat(cover->filepath, deadbeef->pl_find_meta(query->track, ":URI"),
            sizeof(cover->filepath) - strlen(cover->filepath) - 1);
    deadbeef->pl_unlock();
...
static void artwork_reset(void) {
  trace("artwork: reset queue\n");
  queue_clear();
}

static void get_fetcher_preferences(void) {
  artwork_disable_cache =
      deadbeef->conf_get_int("artwork.disable_cache", DEFAULT_DISABLE_CACHE);
  artwork_save_to_music_folders = deadbeef->conf_get_int(
      "artwork.save_to_music_folders", DEFAULT_SAVE_TO_MUSIC_FOLDERS);
  ...
  const char *new_artwork_folders =
      deadbeef->conf_get_str_fast("artwork.folders", NULL);
...
static void artwork_configchanged(void) {
  ...
  if (old_artwork_disable_cache != artwork_disable_cache ||
      old_missing_artwork != missing_artwork ||
...
// on android, cache is always off and music is saved to music folders by
// default
#ifndef ANDROID
    "property \"Disable disk cache\" checkbox artwork.disable_cache 0;\n"
    "property \"Save extracted covers to music folders\" checkbox "
    "artwork.save_to_music_folders 0;\n"
#endif
...
#endif
// android doesn't display any image when cover is not found, and positioning
// algorithm is really different, therefore the below options are useless
#ifndef ANDROID
