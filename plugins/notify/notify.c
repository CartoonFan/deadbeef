...
static void notify_send(DBusMessage *msg) {
...
static void esc_xml(const char *cmd, char *esc, int size) {
    const char *src = cmd;
    char *dst = esc;
    char *end = dst + size - 1;
    while (*src && dst < end) {
...
static void cover_avail_callback(const char *fname, const char *artist,
...
static void show_notification(DB_playItem_t *track) {
    ...
    DBusMessage *msg =
        dbus_message_new_method_call(E_NOTIFICATION_BUS_NAME, E_NOTIFICATION_PATH,
                                     E_NOTIFICATION_INTERFACE, "Notify");
    ...
    if (deadbeef->conf_get_int("notify.albumart", 0) && artwork_plugin) {
        ...
        v_iconname = artwork_plugin->get_album_art(
                         fname, artist, album,
                         deadbeef->conf_get_int("notify.albumart_size", 64),
    ...
    dbus_message_append_args(msg, DBUS_TYPE_STRING, &v_appname, DBUS_TYPE_UINT32,
                             &replaces_id, DBUS_TYPE_STRING, &v_iconname,
                             DBUS_TYPE_STRING, &v_summary, DBUS_TYPE_STRING,
                             &v_body, DBUS_TYPE_INVALID);
...
static int on_songstarted(ddb_event_track_t *ev) {
...
static void init_tf(void) {
    ...
    char format[200];
    deadbeef->conf_get_str("notify.format_title_tf", NOTIFY_DEFAULT_TITLE, format,
                           sizeof(format));
    tf_title = deadbeef->tf_compile(format);
    deadbeef->conf_get_str("notify.format_content_tf", NOTIFY_DEFAULT_CONTENT,
                           format, sizeof(format));
    tf_content = deadbeef->tf_compile(format);
}

static int notify_message(uint32_t id, uintptr_t ctx, uint32_t p1,
                          uint32_t p2) {
    switch (id) {
...
static int notify_disconnect(void) {
    artwork_plugin = NULL;
    return 0;
}
...
static const char settings_dlg[] =
    ...
    "property \"Notification title format\" entry notify.format_title_tf "
    "\"" NOTIFY_DEFAULT_TITLE "\";\n"
    "property \"Notification content format\" entry notify.format_content_tf "
    "\"" NOTIFY_DEFAULT_CONTENT "\";\n"
...
static DB_misc_t plugin = {
    ...
    "dbus and notification daemon to be running.\nNotification "
    "daemon should be provided by your desktop environment.\n",
    .plugin.copyright =
    "OSD Notification plugin for DeaDBeeF Player\n"
    "Copyright (C) 2009-2014 Alexey Yakovenko and contributors\n"
    "\n"
...
DB_plugin_t *notify_load(DB_functions_t *ddb) {
    deadbeef = ddb;
