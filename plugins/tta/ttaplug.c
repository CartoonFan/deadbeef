...
static DB_fileinfo_t *tta_open(uint32_t hints) {
  tta_info_t *info = calloc(sizeof(tta_info_t), 1);
  return &info->info;
}

static int tta_init(DB_fileinfo_t *_info, DB_playItem_t *it) {
...
static void tta_free(DB_fileinfo_t *_info) {
  tta_info_t *info = (tta_info_t *)_info;
  if (info) {
...
static int tta_read(DB_fileinfo_t *_info, char *bytes, int size) {
  tta_info_t *info = (tta_info_t *)_info;
  ...
  while (size > 0) {
    if (info->samples_to_skip > 0 && info->remaining > 0) {
      ...
      if (skip < info->remaining) {
        memmove(info->buffer, info->buffer + skip * samplesize,
                (info->remaining - skip) * samplesize);
      }
      info->remaining -= skip;
...
static int tta_seek_sample(DB_fileinfo_t *_info, int sample) {
  tta_info_t *info = (tta_info_t *)_info;
...
static int tta_seek(DB_fileinfo_t *_info, float time) {
  return tta_seek_sample(_info, time * _info->fmt.samplerate);
}

static DB_playItem_t *tta_insert(ddb_playlist_t *plt, DB_playItem_t *after,
                                 const char *fname) {
  tta_info tta;
  ...
  DB_playItem_t *cue =
      deadbeef->plt_process_cue(plt, after, it, totalsamples, tta.SAMPLERATE);
  if (cue) {
...
static int tta_start(void) { return 0; }

static int tta_stop(void) { return 0; }
...
static DB_decoder_t plugin = {
    ...
    .plugin.descr =
        "tta decoder based on TTA Hardware Players Library Version 1.2",
    .plugin.copyright =
        "TTA plugin for DeaDBeeF\n"
        "Copyright (C) 2009-2014 Alexey Yakovenko\n"
        "\n"
...
DB_plugin_t *tta_load(DB_functions_t *api) {
  deadbeef = api;
