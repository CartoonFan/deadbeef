...
// this would mean "execute playback_random action when Super+n is pressed
globally"
//
...
static void prettify_forward_slash(const char *src, char *dst, int size) {
    char *start = dst;
    const char arrow[] = " → ";
...
static const char *get_display_action_title(const char *title) {
...
DB_plugin_action_t *find_action_by_name(const char *command) {
        ...
        if (p->get_actions) {
            ...
            while (actions) {
                if (actions->name && actions->title &&
                        !strcasecmp(actions->name, command)) {
                    break; // found
                }
...
static int hotkeys_load(void) {
    GtkWidget *hotkeys = lookup_widget(prefwin, "hotkeys_list");
    GtkListStore *hkstore =
        GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(hotkeys)));
    ...
    while (item) {
        ...
        gtk_list_store_set(hkstore, &iter, 0, keycombo, 1, title, 2, ctx_names[ctx],
                           3, isglobal, 4, action->name, 5, ctx, -1);
        n_items++;
...
static void hotkeys_save(void) {
    ...
    while (res) {
        ...
        snprintf(value, sizeof(value), "\"%s\" %d %d %s",
                 g_value_get_string(&keycombo), g_value_get_int(&context),
                 g_value_get_boolean(&global), g_value_get_string(&action));
...
const char *action_tree_append(const char *title, GtkTreeStore *store,
                               GtkTreeIter *root_iter, GtkTreeIter *iter) {
    ...
    for (;;) {
        ...
        *s = 0;
        ...
        gboolean res =
            gtk_tree_model_iter_children(GTK_TREE_MODEL(store), &i, root_iter);
        if (!res) {
...
static gboolean set_current_action(GtkTreeModel *model, GtkTreePath *path,
    ...
    if (name && binding->name && !strcmp(binding->name, name) &&
            binding->ctx == g_value_get_int(&ctx_val)) {
        gtk_tree_view_expand_to_path(GTK_TREE_VIEW(binding->treeview), path);
        gtk_tree_view_set_cursor(GTK_TREE_VIEW(binding->treeview), path, NULL,
                                 FALSE);
        return TRUE;
    }
    return FALSE;
...
void init_action_tree(GtkWidget *actions, const char *act, int ctx) {
    GtkTreeViewColumn *hk_act_col1 = gtk_tree_view_column_new_with_attributes(
                                         _("Action"), gtk_cell_renderer_text_new(), "text", 0, NULL);
    ...
    GtkTreeStore *actions_store =
        gtk_tree_store_new(3, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_INT);
    ...
    gtk_tree_store_set(actions_store, &action_selection_iter, 0,
                       _("Selected track(s)"), -1);
    GtkTreeIter action_playlist_iter;
    gtk_tree_store_append(actions_store, &action_playlist_iter, NULL);
    gtk_tree_store_set(actions_store, &action_playlist_iter, 0,
                       _("Current playlist"), -1);
    GtkTreeIter action_nowplaying_iter;
    gtk_tree_store_append(actions_store, &action_nowplaying_iter, NULL);
    gtk_tree_store_set(actions_store, &action_nowplaying_iter, 0,
                       _("Now playing"), -1);
    ...
    for (int i = 0; plugins[i]; i++) {
        ...
        if (p->get_actions) {
            ...
            while (actions) {
                if (actions->name &&
                        actions
                        ->title) { // only add actions with both the name and the title
                    char title[100];

                    GtkTreeIter iter;
                    const char *t;
                    if (actions->flags & DB_ACTION_COMMON) {
                        t = action_tree_append(actions->title, actions_store,
                                               &action_main_iter, &iter);
                        unescape_forward_slash(t, title, sizeof(title));
                        gtk_tree_store_set(actions_store, &iter, 0, title, 1, actions->name,
                                           2, DDB_ACTION_CTX_MAIN, -1);
                    }
                    if (actions->flags &
                            (DB_ACTION_SINGLE_TRACK | DB_ACTION_MULTIPLE_TRACKS |
                             DB_ACTION_CAN_MULTIPLE_TRACKS)) {
                        t = action_tree_append(actions->title, actions_store,
                                               &action_selection_iter, &iter);
                        unescape_forward_slash(t, title, sizeof(title));
                        gtk_tree_store_set(actions_store, &iter, 0, title, 1, actions->name,
                                           2, DDB_ACTION_CTX_SELECTION, -1);
                        if (!(actions->flags & DB_ACTION_EXCLUDE_FROM_CTX_PLAYLIST)) {
                            t = action_tree_append(actions->title, actions_store,
                                                   &action_playlist_iter, &iter);
                            unescape_forward_slash(t, title, sizeof(title));
                            gtk_tree_store_set(actions_store, &iter, 0, title, 1,
                                               actions->name, 2, DDB_ACTION_CTX_PLAYLIST, -1);
                        }
                        t = action_tree_append(actions->title, actions_store,
                                               &action_nowplaying_iter, &iter);
                        unescape_forward_slash(t, title, sizeof(title));
                        gtk_tree_store_set(actions_store, &iter, 0, title, 1, actions->name,
                                           2, DDB_ACTION_CTX_NOWPLAYING, -1);
                    }
                } else {
                    //                    fprintf (stderr, "WARNING: action %s/%s from
                    //                    plugin %s is missing name and/or title\n",
    ...
    gtk_tree_view_set_model(GTK_TREE_VIEW(actions),
                            GTK_TREE_MODEL(actions_store));

    if (act && ctx != -1) {
        actionbinding_t binding = {.name = act, .ctx = ctx, .treeview = actions};
        gtk_tree_model_foreach(GTK_TREE_MODEL(actions_store), set_current_action,
                               (void *)&binding);
    }
}
...
void on_hotkeys_actions_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (response == GTK_RESPONSE_OK) {
        ...
        set_button_action_label(name, ctx,
                                lookup_widget(prefwin, "hotkeys_actions"));
    }
    gtk_widget_destroy(dlg);
}

void prefwin_init_hotkeys(GtkWidget *_prefwin) {
    DB_plugin_t *hkplug = deadbeef->plug_get_for_id("hotkeys");
    if (!hkplug) {
        // remove hotkeys tab
        gtk_notebook_remove_page(GTK_NOTEBOOK(lookup_widget(_prefwin, "notebook")),
                                 6);
        return;
    }
    ...
    // setup hotkeys list
    GtkTreeViewColumn *hk_col1 = gtk_tree_view_column_new_with_attributes(
                                     _("Key combination"), gtk_cell_renderer_text_new(), "text", 0, NULL);
    gtk_tree_view_column_set_resizable(hk_col1, TRUE);
    GtkTreeViewColumn *hk_col2 = gtk_tree_view_column_new_with_attributes(
                                     _("Action"), gtk_cell_renderer_text_new(), "text", 1, NULL);
    gtk_tree_view_column_set_resizable(hk_col2, TRUE);
    GtkTreeViewColumn *hk_col3 = gtk_tree_view_column_new_with_attributes(
                                     _("Context"), gtk_cell_renderer_text_new(), "text", 2, NULL);
    gtk_tree_view_column_set_resizable(hk_col3, TRUE);
    GtkTreeViewColumn *hk_col4 = gtk_tree_view_column_new_with_attributes(
                                     _("Is global"), gtk_cell_renderer_text_new(), "text", 3, NULL);
...
void set_button_action_label(const char *act, int action_ctx,
                             GtkWidget *button) {
    if (act && action_ctx >= 0) {
        ...
        if (action) {
            ...
            char s[200];
            snprintf(s, sizeof(s), "%s%s%s", ctx_str ? ctx_str : "",
                     ctx_str ? " ⇒ " : "", action->title);
            char s_fixed[200];
...
void on_hotkeys_list_cursor_changed(GtkTreeView *treeview, gpointer user_data) {
    ...
    if (path && gtk_tree_model_get_iter(model, &iter, path)) {
        ...
        gtk_toggle_button_set_active(
            GTK_TOGGLE_BUTTON(lookup_widget(prefwin, "hotkey_is_global")),
            g_value_get_boolean(&val_isglobal));
        ...
        gtk_button_set_label(GTK_BUTTON(lookup_widget(prefwin, "hotkeys_set_key")),
                             keycombo ? keycombo : "");
    } else {
        gtk_widget_set_sensitive(lookup_widget(prefwin, "hotkeys_actions"), FALSE);
        gtk_widget_set_sensitive(lookup_widget(prefwin, "hotkey_is_global"), FALSE);
        gtk_toggle_button_set_active(
            GTK_TOGGLE_BUTTON(lookup_widget(prefwin, "hotkey_is_global")), FALSE);
        gtk_widget_set_sensitive(lookup_widget(prefwin, "hotkeys_set_key"), FALSE);
        gtk_button_set_label(GTK_BUTTON(lookup_widget(prefwin, "hotkeys_set_key")),
                             _("<Not set>"));
    }
...
void on_hotkey_add_clicked(GtkButton *button, gpointer user_data) {
    ...
    GtkTreeIter iter;
    gtk_list_store_append(hkstore, &iter);
    gtk_list_store_set(hkstore, &iter, 0, _("<Not set>"), 1, _("<Not set>"), 2,
                       _("<Not set>"), 3, 0, 4, NULL, 5, -1, -1);
        ...
        // update the tree
        {
            ...
            if (path && gtk_tree_model_get_iter(model, &iter, path)) {
                if (action) {
                    ...
                    char title[100];
                    unescape_forward_slash(t, title, sizeof(title));
                    gtk_list_store_set(GTK_LIST_STORE(model), &iter, 1, title, 4,
                                       action->name, 5, ctx, 2, ctx_names[ctx], -1);
                } else {
                    gtk_list_store_set(GTK_LIST_STORE(model), &iter, 1, _("<Not set>"), 4,
                                       NULL, 2, _("<Not set>"), -1);
                }
            }
        }
...
static const char *get_name_for_keycode(int keycode) {
...
static void get_keycombo_string(guint accel_key, GdkModifierType accel_mods,
                                char *new_value) {
    // build value
    new_value[0] = 0;
    if (!accel_key) {
...
gboolean on_hotkeys_set_key_key_press_event(GtkWidget *widget,
    ...
    gdk_keymap_translate_keyboard_state(
        gdk_keymap_get_for_display(display), event->hardware_keycode,
        accel_mods & (~GDK_SHIFT_MASK), 0, &accel_key, NULL, NULL,
        &consumed_modifiers);
...
static void hotkey_grab_focus(GtkWidget *widget) {
    GdkDisplay *display = gtk_widget_get_display(widget);
    if (gtkui_hotkey_grabbing) {
        return;
    }
    gtkui_hotkey_grabbing = 0;
    if (GDK_GRAB_SUCCESS != gdk_keyboard_grab(gtk_widget_get_window(widget),
...
void on_hotkeys_defaults_clicked(GtkButton *button, gpointer user_data) {
    GtkWidget *dlg = gtk_message_dialog_new(
                         GTK_WINDOW(prefwin), GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING,
                         GTK_BUTTONS_YES_NO, _("All your custom-defined hotkeys will be lost."));
    gtk_window_set_transient_for(GTK_WINDOW(dlg), GTK_WINDOW(prefwin));
    gtk_message_dialog_format_secondary_text(
        GTK_MESSAGE_DIALOG(dlg), _("This operation cannot be undone. Proceed?"));
...
void gtkui_set_default_hotkeys(void) {
    deadbeef->conf_remove_items("hotkey.key");
    ...
    deadbeef->conf_set_str("hotkey.key07",
                           "\"Ctrl m\" 0 0 toggle_stop_after_current");
    deadbeef->conf_set_str("hotkey.key08",
                           "\"Ctrl j\" 0 0 jump_to_current_track");
    deadbeef->conf_set_str("hotkey.key09", "\"F1\" 0 0 help");
    deadbeef->conf_set_str("hotkey.key10", "\"Delete\" 1 0 remove_from_playlist");
    deadbeef->conf_set_str("hotkey.key11",
                           "\"Ctrl w\" 0 0 remove_current_playlist");
    ...
    deadbeef->conf_set_str("hotkey.key32",
                           "\"Ctrl k\" 0 0 toggle_stop_after_album");
    deadbeef->conf_save();
}

void gtkui_import_0_5_global_hotkeys(void) {
    int n = 40;
    deadbeef->conf_lock();
