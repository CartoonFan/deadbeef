...
#define DDB_LISTVIEW(obj)                                                      \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), DDB_TYPE_LISTVIEW, DdbListview))
#define DDB_LISTVIEW_CLASS(obj)                                                \
  (G_TYPE_CHECK_CLASS_CAST((obj), DDB_TYPE_LISTVIEW, DdbListviewClass))
#define DDB_IS_LISTVIEW(obj)                                                   \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), DDB_TYPE_LISTVIEW))
#define DDB_IS_LISTVIEW_CLASS(obj)                                             \
  (G_TYPE_CHECK_CLASS_TYPE((obj), DDB_TYPE_LISTVIEW))
#define DDB_LISTVIEW_GET_CLASS(obj)                                            \
  (G_TYPE_INSTANCE_GET_CLASS((obj), DDB_TYPE_LISTVIEW, DdbListviewClass))
...
typedef struct {
  ...
  int (*get_group)(DdbListview *listview, DdbListviewIter it, char *str,
                   int size, int index);

  void (*drag_n_drop)(DdbListviewIter before, DdbPlaylistHandle playlist_from,
                      uint32_t *indices, int length, int copy);
  void (*external_drag_n_drop)(DdbListviewIter before, char *mem, int length);

  void (*draw_group_title)(DdbListview *listview, cairo_t *drawable,
                           DdbListviewIter iter, int x, int y, int width,
                           int height, int group_depth);
  void (*draw_album_art)(DdbListview *listview, cairo_t *cr, DB_playItem_t *it,
                         void *user_data, int pinned, int next_y, int x, int y,
                         int width, int height);
  void (*draw_column_data)(DdbListview *listview, cairo_t *cr,
                           DdbListviewIter it, int idx, int align,
                           void *user_data, GdkColor *fg_clr, int x, int y,
                           int width, int height, int even);
  ...
  // callbacks
  void (*list_context_menu)(DdbListview *listview, DdbListviewIter iter,
                            int idx, int plt_iter);
  void (*list_empty_region_context_menu)(DdbListview *listview);
  void (*header_context_menu)(DdbListview *listview, int col);
  void (*handle_doubleclick)(DdbListview *listview, DdbListviewIter iter,
                             int idx);
...
struct _DdbListview {
  ...
  ddb_playlist_t
      *plt; // current playlist (refcounted), must be unreffed with the group
...
void ddb_listview_set_binding(DdbListview *listview,
                              DdbListviewBinding *binding);
void ddb_listview_draw_row(DdbListview *listview, int idx,
                           DdbListviewIter iter);
void ddb_listview_select_single(DdbListview *listview, int sel);
void ddb_listview_select_range(DdbListview *ps, int start, int end);
void ddb_listview_scroll_to(DdbListview *listview, int rowpos);
...
void ddb_listview_column_append(DdbListview *listview, const char *title,
                                int width, int align_right, minheight_cb_t,
...
int ddb_listview_column_get_info(DdbListview *listview, int col,
                                 ...
                                 int *align_right, minheight_cb_t *,
                                 int *is_artwork, int *color_override,
                                 GdkColor *color, void **user_data);
