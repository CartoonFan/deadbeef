...
void w_init(void) { rootwidget = w_create("box"); }
...
ddb_gtkui_widget_t *w_get_rootwidget(void) { return rootwidget; }
...
int w_get_design_mode(void) { return design_mode; }

static gboolean w_init_cb(void *data) {
...
void w_replace(ddb_gtkui_widget_t *w, ddb_gtkui_widget_t *from,
               ddb_gtkui_widget_t *to) {
  if (w->replace) {
    w->replace(w, from, to);
    if (to->init) {
...
static const char *w_get_title(ddb_gtkui_widget_t *w) {
...
const char *w_unknown_load(struct ddb_gtkui_widget_s *base, const char *type,
                           const char *s) {
  ...
  if (!(*p)) {
    fprintf(stderr,
            "reached EOL before expected { while trying to load unknown widget "
            "%s\n",
            w->expected_type);
    return NULL;
  }
...
static void w_unknown_save(struct ddb_gtkui_widget_s *base, char *s, int sz) {
  w_unknown_t *w = (w_unknown_t *)base;
  int l = strlen(s);
  s += l;
  sz -= l;
...
static gboolean unknown_draw(GtkWidget *widget, cairo_t *cr,
                             gpointer user_data) {
  ...
  char s[1000];
  snprintf(s, sizeof(s), _("Widget \"%s\" is not available"),
           ((w_unknown_t *)w)->expected_type);

  cairo_show_text(cr, s);
  return TRUE;
}

static gboolean unknown_expose_event(GtkWidget *widget, GdkEventExpose *event,
                                     gpointer user_data) {
...
ddb_gtkui_widget_t *w_unknown_create(const char *type) {
  ...
  g_signal_connect_after((gpointer)w->drawarea, "expose_event",
                         G_CALLBACK(unknown_expose_event), w);
#else
  g_signal_connect_after((gpointer)w->drawarea, "draw",
                         G_CALLBACK(unknown_draw), w);
#endif
  w_override_signals(w->base.widget, w);
  return (ddb_gtkui_widget_t *)w;
}

const char *w_create_from_string(const char *s, ddb_gtkui_widget_t **parent) {
  char t[MAX_TOKEN];
  s = gettoken(s, t);
  if (!s) {
    return NULL;
...
static gboolean w_draw_event(GtkWidget *widget, cairo_t *cr,
                             gpointer user_data) {
...
gboolean w_expose_event(GtkWidget *widget, GdkEventExpose *event,
                        gpointer user_data) {
  cairo_t *cr = gdk_cairo_create(gtk_widget_get_window(widget));
...
void save_widget_to_string(char *str, int sz, ddb_gtkui_widget_t *w) {
...
static void on_replace_activate(GtkMenuItem *menuitem, gpointer user_data) {
  ddb_gtkui_widget_t *ui_widget =
      g_object_get_data(menuitem, associated_widget_data_id);
  for (w_creator_t *cr = w_creators; cr; cr = cr->next) {
    if (cr->type == user_data) {
      // hack for single-instance
      // first replace with a placeholder, so that the original widget is
      // destroyed then do the real replacement
...
void show_widget(GtkWidget *widget, gpointer data) { gtk_widget_show(widget); }

static GtkRequisition prev_req;

void w_menu_deactivate(GtkMenuShell *menushell, gpointer user_data) {
  hidden = 0;
  ddb_gtkui_widget_t *w = user_data;
...
static GtkWidget *create_widget_menu(ddb_gtkui_widget_t *ui_widget) {
  GtkWidget *menu;
  GtkWidget *submenu;
  GtkWidget *item;
  menu = gtk_menu_new();
  ...
  for (w_creator_t *cr = w_creators; cr; cr = cr->next) {
    if (cr->title) {
      ...
      g_signal_connect((gpointer)item, "activate",
                       G_CALLBACK(on_replace_activate), (void *)cr->type);
    }
  }
  ...
  if (strcmp(ui_widget->type, "placeholder")) {
    ...
    gtk_widget_show(item);
    gtk_container_add(GTK_CONTAINER(menu), item);
    g_signal_connect((gpointer)item, "activate", G_CALLBACK(on_delete_activate),
                     ui_widget);

    item = gtk_menu_item_new_with_mnemonic(_("Cut"));
    gtk_widget_show(item);
    gtk_container_add(GTK_CONTAINER(menu), item);
    g_signal_connect((gpointer)item, "activate", G_CALLBACK(on_cut_activate),
                     ui_widget);

    item = gtk_menu_item_new_with_mnemonic(_("Copy"));
    gtk_widget_show(item);
    gtk_container_add(GTK_CONTAINER(menu), item);
    g_signal_connect((gpointer)item, "activate", G_CALLBACK(on_copy_activate),
                     ui_widget);
  }
  ...
  gtk_widget_show(item);
  gtk_container_add(GTK_CONTAINER(menu), item);
  g_signal_connect((gpointer)item, "activate", G_CALLBACK(on_paste_activate),
                   ui_widget);
...
gboolean w_button_press_event(GtkWidget *widget, GdkEventButton *event,
                              gpointer user_data) {
  ...
  g_signal_connect((gpointer)menu, "deactivate", G_CALLBACK(w_menu_deactivate),
                   user_data);
...
void w_override_signals(GtkWidget *widget, gpointer user_data) {
  g_signal_connect((gpointer)widget, "button_press_event",
                   G_CALLBACK(w_button_press_event), user_data);
#if !GTK_CHECK_VERSION(3, 0, 0)
  g_signal_connect((gpointer)widget, "expose_event", G_CALLBACK(w_expose_event),
                   user_data);
#else
  g_signal_connect((gpointer)widget, "draw", G_CALLBACK(w_draw_event),
                   user_data);
#endif
  if (GTK_IS_CONTAINER(widget)) {
  ...
  for (;;) {
    ...
    for (c = w_creators; c; c = c->next) {
      if (!strcmp(c->type, type)) {
        fprintf(stderr,
                "gtkui w_reg_widget: widget type %s already registered\n",
                type);
        return;
      }
    }
...
static int get_num_widgets(ddb_gtkui_widget_t *w, const char *type) {
  int num = 0;
    ...
    if (!strcmp(c->type, type)) {
      if (c->flags & DDB_WF_SINGLE_INSTANCE) {
        ...
        if (num) {
          ...
          w->text =
              strdup(_("Multiple widgets of this type are not supported"));
          return (ddb_gtkui_widget_t *)w;
        }
      }
...
void w_container_add(ddb_gtkui_widget_t *cont, ddb_gtkui_widget_t *child) {
  GtkWidget *container = w_get_container(cont);
  gtk_container_add(GTK_CONTAINER(container), child->widget);
  gtk_widget_show(child->widget);
}

void w_container_remove(ddb_gtkui_widget_t *cont, ddb_gtkui_widget_t *child) {
...
////// placeholder widget
gboolean w_placeholder_draw(GtkWidget *widget, cairo_t *cr,
                            gpointer user_data) {
  if (!design_mode) {
    return FALSE;
  }
...
gboolean w_placeholder_expose_event(GtkWidget *widget, GdkEventExpose *event,
                                    gpointer user_data) {
  cairo_t *cr = gdk_cairo_create(gtk_widget_get_window(widget));
...
ddb_gtkui_widget_t *w_placeholder_create(void) {
...
#if !GTK_CHECK_VERSION(3, 0, 0)
  g_signal_connect_after((gpointer)w->drawarea, "expose_event",
                         G_CALLBACK(w_placeholder_expose_event), w);
#else
  g_signal_connect_after((gpointer)w->drawarea, "draw",
                         G_CALLBACK(w_placeholder_draw), w);
#endif
  w_override_signals(w->base.widget, w);
  return (ddb_gtkui_widget_t *)w;
}
...
// dummy widget
static const char *w_dummy_load(struct ddb_gtkui_widget_s *w, const char *type,
                                const char *s) {
...
static void w_dummy_save(struct ddb_gtkui_widget_s *w, char *s, int sz) {
  char save[1000] = "";
  char *pp = save;
  int ss = sizeof(save);
  int n;
...
void w_dummy_init(ddb_gtkui_widget_t *wbase) {
...
static void w_dummy_destroy(ddb_gtkui_widget_t *wbase) {
  w_dummy_t *w = (w_dummy_t *)wbase;
  if (w->text) {
    free(w->text);
    w->text = NULL;
...
ddb_gtkui_widget_t *w_dummy_create(void) {
...
const char *w_splitter_load(struct ddb_gtkui_widget_s *w, const char *type,
                            const char *s) {
...
void w_splitter_save(struct ddb_gtkui_widget_s *w, char *s, int sz) {
  w_splitter_t *sp = (w_splitter_t *)w;
  int locked = ddb_splitter_get_size_mode(DDB_SPLITTER(sp->box));

  float ratio = ddb_splitter_get_proportion(DDB_SPLITTER(sp->box));
  char spos[100];
  // NOTE: we use pos instead of size1 to ensure compatiblity with older
  // deadbeef versions
  snprintf(spos, sizeof(spos), " locked=%d ratio=%f pos=%d size2=%d", locked,
           ratio, sp->size1, sp->size2);
  strncat(s, spos, sz);
}

void w_splitter_add(ddb_gtkui_widget_t *w, ddb_gtkui_widget_t *child) {
  w_container_add(w, child);
}

GtkWidget *w_splitter_get_container(struct ddb_gtkui_widget_s *b) {
  return ((w_splitter_t *)b)->box;
}

void on_splitter_lock_c1_toggled(GtkCheckMenuItem *checkmenuitem,
                                 gpointer user_data) {
  ...
  if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(checkmenuitem))) {
    ...
    ddb_splitter_set_size_mode(DDB_SPLITTER(sp->box),
                               DDB_SPLITTER_SIZE_MODE_LOCK_C1);
  }
}

void on_splitter_lock_c2_toggled(GtkCheckMenuItem *checkmenuitem,
                                 gpointer user_data) {
  w_splitter_t *sp = (w_splitter_t *)user_data;
  if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(checkmenuitem))) {
...
void on_splitter_lock_prop_toggled(GtkCheckMenuItem *checkmenuitem,
                                   gpointer user_data) {
  w_splitter_t *sp = (w_splitter_t *)user_data;
  if (gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(checkmenuitem))) {
...
void w_splitter_initmenu(struct ddb_gtkui_widget_s *w, GtkWidget *menu) {
  w_splitter_t *sp = (w_splitter_t *)w;
  GtkOrientation orientation =
      ddb_splitter_get_orientation(DDB_SPLITTER(sp->box));

  GSList *group = NULL;

  GtkWidget *item =
      gtk_radio_menu_item_new_with_mnemonic(group, _("Proportional Sizing"));
  ...
  g_signal_connect((gpointer)item, "toggled",
                   G_CALLBACK(on_splitter_lock_prop_toggled), w);

  if (orientation == GTK_ORIENTATION_VERTICAL) {
    item =
        gtk_radio_menu_item_new_with_mnemonic(group, _("Lock Top Pane Height"));
  } else {
    item =
        gtk_radio_menu_item_new_with_mnemonic(group, _("Lock Left Pane Width"));
  }
  group = gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(item));
  gtk_widget_show(item);
  ...
  gtk_container_add(GTK_CONTAINER(menu), item);
  g_signal_connect((gpointer)item, "toggled",
                   G_CALLBACK(on_splitter_lock_c1_toggled), w);

  if (orientation == GTK_ORIENTATION_VERTICAL) {
    item = gtk_radio_menu_item_new_with_mnemonic(group,
                                                 _("Lock Bottom Pane Height"));
  } else {
    item = gtk_radio_menu_item_new_with_mnemonic(group,
                                                 _("Lock Right Pane Width"));
  }
  group = gtk_radio_menu_item_get_group(GTK_RADIO_MENU_ITEM(item));
  gtk_widget_show(item);
  ...
  gtk_container_add(GTK_CONTAINER(menu), item);
  g_signal_connect((gpointer)item, "toggled",
                   G_CALLBACK(on_splitter_lock_c2_toggled), w);
}

void w_splitter_replace(ddb_gtkui_widget_t *cont, ddb_gtkui_widget_t *child,
                        ddb_gtkui_widget_t *newchild) {
  int ntab = 0;
  ddb_gtkui_widget_t *prev = NULL;
  for (ddb_gtkui_widget_t *c = cont->children; c; c = c->next, ntab++) {
    if (c == child) {
      ...
      if (ntab == 0) {
        ddb_splitter_add_child_at_pos(DDB_SPLITTER(container), newchild->widget,
                                      0);
      } else {
        ddb_splitter_add_child_at_pos(DDB_SPLITTER(container), newchild->widget,
                                      1);
...
void w_splitter_remove(ddb_gtkui_widget_t *cont, ddb_gtkui_widget_t *child) {
  GtkWidget *container = w_get_container(cont);
  gtk_container_remove(GTK_CONTAINER(container), child->widget);
}

void w_splitter_init(ddb_gtkui_widget_t *base) {
...
////// vsplitter widget
ddb_gtkui_widget_t *w_vsplitter_create(void) {
...
////// hsplitter widget
ddb_gtkui_widget_t *w_hsplitter_create(void) {
  w_splitter_t *w = malloc(sizeof(w_splitter_t));
  memset(w, 0, sizeof(w_splitter_t));
  w->ratio = 0.5f;
  w->locked = DDB_SPLITTER_SIZE_MODE_PROP;
...
static void w_tabs_destroy(ddb_gtkui_widget_t *w) {
...
const char *w_tabs_load(struct ddb_gtkui_widget_s *widget, const char *type,
                        const char *s) {
...
void w_tabs_save(struct ddb_gtkui_widget_s *widget, char *s, int sz) {
  w_tabs_t *w = (w_tabs_t *)widget;
  ...
  for (int i = 0; i < num_pages; i++) {
    GtkWidget *child =
        gtk_notebook_get_nth_page(GTK_NOTEBOOK(w->base.widget), i);
    const char *text =
        gtk_notebook_get_tab_label_text(GTK_NOTEBOOK(w->base.widget), child);
...
static void tabs_add_tab(gpointer user_data) {
...
static void tabs_remove_tab(gpointer user_data, int tab) {
  w_tabs_t *w = user_data;
  int i = 0;
  int num_pages = gtk_notebook_get_n_pages(GTK_NOTEBOOK(w->base.widget));
  for (ddb_gtkui_widget_t *c = w->base.children; c; c = c->next, i++) {
...
static gboolean tab_button_press_event(GtkWidget *widget, GdkEventButton *event,
                                       gpointer user_data);

static void on_rename_tab_activate(GtkMenuItem *menuitem, gpointer user_data) {
  w_tabs_t *w = user_data;
  ...
  int active = gtk_notebook_get_current_page(GTK_NOTEBOOK(w->base.widget));
  GtkWidget *child =
      gtk_notebook_get_nth_page(GTK_NOTEBOOK(w->base.widget), active);
  gtk_entry_set_text(GTK_ENTRY(e), gtk_notebook_get_tab_label_text(
                                       GTK_NOTEBOOK(w->base.widget), child));
  int res = gtk_dialog_run(GTK_DIALOG(dlg));
  if (res == GTK_RESPONSE_OK) {
    gtk_notebook_set_tab_label_text(GTK_NOTEBOOK(w->base.widget), child,
                                    gtk_entry_get_text(GTK_ENTRY(e)));
  }
  gtk_widget_destroy(dlg);
}

static void on_remove_tab_activate(GtkMenuItem *menuitem, gpointer user_data) {
  w_tabs_t *w = user_data;
  tabs_remove_tab(w, w->clicked_page);
}

static void on_add_tab_activate(GtkMenuItem *menuitem, gpointer user_data) {
  tabs_add_tab(user_data);
}

static void on_move_tab_left_activate(GtkMenuItem *menuitem,
                                      gpointer user_data) {
  w_tabs_t *w = user_data;
  if (w->clicked_page <= 0) {
    return;
  ...
  for (ddb_gtkui_widget_t *c = w->base.children; c; c = c->next, i++) {
    if (i == w->clicked_page - 1) {
      ...
      gtk_notebook_insert_page(GTK_NOTEBOOK(w->base.widget), newchild->widget,
                               label, w->clicked_page - 1);
#if GTK_CHECK_VERSION(3, 0, 0)
      gtk_label_set_ellipsize(GTK_LABEL(label), PANGO_ELLIPSIZE_END);
      gtk_misc_set_padding(GTK_MISC(label), 0, 0);
      gtk_container_child_set(GTK_CONTAINER(w->base.widget), newchild->widget,
                              "tab-expand", TRUE, "tab-fill", TRUE, NULL);
#endif
      gtk_notebook_set_current_page(GTK_NOTEBOOK(w->base.widget),
                                    w->clicked_page - 1);
      w->clicked_page--;
      break;
    }
...
static void on_move_tab_right_activate(GtkMenuItem *menuitem,
                                       gpointer user_data) {
  w_tabs_t *w = user_data;
...
static void on_tab_popup_menu(GtkWidget *widget, gpointer user_data) {
  w_tabs_t *w = user_data;
  GtkWidget *menu;
  GtkWidget *item;
  menu = gtk_menu_new();
  ...
  gtk_container_add(GTK_CONTAINER(menu), item);
  g_signal_connect((gpointer)item, "activate",
                   G_CALLBACK(on_rename_tab_activate), w);

  item = gtk_menu_item_new_with_mnemonic(_("Remove Tab"));
  gtk_widget_show(item);
  gtk_container_add(GTK_CONTAINER(menu), item);
  g_signal_connect((gpointer)item, "activate",
                   G_CALLBACK(on_remove_tab_activate), w);

  item = gtk_menu_item_new_with_mnemonic(_("Add New Tab"));
  gtk_widget_show(item);
  gtk_container_add(GTK_CONTAINER(menu), item);
  g_signal_connect((gpointer)item, "activate", G_CALLBACK(on_add_tab_activate),
                   w);
  ...
  gtk_container_add(GTK_CONTAINER(menu), item);
  g_signal_connect((gpointer)item, "activate",
                   G_CALLBACK(on_move_tab_left_activate), w);

  item = gtk_menu_item_new_with_mnemonic(_("Move Tab Right"));
  gtk_widget_show(item);
  gtk_container_add(GTK_CONTAINER(menu), item);
  g_signal_connect((gpointer)item, "activate",
                   G_CALLBACK(on_move_tab_right_activate), w);
...
static void w_tabs_add(ddb_gtkui_widget_t *cont, ddb_gtkui_widget_t *child) {
  ...
  gtk_label_set_ellipsize(GTK_LABEL(label), PANGO_ELLIPSIZE_END);
  gtk_misc_set_padding(GTK_MISC(label), 0, 0);
  gtk_container_child_set(GTK_CONTAINER(cont->widget), child->widget,
                          "tab-expand", TRUE, "tab-fill", TRUE, NULL);
#endif
}

static void w_tabs_replace(ddb_gtkui_widget_t *cont, ddb_gtkui_widget_t *child,
                           ddb_gtkui_widget_t *newchild) {
  int ntab = 0;
  ddb_gtkui_widget_t *prev = NULL;
  for (ddb_gtkui_widget_t *c = cont->children; c;
       prev = c, c = c->next, ntab++) {
    if (c == child) {
      newchild->next = c->next;
      if (prev) {
        prev->next = newchild;
      ...
      gtk_widget_show(label);
      gtk_widget_show(newchild->widget);
      int pos = gtk_notebook_insert_page(GTK_NOTEBOOK(cont->widget),
                                         newchild->widget, label, ntab);
#if GTK_CHECK_VERSION(3, 0, 0)
      gtk_label_set_ellipsize(GTK_LABEL(label), PANGO_ELLIPSIZE_END);
      gtk_misc_set_padding(GTK_MISC(label), 0, 0);
      gtk_container_child_set(GTK_CONTAINER(cont->widget), newchild->widget,
                              "tab-expand", TRUE, "tab-fill", TRUE, NULL);
#endif
...
static gboolean get_event_coordinates_in_widget(GtkWidget *widget,
                                                GdkEventButton *event, gint *x,
                                                gint *y) {
...
static gboolean on_tabs_button_press_event(GtkWidget *notebook,
...
static void w_tabs_initmenu(struct ddb_gtkui_widget_s *w, GtkWidget *menu) {
  GtkWidget *item;
  item = gtk_menu_item_new_with_mnemonic(_("Add New Tab"));
  gtk_widget_show(item);
  gtk_container_add(GTK_CONTAINER(menu), item);
...
static void w_tabs_init(ddb_gtkui_widget_t *base) {
  ...
  if (w->titles) {
    ...
    while (page < w->num_tabs) {
      GtkWidget *child =
          gtk_notebook_get_nth_page(GTK_NOTEBOOK(w->base.widget), page);
      if (w->titles[page]) {
        gtk_notebook_set_tab_label_text(GTK_NOTEBOOK(w->base.widget), child,
                                        w->titles[page]);
#if GTK_CHECK_VERSION(3, 0, 0)
        GtkLabel *label = GTK_LABEL(
            gtk_notebook_get_tab_label(GTK_NOTEBOOK(w->base.widget), child));
        gtk_label_set_ellipsize(GTK_LABEL(label), PANGO_ELLIPSIZE_END);
        gtk_misc_set_padding(GTK_MISC(label), 0, 0);
#endif
      }
...
ddb_gtkui_widget_t *w_tabs_create(void) {
...
#if !GTK_CHECK_VERSION(3, 0, 0)
  g_signal_connect((gpointer)w->base.widget, "expose_event",
                   G_CALLBACK(w_expose_event), w);
#else
  g_signal_connect((gpointer)w->base.widget, "draw", G_CALLBACK(w_draw_event),
                   w);
#endif
  g_signal_connect((gpointer)w->base.widget, "button_press_event",
                   G_CALLBACK(on_tabs_button_press_event), w);
...
ddb_gtkui_widget_t *w_box_create(void) {
...
//// tabstrip widget
static gboolean tabstrip_refresh_cb(void *ctx) {
  w_tabstrip_t *w = ctx;
  ddb_tabstrip_refresh(DDB_TABSTRIP(w->tabstrip));
  return FALSE;
}
static int w_tabstrip_message(ddb_gtkui_widget_t *w, uint32_t id, uintptr_t ctx,
                              uint32_t p1, uint32_t p2) {
  switch (id) {
  case DB_EV_PLAYLISTCHANGED:
    if (p1 == DDB_PLAYLIST_CHANGE_TITLE || p1 == DDB_PLAYLIST_CHANGE_POSITION ||
  ...
  case DB_EV_CONFIGCHANGED:
    if (ctx) {
      ...
      if (gtkui_tabstrip_override_conf(conf_str) ||
          gtkui_tabstrip_colors_conf(conf_str) ||
          gtkui_tabstrip_font_conf(conf_str)) {
        g_idle_add(tabstrip_refresh_cb, w);
      }
    }
...
ddb_gtkui_widget_t *w_tabstrip_create(void) {
...
static w_trackdata_t *playlist_trackdata(DdbListview *listview,
                                         DB_playItem_t *track) {
...
static gboolean playlist_tabstriprefresh_cb(gpointer p) {
  w_tabbed_playlist_t *tp = p;
  ddb_tabstrip_refresh(tp->tabstrip);
  return FALSE;
}

static gboolean trackinfochanged_cb(gpointer data) {
...
static gboolean paused_cb(gpointer data) {
...
static gboolean playlist_config_changed_cb(gpointer data) {
...
static gboolean playlist_sort_reset_cb(gpointer data) {
  ddb_listview_col_sort_update(DDB_LISTVIEW(data));
  return FALSE;
}

static gboolean playlist_list_refresh_cb(gpointer data) {
  ddb_listview_refresh(DDB_LISTVIEW(data), DDB_REFRESH_LIST);
  return FALSE;
}

static gboolean playlist_header_refresh_cb(gpointer data) {
  ddb_listview_refresh(DDB_LISTVIEW(data), DDB_REFRESH_COLUMNS);
  return FALSE;
}

static gboolean playlist_setup_cb(gpointer data) {
...
static gboolean songfinished_cb(gpointer data) {
  w_trackdata_t *d = data;
  int idx = deadbeef->pl_get_idx_of(d->trk);
  if (idx != -1) {
    ddb_listview_draw_row(d->listview, idx, d->trk);
...
// otherwise gtkui.c will handle it
static gboolean songstarted_cb(gpointer data) {
  w_trackdata_t *d = data;
  int idx = deadbeef->pl_get_idx_of(d->trk);
  if (idx != -1) {
...
static void playlist_set_cursor(DdbListview *listview, DB_playItem_t *it) {
...
static gboolean cursor_moved_cb(gpointer data) {
  w_trackdata_t *d = data;
...
static gboolean focus_selection_cb(gpointer data) {
...
static gboolean trackfocus_cb(gpointer data) {
  deadbeef->pl_lock();
  DB_playItem_t *it = deadbeef->streamer_get_playing_track();
  if (it) {
...
static int w_playlist_message(ddb_gtkui_widget_t *w, uint32_t id, uintptr_t ctx,
                              uint32_t p1, uint32_t p2) {
  ...
  case DB_EV_TRACKINFOCHANGED:
    ...
    if (p1 == DDB_PLAYLIST_CHANGE_CONTENT ||
        p1 == DDB_PLAYLIST_CHANGE_SELECTION && p2 != PL_MAIN ||
        p1 == DDB_PLAYLIST_CHANGE_PLAYQUEUE) {
      ddb_event_track_t *ev = (ddb_event_track_t *)ctx;
      if (ev->track) {
  ...
  case DB_EV_PLAYLISTCHANGED:
    ...
    if (p1 == DDB_PLAYLIST_CHANGE_CONTENT ||
        p1 == DDB_PLAYLIST_CHANGE_SELECTION &&
            (p2 != PL_MAIN || (DdbListview *)ctx != p->list) ||
        p1 == DDB_PLAYLIST_CHANGE_PLAYQUEUE) {
  ...
  case DB_EV_CONFIGCHANGED:
    if (ctx) {
      char *conf_str = (char *)ctx;
      if (gtkui_listview_override_conf(conf_str) ||
          gtkui_listview_font_conf(conf_str)) {
      ...
      } else if (gtkui_listview_font_style_conf(conf_str) ||
                 !strcmp(conf_str, "playlist.pin.groups") ||
                 !strcmp(conf_str, "playlist.groups.spacing")) {
        g_idle_add(playlist_list_refresh_cb, p->list);
      } else if (gtkui_tabstrip_override_conf(conf_str) ||
                 gtkui_tabstrip_colors_conf(conf_str)) {
        g_idle_add(playlist_header_refresh_cb, p->list);
      }
    }
...
static int w_tabbed_playlist_message(ddb_gtkui_widget_t *w, uint32_t id,
                                     uintptr_t ctx, uint32_t p1, uint32_t p2) {
  switch (id) {
  case DB_EV_CONFIGCHANGED:
    if (ctx) {
      char *str = (char *)ctx;
      if (gtkui_tabstrip_override_conf(str) ||
          gtkui_tabstrip_colors_conf(str) || gtkui_tabstrip_font_conf(str) ||
          gtkui_tabstrip_font_style_conf(str)) {
...
static const char *w_playlist_load(struct ddb_gtkui_widget_s *w,
                                   const char *type, const char *s) {
...
static void w_playlist_save(struct ddb_gtkui_widget_s *w, char *s, int sz) {
  ...
  char save[100];
  snprintf(save, sizeof(save), " hideheaders=%d width=%d", ww->hideheaders,
           width);
  strncat(s, save, sz);
}

static void w_playlist_init(ddb_gtkui_widget_t *base) {
...
static void on_playlist_showheaders_toggled(GtkCheckMenuItem *checkmenuitem,
                                            gpointer user_data) {
...
static void w_playlist_initmenu(struct ddb_gtkui_widget_s *w, GtkWidget *menu) {
  GtkWidget *item;
  item = gtk_check_menu_item_new_with_mnemonic(_("Show Column Headers"));
  gtk_widget_show(item);
  gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(item),
                                 !((w_playlist_t *)w)->hideheaders);
  gtk_container_add(GTK_CONTAINER(menu), item);
  g_signal_connect((gpointer)item, "toggled",
                   G_CALLBACK(on_playlist_showheaders_toggled), w);
}

ddb_gtkui_widget_t *w_tabbed_playlist_create(void) {
...
ddb_gtkui_widget_t *w_playlist_create(void) {
...
gboolean fill_selproperties_cb(gpointer data) {
  ...
  GtkListStore *store =
      GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(w->tree)));
...
static void selproperties_selection_changed(gpointer user_data) {
  ...
  selprop_w->refresh_timeout = g_timeout_add_full(
      G_PRIORITY_DEFAULT_IDLE, 10, fill_selproperties_cb, user_data, NULL);
}

static int selproperties_message(ddb_gtkui_widget_t *w, uint32_t id,
                                 uintptr_t ctx, uint32_t p1, uint32_t p2) {
...
static void w_selproperties_init(struct ddb_gtkui_widget_s *widget) {
...
static void
on_selproperties_showheaders_toggled(GtkCheckMenuItem *checkmenuitem,
                                     gpointer user_data) {
  w_selproperties_t *w = user_data;
  int showheaders =
      gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(checkmenuitem));
  deadbeef->conf_set_int("gtkui.selection_properties.show_headers",
                         showheaders);
  gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(w->tree), showheaders);
}

static void w_selproperties_initmenu(struct ddb_gtkui_widget_s *w,
                                     GtkWidget *menu) {
  GtkWidget *item;
  item = gtk_check_menu_item_new_with_mnemonic(_("Show Column Headers"));
  gtk_widget_show(item);
  int showheaders =
      deadbeef->conf_get_int("gtkui.selection_properties.show_headers", 1);
  gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(item), showheaders);
  gtk_container_add(GTK_CONTAINER(menu), item);
  g_signal_connect((gpointer)item, "toggled",
                   G_CALLBACK(on_selproperties_showheaders_toggled), w);
}

ddb_gtkui_widget_t *w_selproperties_create(void) {
...
///// cover art display
static GdkPixbuf *get_cover_art(const int width, const int height,
  ...
  GdkPixbuf *pixbuf = get_cover_art_primary_by_size(
      uri, artist, album, width, height, callback, user_data);
  deadbeef->pl_unlock();
  deadbeef->pl_item_unref(it);
  return pixbuf;
}

static gboolean coverart_invalidate_cb(void *user_data) {
  gtk_widget_queue_draw(user_data);
  return FALSE;
}

static void coverart_invalidate(void *user_data) {
  g_idle_add(coverart_invalidate_cb, user_data);
}

static gboolean coverart_unref_cb(void *user_data) {
  g_object_unref(user_data);
  return FALSE;
}

static void coverart_unref(void *user_data) {
  g_idle_add(coverart_unref_cb, user_data);
}

static gboolean coverart_load(void *user_data) {
  w_coverart_t *w = user_data;
  w->load_timeout_id = 0;
  GdkPixbuf *pixbuf = get_cover_art(w->widget_width, w->widget_height,
...
static void coverart_draw_cairo(GdkPixbuf *pixbuf, GtkAllocation *a,
                                cairo_t *cr, const int filter) {
  ...
  if (pw > a->width || ph > a->height || pw < a->width && ph < a->height) {
    ...
    cairo_translate(cr, (a->width - a->width * scale) / 2.,
                    (a->height - a->height * scale) / 2.);
    cairo_scale(cr, scale, scale);
    cairo_pattern_set_filter(cairo_get_source(cr), filter);
  }
  gdk_cairo_set_source_pixbuf(cr, pixbuf, (a->width - pw) / 2.,
                              (a->height - ph) / 2.);
  cairo_fill(cr);
}

static void coverart_draw_anything(GtkAllocation *a, cairo_t *cr) {
...
static void coverart_draw_exact(GtkAllocation *a, cairo_t *cr,
                                void *user_data) {
  w_coverart_t *w = user_data;
  GdkPixbuf *pixbuf =
      get_cover_art(a->width, a->height, coverart_invalidate, w->drawarea);
  if (pixbuf) {
...
static gboolean coverart_draw(GtkWidget *widget, cairo_t *cr,
                              gpointer user_data) {
  GtkAllocation a;
  gtk_widget_get_allocation(widget, &a);
  ...
  if (w->widget_height == a.height && w->widget_width == a.width) {
    ...
    w->load_timeout_id = g_timeout_add(w->widget_height == -1 ? 100 : 1000,
                                       coverart_load, user_data);
...
static gboolean coverart_expose_event(GtkWidget *widget, GdkEventExpose *event,
                                      gpointer user_data) {
  cairo_t *cr = gdk_cairo_create(gtk_widget_get_window(widget));
...
static int coverart_message(ddb_gtkui_widget_t *base, uint32_t id,
                            uintptr_t ctx, uint32_t p1, uint32_t p2) {
...
static gboolean coverart_destroy_drawarea(GtkWidget *drawarea,
                                          gpointer user_data) {
...
ddb_gtkui_widget_t *w_coverart_create(void) {
...
#if !GTK_CHECK_VERSION(3, 0, 0)
  g_signal_connect_after((gpointer)w->drawarea, "expose_event",
                         G_CALLBACK(coverart_expose_event), w);
#else
  g_signal_connect_after((gpointer)w->drawarea, "draw",
                         G_CALLBACK(coverart_draw), w);
#endif
  g_signal_connect((gpointer)w->drawarea, "destroy",
                   G_CALLBACK(coverart_destroy_drawarea), w);
  w_override_signals(w->base.widget, w);
  return (ddb_gtkui_widget_t *)w;
}

///// scope vis
void w_scope_destroy(ddb_gtkui_widget_t *w) {
...
gboolean w_scope_draw_cb(void *data) {
  w_scope_t *s = data;
  gtk_widget_queue_draw(s->drawarea);
  return TRUE;
}

static void scope_wavedata_listener(void *ctx, ddb_audio_data_t *data) {
  w_scope_t *w = ctx;
  if (w->nsamples != w->resized) {
    ...
    if (w->nsamples > 0) {
      ...
      if (oldsamples) {
        ...
        memcpy(w->samples + w->nsamples - n, oldsamples + oldnsamples - n,
               n * sizeof(float));
      }
    }
    if (oldnsamples) {
  ...
  if (w->samples) {
    ...
    for (int i = 0; i < sz && pos < nsamples; i++, pos += ratio) {
      w->samples[n + i] =
          data->data[ftoi(pos * data->fmt->channels) * data->fmt->channels + 0];
      for (int j = 1; j < data->fmt->channels; j++) {
        w->samples[n + i] +=
            data->data[ftoi(pos * data->fmt->channels) * data->fmt->channels +
                       j];
      }
...
static inline void _draw_vline(uint8_t *data, int stride, int x0, int y0,
                               int y1) {
  if (y0 > y1) {
    int tmp = y0;
    y0 = y1;
...
static inline void _draw_bar(uint8_t *data, int stride, int x0, int y0, int w,
...
gboolean scope_draw_cairo(GtkWidget *widget, cairo_t *cr, gpointer user_data) {
  GtkAllocation a;
  gtk_widget_get_allocation(widget, &a);
  ...
  if (!w->surf || cairo_image_surface_get_width(w->surf) != a.width ||
      cairo_image_surface_get_height(w->surf) != a.height) {
    if (w->surf) {
...
gboolean scope_draw(GtkWidget *widget, cairo_t *cr, gpointer user_data) {
#if USE_OPENGL
  GtkAllocation a;
  gtk_widget_get_allocation(widget, &a);
  w_scope_t *w = user_data;
...
gboolean scope_expose_event(GtkWidget *widget, GdkEventExpose *event,
                            gpointer user_data) {
  cairo_t *cr = gdk_cairo_create(gtk_widget_get_window(widget));
...
void w_scope_init(ddb_gtkui_widget_t *w) {
  w_scope_t *s = (w_scope_t *)w;
  if (s->drawtimer) {
    g_source_remove(s->drawtimer);
    s->drawtimer = 0;
...
void scope_realize(GtkWidget *widget, gpointer data) {
  w_scope_t *w = data;
#if USE_OPENGL
  w->glcontext =
      gtk_widget_create_gl_context(w->drawarea, NULL, TRUE, GDK_GL_RGBA_TYPE);
#endif
}

ddb_gtkui_widget_t *w_scope_create(void) {
  ...
  GdkGLConfig *conf = gdk_gl_config_new_by_mode(
      (GdkGLConfigMode)(GDK_GL_MODE_RGB | GDK_GL_MODE_DOUBLE));
  gboolean cap = gtk_widget_set_gl_capability(w->drawarea, conf, NULL, TRUE,
                                              GDK_GL_RGBA_TYPE);
#endif
...
#if !GTK_CHECK_VERSION(3, 0, 0)
  g_signal_connect_after((gpointer)w->drawarea, "expose_event",
                         G_CALLBACK(scope_expose_event), w);
#else
  g_signal_connect_after((gpointer)w->drawarea, "draw", G_CALLBACK(scope_draw),
                         w);
#endif
  g_signal_connect_after(G_OBJECT(w->drawarea), "realize",
                         G_CALLBACK(scope_realize), w);
  w_override_signals(w->base.widget, w);
...
///// spectrum vis
void w_spectrum_destroy(ddb_gtkui_widget_t *w) {
...
gboolean w_spectrum_draw_cb(void *data) {
  w_spectrum_t *s = data;
  gtk_widget_queue_draw(s->drawarea);
  return TRUE;
}
...
static void spectrum_audio_listener(void *ctx, ddb_audio_data_t *data) {
  w_spectrum_t *w = ctx;
  memcpy(w->data, data->data, DDB_FREQ_BANDS * sizeof(float));
}

static void _spectrum_run(ddb_gtkui_widget_t *w) {
  w_spectrum_t *s = (w_spectrum_t *)w;
  if (s->drawtimer > 0) {
    return;
  }
...
static void _spectrum_stop(ddb_gtkui_widget_t *w) {
  w_spectrum_t *s = (w_spectrum_t *)w;
  if (s->drawtimer > 0) {
    g_source_remove(s->drawtimer);
    s->drawtimer = 0;
...
static gboolean spectrum_draw(GtkWidget *widget, cairo_t *cr,
                              gpointer user_data) {
...
void w_spectrum_init(ddb_gtkui_widget_t *w) {
  w_spectrum_t *s = (w_spectrum_t *)w;
  if (s->drawtimer) {
    g_source_remove(s->drawtimer);
    s->drawtimer = 0;
...
void spectrum_realize(GtkWidget *widget, gpointer data) {
  w_spectrum_t *w = data;
#if USE_OPENGL
  w->glcontext =
      gtk_widget_create_gl_context(w->drawarea, NULL, TRUE, GDK_GL_RGBA_TYPE);
...
static int w_spectrum_message(ddb_gtkui_widget_t *w, uint32_t id, uintptr_t ctx,
                              uint32_t p1, uint32_t p2) {
  switch (id) {
  case DB_EV_SONGCHANGED: {
...
ddb_gtkui_widget_t *w_spectrum_create(void) {
...
#if !GTK_CHECK_VERSION(3, 0, 0)
  g_signal_connect_after((gpointer)w->drawarea, "expose_event",
                         G_CALLBACK(spectrum_expose_event), w);
#else
  g_signal_connect_after((gpointer)w->drawarea, "draw",
                         G_CALLBACK(spectrum_draw), w);
#endif
  g_signal_connect_after(G_OBJECT(w->drawarea), "realize",
                         G_CALLBACK(spectrum_realize), w);
  w_override_signals(w->base.widget, w);
...
// hbox and vbox
static const char *w_hvbox_load(struct ddb_gtkui_widget_s *w, const char *type,
                                const char *s) {
...
static void save_hvbox_packing(GtkWidget *child, gpointer user_data) {
  w_hvbox_save_info_t *info = user_data;
  gboolean expand;
  gboolean fill;
  guint padding;
  GtkPackType pack_type;
  gtk_box_query_child_packing(GTK_BOX(info->hvbox), child, &expand, &fill,
                              &padding, &pack_type);
  char s[10];
...
static void w_hvbox_save(struct ddb_gtkui_widget_s *w, char *s, int sz) {
  char save[1000];

  w_hvbox_save_info_t info;
  memset(&info, 0, sizeof(info));
  info.hvbox = ((w_hvbox_t *)w)->box;
  gtk_container_foreach(GTK_CONTAINER(((w_hvbox_t *)w)->box),
                        save_hvbox_packing, &info);
  gboolean homogeneous =
      gtk_box_get_homogeneous(GTK_BOX(((w_hvbox_t *)w)->box));

  snprintf(save, sizeof(save), " expand=\"%s\" fill=\"%s\" homogeneous=%d",
           info.expand, info.fill, homogeneous);
  strncat(s, save, sz);
}
...
static void hvbox_init_child(GtkWidget *child, void *user_data) {
  hwbox_init_info_t *info = user_data;
  gboolean expand;
  gboolean fill;
  guint padding;
  GtkPackType pack_type;
  gtk_box_query_child_packing(GTK_BOX(info->w->box), child, &expand, &fill,
                              &padding, &pack_type);
  expand = (info->w->expand & (1 << info->n)) ? TRUE : FALSE;
  fill = (info->w->fill & (1 << info->n)) ? TRUE : FALSE;
  gtk_box_set_child_packing(GTK_BOX(info->w->box), child, expand, fill, padding,
                            pack_type);
  info->n++;
}

static void w_hvbox_init(struct ddb_gtkui_widget_s *w) {
  w_hvbox_t *hvbox = (w_hvbox_t *)w;
  hwbox_init_info_t info;
  info.w = hvbox;
  info.n = 0;
  gtk_container_foreach(GTK_CONTAINER(hvbox->box), hvbox_init_child, &info);
  gtk_box_set_homogeneous(GTK_BOX(hvbox->box),
                          hvbox->homogeneous ? TRUE : FALSE);
}

static void w_hvbox_append(struct ddb_gtkui_widget_s *container,
                           struct ddb_gtkui_widget_s *child) {
  w_hvbox_t *b = (w_hvbox_t *)container;
  gtk_box_pack_start(
      GTK_BOX(b->box), child->widget,
      (child->flags & DDB_GTKUI_WIDGET_FLAG_NON_EXPANDABLE) ? FALSE : TRUE,
      TRUE, 0);
  gtk_widget_show(child->widget);
}

static void w_hvbox_remove(struct ddb_gtkui_widget_s *container,
                           struct ddb_gtkui_widget_s *child) {
  w_hvbox_t *b = (w_hvbox_t *)container;
  gtk_container_remove(GTK_CONTAINER(b->box), child->widget);
}

static void w_hvbox_replace(struct ddb_gtkui_widget_s *container,
                            struct ddb_gtkui_widget_s *child,
                            struct ddb_gtkui_widget_s *newchild) {
  w_hvbox_t *b = (w_hvbox_t *)container;
...
static void on_hvbox_expand(GtkMenuItem *menuitem, gpointer user_data) {
  w_append((ddb_gtkui_widget_t *)user_data, w_create("placeholder"));
}

static void on_hvbox_shrink(GtkMenuItem *menuitem, gpointer user_data) {
...
static void on_hvbox_toggle_homogeneous(GtkCheckMenuItem *checkmenuitem,
                                        gpointer user_data) {
...
static void w_hvbox_initmenu(struct ddb_gtkui_widget_s *w, GtkWidget *menu) {
  GtkWidget *item;
  ...
  gtk_check_menu_item_set_active(
      ...
      gtk_box_get_homogeneous(GTK_BOX(((w_hvbox_t *)w)->box)));
  g_signal_connect((gpointer)item, "toggled",
                   G_CALLBACK(on_hvbox_toggle_homogeneous), w);
}

static void on_hvbox_child_toggle_expand(GtkCheckMenuItem *checkmenuitem,
                                         gpointer user_data) {
  ddb_gtkui_widget_t *w = user_data;
  ...
  gtk_box_query_child_packing(GTK_BOX(box->box), w->widget, &expand, &fill,
                              &padding, &packtype);
  gtk_box_set_child_packing(GTK_BOX(box->box), w->widget, !expand, fill,
                            padding, packtype);
}

static void on_hvbox_child_toggle_fill(GtkCheckMenuItem *checkmenuitem,
                                       gpointer user_data) {
  ddb_gtkui_widget_t *w = user_data;
  w_hvbox_t *box = (w_hvbox_t *)w->parent;
  gboolean expand, fill;
...
static void w_hvbox_initchildmenu(struct ddb_gtkui_widget_s *w,
                                  GtkWidget *menu) {
  w_hvbox_t *box = (w_hvbox_t *)w->parent;
  ...
  gtk_container_add(GTK_CONTAINER(menu), item);
  g_signal_connect((gpointer)item, "toggled",
                   G_CALLBACK(on_hvbox_child_toggle_expand), w);
  ...
  gtk_container_add(GTK_CONTAINER(menu), item);
  g_signal_connect((gpointer)item, "toggled",
                   G_CALLBACK(on_hvbox_child_toggle_fill), w);
}

GtkWidget *w_hvbox_get_container(struct ddb_gtkui_widget_s *b) {
  return ((w_hvbox_t *)b)->box;
}

ddb_gtkui_widget_t *w_hbox_create(void) {
...
ddb_gtkui_widget_t *w_vbox_create(void) {
  w_hvbox_t *w = malloc(sizeof(w_hvbox_t));
  memset(w, 0, sizeof(w_hvbox_t));
  w->base.widget = gtk_event_box_new();
  w->base.append = w_hvbox_append;
...
// button widget
static const char *w_button_load(struct ddb_gtkui_widget_s *w, const char *type,
                                 const char *s) {
...
static void w_button_save(struct ddb_gtkui_widget_s *w, char *s, int sz) {
  char save[1000] = "";
  char *pp = save;
  int ss = sizeof(save);
  int n;
  ...
  w_button_t *b = (w_button_t *)w;
  n = snprintf(pp, ss, " color=\"#%02x%02x%02x\"", b->color.red >> 8,
               b->color.green >> 8, b->color.blue >> 8);
  ss -= n;
  pp += n;
  n = snprintf(pp, ss, " textcolor=\"#%02x%02x%02x\"", b->textcolor.red >> 8,
               b->textcolor.green >> 8, b->textcolor.blue >> 8);
  ss -= n;
  pp += n;
  if (b->icon) {
...
static void w_button_init(ddb_gtkui_widget_t *ww) {
  ...
  GtkWidget *label =
      gtk_label_new_with_mnemonic(w->label ? w->label : _("Button"));
  gtk_widget_show(label);
  ...
  if (w->action) {
    g_signal_connect((gpointer)w->button, "clicked",
                     G_CALLBACK(on_button_clicked), w);
  }

  w_override_signals(w->button, w);
}

static void w_button_destroy(ddb_gtkui_widget_t *w) {
  w_button_t *b = (w_button_t *)w;
  if (b->icon) {
    free(b->icon);
  }
...
static void on_button_set_action_clicked(GtkButton *button,
                                         gpointer user_data) {
...
static void on_button_config(GtkMenuItem *menuitem, gpointer user_data) {
  w_button_t *b = user_data;
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(use_textcolor),
                               b->use_textcolor);
  ...
  g_signal_connect((gpointer)action, "clicked",
                   G_CALLBACK(on_button_set_action_clicked), user_data);
  ...
  for (int n = 0; GtkNamedIcons[n]; n++) {
    ...
    if (gtk_stock_lookup(GtkNamedIcons[n], &it)) {
      ...
      gtk_list_store_set(store, &iter, 0, GtkNamedIcons[n], 1, GtkNamedIcons[n],
                         -1);
    }
  ...
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(icon), renderer, "stock-id", 0,
                                 NULL);

  renderer = gtk_cell_renderer_text_new();
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(icon), renderer, FALSE);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(icon), renderer, "text", 1,
                                 NULL);
  ...
  for (;;) {
    ...
    if (response == GTK_RESPONSE_OK || response == GTK_RESPONSE_APPLY) {
      ...
      b->use_textcolor =
          gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(use_textcolor));
...
static void w_button_initmenu(struct ddb_gtkui_widget_s *w, GtkWidget *menu) {
  GtkWidget *item;
...
ddb_gtkui_widget_t *w_button_create(void) {
...
// seekbar
static gboolean redraw_seekbar_cb(gpointer data) {
  w_seekbar_t *w = data;
  int iconified = gdk_window_get_state(gtk_widget_get_window(mainwin)) &
...
static gboolean seekbar_frameupdate(gpointer data) {
  w_seekbar_t *w = data;
...
static void w_seekbar_init(ddb_gtkui_widget_t *base) {
  ...
  w->timer = g_timeout_add(1000 / gtkui_get_gui_refresh_rate(),
                           seekbar_frameupdate, w);
}

static int w_seekbar_message(ddb_gtkui_widget_t *w, uint32_t id, uintptr_t ctx,
                             uint32_t p1, uint32_t p2) {
  switch (id) {
  case DB_EV_CONFIGCHANGED:
    w_seekbar_init(w);
    if (ctx) {
      char *conf_str = (char *)ctx;
      if (gtkui_bar_override_conf(conf_str) ||
          gtkui_bar_colors_conf(conf_str)) {
...
static void w_seekbar_destroy(ddb_gtkui_widget_t *wbase) {
...
ddb_gtkui_widget_t *w_seekbar_create(void) {
...
// play toolbar
ddb_gtkui_widget_t *w_playtb_create(void) {
  ...
  g_signal_connect((gpointer)stopbtn, "clicked", G_CALLBACK(on_stopbtn_clicked),
                   NULL);
  g_signal_connect((gpointer)playbtn, "clicked", G_CALLBACK(on_playbtn_clicked),
                   NULL);
  g_signal_connect((gpointer)pausebtn, "clicked",
                   G_CALLBACK(on_pausebtn_clicked), NULL);
  g_signal_connect((gpointer)prevbtn, "clicked", G_CALLBACK(on_prevbtn_clicked),
                   NULL);
  g_signal_connect((gpointer)nextbtn, "clicked", G_CALLBACK(on_nextbtn_clicked),
                   NULL);
  return (ddb_gtkui_widget_t *)w;
}

// volumebar
static gboolean redraw_volumebar_cb(gpointer data) {
...
static int w_volumebar_message(ddb_gtkui_widget_t *w, uint32_t id,
                               uintptr_t ctx, uint32_t p1, uint32_t p2) {
  switch (id) {
  case DB_EV_CONFIGCHANGED:
    if (ctx) {
...
ddb_gtkui_widget_t *w_volumebar_create(void) {
...
#if GTK_CHECK_VERSION(3, 0, 0)
  gtk_widget_set_events(GTK_WIDGET(w->base.widget),
                        gtk_widget_get_events(GTK_WIDGET(w->base.widget)) |
                            GDK_SCROLL_MASK);
#endif
...
// chiptune voice ctl
static void on_voice_toggled(GtkToggleButton *togglebutton,
                             gpointer user_data) {
...
ddb_gtkui_widget_t *w_ctvoices_create(void) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(w->voices[i]),
                                 voices & (1 << i));
    g_signal_connect((gpointer)w->voices[i], "toggled",
                     G_CALLBACK(on_voice_toggled), w);
  }
...
static void logviewer_logger_callback(struct DB_plugin_s *plugin,
                                      uint32_t layers, const char *text,
                                      void *ctx);

static void w_logviewer_init(struct ddb_gtkui_widget_s *widget) {
...
static void w_logviewer_scroll_changed(GtkAdjustment *adjustment,
                                       gpointer user_data) {
...
static gboolean logviewer_addtext_cb(gpointer data) {
...
void w_logviewer_destroy(ddb_gtkui_widget_t *w) {
...
ddb_gtkui_widget_t *w_logviewer_create(void) {
  ...
  g_signal_connect(adjustment, "value-changed",
                   G_CALLBACK(w_logviewer_scroll_changed), w);
...
gboolean w_logviewer_is_present(void) { return w_logviewer_instancecount > 0; }
