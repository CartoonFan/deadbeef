...
#define trace(...)                                                             \
  { deadbeef->log_detailed(&plugin.gui.plugin, 0, __VA_ARGS__); }

gboolean action_open_files_handler_cb(void *userdata) {
  GSList *lst =
      show_file_chooser(_("Open file(s)..."), GTKUI_FILECHOOSER_OPENFILE, TRUE);
  if (lst) {
...
int action_open_files_handler(struct DB_plugin_action_s *action,
                              ddb_action_context_t ctx) {
  gdk_threads_add_idle(action_open_files_handler_cb, NULL);
  return 0;
}

gboolean action_add_files_handler_cb(void *user_data) {
  GSList *lst = show_file_chooser(_("Add file(s) to playlist..."),
                                  GTKUI_FILECHOOSER_OPENFILE, TRUE);
  if (lst) {
    gtkui_add_files(lst);
  }
  return FALSE;
...
int action_add_files_handler(struct DB_plugin_action_s *action,
                             ddb_action_context_t ctx) {
  gdk_threads_add_idle(action_add_files_handler_cb, NULL);
  return 0;
}

gboolean action_add_folders_handler_cb(void *user_data) {
  GSList *lst = show_file_chooser(_("Add folder(s) to playlist..."),
                                  GTKUI_FILECHOOSER_OPENFOLDER, TRUE);
  if (lst) {
    gtkui_add_dirs(lst);
  }
  return FALSE;
...
int action_add_folders_handler(struct DB_plugin_action_s *action,
                               ddb_action_context_t ctx) {
  gdk_threads_add_idle(action_add_folders_handler_cb, NULL);
  return 0;
}

gboolean action_quit_handler_cb(void *user_data) {
  gtkui_quit();
  return FALSE;
}

int action_quit_handler(DB_plugin_action_t *act, ddb_action_context_t ctx) {
  g_idle_add(action_quit_handler_cb, NULL);
  return 0;
}

gboolean action_deselect_all_handler_cb(void *user_data) {
  deadbeef->pl_lock();
  ...
  deadbeef->pl_unlock();
  deadbeef->sendmessage(DB_EV_PLAYLISTCHANGED, 0, DDB_PLAYLIST_CHANGE_SELECTION,
                        0);
  return FALSE;
}

int action_deselect_all_handler(struct DB_plugin_action_s *action,
                                ddb_action_context_t ctx) {
  g_idle_add(action_deselect_all_handler_cb, NULL);
  return 0;
}

gboolean action_select_all_handler_cb(void *user_data) {
...
int action_select_all_handler(struct DB_plugin_action_s *action,
                              ddb_action_context_t ctx) {
  g_idle_add(action_select_all_handler_cb, NULL);
  return 0;
}

gboolean action_new_playlist_handler_cb(void *user_data) {
...
int action_new_playlist_handler(struct DB_plugin_action_s *action,
                                ddb_action_context_t ctx) {
...
int action_toggle_mainwin_handler(struct DB_plugin_action_s *action,
                                  ddb_action_context_t ctx) {
  g_idle_add(action_toggle_mainwin_handler_cb, NULL);
  return 0;
}

gboolean action_show_mainwin_handler_cb(void *user_data) {
...
int action_show_mainwin_handler(struct DB_plugin_action_s *action,
                                ddb_action_context_t ctx) {
  g_idle_add(action_show_mainwin_handler_cb, NULL);
  return 0;
}

gboolean action_hide_mainwin_handler_cb(void *user_data) {
  int iconified = gdk_window_get_state(gtk_widget_get_window(mainwin)) &
                  GDK_WINDOW_STATE_ICONIFIED;
  if (gtk_widget_get_visible(mainwin) && !iconified) {
...
int action_hide_mainwin_handler(struct DB_plugin_action_s *action,
                                ddb_action_context_t ctx) {
  g_idle_add(action_hide_mainwin_handler_cb, NULL);
  return 0;
}

// FIXME: this functionality was supposed to be enabled in 0.7.0, but was
// forgotten
...
gboolean action_add_location_handler_cb(void *user_data) {
    ...
    if (entry) {
      ...
      if (text) {
        ...
        if (!deadbeef->plt_add_files_begin(plt, 0)) {
          ...
          DB_playItem_t *it =
              deadbeef->plt_insert_file2(0, plt, tail, text, NULL, NULL, NULL);
#ifndef DISABLE_CUSTOM_TITLE
          if (it &&
              deadbeef->conf_get_int("gtkui.location_set_custom_title", 0)) {
            deadbeef->pl_replace_meta(it, ":CUSTOM_TITLE",
...
int action_add_location_handler(DB_plugin_action_t *act,
                                ddb_action_context_t ctx) {
...
gboolean action_show_help_handler_cb(void *user_data) {
  char fname[PATH_MAX];
  snprintf(fname, sizeof(fname), "%s/%s",
           deadbeef->get_system_dir(DDB_SYS_DIR_DOC), _("help.txt"));
  gtkui_show_info_window(fname, _("Help"), &helpwindow);
  return FALSE;
}

int action_show_help_handler(DB_plugin_action_t *act,
                             ddb_action_context_t ctx) {
  gdk_threads_add_idle(action_show_help_handler_cb, NULL);
  return 0;
}

int action_remove_from_playlist_handler(DB_plugin_action_t *act,
                                        ddb_action_context_t ctx) {
  if (ctx == DDB_ACTION_CTX_SELECTION) {
    ...
    if (plt) {
      ...
      if (cursor != -1) {
        DB_playItem_t *it =
            deadbeef->plt_get_item_for_idx(plt, cursor, PL_MAIN);
        if (it) {
...
static void
_warningMessageForCtx(ddbDeleteFromDiskController_t ctl,
                      ddb_action_context_t ctx, unsigned trackcount,
    ...
    const char *buf2 =
        deadbeef->conf_get_int("gtkui.move_to_trash", 1)
            ? _(" The files will be moved to trash.\n\n(This dialog can be "
                "turned off in GTKUI plugin settings)")
            : _(" The files will be lost.\n\n(This dialog can be turned off in "
                "GTKUI plugin settings)");

    if (ctx == DDB_ACTION_CTX_SELECTION) {
      int selected_files = trackcount;
      if (selected_files == 1) {
        snprintf(buf, sizeof(buf),
                 _("Do you really want to delete the selected file?%s"), buf2);
      } else {
        snprintf(buf, sizeof(buf),
                 _("Do you really want to delete all %d selected files?%s"),
                 selected_files, buf2);
      }
    } else if (ctx == DDB_ACTION_CTX_PLAYLIST) {
      int files = trackcount;
      snprintf(buf, sizeof(buf),
               _("Do you really want to delete all %d files from the current "
                 "playlist?%s"),
               files, buf2);
    } else if (ctx == DDB_ACTION_CTX_NOWPLAYING) {
      snprintf(buf, sizeof(buf),
               _("Do you really want to delete the currently playing file?%s"),
               buf2);
    }

    GtkWidget *dlg = gtk_message_dialog_new(
        GTK_WINDOW(mainwin), GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING,
        GTK_BUTTONS_YES_NO, _("Delete files from disk"));
...
static int _deleteFile(ddbDeleteFromDiskController_t ctl, const char *uri) {
...
static void _deleteCompleted(ddbDeleteFromDiskController_t ctl) {
  ddbDeleteFromDiskControllerFree(ctl);
  _deleteCtl = NULL;
}

gboolean action_delete_from_disk_handler_cb(void *data) {
  if (_deleteCtl) {
    return FALSE;
  }
  ...
  _deleteCtl = ddbDeleteFromDiskControllerInitWithPlaylist(
      ddbDeleteFromDiskControllerAlloc(), plt, ctx);

  ddbDeleteFromDiskControllerSetShouldSkipDeletedTracks(
      _deleteCtl, deadbeef->conf_get_int("gtkui.skip_deleted_songs", 0));
...
void delete_from_disk_with_track_list(ddbUtilTrackList_t trackList) {
  if (_deleteCtl) {
    return;
  }
  ...
  _deleteCtl = ddbDeleteFromDiskControllerInitWithTrackList(
      ddbDeleteFromDiskControllerAlloc(), trackList);
...
int action_delete_from_disk_handler(DB_plugin_action_t *act,
                                    ddb_action_context_t ctx) {
  gdk_threads_add_idle(action_delete_from_disk_handler_cb,
                       (void *)(intptr_t)ctx);
  return 0;
}
...
gboolean action_show_track_properties_handler_cb(void *data) {
...
int action_show_track_properties_handler(DB_plugin_action_t *act,
                                         ddb_action_context_t ctx) {
  trkproperties_action_ctx_t *data =
      calloc(1, sizeof(trkproperties_action_ctx_t));
  data->ctx = ctx;
...
int action_find_handler(DB_plugin_action_t *act, ddb_action_context_t ctx) {
...
gboolean action_cursor_follows_playback_handler_cb(void *data) {
  int val =
      1 - deadbeef->conf_get_int("playlist.scroll.cursorfollowplayback", 1);
...
static void load_playlist_thread(void *data) {
  char *fname = data;
  ddb_playlist_t *plt = deadbeef->plt_get_curr();
  if (plt) {
    if (!deadbeef->plt_add_files_begin(plt, 0)) {
...
gboolean action_load_playlist_handler_cb(void *data) {
  GSList *lst = show_file_chooser(_("Load Playlist"),
                                  GTKUI_FILECHOOSER_LOADPLAYLIST, FALSE);
...
int action_load_playlist_handler(DB_plugin_action_t *act,
                                 ddb_action_context_t ctx) {
  gdk_threads_add_idle(action_load_playlist_handler_cb, NULL);
  return 0;
}

gboolean action_save_playlist_handler_cb(void *data) {
  GSList *lst = show_file_chooser(_("Save Playlist As"),
                                  GTKUI_FILECHOOSER_SAVEPLAYLIST, FALSE);
...
int action_save_playlist_handler(DB_plugin_action_t *act,
                                 ddb_action_context_t ctx) {
  gdk_threads_add_idle(action_save_playlist_handler_cb, NULL);
  return 0;
}

gboolean action_toggle_menu_handler_cb(void *data) {
...
int action_toggle_menu_handler(DB_plugin_action_t *act,
                               ddb_action_context_t ctx) {
  g_idle_add(action_toggle_menu_handler_cb, NULL);
  return 0;
}

gboolean action_toggle_statusbar_handler_cb(void *data) {
  ...
  if (sb) {
    ...
    gtk_check_menu_item_set_active(
        GTK_CHECK_MENU_ITEM(lookup_widget(mainwin, "view_status_bar")), val);
...
int action_toggle_statusbar_handler(DB_plugin_action_t *act,
                                    ddb_action_context_t ctx) {
  g_idle_add(action_toggle_statusbar_handler_cb, NULL);
  return 0;
}

gboolean action_toggle_designmode_handler_cb(void *data) {
...
int action_toggle_designmode_handler(DB_plugin_action_t *act,
                                     ddb_action_context_t ctx) {
  g_idle_add(action_toggle_designmode_handler_cb, NULL);
  return 0;
}

gboolean action_preferences_handler_cb(void *data) {
  gtkui_run_preferences_dlg();
  return FALSE;
}

int action_preferences_handler(DB_plugin_action_t *act,
                               ddb_action_context_t ctx) {
  gdk_threads_add_idle(action_preferences_handler_cb, NULL);
  return 0;
}

gboolean action_sort_custom_handler_cb(void *data) {
  ...
  gtk_combo_box_set_active(combo,
                           deadbeef->conf_get_int("gtkui.sortby_order", 0));
  deadbeef->conf_lock();
  gtk_entry_set_text(entry,
                     deadbeef->conf_get_str_fast("gtkui.sortby_fmt_v2", ""));
  ...
  if (r == GTK_RESPONSE_OK) {
    ...
    ddb_playlist_t *plt = deadbeef->plt_get_curr();
    deadbeef->plt_sort_v2(plt, PL_MAIN, -1, fmt,
                          order == 0 ? DDB_SORT_ASCENDING
                                     : DDB_SORT_DESCENDING);
    deadbeef->plt_save_config(plt);
    deadbeef->plt_unref(plt);
...
int action_sort_custom_handler(DB_plugin_action_t *act,
                               ddb_action_context_t ctx) {
  gdk_threads_add_idle(action_sort_custom_handler_cb, NULL);
  return 0;
}

int action_crop_selected_handler(DB_plugin_action_t *act,
                                 ddb_action_context_t ctx) {
...
gboolean action_toggle_eq_handler_cb(void *data) {
...
int action_toggle_eq_handler(DB_plugin_action_t *act,
                             ddb_action_context_t ctx) {
  g_idle_add(action_toggle_eq_handler_cb, NULL);
  return 0;
}

gboolean action_show_eq_handler_cb(void *data) {
  GtkWidget *menuitem = lookup_widget(mainwin, "view_eq");
  gboolean act = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(menuitem));
  if (!act) {
...
int action_show_eq_handler(DB_plugin_action_t *act, ddb_action_context_t ctx) {
  g_idle_add(action_show_eq_handler_cb, NULL);
  return 0;
}

gboolean action_hide_eq_handler_cb(void *data) {
  GtkWidget *menuitem = lookup_widget(mainwin, "view_eq");
  gboolean act = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(menuitem));
  if (act) {
...
int action_hide_eq_handler(DB_plugin_action_t *act, ddb_action_context_t ctx) {
  g_idle_add(action_hide_eq_handler_cb, NULL);
  return 0;
}

gboolean action_playback_loop_off_handler_cb(void *data) {
  gtk_check_menu_item_set_active(
      GTK_CHECK_MENU_ITEM(lookup_widget(mainwin, "loop_disable")), 1);
  return FALSE;
}

int action_playback_loop_off_handler(DB_plugin_action_t *act,
                                     ddb_action_context_t ctx) {
  g_idle_add(action_playback_loop_off_handler_cb, NULL);
  return 0;
}

gboolean action_playback_loop_single_handler_cb(void *data) {
  gtk_check_menu_item_set_active(
      GTK_CHECK_MENU_ITEM(lookup_widget(mainwin, "loop_single")), 1);
  return FALSE;
}

int action_playback_loop_single_handler(DB_plugin_action_t *act,
                                        ddb_action_context_t ctx) {
  g_idle_add(action_playback_loop_single_handler_cb, NULL);
  return 0;
}

gboolean action_playback_loop_all_handler_cb(void *data) {
  gtk_check_menu_item_set_active(
      GTK_CHECK_MENU_ITEM(lookup_widget(mainwin, "loop_all")), 1);
  return FALSE;
}

int action_playback_loop_all_handler(DB_plugin_action_t *act,
                                     ddb_action_context_t ctx) {
  g_idle_add(action_playback_loop_all_handler_cb, NULL);
  return 0;
}

gboolean action_playback_order_random_handler_cb(void *data) {
  gtk_check_menu_item_set_active(
      GTK_CHECK_MENU_ITEM(lookup_widget(mainwin, "order_random")), 1);
  return FALSE;
}
...
gboolean action_playback_order_shuffle_albums_handler_cb(void *data) {
  gtk_check_menu_item_set_active(
      GTK_CHECK_MENU_ITEM(lookup_widget(mainwin, "order_shuffle_albums")), 1);
  return FALSE;
}
...
gboolean action_playback_order_shuffle_handler_cb(void *data) {
  gtk_check_menu_item_set_active(
      GTK_CHECK_MENU_ITEM(lookup_widget(mainwin, "order_shuffle")), 1);
  return FALSE;
}
...
gboolean action_playback_order_linear_handler_cb(void *data) {
  gtk_check_menu_item_set_active(
      GTK_CHECK_MENU_ITEM(lookup_widget(mainwin, "order_linear")), 1);
  return FALSE;
}
...
gboolean action_playback_order_cycle_handler_cb(void *data) {
...
gboolean action_playback_loop_cycle_handler_cb(void *data) {
...
int action_playback_loop_cycle_handler(DB_plugin_action_t *act,
                                       ddb_action_context_t ctx) {
  g_idle_add(action_playback_loop_cycle_handler_cb, NULL);
  return 0;
}

gboolean action_toggle_logwindow_handler_cb(void *data) {
  gtkui_toggle_log_window();
  return FALSE;
}

int action_toggle_logwindow_handler(DB_plugin_action_t *act,
                                    ddb_action_context_t ctx) {
