...
// fwd decls
static void ddb_listview_build_groups(DdbListview *listview);

static int ddb_listview_resize_subgroup(DdbListview *listview,
                                        DdbListviewGroup *grp, int group_depth,
                                        int min_height,
                                        int min_no_artwork_height);
static void ddb_listview_resize_groups(DdbListview *listview);
static void ddb_listview_free_group(DdbListview *listview,
                                    DdbListviewGroup *group);
static void ddb_listview_free_all_groups(DdbListview *listview);

static void ddb_listview_update_fonts(DdbListview *ps);
static void ddb_listview_header_update_fonts(DdbListview *ps);

////// list functions ////
static void ddb_listview_list_render(DdbListview *ps, cairo_t *cr,
                                     GdkRectangle *clip);
static void ddb_listview_list_render_row_background(
    DdbListview *ps, cairo_t *cr, DdbListviewIter it, int even, int cursor,
    int x, int y, int w, int h, GdkRectangle *clip);
static void
ddb_listview_list_render_row_foreground(DdbListview *ps, cairo_t *cr,
                                        DdbListviewIter it, int even, int idx,
                                        int y, int w, int h, int x1, int x2);
static void ddb_listview_list_render_album_art(DdbListview *ps, cairo_t *cr,
                                               DdbListviewGroup *grp, int min_y,
                                               int grp_next_y, int y,
                                               GdkRectangle *clip);
static void ddb_listview_list_track_dragdrop(DdbListview *ps, int x, int y);
int ddb_listview_dragdrop_get_row_from_coord(DdbListview *listview, int x,
                                             int y);
void ddb_listview_list_mousemove(DdbListview *ps, GdkEventMotion *event, int x,
                                 int y);
static gboolean ddb_listview_list_setup_vscroll(void *user_data);
static gboolean ddb_listview_list_setup_hscroll(void *user_data);
int ddb_listview_get_row_pos(DdbListview *listview, int pos,
                             int *accumulated_title_height);

////// header functions ////
static void ddb_listview_header_render(DdbListview *ps, cairo_t *cr, int x1,
                                       int x2);

static int ddb_listview_header_get_column_idx_for_coord(DdbListview *pl,
                                                        int click_x);

////// column management functions ////
void ddb_listview_column_move(DdbListview *listview, DdbListviewColumn *which,
                              int inspos);
void ddb_listview_column_free(DdbListview *listview, DdbListviewColumn *c);

// signal handlers
static void ddb_listview_vscroll_value_changed(GtkRange *widget,
                                               gpointer user_data);
static void ddb_listview_hscroll_value_changed(GtkRange *widget,
                                               gpointer user_data);

#if GTK_CHECK_VERSION(3, 0, 0)
static gboolean ddb_listview_header_draw(GtkWidget *widget, cairo_t *cr,
                                         gpointer user_data);
#else
static gboolean ddb_listview_header_expose_event(GtkWidget *widget,
                                                 GdkEventExpose *event,
                                                 gpointer user_data);
#endif

static void ddb_listview_header_realize(GtkWidget *widget, gpointer user_data);

static void ddb_listview_list_realize(GtkWidget *widget, gpointer user_data);

static gboolean ddb_listview_header_motion_notify_event(GtkWidget *widget,
                                                        GdkEventMotion *event,
                                                        gpointer user_data);

static gboolean ddb_listview_header_button_press_event(GtkWidget *widget,
                                                       GdkEventButton *event,
                                                       gpointer user_data);

static gboolean ddb_listview_header_button_release_event(GtkWidget *widget,
                                                         GdkEventButton *event,
                                                         gpointer user_data);

static gboolean ddb_listview_header_enter(GtkWidget *widget,
                                          GdkEventCrossing *event,
                                          gpointer user_data);

static gboolean ddb_listview_list_configure_event(GtkWidget *widget,
                                                  GdkEventConfigure *event,
                                                  gpointer user_data);

#if GTK_CHECK_VERSION(3, 0, 0)
static gboolean ddb_listview_list_draw(GtkWidget *widget, cairo_t *cr,
                                       gpointer user_data);
#else
static gboolean ddb_listview_list_expose_event(GtkWidget *widget,
                                               GdkEventExpose *event,
                                               gpointer user_data);

#endif

gboolean ddb_listview_list_button_press_event(GtkWidget *widget,
                                              GdkEventButton *event,
                                              gpointer user_data);

gboolean ddb_listview_list_popup_menu(GtkWidget *widget, gpointer user_data);

static void ddb_listview_list_drag_data_received(
    GtkWidget *widget, GdkDragContext *drag_context, gint x, gint y,
    GtkSelectionData *data, guint info, guint time, gpointer user_data);

static gboolean ddb_listview_list_drag_motion(GtkWidget *widget,
                                              GdkDragContext *drag_context,
                                              gint x, gint y, guint time,
                                              gpointer user_data);

static gboolean ddb_listview_list_drag_drop(GtkWidget *widget,
                                            GdkDragContext *drag_context,
                                            gint x, gint y, guint time,
                                            gpointer user_data);

static void ddb_listview_list_drag_data_get(GtkWidget *widget,
                                            GdkDragContext *drag_context,
                                            GtkSelectionData *data, guint info,
                                            guint time, gpointer user_data);

static void ddb_listview_list_drag_end(GtkWidget *widget,
                                       GdkDragContext *drag_context,
                                       gpointer user_data);

static gboolean ddb_listview_list_drag_failed(GtkWidget *widget,
                                              GdkDragContext *arg1,
                                              GtkDragResult arg2,
                                              gpointer user_data);

static void ddb_listview_list_drag_leave(GtkWidget *widget,
                                         GdkDragContext *drag_context,
                                         guint time, gpointer user_data);

static gboolean ddb_listview_list_button_release_event(GtkWidget *widget,
                                                       GdkEventButton *event,
                                                       gpointer user_data);

gboolean ddb_listview_motion_notify_event(GtkWidget *widget,
                                          GdkEventMotion *event,
                                          gpointer user_data);
gboolean ddb_listview_list_button_press_event(GtkWidget *widget,
                                              GdkEventButton *event,
...
static gboolean ddb_listview_scroll_event(GtkWidget *widget, GdkEvent *event,
                                          gpointer user_data);

static gboolean ddb_listview_list_key_press_event(GtkWidget *widget,
                                                  GdkEventKey *event,
                                                  gpointer user_data);

static gboolean header_tooltip_handler(GtkWidget *widget, gint x, gint y,
                                       gboolean keyboard_mode,
                                       GtkTooltip *tooltip, gpointer p);

static gboolean list_tooltip_handler(GtkWidget *widget, gint x, gint y,
                                     gboolean keyboard_mode,
                                     GtkTooltip *tooltip, gpointer p);

static void ddb_listview_class_init(DdbListviewClass *class) {
...
static void ddb_listview_init(DdbListview *listview) {
  ...
  gtk_widget_set_events(listview->header,
  ...
  int events = GDK_EXPOSURE_MASK | GDK_POINTER_MOTION_MASK |
               GDK_BUTTON_MOTION_MASK | GDK_BUTTON_PRESS_MASK |
  ...
  g_signal_connect((gpointer)listview->list, "configure_event",
                   G_CALLBACK(ddb_listview_list_configure_event), NULL);

  g_signal_connect((gpointer)listview->scrollbar, "value_changed",
                   G_CALLBACK(ddb_listview_vscroll_value_changed), NULL);
  g_signal_connect((gpointer)listview->hscrollbar, "value_changed",
                   G_CALLBACK(ddb_listview_hscroll_value_changed), NULL);
#if !GTK_CHECK_VERSION(3, 0, 0)
  g_signal_connect((gpointer)listview->header, "expose_event",
                   G_CALLBACK(ddb_listview_header_expose_event), NULL);
#else
  g_signal_connect((gpointer)listview->header, "draw",
                   G_CALLBACK(ddb_listview_header_draw), NULL);
#endif
  g_signal_connect((gpointer)listview->header, "realize",
                   G_CALLBACK(ddb_listview_header_realize), NULL);
  g_signal_connect((gpointer)listview->header, "motion_notify_event",
                   G_CALLBACK(ddb_listview_header_motion_notify_event), NULL);
  ...
  g_signal_connect((gpointer)listview->header, "button_release_event",
                   G_CALLBACK(ddb_listview_header_button_release_event), NULL);
  g_signal_connect((gpointer)listview->header, "enter-notify-event",
                   G_CALLBACK(ddb_listview_header_enter), NULL);
#if !GTK_CHECK_VERSION(3, 0, 0)
  g_signal_connect((gpointer)listview->list, "expose_event",
                   G_CALLBACK(ddb_listview_list_expose_event), NULL);
#else
  g_signal_connect((gpointer)listview->list, "draw",
                   G_CALLBACK(ddb_listview_list_draw), NULL);
#endif
  g_signal_connect((gpointer)listview->list, "realize",
                   G_CALLBACK(ddb_listview_list_realize), NULL);
  ...
  g_signal_connect((gpointer)listview->list, "popup_menu",
                   G_CALLBACK(ddb_listview_list_popup_menu), NULL);
  g_signal_connect((gpointer)listview->list, "scroll_event",
                   G_CALLBACK(ddb_listview_scroll_event), NULL);
  g_signal_connect((gpointer)listview->list, "drag_motion",
                   G_CALLBACK(ddb_listview_list_drag_motion), NULL);
  g_signal_connect((gpointer)listview->list, "drag_drop",
                   G_CALLBACK(ddb_listview_list_drag_drop), NULL);
  g_signal_connect((gpointer)listview->list, "drag_data_get",
                   G_CALLBACK(ddb_listview_list_drag_data_get), NULL);
  g_signal_connect((gpointer)listview->list, "drag_end",
                   G_CALLBACK(ddb_listview_list_drag_end), NULL);
  g_signal_connect((gpointer)listview->list, "drag_failed",
                   G_CALLBACK(ddb_listview_list_drag_failed), NULL);
  g_signal_connect((gpointer)listview->list, "drag_leave",
                   G_CALLBACK(ddb_listview_list_drag_leave), NULL);
  g_signal_connect((gpointer)listview->list, "button_release_event",
                   G_CALLBACK(ddb_listview_list_button_release_event), NULL);
  g_signal_connect((gpointer)listview->list, "motion_notify_event",
                   G_CALLBACK(ddb_listview_motion_notify_event), NULL);
  g_signal_connect((gpointer)listview->list, "drag_data_received",
                   G_CALLBACK(ddb_listview_list_drag_data_received), NULL);

  g_signal_connect((gpointer)listview->list, "key_press_event",
                   G_CALLBACK(ddb_listview_list_key_press_event), NULL);
  ...
  g_signal_connect(G_OBJECT(listview->header), "query-tooltip",
                   G_CALLBACK(header_tooltip_handler), listview);
  g_object_set_property(G_OBJECT(listview->list), "has-tooltip", &value);
  g_signal_connect(G_OBJECT(listview->list), "query-tooltip",
                   G_CALLBACK(list_tooltip_handler), listview);
}

GtkWidget *ddb_listview_new() {
  return g_object_newv(
      ddb_listview_get_type(), 0,
      NULL); // GTK_WIDGET(gtk_type_new(ddb_listview_get_type()));
}
...
void ddb_listview_refresh(DdbListview *listview, uint32_t flags) {
  ...
  if (flags & DDB_REFRESH_VSCROLL) {
    g_idle_add_full(GTK_PRIORITY_RESIZE, ddb_listview_list_setup_vscroll,
                    listview, NULL);
  }
  if (flags & DDB_REFRESH_HSCROLL) {
    g_idle_add_full(GTK_PRIORITY_RESIZE, ddb_listview_list_setup_hscroll,
                    listview, NULL);
  }
...
static gboolean ddb_listview_reconf_scrolling(void *ps) {
  ddb_listview_list_setup_vscroll(ps);
  ddb_listview_list_setup_hscroll(ps);
  return FALSE;
}

static void ddb_listview_update_scroll_ref_point(DdbListview *ps);

static void set_fwidth(DdbListview *ps, float list_width);

static void autoresize_columns(DdbListview *listview, int list_width,
                               int list_height);

static void _update_fwidth(DdbListview *ps, int prev_width) {
  GtkAllocation a;
...
static gboolean _initial_resizing_finished(void *ctx) {
  DdbListview *ps = ctx;
  ps->view_realized = 1;
  GtkAllocation a;
  gtk_widget_get_allocation(GTK_WIDGET(ps), &a);
...
static void ddb_listview_list_realize(GtkWidget *widget, gpointer user_data) {
  DdbListview *listview =
      DDB_LISTVIEW(g_object_get_data(G_OBJECT(widget), "owner"));
  if (listview->binding->drag_n_drop) {
    GtkTargetEntry entry = {.target = TARGET_PLAYITEMS,
                            .flags = GTK_TARGET_SAME_APP,
                            .info = TARGET_SAMEWIDGET};
    // setup drag-drop target
    gtk_drag_dest_set(widget, GTK_DEST_DEFAULT_MOTION | GTK_DEST_DEFAULT_DROP,
                      &entry, 1, GDK_ACTION_COPY | GDK_ACTION_MOVE);
    gtk_drag_dest_add_uri_targets(widget);
  }
  ddb_listview_update_fonts(listview);

  // defer column autoresizing until after the initial window resizing settles
  // down
  g_timeout_add(100, _initial_resizing_finished, listview);
}

static int total_columns_width(DdbListview *listview) {
  int size = 0;
...
static void ddb_listview_list_update_total_width(DdbListview *listview,
                                                 int columns_width, int width) {
  listview->totalwidth = max(columns_width, width);
}

static DdbListviewIter next_playitem(DdbListview *listview,
                                     DdbListviewIter it) {
...
void ddb_listview_groupcheck(DdbListview *listview) {
...
static int ddb_listview_is_album_art_column(DdbListview *listview, int x) {
  int col_x = -listview->hscrollpos;
  for (DdbListviewColumn *c = listview->columns; c && col_x <= x; c = c->next) {
    if (x <= col_x + c->width &&
        listview->binding->is_album_art_column(c->user_data)) {
      return 1;
    }
    col_x += c->width;
  }
...
static int ddb_listview_get_row_pos_subgroup(DdbListview *listview,
                                             DdbListviewGroup *grp, int y,
                                             int idx, int row_idx, int *accum) {
  while (grp) {
    int title_height = 0;
    ...
    if (idx + grp->num_items > row_idx) {
      int i;
      if (grp->subgroups) {
        i = ddb_listview_get_row_pos_subgroup(
            listview, grp->subgroups, y + title_height, idx, row_idx, accum);
      } else {
...
int ddb_listview_get_row_pos(DdbListview *listview, int row_idx,
                             int *accumulated_title_height) {
  int accum = 0;
  deadbeef->pl_lock();
  ddb_listview_groupcheck(listview);
  int y = ddb_listview_get_row_pos_subgroup(listview, listview->groups, 0, 0,
                                            row_idx, &accum);
  deadbeef->pl_unlock();
...
static int ddb_listview_is_empty_region(DdbListviewPickContext *pick_ctx) {
...
static int ddb_listview_list_pickpoint_subgroup(
    DdbListview *listview, DdbListviewGroup *grp, int x, int y, int idx,
    int grp_y, int group_level, int pin_offset,
  ...
  while (grp) {
    ...
    const int grp_title_height =
        grp->group_label_visible ? listview->grouptitle_height : 0;
    if (y >= grp_y && y < grp_y + h) {
      pick_ctx->grp = grp;
      y -= grp_y;
      if (y < grp_title_height ||
          (pin_offset < ry && ry < grp_title_height + pin_offset &&
           gtkui_groups_pinned)) {
        // group title
        ...
        pick_ctx->grp_idx = 0;
                 ...
                 group_level == listview->artwork_subgroup_level) {
        pick_ctx->type = PICK_ALBUM_ART;
        pick_ctx->item_grp_idx = idx;
        pick_ctx->grp_idx =
            min((y - grp_title_height) / rowheight, grp->num_items - 1);
        pick_ctx->item_idx = idx + pick_ctx->grp_idx;
      } else if (grp->subgroups &&
                 ddb_listview_list_pickpoint_subgroup(
                     listview, grp->subgroups, x, orig_y, idx,
                     grp_y + grp_title_height, group_level + 1,
                     pin_offset + grp_title_height, pick_ctx)) {
...
// returns -1 if nothing was hit, otherwise returns pointer to a group, and item
// idx item idx may be set to -1 if group title was hit
static void ddb_listview_list_pickpoint(DdbListview *listview, int x, int y,
                                        DdbListviewPickContext *pick_ctx) {
  int idx = 0;
  int grp_y = 0;
  const int ry = y - listview->scrollpos;
  ...
  deadbeef->pl_lock();
  ddb_listview_groupcheck(listview);
  int found = ddb_listview_list_pickpoint_subgroup(listview, listview->groups,
                                                   x, y, 0, 0, 0, 0, pick_ctx);
  deadbeef->pl_unlock();
...
static void draw_cairo_rectangle(cairo_t *cr, GdkColor *color, int x, int y,
                                 int width, int height) {
  cairo_set_source_rgb(cr, color->red / 65535., color->green / 65535.,
                       color->blue / 65535.);
  cairo_rectangle(cr, x, y, width, height);
  cairo_fill(cr);
}

static void draw_cairo_line(cairo_t *cr, GdkColor *color, int x1, int y1,
                            int x2, int y2) {
  cairo_set_source_rgb(cr, color->red / 65535., color->green / 65535.,
                       color->blue / 65535.);
...
#if GTK_CHECK_VERSION(3, 0, 0)
static void render_column_button(DdbListview *listview, cairo_t *cr,
                                 GtkStateFlags state, int x, int y, int w,
                                 int h, GdkColor *clr) {
  ...
  //    gtk_style_context_add_region(context, GTK_STYLE_REGION_COLUMN_HEADER,
  //    0);
...
static void render_treeview_background(DdbListview *listview, cairo_t *cr,
                                       int selected, int even, int x, int y,
                                       int w, int h, GdkRectangle *clip) {
  if (gtkui_override_listview_colors()) {
    GdkColor clr;
    if (selected) {
      gtkui_get_listview_selection_color(&clr);
    } else {
      even ? gtkui_get_listview_even_row_color(&clr)
           : gtkui_get_listview_odd_row_color(&clr);
    }
    ...
    gtk_style_context_save(context);
    gtk_style_context_set_state(context, selected ? GTK_STATE_FLAG_SELECTED
                                                  : GTK_STATE_FLAG_NORMAL);
    gtk_style_context_add_region(context, GTK_STYLE_REGION_ROW,
                                 even ? GTK_REGION_EVEN : GTK_REGION_ODD);
    gtk_render_background(context, cr, x, y, w, h);
    gtk_style_context_restore(context);
#else
    GTK_WIDGET_SET_FLAGS(theme_treeview, GTK_HAS_FOCUS);
    gtk_paint_flat_box(gtk_widget_get_style(theme_treeview),
                       gtk_widget_get_window(listview->list),
                       selected ? GTK_STATE_SELECTED : GTK_STATE_NORMAL,
                       GTK_SHADOW_NONE, clip, theme_treeview,
                       even ? "cell_even_ruled" : "cell_odd_ruled", x, y, w, h);
    GTK_WIDGET_UNSET_FLAGS(theme_treeview, GTK_HAS_FOCUS);
#endif
  }
}

static void fill_list_background(DdbListview *listview, cairo_t *cr, int x,
                                 int y, int w, int h, GdkRectangle *clip) {
  if (!gtkui_override_listview_colors()) {
#if GTK_CHECK_VERSION(3, 0, 0)
    gtk_render_background(gtk_widget_get_style_context(mainwin), cr, x, y, w,
                          h);
    gtk_render_background(gtk_widget_get_style_context(listview->list), cr, x,
                          y, w, h);
#else
    gtk_paint_flat_box(gtk_widget_get_style(mainwin),
                       gtk_widget_get_window(listview->list), GTK_STATE_NORMAL,
                       GTK_SHADOW_NONE, clip, mainwin, NULL, x, y, w, h);
    gtk_paint_flat_box(gtk_widget_get_style(listview->list),
                       gtk_widget_get_window(listview->list), GTK_STATE_NORMAL,
                       GTK_SHADOW_NONE, clip, listview->list, NULL, x, y, w, h);
#endif
  }
  render_treeview_background(listview, cr, FALSE, TRUE, x, y, w, h, clip);
}

static void ddb_listview_list_render_subgroup(
    DdbListview *listview, cairo_t *cr, GdkRectangle *clip,
    DdbListviewGroup *grp, int idx, int grp_y, const int cursor_index,
    const int current_group_depth, int title_offset,
    const int subgroup_artwork_offset, const int pin_offset) {
  ...
  while (grp && grp_y < clip->y + clip->height) {
    const int title_height =
        grp->group_label_visible ? listview->grouptitle_height : 0;
    const int is_pinned =
        gtkui_groups_pinned && grp_y < pin_offset && grp_y + grp->height >= 0;

    // only render list items when at the deepest group level
    if (!grp->subgroups) {
      DdbListviewIter it = grp->head;
      listview->binding->ref(it);
      for (int i = 0, yy = grp_y + title_height;
           it && i < grp->num_items && yy < clip->y + clip->height;
           i++, yy += row_height) {
        if (yy + row_height >= clip->y &&
            (!gtkui_groups_pinned || yy + row_height >= pin_offset)) {
          ddb_listview_list_render_row_background(
              listview, cr, it, i & 1, idx + i == cursor_index, scrollx, yy,
              total_width, row_height, clip);
          ddb_listview_list_render_row_foreground(
              listview, cr, it, i & 1, idx + i, yy, total_width, row_height,
    ...
    if (current_group_depth == listview->artwork_subgroup_level) {
      ...
      subgroup_title_offset =
          title_offset +
          (grp->group_label_visible ? listview->subgroup_title_padding : 0);
    }

    if (grp->subgroups) {
      // render subgroups before album art and titles
      ddb_listview_list_render_subgroup(
          listview, cr, clip, grp->subgroups, idx, grp_y + title_height,
          cursor_index, current_group_depth + 1, subgroup_title_offset,
          subgroup_artwork_offset, pin_offset + title_height);
    }
    ...
    if (current_group_depth == listview->artwork_subgroup_level) {
      ...
      ddb_listview_list_render_album_art(listview, cr, grp, min_y, grp_next_y,
                                         grp_y + title_height, clip);
    }

    if (is_pinned && clip->y <= title_height + pin_offset) {
      // draw pinned group title
      int y = min(pin_offset, grp_next_y - title_height);
      fill_list_background(listview, cr, scrollx, y, total_width, title_height,
                           clip);
      if (listview->binding->draw_group_title && title_height > 0) {
        listview->binding->draw_group_title(
            listview, cr, grp->head, title_offset, y,
            total_width - title_offset, title_height, current_group_depth);
      }
    } else if (clip->y <= grp_y + title_height) {
      // draw normal group title
      if (listview->binding->draw_group_title && title_height > 0) {
        listview->binding->draw_group_title(listview, cr, grp->head,
                                            title_offset, grp_y, total_width,
                                            title_height, current_group_depth);
      }
    }
...
static void ddb_listview_list_render(DdbListview *listview, cairo_t *cr,
                                     GdkRectangle *clip) {
  ...
  // Calculate which side of the playlist the (first) album art cover is on to
  // tell where to draw subgroup titles
  int subgroup_artwork_offset = listview->subgroup_title_padding;
  int x = 0;
  for (DdbListviewColumn *c = listview->columns; c;
       x += c->width, c = c->next) {
  ...
  fill_list_background(listview, cr, clip->x, clip->y, clip->width,
                       clip->height, clip);

  ddb_listview_list_render_subgroup(
      listview, cr, clip, listview->groups, 0, -listview->scrollpos,
      cursor_index, 0, -listview->hscrollpos, subgroup_artwork_offset, 0);
...
static void ddb_listview_draw_dnd_marker(DdbListview *ps, cairo_t *cr) {
...
static void ddb_listview_update_fonts(DdbListview *ps) {
  ...
  if (row_height != ps->rowheight ||
      grptitle_height != ps->calculated_grouptitle_height) {
...
static void draw_list_rectangle(DdbListview *ps, cairo_t *cr,
                                GdkRectangle *clip) {
  ...
  if (ps->drag_motion_y >= 0 &&
      ps->drag_motion_y - ps->scrollpos - 3 < clip->y + clip->height &&
      ps->drag_motion_y - ps->scrollpos + 3 >= clip->y) {
    ddb_listview_draw_dnd_marker(ps, cr);
  }
}

#if GTK_CHECK_VERSION(3, 0, 0)
static int list_is_realized(DdbListview *listview) {
...
#else
static int list_is_realized(DdbListview *listview) {
  return GTK_OBJECT_FLAGS(listview) & GTK_REALIZED &&
         gtk_widget_get_style(theme_treeview)->depth != -1;
}
static gboolean ddb_listview_list_expose_event(GtkWidget *widget,
                                               GdkEventExpose *event,
                                               gpointer user_data) {
...
static void scroll_by(GtkWidget *scrollbar, gdouble delta) {
  GtkRange *range = GTK_RANGE(scrollbar);
  gdouble step = pow(
      gtk_adjustment_get_page_size(gtk_range_get_adjustment(range)), 2. / 3.);
...
static void invalidate_album_art_cells(DdbListview *listview, int x1, int x2,
                                       int y, int h) {
  int x = -listview->hscrollpos;
  for (DdbListviewColumn *c = listview->columns; c && x < x2;
       x += c->width, c = c->next) {
    if (x + c->width > x1 &&
        listview->binding->is_album_art_column(c->user_data)) {
...
static int find_subgroup_title_heights(DdbListview *ps, DdbListviewGroup *group,
                                       int group_y, int at_y) {
...
static void invalidate_group(DdbListview *ps, int at_y) {
  ...
  int group_titles_height =
      group->group_label_visible ? ps->grouptitle_height : 0;
  if (group->subgroups) {
    group_titles_height += find_subgroup_title_heights(
        ps, group->subgroups, next_group_y - group->height, at_y);
  }

  int group_height = next_group_y - at_y;
  if (next_group_y > at_y) {
    gtk_widget_queue_draw_area(ps->list, 0, 0, ps->list_width,
                               min(group_titles_height, group_height));
  }
  if (group_height > group_titles_height) {
    invalidate_album_art_cells(ps, 0, ps->list_width, group_titles_height,
                               group_height);
  }
}
...
static gboolean ddb_listview_list_drag_motion(GtkWidget *widget,
  ...
  if (!source_is_listview) {
    ...
    // source is listview, so we can support both GDK_ACTION_MOVE and
    // GDK_ACTION_COPY
...
static gchar **ddb_listview_build_drag_uri_list(DdbListview *ps) {
  deadbeef->pl_lock();
...
static void ddb_listview_list_drag_data_get(GtkWidget *widget,
  ...
  case TARGET_SAMEWIDGET: {
    ...
    *ptr = ps->drag_source_playlist;
    ...
    gtk_selection_data_set(selection_data, target, sizeof(uint32_t) * 8,
                           (gchar *)ptr, (nsel + 1) * sizeof(uint32_t));
    free(ptr);
  } break;
  default:
...
static void ddb_listview_list_drag_data_received(
    GtkWidget *widget, GdkDragContext *drag_context, gint x, gint y,
    GtkSelectionData *data, guint target_type, guint time, gpointer user_data) {
  DdbListview *ps = DDB_LISTVIEW(g_object_get_data(G_OBJECT(widget), "owner"));
  ...
  if (target_type == TARGET_URILIST) { // uris
    ...
    memcpy(mem, ptr, len);
    mem[len] = 0;
    // we don't pass control structure, but there's only one drag-drop view
    // currently
    ...
    if (it) {
      UNREF(it);
             ...
             gtk_selection_data_get_format(data) ==
                 32) { // list of 32bit ints, DDB_URI_LIST target
    ddb_listview_clear_sort(ps);
    ...
    if (p) {
      // FIXME
      ps->binding->drag_n_drop(
          drop_before, p, d, length,
          gdk_drag_context_get_selected_action(drag_context) == GDK_ACTION_COPY
              ? 1
              : 0);
...
static void adjust_scrollbar(GtkWidget *scrollbar, int upper, int page_size) {
  GtkRange *range = GTK_RANGE(scrollbar);
  if (page_size >= upper) {
  ...
  if (scrollpos > 0 && page_size != old_page_size &&
      scrollpos >= old_upper - old_page_size) {
    scrollpos = upper - page_size;
  }
  gtk_range_set_adjustment(
      range, GTK_ADJUSTMENT(gtk_adjustment_new(0, 0, upper, SCROLL_STEP,
...
void ddb_listview_draw_row(DdbListview *listview, int row, DdbListviewIter it) {
  int y = ddb_listview_get_row_pos(listview, row, NULL) - listview->scrollpos;
  if (y + listview->rowheight > 0 && y <= listview->list_height) {
    gtk_widget_queue_draw_area(listview->list, 0, y, listview->list_width,
                               listview->rowheight);
  }
}
...
static void ddb_listview_list_render_row_background(
  ...
  if (cursor && gtk_widget_has_focus(ps->list)) {
    // not all gtk engines/themes render focus rectangle in treeviews but we
    // want it anyway
    GdkColor clr;
    gtkui_get_listview_cursor_color(&clr);
    cairo_set_source_rgb(cr, clr.red / 65535., clr.green / 65535.,
                         clr.blue / 65535.);
...
ddb_listview_list_render_row_foreground(DdbListview *ps, cairo_t *cr,
                                        ...
                                        int y, int w, int h, int x1, int x2) {
  int x = -ps->hscrollpos;
  for (DdbListviewColumn *c = ps->columns; c && x < x2;
       x += c->width, c = c->next) {
    if (x + c->width > x1 && !ps->binding->is_album_art_column(c->user_data)) {
      ps->binding->draw_column_data(
          ps, cr, it, idx, c->align_right, c->user_data,
          c->color_override ? &c->color : NULL, x, y, c->width, h, even);
    }
  }
}
...
static void ddb_listview_list_render_album_art(DdbListview *ps, cairo_t *cr,
                                               ...
                                               GdkRectangle *clip) {
  int x = -ps->hscrollpos;
  for (DdbListviewColumn *c = ps->columns; c && x < clip->x + clip->width;
       x += c->width, c = c->next) {
    if (ps->binding->is_album_art_column(c->user_data) &&
        x + c->width > clip->x) {
      fill_list_background(ps, cr, x, y, c->width,
                           grp->height - ps->grouptitle_height, clip);
      if (ps->grouptitle_height > 0) {
        ps->binding->draw_album_art(ps, cr, grp->head, c->user_data, min_y,
                                    grp_next_y, x, y, c->width,
                                    grp->height - ps->grouptitle_height);
      }
    }
  }
}

static void ddb_listview_header_expose(DdbListview *ps, cairo_t *cr, int x,
                                       int y, int w, int h) {
  ddb_listview_header_render(ps, cr, x, x + w);
}

// Deselect all items in the current list
static void ddb_listview_deselect_all(DdbListview *listview) {
  int notify_singly =
      listview->binding->sel_count() <= NUM_ROWS_TO_NOTIFY_SINGLY;
  DdbListviewIter it;
  int idx = 0;
...
static void ddb_listview_select_group(DdbListview *listview,
                                      DdbListviewGroup *grp, int first_item_idx,
                                      int deselect) {
  if (grp == NULL) {
    return;
  }
  ...
  for (int group_idx = 0; it && group_idx < grp->num_items; group_idx++) {
    ...
    if (notify_singly) {
      ...
      listview->binding->selection_changed(listview, it,
                                           first_item_idx + group_idx);
    }
    it = next_playitem(listview, it);
  }
  if (it) {
...
// item_idx: index of first item in the group or -1
static void ddb_listview_toggle_group_selection(DdbListview *listview,
                                                DdbListviewGroup *grp,
                                                int item_idx) {
  if (grp == NULL) {
    return;
...
void ddb_listview_select_single(DdbListview *ps, int sel) {
  deadbeef->pl_lock();
...
void ddb_listview_update_cursor(DdbListview *ps, int cursor) {
...
static gboolean set_cursor_and_scroll_cb(gpointer data) {
  ...
  int accumulated_title_height;
  int cursor_scroll =
      ddb_listview_get_row_pos(sc->pl, sc->cursor, &accumulated_title_height);
  int newscroll = sc->pl->scrollpos;
  if (!gtkui_groups_pinned && cursor_scroll < sc->pl->scrollpos) {
    newscroll = cursor_scroll;
  } else if (gtkui_groups_pinned &&
             cursor_scroll < sc->pl->scrollpos + accumulated_title_height) {
    newscroll = cursor_scroll - accumulated_title_height;
  } else if (cursor_scroll + sc->pl->rowheight >=
             sc->pl->scrollpos + sc->pl->list_height) {
...
void ddb_listview_set_cursor_and_scroll(DdbListview *listview, int cursor) {
...
void ddb_listview_select_range(DdbListview *ps, int start, int end) {
  int nchanged = 0;
  int idx = 0;
  DdbListviewIter it;
...
static int ddb_listview_is_group_selected(DdbListview *ps,
                                          DdbListviewGroup *grp) {
  if (!ps || !grp) {
    return 0;
  }
...
void ddb_listview_click_selection(DdbListview *ps, int ex, int ey,
                                  DdbListviewPickContext *pick_ctx, int dnd,
                                  int button) {
  deadbeef->pl_lock();
  ps->areaselect = 0;
  ddb_listview_groupcheck(ps);
  ...
  if (dnd) {
    ...
    } else if (selected && pick_ctx->type != PICK_EMPTY_SPACE &&
               ps->binding->drag_n_drop) {
      ps->dragwait = 1;
    }
  }
  ...
  if (pick_ctx->type == PICK_EMPTY_SPACE) {
    ...
    ddb_listview_deselect_all(ps);
  } else if (pick_ctx->item_idx != -1 && (pick_ctx->type == PICK_GROUP_TITLE ||
                                          pick_ctx->type == PICK_ALBUM_ART)) {
...
//   }}}
// }}}
void ddb_listview_list_mouse1_pressed(DdbListview *ps, int state, int ex,
                                      int ey, GdkEventType type) {
  ...
  int group_clicked =
      (pick_ctx.type == PICK_ALBUM_ART || pick_ctx.type == PICK_GROUP_TITLE)
          ? 1
          : 0;

  int cursor = ps->binding->cursor();
  if (type == GDK_2BUTTON_PRESS && fabs(ps->lastpos[0] - ex) < 3 &&
      fabs(ps->lastpos[1] - ey) < 3) {
    // doubleclick - play this item
    if (pick_ctx.item_idx != -1 && !ddb_listview_is_empty_region(&pick_ctx) &&
        cursor != -1) {
      int idx = cursor;
  ...
  if (!(state & (selmask | GDK_SHIFT_MASK))) {
    ...
    if (pick_ctx.type != PICK_EMPTY_SPACE && pick_ctx.item_idx != -1) {
      if (group_clicked) {
        // toggle group items
        ddb_listview_toggle_group_selection(ps, pick_ctx.grp,
                                            pick_ctx.item_grp_idx);
...
void ddb_listview_list_mouse1_released(DdbListview *ps, int state, int ex,
                                       int ey, double time) {
...
static gboolean ddb_listview_list_scroll_cb(gpointer data) {
  DdbListview *ps = (DdbListview *)data;
  ps->scroll_active = 1;
  struct timeval tm;
  gettimeofday(&tm, NULL);
  float dt = tm.tv_sec - ps->tm_prevscroll.tv_sec +
             (tm.tv_usec - ps->tm_prevscroll.tv_usec) / 1000000.0;
  ...
  if (ps->scroll_mode == 0) {
    ddb_listview_list_mousemove(ps, NULL, ps->scroll_pointer_x,
                                ps->scroll_pointer_y);
  } else if (ps->scroll_mode == 1) {
    ddb_listview_list_track_dragdrop(ps, ps->scroll_pointer_x,
                                     ps->scroll_pointer_y);
  }
  if (ps->scroll_direction < 0) {
...
static void ddb_listview_select_tracks_within_region(DdbListview *ps, int x,
  ...
  if (start_idx == end_idx) {
    if (pick_ctx_start.type != PICK_EMPTY_SPACE ||
        pick_ctx_end.type != PICK_EMPTY_SPACE) {
...
void ddb_listview_list_mousemove(DdbListview *ps, GdkEventMotion *ev, int ex,
                                 int ey) {
  deadbeef->pl_lock();
  GtkWidget *widget = ps->list;
  int move_threshold =
      gtk_drag_check_threshold(widget, ps->lastpos[0], ps->lastpos[1], ex, ey);
  ...
  if (move_threshold) {
    if (ps->dragwait) {
      ...
      gtk_drag_begin(widget, lst, GDK_ACTION_COPY | GDK_ACTION_MOVE, 1,
                     (GdkEvent *)ev);
    } else if (ps->areaselect) {
      ddb_listview_select_tracks_within_region(
          ps, ps->areaselect_x, ps->areaselect_y, ey + ps->scrollpos);
...
int ddb_listview_dragdrop_get_row_from_coord(DdbListview *listview, int x,
  ...
  if (pick_ctx.type == PICK_ITEM || pick_ctx.type == PICK_ALBUM_ART) {
    ...
    int it_y =
        ddb_listview_get_row_pos(listview, row_idx, NULL) - listview->scrollpos;
...
static void ddb_listview_list_track_dragdrop(DdbListview *ps, int x, int y) {
  ...
  if (y == -1) {
    ...
    if (sel == -1) {
      if (ps->binding->count() == 0) {
        ...
        // after last row
        ps->drag_motion_y =
            ddb_listview_get_row_pos(ps, ps->binding->count() - 1, NULL) +
            ps->rowheight;
      }
    } else {
  ...
  if (prev_drag_y != ps->drag_motion_y) {
    if (prev_drag_y != -1) {
      ...
      gtk_widget_queue_draw_area(ps->list, 0, prev_drag_y - ps->scrollpos - 3,
                                 ps->list_width, 7);
    }
    if (ps->drag_motion_y != -1) {
      // new track
      gtk_widget_queue_draw_area(ps->list, 0,
                                 ps->drag_motion_y - ps->scrollpos - 3,
                                 ps->list_width, 7);
    }
  }
...
static void draw_header_fg(DdbListview *ps, cairo_t *cr, DdbListviewColumn *c,
                           GdkColor *clr, int x, int xx, int h) {
  int text_width = xx - x - 10;
  if (c->sort_order) {
    int arrow_sz = 10;
    text_width = max(0, text_width - arrow_sz);
#if GTK_CHECK_VERSION(3, 0, 0)
    gtk_render_arrow(gtk_widget_get_style_context(theme_treeview), cr,
                     c->sort_order * G_PI, xx - arrow_sz - 5,
                     h / 2 - arrow_sz / 2, arrow_sz);
#else
    int dir = c->sort_order == 1 ? GTK_ARROW_DOWN : GTK_ARROW_UP;
    gtk_paint_arrow(ps->header->style, ps->header->window, GTK_STATE_NORMAL,
                    GTK_SHADOW_NONE, NULL, ps->header, NULL, dir, TRUE,
                    xx - arrow_sz - 5, h / 2 - arrow_sz / 2, arrow_sz,
                    arrow_sz);
#endif
  }
  ...
  cairo_clip(cr);
  draw_text_custom(&ps->hdrctx, x + 5, 3, text_width, 0, DDB_COLUMN_FONT, 0, 0,
                   c->title);
...
static void ddb_listview_header_render(DdbListview *ps, cairo_t *cr, int x1,
  ...
  draw_cairo_line(cr, &gtk_widget_get_style(ps->header)->mid[GTK_STATE_NORMAL],
                  0, h, a.width, h);
#else
#if GTK_CHECK_VERSION(3, 0, 0)
  render_column_button(ps, cr, GTK_STATE_FLAG_NORMAL, 0, -1, a.width, h + 2,
                       &gdkfg);
#else
  gtk_paint_box(gtk_widget_get_style(theme_button),
                gtk_widget_get_window(ps->header), GTK_STATE_NORMAL,
                GTK_SHADOW_OUT, NULL, ps->header, "button", -2, -2, a.width + 4,
                h + 4);
  ...
  int x = -ps->hscrollpos;
  int idx = 0;
  // Add a column header pseudo-button for each configured treeview column, by
  // drawing lines across the background
  for (DdbListviewColumn *c = ps->columns; c && x < x2; c = c->next, idx++) {
    int xx = x + c->width;

    // Only render for columns within the clip region, and not any column which
    // is being dragged
    if (idx != ps->header_dragging && xx >= x1) {
      // Paint the button text
      draw_header_fg(ps, cr, c, &gdkfg, x, xx, h);

      // Add a vertical line near the right side of the column width, but not
      // right next to an empty slot
  ...
  if (ps->header_dragging != -1) {
    ...
    // Mark the position where the dragged column used to be with an
    // indented/active/dark position
...
#else
      gtk_paint_box(gtk_widget_get_style(theme_button),
                    gtk_widget_get_window(ps->header), GTK_STATE_ACTIVE,
                    GTK_SHADOW_ETCHED_IN, NULL, ps->header, "button", xx, 0, w,
                    h);
#endif
    }

    // Draw a highlighted/selected "button" wherever the dragged column is
    // currently positioned
    xx = ps->col_movepos - ps->hscrollpos - 2;
    if (w > 0 && xx < x2) {
#if GTK_CHECK_VERSION(3, 0, 0)
      render_column_button(ps, cr,
                           GTK_STATE_FLAG_PRELIGHT | GTK_STATE_FLAG_FOCUSED, xx,
                           0, w, h, &gdkfg);
#else
      gtk_paint_box(gtk_widget_get_style(theme_button),
                    gtk_widget_get_window(ps->header), GTK_STATE_SELECTED,
                    GTK_SHADOW_OUT, NULL, ps->header, "button", xx, 0, w, h);
...
static gboolean ddb_listview_header_expose_event(GtkWidget *widget,
  ...
  DdbListview *ps = DDB_LISTVIEW(g_object_get_data(G_OBJECT(widget), "owner"));
  cairo_t *cr = gdk_cairo_create(gtk_widget_get_window(widget));
  ddb_listview_header_expose(ps, cr, event->area.x, event->area.y,
...
static void ddb_listview_column_size_changed(DdbListview *listview,
                                             DdbListviewColumn *c) {
  if (listview->binding->is_album_art_column(c->user_data)) {
    ...
    if (!listview->lock_columns) {
      ...
      gtk_range_set_value(GTK_RANGE(listview->scrollbar),
                          pos - listview->ref_point_offset);
    }
  }
}

static void ddb_listview_update_scroll_ref_point_subgroup(DdbListview *ps,
                                                          DdbListviewGroup *grp,
                                                          int abs_idx,
                                                          int grp_y) {
  // find 1st group
  ...
  int grp_content_pos =
      grp_y + (grp->group_label_visible ? ps->grouptitle_height : 0);

  if (grp->subgroups) {
    // search subgroups for anchor
    ddb_listview_update_scroll_ref_point_subgroup(ps, grp->subgroups, abs_idx,
                                                  grp_content_pos);
  } else {
    // choose first visible item as anchor
    int first_item_idx =
        max(0, (ps->scrollpos - grp_content_pos) / ps->rowheight);
    ps->ref_point = abs_idx + first_item_idx;
    ps->ref_point_offset =
        grp_content_pos + (first_item_idx * ps->rowheight) - ps->scrollpos;
  }
}
...
static void ddb_listview_update_scroll_ref_point(DdbListview *ps) {
  ...
  if (ps->groups) {
    ...
    if (ps->scrollpos < cursor_pos &&
        cursor_pos < ps->scrollpos + ps->list_height &&
        cursor_pos < ps->fullheight) {
...
static void set_column_width(DdbListview *listview, DdbListviewColumn *c,
                             float new_width) {
...
void ddb_listview_init_autoresize(DdbListview *ps, int totalwidth) {
  if (totalwidth > 0) {
    DdbListviewColumn *c;
...
static int groups_full_height(DdbListview *listview, DdbListviewColumn *c,
                              int new_width) {
  int min_height = c->minheight_cb(c->user_data, new_width);
  int full_height = 0;
  deadbeef->pl_lock();
  for (DdbListviewGroup *grp = listview->groups; grp; grp = grp->next) {
    full_height += listview->grouptitle_height +
                   max(grp->num_items * listview->rowheight, min_height);
  }
  deadbeef->pl_unlock();
  return full_height;
}
...
static int unsafe_group_height(DdbListview *listview, DdbListviewColumn *c,
                               int new_width, int list_width, int list_height) {
  if (!c->minheight_cb) {
    return 0;
  }
  ...
  if (listview->fullheight > list_height) {
    if (groups_full_height(listview, c, new_width) <= list_height) {
      int width_wo_scrollbar =
          roundf((list_width + scrollbar_width) * c->fwidth);
...
static void autoresize_columns(DdbListview *listview, int list_width,
  ...
  if (listview->fwidth > 1) {
    do {
      ...
      for (DdbListviewColumn *c = listview->columns; c; c = c->next) {
        int new_width =
            max(MIN_COLUMN_WIDTH, roundf(working_width * c->fwidth));
        if (unsafe_group_height(listview, c, new_width, list_width,
                                list_height)) {
          new_width = c->width;
        }
    ...
    do {
      ...
      for (DdbListviewColumn *c = listview->columns; c; c = c->next) {
        ...
        } else if (unsafe_group_height(listview, c, new_width, list_width,
                                       list_height)) {
          new_width = c->width;
        }
        total_width += new_width;
...
static gboolean ddb_listview_list_configure_event(GtkWidget *widget,
  ...
  // the values in event->width/height are broken since GTK-3.22.1, so let's use
  // widget's allocation instead, and hope this is reliable.
  GtkAllocation a;
  ...
  if (a.width != prev_width || a.height != ps->list_height) {
    ...
    g_idle_add_full(GTK_PRIORITY_RESIZE, ddb_listview_reconf_scrolling, ps,
                    NULL);
  }
...
void ddb_listview_size_columns_without_scrollbar(DdbListview *listview) {
  if (deadbeef->conf_get_int("gtkui.autoresize_columns", 0) &&
      gtk_widget_get_visible(listview->scrollbar)) {
    GtkAllocation a;
    gtk_widget_get_allocation(listview->scrollbar, &a);
    autoresize_columns(listview, listview->list_width + a.width,
...
static void set_header_cursor(DdbListview *listview, gdouble mousex) {
  int x = -listview->hscrollpos;
  for (DdbListviewColumn *c = listview->columns; c; c = c->next) {
    if (mousex >= x + c->width - 4 && mousex <= x + c->width) {
      gdk_window_set_cursor(gtk_widget_get_window(listview->header),
                            listview->cursor_sz);
      return;
    }
    x += c->width;
...
static gboolean ddb_listview_header_motion_notify_event(GtkWidget *widget,
  ...
  if (ps->header_prepare) {
    if (ps->header_dragging != -1 &&
        gtk_drag_check_threshold(widget, round(ps->prev_header_x), 0,
                                 round(event->x), 0)) {
      ps->header_prepare = 0;
    } else {
      return FALSE;
  ...
  if (ps->header_dragging >= 0) {
    ...
    if (c) {
      ...
      for (int xx = 0, ii = 0; cc; xx += cc->width, cc = cc->next, ii++) {
        if (ps->header_dragging > ii && left < xx + cc->width / 2 ||
            ps->header_dragging < ii && right > xx + cc->width / 2) {
    ...
    set_column_width(
        ps, c,
        max(MIN_COLUMN_WIDTH, round(event->x) - ps->header_dragpt[0] - x));

    ddb_listview_column_size_changed(ps, c);
    g_idle_add_full(GTK_PRIORITY_RESIZE, ddb_listview_list_setup_hscroll, ps,
                    NULL);
    gtk_widget_queue_draw(ps->header);
    gtk_widget_queue_draw(ps->list);
  } else {
...
void ddb_listview_col_sort_update(DdbListview *listview) {
...
static gboolean ddb_listview_header_button_release_event(GtkWidget *widget,
  ...
  if (event->button == 1) {
    if (ps->header_sizing != -1) {
      ...
      ddb_listview_list_update_total_width(ps, total_columns_width(ps),
                                           ps->list_width);
      ps->header_sizing = -1;
...
static gboolean ddb_listview_header_enter(GtkWidget *widget,
                                          ...
                                          gpointer user_data) {
  DdbListview *ps = DDB_LISTVIEW(g_object_get_data(G_OBJECT(widget), "owner"));
  if (!ps->header_prepare && ps->header_dragging == -1 &&
      ps->header_sizing == -1) {
    int x = event->x;
#if GTK_CHECK_VERSION(3, 0, 0)
    if (event->send_event) {
      ...
      gdk_window_get_device_position(
          win, gdk_device_manager_get_client_pointer(device_manager), &x, NULL,
          NULL);
    }
#endif
...
gboolean ddb_listview_list_button_press_event(GtkWidget *widget,
  ...
  DdbListview *ps = DDB_LISTVIEW(g_object_get_data(G_OBJECT(widget), "owner"));
  if (TEST_LEFT_CLICK(event)) {
    ddb_listview_list_mouse1_pressed(ps, event->state, event->x, event->y,
                                     event->type);
  } else if (TEST_RIGHT_CLICK(event)) {
    // get item under cursor
    DdbListviewPickContext pick_ctx;
    ddb_listview_list_pickpoint(ps, event->x, event->y + ps->scrollpos,
                                &pick_ctx);

    ddb_listview_click_selection(ps, event->x, event->y, &pick_ctx, 0,
                                 event->button);
...
static gboolean ddb_listview_list_button_release_event(GtkWidget *widget,
  ...
  DdbListview *ps = DDB_LISTVIEW(g_object_get_data(G_OBJECT(widget), "owner"));
  if (event->button == 1) {
    ddb_listview_list_mouse1_released(ps, event->state, event->x, event->y,
                                      event->time);
  }
  return FALSE;
}
...
void ddb_listview_set_binding(DdbListview *listview,
                              DdbListviewBinding *binding) {
  listview->binding = binding;
}

void ddb_listview_scroll_to(DdbListview *listview, int pos) {
  pos = ddb_listview_get_row_pos(listview, pos, NULL);
  if (pos < listview->scrollpos ||
      pos + listview->rowheight >=
          listview->scrollpos + listview->list_height) {
    gtk_range_set_value(GTK_RANGE(listview->scrollbar),
                        pos - listview->list_height / 2);
  }
}

/////// column management code

int ddb_listview_column_get_count(DdbListview *listview) {
  int cnt = 0;
...
ddb_listview_column_alloc(const char *title, int align_right,
                          minheight_cb_t minheight_cb, int is_artwork,
                          int color_override, GdkColor color, void *user_data) {
...
void ddb_listview_column_append(DdbListview *listview, const char *title,
                                int width, int align_right,
                                minheight_cb_t minheight_cb, int is_artwork,
                                int color_override, GdkColor color,
                                void *user_data) {
  ddb_listview_column_insert(listview, -1, title, width, align_right,
                             minheight_cb, is_artwork, color_override, color,
                             user_data);
}

void ddb_listview_column_insert(DdbListview *listview, int before,
                                const char *title, int width, int align_right,
                                minheight_cb_t minheight_cb, int is_artwork,
                                int color_override, GdkColor color,
                                void *user_data) {
  DdbListviewColumn *c =
      ddb_listview_column_alloc(title, align_right, minheight_cb, is_artwork,
                                color_override, color, user_data);
...
static void remove_column(DdbListview *listview, DdbListviewColumn **c_ptr) {
...
void ddb_listview_column_remove(DdbListview *listview, int idx) {
  DdbListviewColumn *c = listview->columns;
  if (idx == 0) {
...
int ddb_listview_column_get_info(DdbListview *listview, int col,
                                 const char **title, int *width,
                                 int *align_right, minheight_cb_t *minheight_cb,
                                 int *is_artwork, int *color_override,
                                 GdkColor *color, void **user_data) {
  DdbListviewColumn *c;
  int idx = 0;
...
int ddb_listview_column_set_info(DdbListview *listview, int col,
                                 const char *title, int width, int align_right,
                                 minheight_cb_t minheight_cb, int is_artwork,
                                 int color_override, GdkColor color,
                                 void *user_data) {
...
void ddb_listview_invalidate_album_art_columns(DdbListview *listview) {
  invalidate_album_art_cells(listview, 0, listview->list_width, 0,
                             listview->list_height);
}
...
static int ddb_listview_min_group_height(DdbListviewColumn *columns) {
  int min_height = 0;
...
static DdbListviewGroup *new_group(DdbListview *listview, DdbListviewIter it,
...
static int calc_subgroups_height(DdbListviewGroup *grp) {
  int height = 0;
  while (grp) {
    height += grp->height;
    grp = grp->next;
...
static int calc_group_height(DdbListview *listview, DdbListviewGroup *grp,
                             int min_height, int is_last) {
...
static int build_groups(DdbListview *listview) {
  ...
  int min_no_artwork_height =
      ddb_listview_min_no_artwork_group_height(listview->columns);
  int full_height = 0;
  // groups
  if (listview->grouptitle_height) {
    ...
    for (int i = 0; i < group_depth; i++) {
      last_group[i] = grp;
      grp = grp->subgroups;
      listview->binding->get_group(listview, it, group_titles[i],
                                   sizeof(*group_titles), i);
    ...
    while ((it = next_playitem(listview, it))) {
      ...
      for (int i = 0; i < group_depth; i++) {
        ...
        listview->binding->get_group(listview, it, next_title,
                                     sizeof(next_title), i);
      ...
      if (make_new_group_offset >= 0) {
        ...
        for (int i = group_depth - 1; i >= make_new_group_offset; i--) {
          ...
          listview->binding->get_group(listview, it, next_title,
                                       sizeof(next_title), i);
          int height = calc_group_height(listview, last_group[i],
                                         i == listview->artwork_subgroup_level
                                             ? min_height
                                             : min_no_artwork_height,
                                         !(it > 0));
          if (i == 0) {
            full_height += height;
          }
          DdbListviewGroup *new_grp =
              it ? new_group(listview, it, next_title[0] != 0) : NULL;
...
static int ddb_listview_resize_subgroup(DdbListview *listview,
  ...
  while (grp) {
    if (grp->subgroups) {
      ddb_listview_resize_subgroup(listview, grp->subgroups, group_depth + 1,
                                   min_height, min_no_artwork_height);
    }
    full_height += calc_group_height(
        listview, grp,
        group_depth == listview->artwork_subgroup_level ? min_height
                                                        : min_no_artwork_height,
        !grp->next);
    grp = grp->next;
  }
...
static void ddb_listview_resize_groups(DdbListview *listview) {
  ...
  int min_no_artwork_height =
      ddb_listview_min_no_artwork_group_height(listview->columns);
  int full_height = ddb_listview_resize_subgroup(
      listview, listview->groups, 0, min_height, min_no_artwork_height);

  if (full_height != listview->fullheight) {
    listview->fullheight = full_height;
    adjust_scrollbar(listview->scrollbar, listview->fullheight,
                     listview->list_height);
  }
}

static gboolean unlock_columns_cb(gpointer p) {
  DDB_LISTVIEW(p)->lock_columns = 0;
  return FALSE;
}

int ddb_listview_list_setup(DdbListview *listview, int scroll_to) {
...
void ddb_listview_show_header(DdbListview *listview, int show) {
  show ? gtk_widget_show(listview->header) : gtk_widget_hide(listview->header);
}

void ddb_listview_clear_sort(DdbListview *listview) {
  DdbListviewColumn *c;
...
static gboolean ddb_listview_list_key_press_event(GtkWidget *widget,
  ...
  DdbListview *listview =
      DDB_LISTVIEW(g_object_get_data(G_OBJECT(widget), "owner"));
  if (!listview->binding->list_handle_keypress(listview, event->keyval,
                                               event->state, PL_MAIN)) {
...
static void set_tooltip(GtkTooltip *tooltip, const char *text, int x, int y,
                        int width, int height) {
...
static gboolean header_tooltip_handler(GtkWidget *widget, gint x, gint y,
  ...
  DdbListviewColumn *c;
  int col_x = -listview->hscrollpos;
  for (c = listview->columns; c && col_x + c->width < x;
       col_x += c->width, c = c->next)
    ;
  ...
  if (pick_ctx.type == PICK_ITEM) {
    ...
    if (it) {
      ...
      if (c) {
        ...
        GdkColor clr = {0};
        int row_y =
            ddb_listview_get_row_pos(listview, idx, NULL) - listview->scrollpos;
        listview->binding->draw_column_data(
            listview, cr, it, idx, c->align_right, c->user_data, &clr, col_x,
            row_y, c->width, listview->rowheight, 0);
        cairo_destroy(cr);
        if (draw_is_ellipsized(&listview->listctx)) {
          set_tooltip(tooltip, draw_get_text(&listview->listctx), col_x, row_y,
                      c->width, listview->rowheight);
...
void ddb_listview_cancel_autoredraw(DdbListview *listview) {
