...
static void gtkpl_adddir_cb(gpointer data, gpointer userdata) {
    ddb_playlist_t *plt = deadbeef->plt_get_curr();
...
void gtkpl_add_dirs(GSList *lst) {
    ddb_playlist_t *plt = deadbeef->plt_get_curr();
...
static void gtkpl_addfile_cb(gpointer data, gpointer userdata) {
    ddb_playlist_t *plt = deadbeef->plt_get_curr();
...
void gtkpl_add_files(GSList *lst) {
    ddb_playlist_t *plt = deadbeef->plt_get_curr();
    if (deadbeef->plt_add_files_begin(plt, 0) < 0) {
        g_slist_free(lst);
        deadbeef->plt_unref(plt);
...
static void add_dirs_worker(void *data) {
...
static void add_files_worker(void *data) {
    GSList *lst = (GSList *)data;
    gtkpl_add_files(lst);
}
...
static void open_files_worker(void *data) {
    GSList *lst = (GSList *)data;
    gtkpl_add_files(lst);
    deadbeef->pl_save_current();
...
void strcopy_special(char *dest, const char *src, int len) {
    while (len > 0) {
        if (*src == '%' && len >= 3) {
            int charcode = 0;
            int byte;
...
static gboolean set_dnd_cursor_idle(gpointer data) {
    if (!data) {
...
void gtkpl_add_fm_dropped_files(DB_playItem_t *drop_before, char *ptr,
                                int length) {
    ddb_playlist_t *plt = deadbeef->plt_get_curr();
    if (deadbeef->plt_add_files_begin(plt, 0) < 0) {
        free(ptr);
    ...
    while (*p) {
        ...
        if (pe - p < 4096 && pe - p > 7) {
            ...
            // fname[pe - p] = 0;
            int abort = 0;
            DdbListviewIter inserted =
                deadbeef->plt_insert_dir2(0, plt, after, fname, &abort, NULL, NULL);
            if (!inserted && !abort) {
...
static void fmdrop_worker(void *ctx) {
...
static GtkFileFilter *set_file_filter(GtkFileChooser *dlg, const char *name) {
    if (!name) {
...
static gboolean file_filter_func(const GtkFileFilterInfo *filter_info,
                                 gpointer data) {
    // get ext
...
static GtkFileFilter *set_file_filter(GtkFileChooser *dlg, const char *name) {
    ...
    gtk_file_filter_add_custom(flt, GTK_FILE_FILTER_FILENAME, file_filter_func,
                               NULL, NULL);
    gtk_file_chooser_add_filter(GTK_FILE_CHOOSER(dlg), flt);
    gtk_file_chooser_set_filter(GTK_FILE_CHOOSER(dlg), flt);
    flt = gtk_file_filter_new();
...
static GtkFileFilter *set_file_filter_loadplaylist(GtkFileChooser *dlg) {
...
static gboolean playlist_filter_func(const GtkFileFilterInfo *filter_info,
                                     gpointer data) {
    // get ext
    const char *p = strrchr(filter_info->filename, '.');
    if (!p) {
...
static void set_file_filter_loadplaylist(GtkFileChooser *dlg) {
    GtkFileFilter *flt;
    flt = gtk_file_filter_new();
    gtk_file_filter_set_name(flt, _("Supported playlist formats"));
...
static void set_file_filter_saveplaylist(GtkFileChooser *dlg) {
    GtkFileFilter *flt;
    flt = gtk_file_filter_new();
...
static void on_follow_symlinks_toggled(GtkToggleButton *togglebutton,
                                       gpointer user_data) {
    deadbeef->conf_set_int("add_folders_follow_symlinks",
                           gtk_toggle_button_get_active(togglebutton));
}
#endif

static GtkFileChooser *get_file_chooser(const gchar *title,
                                        GtkFileChooserAction action,
                                        gboolean select_multiple) {
#ifdef USE_GTK_NATIVE_FILE_CHOOSER
    GtkFileChooserNative *dlg = gtk_file_chooser_native_new(
                                    title, GTK_WINDOW(mainwin), action, NULL, NULL);
#else
    GtkWidget *dlg = gtk_file_chooser_dialog_new(
                         title, GTK_WINDOW(mainwin), action, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
    ...
    if (action == GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER) {
        ...
        gtk_toggle_button_set_active(
            ...
            deadbeef->conf_get_int("add_folders_follow_symlinks", 0));
        g_signal_connect((gpointer)check, "toggled",
                         G_CALLBACK(on_follow_symlinks_toggled), NULL);
...
static int run_file_chooser(GtkFileChooser *dlg) {
#ifdef USE_GTK_NATIVE_FILE_CHOOSER
...
static void destroy_file_chooser(GtkFileChooser *dlg) {
#ifdef USE_GTK_NATIVE_FILE_CHOOSER
...
GSList *show_file_chooser(const gchar *title, enum GtkuiFileChooserType type,
                          gboolean select_multiple) {
    GtkFileChooserAction action;
    switch (type) {
    ...
    gtk_file_chooser_set_current_folder_uri(
        GTK_FILE_CHOOSER(dlg), deadbeef->conf_get_str_fast(conf_lastdir, ""));
    ...
    // store folder
    gchar *folder =
        gtk_file_chooser_get_current_folder_uri(GTK_FILE_CHOOSER(dlg));
    if (folder) {
    ...
    if (response == GTK_RESPONSE_ACCEPT) {
        ...
        // workaround: Gtk's win32 file chooser uses g_slist_prepend internally and
        // forgets to reverse it before returning
