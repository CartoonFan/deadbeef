...
const char *gtkui_default_titlebar_playing =
    "%artist% - %title% - DeaDBeeF-%_deadbeef_version%";
...
void gtkpl_free(DdbListview *pl) {
#if 0
...
static void format_timestr(char *buf, int sz, float time) {
  time = roundf(time);
  ...
  if (daystotal == 0) {
    ...
    snprintf(buf, sz, _("%d days %d:%02d:%02d"), daystotal, hourtotal, mintotal,
             sectotal);
  }
}

static gboolean update_songinfo(gpointer unused) {
  ...
  snprintf(sbtext_new, sizeof(sbtext_new), "%s | %d tracks | %s %s", buffer,
           deadbeef->pl_getcount(PL_MAIN), totaltime_str, _("total playtime"));
...
void set_tray_tooltip(const char *text) {
  if (trayicon) {
...
gboolean on_trayicon_scroll_event(GtkWidget *widget, GdkEventScroll *event,
                                  gpointer user_data) {
...
static gboolean show_traymenu_cb(gpointer data) {
  if (!traymenu) {
    traymenu = create_traymenu();
  }
  gtk_menu_popup(GTK_MENU(traymenu), NULL, NULL, NULL, NULL, 0,
                 gtk_get_current_event_time());
  return FALSE;
}

static void show_traymenu(void) { g_idle_add(show_traymenu_cb, NULL); }

#if GTK_CHECK_VERSION(3, 0, 0)
static gboolean mainwin_hide_cb(gpointer data) {
...
gboolean on_trayicon_activate(GtkWidget *widget, gpointer user_data) {
...
gboolean on_trayicon_button_press_event(GtkWidget *widget,
...
gboolean on_trayicon_popup_menu(GtkWidget *widget, guint button, guint time,
                                gpointer user_data) {
  gtk_menu_popup(GTK_MENU(traymenu), NULL, NULL, gtk_status_icon_position_menu,
                 trayicon, button, time);
  return FALSE;
}

static gboolean activate_cb(gpointer nothing) {
  if (w_get_rootwidget() == NULL)
    init_widget_layout();
  wingeom_restore(mainwin, "mainwin", 40, 40, 500, 300, 0);
...
static void titlebar_tf_free(void) {
...
void gtkui_titlebar_tf_init(void) {
  titlebar_tf_free();

  char fmt[500];
  deadbeef->conf_get_str("gtkui.titlebar_playing_tf",
                         gtkui_default_titlebar_playing, fmt, sizeof(fmt));
  titlebar_playing_bc = deadbeef->tf_compile(fmt);
  deadbeef->conf_get_str("gtkui.titlebar_stopped_tf",
                         gtkui_default_titlebar_stopped, fmt, sizeof(fmt));
  titlebar_stopped_bc = deadbeef->tf_compile(fmt);

  static const char statusbar_tf_with_seltime_fmt[] =
      "$if2($strcmp(%%ispaused%%,),%s | )$if2($upper(%%codec%%),-) |[ "
      "%%playback_bitrate%% kbps |][ %%samplerate%%Hz |][ %%:BPS%% %s |][ "
      "%%channels%% |] %%playback_time%% / %%length%% | "
      "%%selection_playback_time%% %s";
  ...
  const char statusbar_stopped_tf_with_seltime_fmt[] =
      "%s | %%selection_playback_time%% %s";
  ...
  if (deadbeef->conf_get_int("gtkui.statusbar_seltime", 0)) {
    snprintf(statusbar_tf, sizeof(statusbar_tf), statusbar_tf_with_seltime_fmt,
             _("Paused"), _("bit"), _("selection playtime"));
    snprintf(statusbar_stopped_tf, sizeof(statusbar_stopped_tf),
             statusbar_stopped_tf_with_seltime_fmt, _("Stopped"),
             _("selection playtime"));
  } else {
    snprintf(statusbar_tf, sizeof(statusbar_tf), statusbar_tf_fmt, _("Paused"),
             _("bit"));
    snprintf(statusbar_stopped_tf, sizeof(statusbar_stopped_tf),
             statusbar_stopped_tf_fmt, _("Stopped"));
  }
...
static gboolean set_title_cb(gpointer data) {
  gtkui_set_titlebar(NULL);
  return FALSE;
}

void gtkui_set_titlebar(DB_playItem_t *it) {
  if (!it) {
  ...
  deadbeef->tf_eval(&ctx, it ? titlebar_playing_bc : titlebar_stopped_bc, str,
                    sizeof(str));
  if (ctx.plt) {
...
static gboolean playlistcontentchanged_cb(gpointer none) {
  trkproperties_fill_metadata();
  return FALSE;
}

static gboolean gtkui_on_frameupdate(gpointer data) {
  update_songinfo(NULL);

  return TRUE;
}

static gboolean gtkui_update_status_icon(gpointer unused) {
  ...
  if (!gtk_icon_theme_has_icon(theme, icon_name))
    ...
    GtkIconInfo *icon_info = gtk_icon_theme_lookup_icon(
        theme, icon_name, 48, GTK_ICON_LOOKUP_USE_BUILTIN);
    const gboolean icon_is_builtin =
  ...
  if (!gtk_icon_theme_has_icon(theme, icon_name)) {
    char iconpath[1024];
    snprintf(iconpath, sizeof(iconpath), "%s/deadbeef.png",
             deadbeef->get_system_dir(DDB_SYS_DIR_PREFIX));
...
#if !GTK_CHECK_VERSION(2, 14, 0)
  g_signal_connect((gpointer)trayicon, "activate",
                   G_CALLBACK(on_trayicon_activate), NULL);
#else
  printf("connecting button tray signals\n");
  g_signal_connect((gpointer)trayicon, "scroll_event",
                   G_CALLBACK(on_trayicon_scroll_event), NULL);
  g_signal_connect((gpointer)trayicon, "button_press_event",
                   G_CALLBACK(on_trayicon_button_press_event), NULL);
#endif
  g_signal_connect((gpointer)trayicon, "popup_menu",
                   G_CALLBACK(on_trayicon_popup_menu), NULL);
...
static void override_builtin_statusicon(int override) {
  gtkui_override_statusicon = override;
  g_idle_add(gtkui_update_status_icon, NULL);
}

static void gtkui_hide_status_icon() {
  if (trayicon) {
    g_object_set(trayicon, "visible", FALSE, NULL);
  }
}

int gtkui_get_curr_playlist_mod(void) {
...
void gtkui_setup_gui_refresh(void) {
...
static gboolean gtkui_on_configchanged(void *data) {
  // order and looping
  const char *w;

  // order
  const char *orderwidgets[4] = {"order_linear", "order_shuffle",
                                 "order_random", "order_shuffle_albums"};
  w = orderwidgets[deadbeef->streamer_get_shuffle()];
  gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(lookup_widget(mainwin, w)),
                                 TRUE);
  ...
  gtk_check_menu_item_set_active(
      ...
      deadbeef->conf_get_int("playlist.scroll.followplayback", 1) ? TRUE
                                                                  : FALSE);
  ...
  int stop_after_current =
      deadbeef->conf_get_int("playlist.stop_after_current", 0);
  gtk_check_menu_item_set_active(
  ...
  // play state images
  gtkui_unicode_playstate =
      deadbeef->conf_get_int("gtkui.unicode_playstate", 0);

  // seekbar overlay
  gtkui_disable_seekbar_overlay =
      deadbeef->conf_get_int("gtkui.disable_seekbar_overlay", 0);
...
static gboolean outputchanged_cb(gpointer nothing) {
  preferences_fill_soundcards();
  return FALSE;
}

void save_playlist_as(void) {
...
int gtkui_add_new_playlist(void) {
...
void gtkui_copy_playlist_int(ddb_playlist_t *src, ddb_playlist_t *dst) {
  deadbeef->pl_lock();
...
int gtkui_copy_playlist(ddb_playlist_t *plt) {
  char orig_title[100];
...
int gtkui_get_gui_refresh_rate() {
...
static void send_messages_to_widgets(ddb_gtkui_widget_t *w, uint32_t id,
...
gboolean add_mainmenu_actions_cb(void *data) {
  add_mainmenu_actions();
  return FALSE;
}

int gtkui_plt_add_dir(ddb_playlist_t *plt, const char *dirname,
                      int (*cb)(DB_playItem_t *it, void *data),
                      void *user_data);

int gtkui_plt_add_file(ddb_playlist_t *plt, const char *filename,
                       int (*cb)(DB_playItem_t *it, void *data),
                       void *user_data);

int gtkui_pl_add_files_begin(ddb_playlist_t *plt);

void gtkui_pl_add_files_end(void);

DB_playItem_t *gtkui_plt_load(ddb_playlist_t *plt, DB_playItem_t *after,
                              const char *fname, int *pabort,
                              int (*cb)(DB_playItem_t *it, void *data),
                              void *user_data);

static int is_current_playlist(DB_playItem_t *it) {
  ddb_playlist_t *plt = deadbeef->plt_get_curr();
  if (plt) {
...
// This will be overwritten if there is a current main listview, or when the
// playlist is loaded by a listview
static void playlist_set_intended_scroll(DB_playItem_t *it) {
...
static gboolean pre_songstarted_cb(gpointer data) {
...
static gboolean pre_trackfocus_cb(gpointer data) {
  deadbeef->pl_lock();
...
// These functions handle cases that need a playlist switch, or when there is no
// main listview to take the message
static void playlist_set_cursor(ddb_playlist_t *plt, DB_playItem_t *it) {
  int idx = deadbeef->plt_get_item_idx(plt, it, PL_MAIN);
  if (idx != -1) {
    if (deadbeef->pl_is_selected(it) ||
        idx != deadbeef->plt_get_cursor(plt, PL_MAIN) ||
        deadbeef->plt_getselcount(plt) != 1) {
      deadbeef->plt_deselect_all(plt);
      deadbeef->pl_set_selected(it, 1);
      deadbeef->plt_set_cursor(plt, PL_MAIN, idx);
      ddb_event_track_t *event =
          (ddb_event_track_t *)deadbeef->event_alloc(DB_EV_CURSOR_MOVED);
      event->track = it;
      deadbeef->pl_item_ref(it);
...
static int gtkui_message(uint32_t id, uintptr_t ctx, uint32_t p1, uint32_t p2) {
...
static const char gtkui_def_layout[] =
    "vbox expand=\"0 1\" fill=\"1 1\" homogeneous=0 {hbox expand=\"0 1 0\" "
    "fill=\"1 1 1\" homogeneous=0 {playtb {} seekbar {} volumebar {} } "
    "tabbed_playlist hideheaders=0 {} } ";

static void init_widget_layout(void) {
  w_init();
  ddb_gtkui_widget_t *rootwidget = w_get_rootwidget();
  gtk_widget_show(rootwidget->widget);
  gtk_box_pack_start(GTK_BOX(lookup_widget(mainwin, "plugins_bottom_vbox")),
                     rootwidget->widget, TRUE, TRUE, 0);
  ...
  if (!strcmp(layout, "-")) {
    ...
    if (!strcmp(layout_060, "-")) {
      ...
      snprintf(layout, sizeof(layout),
               "vbox expand=\"0 1\" fill=\"1 1\" homogeneous=0 {hbox "
               "expand=\"0 1 0\" fill=\"1 1 1\" homogeneous=0 {playtb {} "
               "seekbar {} volumebar {} } %s }",
               layout_060);
...
gboolean gtkui_connect_cb(void *none) {
  // equalizer
...
int gtkui_add_file_info_cb(ddb_fileadd_data_t *data, void *user_data) {
  if (data->visibility == 0) {
    ...
    g_idle_add(gtkui_set_progress_text_idle,
               (gpointer)strdup(fname)); // slowwwww
    deadbeef->pl_unlock();
  }
  return 0;
}

void gtkui_add_file_begin_cb(ddb_fileadd_data_t *data, void *user_data) {
  if (data->visibility == 0) {
    progress_show();
  }
}

void gtkui_add_file_end_cb(ddb_fileadd_data_t *data, void *user_data) {
  if (data->visibility == 0) {
    progress_hide();
  }
}
...
static void add_window_init_hook(void (*callback)(void *userdata),
                                 void *userdata) {
  if (window_init_hooks_count >= WINDOW_INIT_HOOK_MAX) {
    fprintf(stderr,
            "gtkui: add_window_init_hook can't add another hook, maximum "
            "number of hooks (%d) exceeded\n",
...
void gtkui_toggle_log_window(void) {
...
void gtkui_show_log_window_internal(gboolean show) {
...
void gtkui_show_log_window(gboolean show) {
  if (show) {
...
gboolean on_gtkui_log_window_delete(GtkWidget *widget, GdkEventAny *event,
                                    GtkWidget **pwindow) {
  gtkui_show_log_window(FALSE);
...
void logwindow_scroll_changed(GtkAdjustment *adjustment, gpointer user_data) {
...
GtkWidget *gtkui_create_log_window(void) {
  GtkWidget *pwindow = create_log_window();
  GtkWidget *scrolledwindow14 = lookup_widget(pwindow, "scrolledwindow14");
  GtkAdjustment *adjustment = gtk_scrolled_window_get_vadjustment(
      GTK_SCROLLED_WINDOW(scrolledwindow14));
  g_signal_connect(adjustment, "value-changed",
                   G_CALLBACK(logwindow_scroll_changed), NULL);
  g_signal_connect(pwindow, "delete_event",
                   G_CALLBACK(on_gtkui_log_window_delete), pwindow);
...
static gboolean logwindow_addtext_cb(gpointer data) {
...
static void logwindow_logger_callback(struct DB_plugin_s *plugin,
...
void gtkui_mainwin_init(void) {
  // register widget types
  w_reg_widget(_("Playlist with tabs"), DDB_WF_SINGLE_INSTANCE,
               w_tabbed_playlist_create, "tabbed_playlist", NULL);
  w_reg_widget(_("Playlist"), DDB_WF_SINGLE_INSTANCE, w_playlist_create,
               "playlist", NULL);
  w_reg_widget(NULL, 0, w_box_create, "box", NULL);
  w_reg_widget(NULL, 0, w_dummy_create, "dummy", NULL);
  w_reg_widget(_("Splitter (top and bottom)"), 0, w_vsplitter_create,
               "vsplitter", NULL);
  w_reg_widget(_("Splitter (left and right)"), 0, w_hsplitter_create,
               "hsplitter", NULL);
  ...
  w_reg_widget(_("Selection properties"), 0, w_selproperties_create,
               "selproperties", NULL);
...
#if GTK_CHECK_VERSION(3, 0, 0)
  init_widget_layout();
  gtk_widget_set_events(GTK_WIDGET(mainwin),
                        gtk_widget_get_events(GTK_WIDGET(mainwin)) |
                            GDK_SCROLL_MASK);
  ...
  fileadded_listener_id =
      deadbeef->listen_file_added(gtkui_add_file_info_cb, NULL);
  fileadd_beginend_listener_id = deadbeef->listen_file_add_beginend(
      gtkui_add_file_begin_cb, gtkui_add_file_end_cb, NULL);
...
gboolean gtkui_set_progress_text_idle(gpointer data) {
  char *text = (char *)data;
  if (text) {
...
void gtkui_playlist_set_curr(int playlist) {
  deadbeef->plt_set_curr_idx(playlist);
  deadbeef->conf_set_int("playlist.current", playlist);
}

void on_gtkui_info_window_delete(GtkWidget *widget,
...
void gtkui_show_info_window(const char *fname, const char *title,
                            GtkWidget **pwindow) {
  if (*pwindow) {
    return;
  }
...
gboolean gtkui_quit_cb(void *ctx) {
  w_save();

  if (deadbeef->have_background_jobs()) {
    GtkWidget *dlg = gtk_message_dialog_new(
        GTK_WINDOW(mainwin), GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING,
        GTK_BUTTONS_YES_NO,
        _("The player is currently running background tasks. If you quit now, "
          "the tasks will be cancelled or interrupted. This may result in data "
          "loss."));
...
void gtkui_quit(void) { gdk_threads_add_idle(gtkui_quit_cb, NULL); }

static void import_legacy_tf(const char *key_from, const char *key_to) {
  deadbeef->conf_lock();
...
static int gtkui_connect(void) { return 0; }

static int gtkui_disconnect(void) {
  supereq_plugin = NULL;
  return 0;
}

static gboolean quit_gtk_cb(gpointer nothing) {
...
GtkWidget *gtkui_get_mainwin(void) { return mainwin; }

static DB_plugin_action_t action_rg_remove_info = {
    .title = "ReplayGain/Remove ReplayGain Information",
    .name = "rg_remove_info",
    .flags =
        DB_ACTION_SINGLE_TRACK | DB_ACTION_MULTIPLE_TRACKS | DB_ACTION_ADD_MENU,
...
static DB_plugin_action_t *gtkui_get_actions(DB_playItem_t *it) {
  return &action_view_log;
}

#if !GTK_CHECK_VERSION(3, 0, 0)
DB_plugin_t *ddb_gui_GTK2_load(DB_functions_t *api) {
  deadbeef = api;
  return DB_PLUGIN(&plugin);
}
#else
DB_plugin_t *ddb_gui_GTK3_load(DB_functions_t *api) {
  deadbeef = api;
  return DB_PLUGIN(&plugin);
}
#endif
...
static const char settings_dlg[] =
    "property \"Ask confirmation to delete files from disk\" checkbox "
    "gtkui.delete_files_ask 1;\n"
    "property \"Status icon volume control sensitivity\" entry "
    "gtkui.tray_volume_sensitivity 1;\n"
    "property \"Custom status icon\" entry gtkui.custom_tray_icon \"" TRAY_ICON
    "\" ;\n"
    "property \"Add separators between plugin context menu items\" checkbox "
    "gtkui.action_separators 0;\n"
    "property \"Use unicode chars instead of images for track state\" checkbox "
    "gtkui.unicode_playstate 0;\n"
    "property \"Disable seekbar overlay text\" checkbox "
    "gtkui.disable_seekbar_overlay 0;\n";
