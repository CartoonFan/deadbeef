...
gboolean on_mainwin_key_press_event(GtkWidget *widget, GdkEventKey *event,
    ...
    if (hkplug) {
        ...
        DB_plugin_action_t *act =
            ((DB_hotkeys_plugin_t *)hkplug)
            ->get_action_for_keycombo(accel_key, accel_mods, 0, &ctx);
...
void on_about1_activate(GtkMenuItem *menuitem, gpointer user_data) {
    ...
    snprintf(fname, sizeof(fname), "%s/%s",
             deadbeef->get_system_dir(DDB_SYS_DIR_DOC), "about.txt");
...
void on_changelog1_activate(GtkMenuItem *menuitem, gpointer user_data) {
    ...
    char fname[PATH_MAX];
    snprintf(fname, sizeof(fname), "%s/%s",
             deadbeef->get_system_dir(DDB_SYS_DIR_DOC), "ChangeLog");
...
void on_gpl1_activate(GtkMenuItem *menuitem, gpointer user_data) {
    char fname[PATH_MAX];
    snprintf(fname, sizeof(fname), "%s/%s",
             deadbeef->get_system_dir(DDB_SYS_DIR_DOC), "COPYING.GPLv2");
    gtkui_show_info_window(fname, "GNU GENERAL PUBLIC LICENSE Version 2",
                           &gplwindow);
}
...
void on_lgpl1_activate(GtkMenuItem *menuitem, gpointer user_data) {
    char fname[PATH_MAX];
    snprintf(fname, sizeof(fname), "%s/%s",
             deadbeef->get_system_dir(DDB_SYS_DIR_DOC), "COPYING.LGPLv2.1");
    gtkui_show_info_window(fname, "GNU LESSER GENERAL PUBLIC LICENSE Version 2.1",
                           &lgplwindow);
}
...
gboolean on_helpwindow_key_press_event(GtkWidget *widget, GdkEventKey *event,
    ...
    if (event->keyval == GDK_Escape) {
        GtkWidget **pwindow =
            (GtkWidget **)g_object_get_data(G_OBJECT(widget), "pointer");
        if (pwindow) {
            *pwindow = NULL;
        }
        gtk_widget_hide(widget);
...
void on_column_id_changed(GtkComboBox *combobox, gpointer user_data) {
    ...
    gtk_widget_set_sensitive(
        fmt,
        act == find_first_preset_column_type(DB_COLUMN_CUSTOM) ? TRUE : FALSE);

    if (!editcolumn_title_changed) {
        GtkWidget *title = lookup_widget(toplevel, "title");
        if (title) {
            gtk_entry_set_text(GTK_ENTRY(title), gtk_combo_box_text_get_active_text(
...
gboolean on_mainwin_button_press_event(GtkWidget *widget, GdkEventButton *event,
    ...
    if (event->x >= c.x && event->x < c.x + c.width && event->y >= c.y &&
            event->y < c.y + c.height) {
...
void on_translators1_activate(GtkMenuItem *menuitem, gpointer user_data) {
    ...
    char fname[PATH_MAX];
    snprintf(fname, sizeof(fname), "%s/%s",
             deadbeef->get_system_dir(DDB_SYS_DIR_DOC), "translators.txt");
...
void on_log_clear_clicked(GtkButton *button, gpointer user_data) {
    GtkWidget *textview = lookup_widget(
                              gtk_widget_get_toplevel(GTK_WIDGET(button)), "logwindow_textview");
