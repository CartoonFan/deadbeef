...
#define gtk_menu_popup_at_pointer(menu, trigger_event)                         \
...
static struct pl_preset_column_format
    pl_preset_column_formats[PRESET_COLUMN_NUMITEMS];

static ddbUtilTrackList_t _menuTrackList;

static void _capture_selected_track_list(void) {
  ...
  _menuTrackList = ddbUtilTrackListInitWithWithTracks(
      ddbUtilTrackListAlloc(), plt, DDB_ACTION_CTX_SELECTION, tracks, count);
...
static void init_preset_column_struct(void) {
  // There can only be one instance of columns of type DB_COLUMN_FILENUMBER,
  // DB_COLUMN_PLAYING, DB_COLUMN_ALBUM_ART and DB_COLUMN_CUSTOM
...
int find_first_preset_column_type(int type) {
  int idx = -1;
...
void pl_common_init(void) {
  ...
  gtk_box_pack_start(GTK_BOX(gtk_bin_get_child(GTK_BIN(mainwin))),
                     theme_treeview, FALSE, FALSE, 0);
...
static col_info_t *create_col_info(DdbListview *listview, int id) {
...
void pl_common_free_col_info(void *data) {
  if (!data) {
    return;
  }
...
int pl_common_rewrite_column_config(DdbListview *listview, const char *name) {
  ...
  for (int i = 0; i < cnt; i++) {
    ...
    int color_override;
    GdkColor color;
    ddb_listview_column_get_info(listview, i, &title, &width, &align, NULL,
                                 NULL, &color_override, &color, (void **)&info);

    char *esctitle = parser_escape_string(title);
    char *escformat = info->format ? parser_escape_string(info->format) : NULL;
    char *escsortformat =
        info->sort_format ? parser_escape_string(info->sort_format) : NULL;

    size_t written =
        snprintf(p, n,
                 "{\"title\":\"%s\",\"id\":\"%d\",\"format\":\"%s\",\"sort_"
                 "format\":\"%s\",\"size\":\"%d\",\"align\":\"%d\",\"color_"
                 "override\":\"%d\",\"color\":\"#ff%02x%02x%02x\"}%s",
                 esctitle, info->id, escformat ? escformat : "",
                 escsortformat ? escsortformat : "", width, align,
                 color_override, color.red >> 8, color.green >> 8,
                 color.blue >> 8, i < cnt - 1 ? "," : "");
    free(esctitle);
    if (escformat) {
      free(escformat);
    }
    ...
    n -= written;
    if (n <= 0) {
      fprintf(stderr, "Column configuration is too large, doesn't fit in the "
                      "buffer. Won't be written.\n");
      return -1;
    }
  }
  strcpy(p, "]");
...
static gboolean tf_redraw_cb(gpointer user_data) {
...
static int min_group_height(void *user_data, int width) {
...
///// cover art display
int pl_common_is_album_art_column(void *user_data) {
  col_info_t *info = (col_info_t *)user_data;
  return info->id == DB_COLUMN_ALBUM_ART;
}

static GdkPixbuf *get_cover_art(DB_playItem_t *it, int width, int height,
  ...
  GdkPixbuf *pixbuf = get_cover_art_thumb_by_size(uri, artist, album, width,
                                                  height, callback, user_data);
  deadbeef->pl_unlock();
  return pixbuf;
}

static gboolean cover_invalidate_cb(void *user_data) {
...
static void cover_invalidate(void *user_data) {
  g_idle_add(cover_invalidate_cb, user_data);
}

static gboolean cover_load(void *user_data) {
  col_info_t *info = user_data;
  info->cover_load_timeout_id = 0;
  ...
  while (group && group_y < end_pos) {
    GdkPixbuf *pixbuf = get_cover_art(group->head, info->new_cover_size,
                                      info->new_cover_size, NULL, NULL);
    if (pixbuf) {
...
static void cover_draw_cairo(GdkPixbuf *pixbuf, int x, int min_y, int max_y,
                             int width, int height, cairo_t *cr, int filter) {
  ...
  if (pw > width || ph > height || pw < width && ph < height) {
    ...
    cairo_translate(cr, (width - width * scale) / 2.,
                    min(min_y, max_y - ph * scale) - real_y);
...
static void cover_draw_anything(DB_playItem_t *it, int x, int min_y, int max_y,
  ...
  if (pixbuf) {
    cover_draw_cairo(pixbuf, x, min_y, max_y, width, height, cr,
                     CAIRO_FILTER_FAST);
    g_object_unref(pixbuf);
  }
}

static void cover_draw_exact(DB_playItem_t *it, int x, int min_y, int max_y,
                             int width, int height, cairo_t *cr,
                             void *user_data) {
  GdkPixbuf *pixbuf =
      get_cover_art(it, width, height, cover_invalidate, user_data);
  if (!pixbuf) {
...
void pl_common_draw_album_art(DdbListview *listview, cairo_t *cr,
                              DB_playItem_t *it, void *user_data, int min_y,
                              int next_y, int x, int y, int width, int height) {
  ...
  if (info->cover_size == art_width) {
    cover_draw_exact(it, art_x, min_y, next_y, art_width, art_height, cr,
                     user_data);
  } else {
...
#define CHANNEL_BLENDR(CHANNELA, CHANNELB, BLEND)                              \
  (sqrt(((1.0 - BLEND) * (CHANNELA * CHANNELA)) +                              \
        (BLEND * (CHANNELB * CHANNELB))))

PangoAttrList *convert_escapetext_to_pango_attrlist(char *text, float *fg,
                                                    float *bg,
                                                    float *highlight) {
...
void pl_common_draw_column_data(DdbListview *listview, cairo_t *cr,
                                DdbListviewIter it, int idx, int iter,
                                int align, void *user_data, GdkColor *fg_clr,
                                int x, int y, int width, int height, int even) {
  col_info_t *info = user_data;

  DB_playItem_t *playing_track = deadbeef->streamer_get_playing_track();

  if (!gtkui_unicode_playstate && it && it == playing_track &&
      info->id == DB_COLUMN_PLAYING) {
    ...
    if (pixbuf) {
      gdk_cairo_set_source_pixbuf(cr, pixbuf, x + width / 2 - 8,
                                  y + height / 2 - 8);
    ...
    if (it == playing_track && info->id == DB_COLUMN_PLAYING) {
      ...
      if (ctx.update > 0) {
        ...
        if ((ctx.flags & DDB_TF_CONTEXT_HAS_INDEX) && ctx.iter == PL_MAIN) {
          ...
          listview->tf_redraw_track_idx =
              deadbeef->plt_get_item_idx(ctx.plt, it, ctx.iter);
        }
        listview->tf_redraw_timeout_id =
            g_timeout_add(ctx.update, tf_redraw_cb, listview);
    ...
    float fg[3] = {(float)color->red / 0xffff, (float)color->green / 0xffff,
                   (float)color->blue / 0xffff};
    ...
    if (!gtkui_override_listview_colors()) {
      highlight_color =
          &gtk_widget_get_style(theme_treeview)->fg[GTK_STATE_NORMAL];
    } else {
      highlight_color = (gtkui_get_listview_group_text_color(&hlclr), &hlclr);
    }

    float highlight[] = {highlight_color->red / 65535.,
                         highlight_color->green / 65535.,
                         highlight_color->blue / 65535.};
    ...
    if (!gtkui_override_listview_colors()) {
      if (deadbeef->pl_is_selected(it)) {
        background_color =
            &gtk_widget_get_style(theme_treeview)->bg[GTK_STATE_SELECTED];
      } else {
        background_color =
            &gtk_widget_get_style(theme_treeview)->bg[GTK_STATE_NORMAL];
      }
    } else {
      if (deadbeef->pl_is_selected(it)) {
    ...
    float bg[] = {background_color->red / 65535.,
                  background_color->green / 65535.,
                  background_color->blue / 65535.};

    if (is_dimmed) {
      PangoAttrList *attrs =
          convert_escapetext_to_pango_attrlist(text, fg, bg, highlight);
      pango_layout_set_attributes(listview->listctx.pangolayout, attrs);
      pango_attr_list_unref(attrs);
      draw_text_custom(&listview->listctx, x + 5, y + 3, width - 10, align,
                       DDB_LIST_FONT, bold, italic, text);
...
static GtkWidget *find_popup(GtkWidget *widget) {
...
static DdbListview *get_context_menu_listview(GtkMenuItem *menuitem) {
  return DDB_LISTVIEW(
      g_object_get_data(G_OBJECT(find_popup(GTK_WIDGET(menuitem))), "ps"));
}

static int get_context_menu_column(GtkMenuItem *menuitem) {
  return GPOINTER_TO_INT(g_object_get_data(
      G_OBJECT(gtk_widget_get_parent(GTK_WIDGET(menuitem))), "column"));
}

static void add_to_playback_queue_activate(GtkMenuItem *menuitem,
                                           gpointer user_data) {
...
static void remove_from_playback_queue_activate(GtkMenuItem *menuitem,
                                                gpointer user_data) {
  DdbListview *listview = get_context_menu_listview(menuitem);
  DB_playItem_t *it = listview->binding->head();
  while (it) {
...
static void reload_metadata_activate(GtkMenuItem *menuitem,
                                     gpointer user_data) {
  DdbListview *listview = get_context_menu_listview(menuitem);
  DB_playItem_t *it = listview->binding->head();
  while (it) {
    ...
    int match = deadbeef->pl_is_selected(it) &&
                deadbeef->is_local_file(deadbeef->pl_find_meta(it, ":URI")) &&
                dec;
    deadbeef->pl_unlock();
...
static void properties_activate(GtkMenuItem *menuitem, gpointer user_data) {
...
static void on_remove2_activate(GtkMenuItem *menuitem, gpointer user_data) {
...
static void on_toggle_set_custom_title(GtkToggleButton *togglebutton,
                                       gpointer user_data) {
...
static void on_set_custom_title_activate(GtkMenuItem *menuitem,
                                         gpointer user_data) {
  DdbListview *lv = user_data;
  ...
  const char *custom_title =
      deadbeef->pl_find_meta((DB_playItem_t *)it, ":CUSTOM_TITLE");
  if (custom_title) {
  ...
  g_signal_connect((gpointer)sct, "toggled",
                   G_CALLBACK(on_toggle_set_custom_title), dlg);
  ...
  if (response == GTK_RESPONSE_OK) {
    if (it && deadbeef->conf_get_int("gtkui.location_set_custom_title", 0)) {
      deadbeef->pl_replace_meta((DB_playItem_t *)it, ":CUSTOM_TITLE",
                                gtk_entry_get_text(GTK_ENTRY(ct)));
    } else {
...
static void on_remove_from_disk_activate(GtkMenuItem *menuitem,
                                         gpointer user_data) {
  delete_from_disk_with_track_list(_menuTrackList);
}

static void actionitem_activate(GtkMenuItem *menuitem,
...
#define HOOKUP_OBJECT(component, widget, name)                                 \
  g_object_set_data_full(G_OBJECT(component), name, g_object_ref(widget),      \
...
void list_empty_region_context_menu(DdbListview *listview) {
  ...
  gtk_widget_add_accelerator(paste, "activate", accel_group, GDK_v,
  ...
  g_signal_connect((gpointer)paste, "activate", G_CALLBACK(on_paste_activate),
                   NULL);

  gtk_menu_popup_at_pointer(GTK_MENU(playlist_menu), NULL);
}

void list_context_menu(DdbListview *listview, DdbListviewIter it, int idx,
                       int iter) {
  ...
  add_to_playback_queue1 =
      gtk_menu_item_new_with_mnemonic(_("Add To Playback Queue"));
  gtk_widget_show(add_to_playback_queue1);
  gtk_container_add(GTK_CONTAINER(playlist_menu), add_to_playback_queue1);

  remove_from_playback_queue1 =
      gtk_menu_item_new_with_mnemonic(_("Remove From Playback Queue"));
  if (listview->binding->sel_count() > 1) {
    ...
    for (int i = 0; i < pqlen && no_playqueue_items; i++) {
      ...
      if (deadbeef->pl_get_playlist(pqitem) == plt &&
          listview->binding->is_selected(pqitem)) {
  ...
  for (i = 0; plugins[i]; i++) {
    ...
    for (action = actions; action; action = action->next) {
      if ((action->flags & DB_ACTION_COMMON) ||
          !((action->callback2 && (action->flags & DB_ACTION_ADD_MENU)) ||
            action->callback) ||
          !(action->flags &
            (DB_ACTION_MULTIPLE_TRACKS | DB_ACTION_SINGLE_TRACK)))
        continue;
      ...
      for (;;) {
        ...
        if (slash && *(slash - 1) != '\\') {
          ...
          *t = 0;
          ...
          popup = GTK_WIDGET(
              g_object_get_data(G_OBJECT(find_popup(prev_menu)), name));
          if (!popup) {
      ...
      *t = 0;
      ...
      gtk_container_add(popup ? GTK_CONTAINER(popup)
                              : GTK_CONTAINER(playlist_menu),
                        actionitem);

      g_signal_connect((gpointer)actionitem, "activate",
                       G_CALLBACK(actionitem_activate), action);
      if ((selected_count > 1 &&
           !(action->flags & DB_ACTION_MULTIPLE_TRACKS)) ||
  ...
  if (added_entries > 0 &&
      !deadbeef->conf_get_int("gtkui.action_separators", 0)) {
    separator8 = gtk_separator_menu_item_new();
    gtk_widget_show(separator8);
    gtk_container_add(GTK_CONTAINER(playlist_menu), separator8);
    gtk_widget_set_sensitive(separator8, FALSE);
  ...
  g_signal_connect((gpointer)add_to_playback_queue1, "activate",
                   G_CALLBACK(add_to_playback_queue_activate), NULL);
  g_signal_connect((gpointer)remove_from_playback_queue1, "activate",
                   G_CALLBACK(remove_from_playback_queue_activate), NULL);
  g_signal_connect((gpointer)reload_metadata, "activate",
                   G_CALLBACK(reload_metadata_activate), NULL);
  g_signal_connect((gpointer)cut, "activate", G_CALLBACK(on_cut_activate),
                   NULL);
  g_signal_connect((gpointer)copy, "activate", G_CALLBACK(on_copy_activate),
                   NULL);
  g_signal_connect((gpointer)paste, "activate", G_CALLBACK(on_paste_activate),
                   NULL);
  g_signal_connect((gpointer)remove2, "activate",
                   G_CALLBACK(on_remove2_activate), NULL);
  if (!hide_remove_from_disk) {
    g_signal_connect((gpointer)remove_from_disk, "activate",
                     G_CALLBACK(on_remove_from_disk_activate), NULL);
  }
#ifndef DISABLE_CUSTOM_TITLE
  g_signal_connect((gpointer)set_custom_title, "activate",
                   G_CALLBACK(on_set_custom_title_activate), listview);
#endif
  g_signal_connect((gpointer)properties1, "activate",
                   G_CALLBACK(properties_activate), NULL);

  gtk_menu_popup_at_pointer(GTK_MENU(playlist_menu), NULL);
}

static char *strtok_stringdelim_r(char *str, const char *delim,
                                  char **next_start) {
  char *end;
...
static void groups_changed(DdbListview *listview, const char *format) {
  if (!format) {
    return;
  }
  ...
  char *saveptr = NULL;
  char *token;
  while ((token = strtok_stringdelim_r(mutable_format, SUBGROUP_DELIMITER,
                                       &saveptr)) != NULL) {
    if (strlen(token) > 0) {
      DdbListviewGroupFormat *new_fmt =
          calloc(sizeof(DdbListviewGroupFormat), 1);
      if (!fmt) {
...
gboolean list_handle_keypress(DdbListview *ps, int keyval, int state,
                              int iter) {
  ...
  if (state & GDK_SHIFT_MASK) {
    if (cursor != prev) {
      ...
      // select all between shift_sel_anchor and deadbeef->pl_get_cursor
      // (ps->iterator)
...
static void on_group_by_none_activate(GtkMenuItem *menuitem,
                                      gpointer user_data) {
  ddb_playlist_t *plt = deadbeef->plt_get_curr();
  if (plt) {
...
static void on_pin_groups_active(GtkMenuItem *menuitem, gpointer user_data) {
  int old_val = deadbeef->conf_get_int("playlist.pin.groups", 0);
  deadbeef->conf_set_int("playlist.pin.groups", old_val ? 0 : 1);
  deadbeef->sendmessage(DB_EV_CONFIGCHANGED, (uintptr_t) "playlist.pin.groups",
                        0, 0);
...
static void on_group_by_artist_date_album_activate(GtkMenuItem *menuitem,
                                                   gpointer user_data) {
  ddb_playlist_t *plt = deadbeef->plt_get_curr();
  if (plt) {
    deadbeef->plt_modified(plt);
...
static void on_group_by_artist_activate(GtkMenuItem *menuitem,
                                        gpointer user_data) {
  ddb_playlist_t *plt = deadbeef->plt_get_curr();
  if (plt) {
    deadbeef->plt_modified(plt);
...
static void _make_format(char *format, size_t size,
                         DdbListviewGroupFormat *fmt) {
  format[0] = 0;
  while (fmt) {
    if (format[0] != 0) {
...
static void on_group_by_custom_activate(GtkMenuItem *menuitem,
                                        gpointer user_data) {
...
int pl_common_load_column_config(DdbListview *listview, const char *key) {
  deadbeef->conf_lock();
  ...
  for (int i = 0; i < json_array_size(root); i++) {
    json_t *title, *align, *id, *format, *sort_format, *width, *color_override,
        *color;
    ...
    ddb_listview_column_append(
        listview, stitle, iwidth, ialign,
        inf->id == DB_COLUMN_ALBUM_ART ? min_group_height : NULL,
        inf->id == DB_COLUMN_ALBUM_ART, icolor_override, gdkcolor, inf);
  }
  json_decref(root);
  return 0;
error:
...
static void init_column(col_info_t *inf, int id, const char *format,
                        const char *sort_format) {
  if (inf->format) {
    free(inf->format);
    inf->format = NULL;
  }
  ...
  } else if (pl_preset_column_formats[id].id == DB_COLUMN_STANDARD &&
             pl_preset_column_formats[id].format) {
...
static void populate_column_id_combo_box(GtkComboBoxText *combo_box) {
  for (int i = 0; i < PRESET_COLUMN_NUMITEMS; i++) {
    gtk_combo_box_text_append_text(combo_box,
                                   pl_preset_column_formats[i].title);
  }
}

int editcolumn_title_changed = 0;

static void on_add_column_activate(GtkMenuItem *menuitem, gpointer user_data) {
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(lookup_widget(dlg, "color_override")), 0);

  gtk_color_button_set_color(GTK_COLOR_BUTTON(lookup_widget(dlg, "color")),
                             &color);
  gint response = gtk_dialog_run(GTK_DIALOG(dlg));
  if (response == GTK_RESPONSE_OK) {
    const gchar *title =
        gtk_entry_get_text(GTK_ENTRY(lookup_widget(dlg, "title")));
    const gchar *format =
        gtk_entry_get_text(GTK_ENTRY(lookup_widget(dlg, "format")));
    const gchar *sort_format =
        gtk_entry_get_text(GTK_ENTRY(lookup_widget(dlg, "sort_format")));
    int sel = gtk_combo_box_get_active(GTK_COMBO_BOX(lookup_widget(dlg, "id")));

    int clr_override = gtk_toggle_button_get_active(
        GTK_TOGGLE_BUTTON(lookup_widget(dlg, "color_override")));
    GdkColor clr;
    gtk_color_button_get_color(GTK_COLOR_BUTTON(lookup_widget(dlg, "color")),
                               &clr);
    ...
    int align =
        gtk_combo_box_get_active(GTK_COMBO_BOX(lookup_widget(dlg, "align")));
    DdbListview *listview = get_context_menu_listview(menuitem);
    int before = get_context_menu_column(menuitem);
    ddb_listview_column_insert(
        listview, before, title, 100, align,
        inf->id == DB_COLUMN_ALBUM_ART ? min_group_height : NULL,
        inf->id == DB_COLUMN_ALBUM_ART, clr_override, clr, inf);
    ddb_listview_refresh(listview, DDB_LIST_CHANGED | DDB_REFRESH_COLUMNS |
                                       DDB_REFRESH_LIST | DDB_REFRESH_HSCROLL);
  }
  gtk_widget_destroy(dlg);
}

static void on_edit_column_activate(GtkMenuItem *menuitem, gpointer user_data) {
  ...
  int color_override;
  GdkColor color;
  int res = ddb_listview_column_get_info(
      listview, active_column, &title, &width, &align_right, NULL, NULL,
      &color_override, &color, (void **)&inf);
  if (res == -1) {
  ...
  if (inf->id == DB_COLUMN_STANDARD) {
    for (int i = 0; i < PRESET_COLUMN_NUMITEMS; i++) {
      if (pl_preset_column_formats[i].id == DB_COLUMN_STANDARD && inf->format &&
          pl_preset_column_formats[i].format &&
          !strcmp(inf->format, pl_preset_column_formats[i].format)) {
        idx = i;
        break;
      }
    }
  ...
  if (inf->sort_format) {
    gtk_entry_set_text(GTK_ENTRY(lookup_widget(dlg, "sort_format")),
                       inf->sort_format);
  }
  gtk_combo_box_set_active(GTK_COMBO_BOX(lookup_widget(dlg, "align")),
                           align_right);
  gtk_entry_set_text(GTK_ENTRY(lookup_widget(dlg, "title")), title);
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(lookup_widget(dlg, "color_override")), color_override);
  ...
  if (response == GTK_RESPONSE_OK) {
    ...
    ddb_listview_column_set_info(
        listview, active_column, title, width, align,
        inf->id == DB_COLUMN_ALBUM_ART ? min_group_height : NULL,
        inf->id == DB_COLUMN_ALBUM_ART, clr_override, clr, inf);
...
static void on_remove_column_activate(GtkMenuItem *menuitem,
                                      gpointer user_data) {
  int active_column = get_context_menu_column(menuitem);
  if (active_column == -1)
    return;
...
static GtkWidget *create_headermenu(DdbListview *listview, int column,
                                    int groupby) {
  ...
  if (groupby) {
    ...
    pin_groups =
        gtk_check_menu_item_new_with_mnemonic(_("Pin groups when scrolling"));
    gtk_widget_show(pin_groups);
    gtk_container_add(GTK_CONTAINER(headermenu), pin_groups);
    gtk_check_menu_item_set_active(
        GTK_CHECK_MENU_ITEM(pin_groups),
        (gboolean)deadbeef->conf_get_int("playlist.pin.groups", 0));
    ...
    g_signal_connect((gpointer)none, "activate",
                     G_CALLBACK(on_group_by_none_activate), NULL);

    g_signal_connect((gpointer)pin_groups, "activate",
                     G_CALLBACK(on_pin_groups_active), NULL);

    g_signal_connect((gpointer)artist_date_album, "activate",
                     G_CALLBACK(on_group_by_artist_date_album_activate), NULL);

    g_signal_connect((gpointer)artist, "activate",
                     G_CALLBACK(on_group_by_artist_activate), NULL);

    g_signal_connect((gpointer)custom, "activate",
                     G_CALLBACK(on_group_by_custom_activate), NULL);
  }

  g_signal_connect((gpointer)add_column, "activate",
                   G_CALLBACK(on_add_column_activate), listview);
  g_signal_connect((gpointer)edit_column, "activate",
                   G_CALLBACK(on_edit_column_activate), listview);
  g_signal_connect((gpointer)remove_column, "activate",
                   G_CALLBACK(on_remove_column_activate), listview);

  return headermenu;
}

void pl_common_header_context_menu(DdbListview *ps, int column) {
...
void pl_common_add_column_helper(DdbListview *listview, const char *title,
                                 int width, int id, const char *format,
                                 const char *sort_format, int align_right) {
  if (!format) {
    format = "";
  }
  if (!sort_format) {
  ...
  ddb_listview_column_append(listview, title, width, align_right,
                             inf->id == DB_COLUMN_ALBUM_ART ? min_group_height
                                                            : NULL,
                             inf->id == DB_COLUMN_ALBUM_ART, 0, color, inf);
}

int pl_common_get_group(DdbListview *listview, DdbListviewIter it, char *str,
                        int size, int index) {
...
void pl_common_draw_group_title(DdbListview *listview, cairo_t *drawable,
                                DdbListviewIter it, int iter, int x, int y,
                                int width, int height, int group_depth) {
  if (listview->group_formats->format && listview->group_formats->format[0]) {
    char str[1024] = "";
    DdbListviewGroupFormat *fmt = listview->group_formats;
    while (group_depth--) {
    ...
    if (theming) {
      GdkColor *clr =
          &gtk_widget_get_style(theme_treeview)->fg[GTK_STATE_NORMAL];
      float rgb[] = {clr->red / 65535., clr->green / 65535.,
                     clr->blue / 65535.};
    ...
    if (text_width > 0) {
      draw_text_custom(&listview->grpctx, x + 5,
                       y + height / 2 -
                           draw_get_listview_rowheight(&listview->grpctx) / 2 +
                           3,
                       text_width, 0, DDB_GROUP_FONT, 0, 0, str);
      draw_get_layout_extents(&listview->grpctx, &ew, NULL);

      int len = strlen(str);
      int line_x = x + 10 + ew + (len ? ew / len / 2 : 0);
      if (line_x + 20 < x + width) {
        draw_line(&listview->grpctx, line_x, y + height / 2, x + width,
                  y + height / 2);
      }
    }
  }
...
void pl_common_selection_changed(DdbListview *ps, int iter, DB_playItem_t *it) {
  if (it) {
    ddb_event_track_t *ev =
        (ddb_event_track_t *)deadbeef->event_alloc(DB_EV_TRACKINFOCHANGED);
    ev->track = it;
    deadbeef->pl_item_ref(ev->track);
    deadbeef->event_send((ddb_event_t *)ev, DDB_PLAYLIST_CHANGE_SELECTION,
                         iter);
  } else {
    deadbeef->sendmessage(DB_EV_PLAYLISTCHANGED, (uintptr_t)ps,
                          DDB_PLAYLIST_CHANGE_SELECTION, iter);
  }
}

void pl_common_col_sort(int sort_order, int iter, void *user_data) {
  col_info_t *c = (col_info_t *)user_data;
  ddb_playlist_t *plt = deadbeef->plt_get_curr();
  char *format =
      (c->sort_format && strlen(c->sort_format)) ? c->sort_format : c->format;
  deadbeef->plt_sort_v2(plt, iter, c->id, format,
                        sort_order == 2 ? DDB_SORT_DESCENDING
                                        : DDB_SORT_ASCENDING);
  deadbeef->plt_unref(plt);
}

void pl_common_set_group_format(DdbListview *listview, const char *format_conf,
                                const char *artwork_level_conf,
                                const char *subgroup_padding_conf) {
  deadbeef->conf_lock();
  char *format = strdup(deadbeef->conf_get_str_fast(format_conf, ""));
  listview->artwork_subgroup_level =
      deadbeef->conf_get_int(artwork_level_conf, 0);
  listview->subgroup_title_padding =
      deadbeef->conf_get_int(subgroup_padding_conf, 10);
  deadbeef->conf_unlock();
  ...
  char *saveptr = NULL;
  char *token;
  while ((token = strtok_stringdelim_r(format, SUBGROUP_DELIMITER, &saveptr)) !=
         NULL) {
    if (strlen(token) > 0) {
      DdbListviewGroupFormat *new_fmt =
          calloc(sizeof(DdbListviewGroupFormat), 1);
...
static int import_column_from_0_6(const uint8_t *def, char *json_out,
                                  int outsize) {
...
#define MAX_COLUMN_TF 2048
  ...
  int ret = snprintf(json_out, outsize,
                     "{\"title\":\"%s\",\"id\":\"%d\",\"format\":\"%s\","
                     "\"size\":\"%d\",\"align\":\"%d\"}",
                     title, out_id, format, width, align);
  return min(ret, outsize);
}

int import_column_config_0_6(const char *oldkeyprefix, const char *newkey) {
