...
static size_t http_content_header_handler(void *ptr, size_t size, size_t nmemb,
                                          void *stream);
...
static int http_need_abort(uint64_t identifier);

static void http_cancel_abort(uint64_t identifier);

static void vfs_curl_abort_with_identifier(uint64_t identifier);

static size_t http_curl_write_wrapper(HTTP_FILE *fp, void *ptr, size_t size) {
  size_t avail = size;
  while (avail > 0) {
...
void vfs_curl_set_meta(DB_playItem_t *it, const char *meta, const char *value) {
...
int http_parse_shoutcast_meta(HTTP_FILE *fp, const char *meta, size_t size) {
        ...
        if (songstarted) {
          ...
          if (emulate_trackchange) {
            ddb_event_trackchange_t *ev =
                (ddb_event_trackchange_t *)deadbeef->event_alloc(
                    DB_EV_SONGCHANGED);
          ...
          ddb_event_track_t *ev =
              (ddb_event_track_t *)deadbeef->event_alloc(DB_EV_SONGSTARTED);
          ev->track = fp->track;
...
static void http_stream_reset(HTTP_FILE *fp) {
  // FIXME: calling this without reopening the entire stream causes metadata
  // counter unsync.
  fp->gotheader = 0;
  fp->icyheader = 0;
  fp->gotsomeheader = 0;
...
static const uint8_t *parse_header(const uint8_t *p, const uint8_t *e,
                                   uint8_t *key, int keysize, uint8_t *value,
                                   int valuesize) {
...
static size_t http_content_header_handler_int(void *ptr, size_t size,
                                              void *stream,
                                              int *end_of_headers) {
...
size_t vfs_curl_handle_icy_headers(size_t avail, HTTP_FILE *fp, char *ptr) {
  size_t size = avail;
  ...
  if (fp->icyheader) {
    if (fp->nheaderpackets > 10) {
      fprintf(stderr, "vfs_curl: warning: seems like stream has unterminated "
                      "ICY headers\n");
      fp->icy_metaint = 0;
      fp->wait_meta = 0;
      fp->gotheader = 1;
...
static size_t _handle_icy_metadata(size_t avail, HTTP_FILE *fp, char *ptr,
                                   int *error) {
  size_t size = avail;
  while (fp->icy_metaint > 0) {
    ...
    if (fp->metadata_size > 0) {
      if (fp->metadata_size > fp->metadata_have_size) {
        trace("metadata fetch mode, avail: %d, metadata_size: %d, "
              "metadata_have_size: %d)\n",
    ...
    if (avail < 0) {
      trace("vfs_curl: something bad happened in metadata parser. can't "
            "continue streaming.\n");
      *error = 1;
      return 0;
    }
...
static size_t http_curl_write(void *_ptr, size_t size, size_t nmemb,
                              void *stream) {
  char *ptr = _ptr;
...
static int http_curl_control(void *stream, double dltotal, double dlnow,
  ...
  // trace ("http_curl_control: status = %d, response = %d, interval: %f
...
void vfs_curl_free_file(HTTP_FILE *fp) {
  if (fp->content_type) {
    free(fp->content_type);
  }
  if (fp->track) {
...
static void http_thread_func(void *ctx) {
  ...
  for (;;) {
    ...
    char ua[100];
    deadbeef->conf_get_str("network.http_user_agent", "deadbeef", ua,
                           sizeof(ua));
...
static void http_set_track(DB_FILE *f, DB_playItem_t *it) {
...
static void http_close(DB_FILE *stream) {
  trace("http_close %p\n", stream);
  assert(stream);
  HTTP_FILE *fp = (HTTP_FILE *)stream;
...
static size_t http_read(void *ptr, size_t size, size_t nmemb, DB_FILE *stream) {
  assert(stream);
  assert(ptr);
  HTTP_FILE *fp = (HTTP_FILE *)stream;
  ...
  fp->seektoend = 0;
  if (fp->status == STATUS_ABORTED ||
      (fp->status == STATUS_FINISHED && fp->remaining == 0)) {
    errno = ECONNABORTED;
    return 0;
  }
  if (!fp->tid) {
  ...
  while ((fp->remaining > 0 ||
    ...
    while ((fp->remaining == 0 || fp->skipbytes > 0) &&
           fp->status != STATUS_FINISHED && fp->status != STATUS_ABORTED) {
    ...
    part1 = min(part1, cp);
    //        trace ("readpos=%d, remaining=%d, req=%d, cp=%d, part1=%d,
...
static int http_seek(DB_FILE *stream, int64_t offset, int whence) {
  ...
  if (whence == SEEK_SET) {
    if (fp->pos == offset) {
      ...
      return 0;
               ...
               fp->pos - offset <= BUFFER_SIZE - fp->remaining) {
      fp->skipbytes = 0;
...
static int64_t http_tell(DB_FILE *stream) {
  assert(stream);
  HTTP_FILE *fp = (HTTP_FILE *)stream;
  if (fp->seektoend) {
    return fp->length;
...
static void http_rewind(DB_FILE *stream) {
  trace("http_rewind\n");
  assert(stream);
  HTTP_FILE *fp = (HTTP_FILE *)stream;
  if (fp->tid) {
...
static int64_t http_getlength(DB_FILE *stream) {
...
static const char *http_get_content_type(DB_FILE *stream) {
...
static int vfs_curl_start(void) {
  allow_new_streams = 1;
  biglock = deadbeef->mutex_create();
  return 0;
}

static int vfs_curl_stop(void) {
  allow_new_streams = 0;
  if (biglock) {
...
const char **http_get_schemes(void) { return scheme_names; }

int http_is_streaming(void) { return 1; }

static uint64_t vfs_curl_get_identifier(DB_FILE *f) {
  deadbeef->mutex_lock(biglock);
...
static DB_vfs_t plugin = {
    ...
    .plugin.descr = "http and ftp streaming module using libcurl, with ICY "
                    "protocol support",
    .plugin.copyright =
        "CURL VFS plugin for DeaDBeeF Player\n"
        "Copyright (C) 2009-2014 Alexey Yakovenko\n"
...
DB_plugin_t *vfs_curl_load(DB_functions_t *api) {
  deadbeef = api;
