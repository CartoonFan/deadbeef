...
static const int aac_sample_rates[16] = {96000, 88200, 64000, 48000, 44100,
...
int aac_sync(const uint8_t *buf, int *channels, int *sample_rate, int *bit_rate,
             int *samples) {
    int size, rdb;
    ...
    const char *profiles[4] = {
        "0 Main profile AAC MAIN", "1 Low Complexity profile (LC)AAC LC",
        "2 Scalable Sample Rate profile (SSR)AAC SSR", "3 (reserved)AAC LTP"
    };
    ...
    trace("samplerate %d (#%d)\n", aac_sample_rates[sample_freq_index],
          sample_freq_index);
