...
static void aacDecoderClose_FAAD2(aacDecoderHandle_t *_dec) {
    faad2Decoder_t *dec = (faad2Decoder_t *)_dec;
    NeAACDecClose(dec->dec);
    free(_dec);
}

static int aacDecoderInit_FAAD2(aacDecoderHandle_t *_dec, uint8_t *buff,
                                size_t buffSize, unsigned *samplerate,
                                unsigned *channels) {
    faad2Decoder_t *dec = (faad2Decoder_t *)_dec;
    unsigned long sr;
    unsigned char ch;
...
static int aacDecoderInitRaw_FAAD2(aacDecoderHandle_t *_dec, uint8_t *buff,
                                   size_t buffSize, unsigned *samplerate,
                                   unsigned *channels) {
    faad2Decoder_t *dec = (faad2Decoder_t *)_dec;
    unsigned long sr;
...
static const uint8_t *aacDecoderGetASC_FAAD2(aacDecoderHandle_t *_dec) {
    faad2Decoder_t *dec = (faad2Decoder_t *)_dec;
    NeAACDecConfigurationPtr conf = NeAACDecGetCurrentConfiguration(dec->dec);
    return (uint8_t *)conf;
}

static int aacDecoderSetASC_FAAD2(aacDecoderHandle_t *_dec,
                                  const uint8_t *asc) {
    faad2Decoder_t *dec = (faad2Decoder_t *)_dec;
...
static uint8_t *ascDecoderDecodeFrame_FAAD2(aacDecoderHandle_t *_dec,
        aacDecoderFrameInfo_t *frameInfo,
        const uint8_t *buffer,
        size_t bufferSize) {
    faad2Decoder_t *dec = (faad2Decoder_t *)_dec;
    NeAACDecFrameInfo fi = {0};
    void *samples = NeAACDecDecode(dec->dec, &fi, (unsigned char *)buffer,
                                   (unsigned long)bufferSize);

    frameInfo->bytesconsumed = fi.bytesconsumed;
    frameInfo->samples = fi.samples;
    frameInfo->channels = fi.channels;
    memcpy(frameInfo->channel_position, fi.channel_position,
           sizeof(fi.channel_position));
...
aacDecoderHandle_t *aacDecoderOpenFAAD2(void) {
