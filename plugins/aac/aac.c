...
// 6: 6 channels: front-center, front-left, front-right, back-left, back-right,
// LFE-channel 7: 8 channels: front-center, front-left, front-right, side-left,
// side-right, back-left, back-right, LFE-channel 8-15: Reserved
...
static DB_fileinfo_t *aac_open(uint32_t hints) {
  aac_info_t *info = calloc(sizeof(aac_info_t), 1);
  return (DB_fileinfo_t *)info;
}

static int64_t parse_aac_stream(DB_FILE *fp, int *psamplerate, int *pchannels,
                                float *pduration, int64_t *ptotalsamples) {
...
static int aac_probe(DB_FILE *fp, float *duration, int *samplerate,
                     int *channels, int64_t *totalsamples) {

  deadbeef->rewind(fp);
  if (parse_aac_stream(fp, samplerate, channels, duration, totalsamples) ==
      -1) {
    return -1;
  }
  return 0;
...
static int aac_init(DB_fileinfo_t *_info, DB_playItem_t *it) {
  ...
  if (info->mp4file) {
    ...
    while (info->trak) {
      mp4p_atom_t *aac_atom =
          mp4p_atom_find(info->trak, "trak/mdia/minf/stbl/stsd/mp4a");
      if (aac_atom) {
        aac = aac_atom->data;
  ...
  if (aac) {
    mp4p_atom_t *stts_atom =
        mp4p_atom_find(info->trak, "trak/mdia/minf/stbl/stts");

    uint64_t total_sample_duration = mp4p_stts_total_sample_duration(stts_atom);
    mp4p_atom_t *stsz_atom =
        mp4p_atom_find(info->trak, "trak/mdia/minf/stbl/stsz");
    ...
    mp4p_atom_t *esds_atom =
        mp4p_atom_find(info->trak, "trak/mdia/minf/stbl/stsd/mp4a/esds");
    if (!esds_atom) {
      return -1;
    }
    ...
    if (aacDecoderInit(info->dec, asc, esds->asc_size, &samplerate, &channels) <
        0) {
      return -1;
    }
    ...
    if (!info->file->vfs->is_streaming()) {
      ...
      offs = parse_aac_stream(info->file, &samplerate, &channels, &duration,
                              &totalsamples);
    } else {
      offs =
          parse_aac_stream(info->file, &samplerate, &channels, &duration, NULL);
    }
    if (offs == -1) {
      return -1;
    }
    ...
    info->dec = aacDecoderOpenFAAD2();
    if (aacDecoderInitRaw(info->dec, asc, sizeof(asc), &usamplerate,
                          &uchannels) < 0) {
      return -1;
    }
  ...
  trace("totalsamples: %d, endsample: %d, samples-from-duration: %d, "
        ...
        (int)totalsamples, (int)info->endsample,
        (int)deadbeef->pl_get_item_duration(it) * 44100, _info->fmt.samplerate,
        _info->fmt.channels);
...
static void aac_free(DB_fileinfo_t *_info) {
  aac_info_t *info = (aac_info_t *)_info;
  if (info) {
    if (info->file) {
...
static int aac_read(DB_fileinfo_t *_info, char *bytes, int size) {
  aac_info_t *info = (aac_info_t *)_info;
  if (info->eof) {
    return 0;
  }
  ...
  while (size > 0) {
    ...
    if (info->skipsamples > 0 && info->out_remaining > 0) {
      ...
      if (skip < info->out_remaining) {
        memmove(info->out_buffer, info->out_buffer + skip * samplesize,
                (info->out_remaining - skip) * samplesize);
      }
    ...
    if (info->out_remaining > 0) {
      ...
      if (info->noremap) {
        ...
        if (info->remap[0] == -1) {
          // build remap mtx
          // FIXME: should build channelmask 1st; then remap based on
          // channelmask
          ...
          if ((_info->fmt.channels == 1 &&
               info->remap[0] == FRONT_CHANNEL_CENTER) ||
              (_info->fmt.channels == 2 &&
               info->remap[0] == FRONT_CHANNEL_LEFT &&
               info->remap[1] == FRONT_CHANNEL_RIGHT)) {
            info->noremap = 1;
          }
        }
    ...
    if (info->mp4file) {
      ...
      mp4p_atom_t *stbl_atom =
          mp4p_atom_find(info->trak, "trak/mdia/minf/stbl");
      ...
      samples =
          aacDecoderDecodeFrame(info->dec, &info->frame_info, mp4packet, size);
      ...
      if (info->remaining < AAC_MAX_PACKET_SIZE) {
        ...
        size_t res =
            deadbeef->fread(info->buffer + info->remaining, 1,
                            AAC_MAX_PACKET_SIZE - info->remaining, info->file);
        info->remaining += res;
        trace("remain: %d\n", info->remaining);
        if (!info->remaining) {
          break;
      ...
      trace("NeAACDecDecode %d bytes\n", info->remaining) samples =
          aacDecoderDecodeFrame(info->dec, &info->frame_info, info->buffer,
                                info->remaining);
      bitrate_bytes += info->frame_info.bytesconsumed;
      bitrate_samples += info->frame_info.samples / info->frame_info.channels;
      trace("samples =%p\n", samples);
      if (!samples) {
        //                trace ("NeAACDecDecode failed with error %s (%d),
        //                consumed=%d\n",
        //                NeAACDecGetErrorMessage(info->frame_info.error),
        //                (int)info->frame_info.error,
        //                (int)info->frame_info.bytesconsumed);
      ...
      if (consumed == info->remaining) {
        ...
        memmove(info->buffer, info->buffer + consumed,
    ...
    if (info->frame_info.samples > 0) {
      ...
      info->out_remaining =
          (int)(info->frame_info.samples / info->frame_info.channels);
    }
  }
  ...
  if (bitrate_samples > 0) {
    bitrate =
        bitrate_bytes * 8 * _info->fmt.samplerate / bitrate_samples / 1000;
  }
...
int seek_raw_aac(aac_info_t *info, int sample) {
  uint8_t buf[ADTS_HEADER_SIZE * 8];
  ...
  do {
    ...
    if (size == 0) {
      ...
      frame++;
      if (deadbeef->fseek(info->file, size - (int)sizeof(buf), SEEK_CUR) ==
          -1) {
        break;
      }
      bufsize = 0;
...
static int aac_seek_sample(DB_fileinfo_t *_info, int sample) {
  aac_info_t *info = (aac_info_t *)_info;
  ...
  if (info->mp4file) {
    ...
    uint64_t seeksample =
        (int)((int64_t)sample * info->aac_samplerate / _info->fmt.samplerate);

    uint64_t startsample = 0;
    info->mp4sample = mp4p_stts_mp4sample_containing_sample(
        stts_atom, seeksample, &startsample);
  ...
  _info->readpos =
      (float)(info->currentsample - info->startsample) / _info->fmt.samplerate;

  return 0;
}

static int aac_seek(DB_fileinfo_t *_info, float t) {
...
static aac_chapter_t *aac_load_itunes_chapters(aac_info_t *info,
                                               mp4p_chap_t *chap,
                                               /* out */ int *num_chapters,
                                               int samplerate) {
  *num_chapters = 0;
  ...
  for (int i = 0; i < chap->number_of_entries; i++) {
    ...
    while (trak_atom) {
      ...
      if (!mp4p_atom_type_compare(trak_atom, "trak") &&
          mp4p_trak_has_chapters(trak_atom)) {
    ...
    mp4p_atom_t *stts_atom =
        mp4p_atom_find(trak_atom, "trak/mdia/minf/stbl/stts");
    ...
    if (stts_atom == NULL || mdhd_atom == NULL || stbl_atom == NULL ||
        stsz_atom == NULL) {
      return NULL;
    }
    ...
    aac_chapter_t *chapters =
        calloc(stsz->number_of_entries, sizeof(aac_chapter_t));
    *num_chapters = 0;
    ...
    for (int sample = 0; sample < stsz->number_of_entries; sample++) {
      ...
      uint64_t samplecount = mp4p_stts_sample_duration(stts_atom, sample) /
                             mdhd->time_scale * samplerate;
...
aac_insert_with_chapters(ddb_playlist_t *plt, DB_playItem_t *after,
                         DB_playItem_t *origin, aac_chapter_t *chapters,
                         int num_chapters, int64_t totalsamples,
                         int samplerate) {
  deadbeef->pl_lock();
  ...
  for (int i = 0; i < num_chapters; i++) {
    ...
    // poor-man utf8 check
    if (!chapters[i].title ||
        deadbeef->junk_detect_charset(chapters[i].title)) {
      char s[1000];
    ...
    float duration =
        (float)(chapters[i].endsample - chapters[i].startsample + 1) /
        samplerate;
...
static int _mp4_insert(DB_playItem_t **after, const char *fname, DB_FILE *fp,
                       ddb_playlist_t *plt) {
  ...
  if (chap_atom) {
    ...
    if (chap->number_of_entries > 0) {
      chapters = aac_load_itunes_chapters(&info, chap, &num_chapters,
  ...
  if (chapters && num_chapters > 0) {
    DB_playItem_t *cue =
        aac_insert_with_chapters(plt, *after, it, chapters, num_chapters,
                                 totalsamples, info.info.fmt.samplerate);
  ...
  if (cuesheet) {
    cue = deadbeef->plt_insert_cue_from_buffer(
        plt, *after, it, (const uint8_t *)cuesheet, (int)strlen(cuesheet),
        (int)totalsamples, info.info.fmt.samplerate);
    if (cue) {
      mp4p_atom_free_list(info.mp4file);
      deadbeef->pl_item_unref(it);
      deadbeef->pl_item_unref(cue);
  ...
  cue = deadbeef->plt_insert_cue(plt, *after, it, (int)totalsamples,
                                 info.info.fmt.samplerate);
  if (cue) {
    deadbeef->pl_item_unref(it);
    deadbeef->pl_item_unref(cue);
...
static DB_playItem_t *aac_insert(ddb_playlist_t *plt, DB_playItem_t *after,
                                 const char *fname) {
  ...
  if (duration > 0) {
    ...
    if (cuesheet) {
      cue = deadbeef->plt_insert_cue_from_buffer(
          plt, after, it, (uint8_t *)cuesheet, (int)strlen(cuesheet),
    ...
    cue =
        deadbeef->plt_insert_cue(plt, after, it, (int)totalsamples, samplerate);
    if (cue) {
      deadbeef->pl_item_unref(it);
      deadbeef->pl_item_unref(cue);
      return cue;
...
DB_plugin_t *aac_load(DB_functions_t *api) {
  deadbeef = api;
