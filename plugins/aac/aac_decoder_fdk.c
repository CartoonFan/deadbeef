...
void aacDecoderClose_FDK(aacDecoderHandle_t *_dec) {
  fdkDecoder_t *dec = (fdkDecoder_t *)_dec;
  aacDecoder_Close(dec->dec);
  free(_dec);
}

int aacDecoderInit_FDK(aacDecoderHandle_t *_dec, uint8_t *asc, size_t ascSize,
...
uint8_t *ascDecoderDecodeFrame_FDK(aacDecoderHandle_t *_dec,
                                   aacDecoderFrameInfo_t *frameInfo,
                                   const uint8_t *buffer, size_t bufferSize) {
  fdkDecoder_t *dec = (fdkDecoder_t *)_dec;

  UINT bytesValid = 0;
  AAC_DECODER_ERROR err = aacDecoder_Fill(dec->dec, (UCHAR **)&buffer,
                                          (UINT *)bufferSize, &bytesValid);
  if (err != AAC_DEC_OK) {
    return NULL;
  }
  ...
  err = aacDecoder_DecodeFrame(dec->dec, (INT_PCM *)outBuffer,
                               (INT)(outBufferSize / sizeof(INT_PCM)),
                               AACDEC_FLUSH);
