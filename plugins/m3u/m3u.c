...
DB_playItem_t *load_m3u_from_buffer(DB_playItem_t *after, const char *buffer,
                                    int64_t sz,
                                    int (*cb)(DB_playItem_t *, void *),
                                    const char *fname, int *pabort,
                                    ddb_playlist_t *plt, void *user_data) {
    const char *slash = strrchr(fname, '/');
    if (sz >= 3 && (uint8_t)buffer[0] == 0xef && (uint8_t)buffer[1] == 0xbb &&
    ...
    while (p < end) {
        ...
        if (title[0]) {
            ...
            if (cs) {
                char tmp[1000];
                if (deadbeef->junk_iconv(title, (int)strlen(title), tmp, sizeof(tmp),
                                         cs, "utf-8") >= 0) {
                    strcpy(title, tmp);
                }
            }
        ...
        if (artist[0]) {
            ...
            if (cs) {
                char tmp[1000];
                if (deadbeef->junk_iconv(artist, (int)strlen(artist), tmp, sizeof(tmp),
                                         cs, "utf-8") >= 0) {
                    strcpy(artist, tmp);
                }
            }
        ...
        if (is_fullpath) { // full path
            ...
            trace("pl_insert_m3u: adding file %s\n", fullpath);
            it = deadbeef->plt_insert_file2(0, plt, after, fullpath, pabort, cb,
                                            user_data);
        }
        if (it) {
            after = it;
...
static DB_playItem_t *load_m3u(ddb_playlist_t *plt, DB_playItem_t *after,
                               const char *fname, int *pabort,
    ...
    after = load_m3u_from_buffer(after, buffer, sz, cb, fname, pabort, plt,
                                 user_data);
...
static DB_playItem_t *pls_insert_file(ddb_playlist_t *plt, DB_playItem_t *after,
                                      const char *fname, const char *uri,
                                      int *pabort,
                                      int (*cb)(DB_playItem_t *it, void *data),
                                      void *user_data, const char *title,
                                      const char *length) {
...
static DB_playItem_t *load_pls(ddb_playlist_t *plt, DB_playItem_t *after,
                               const char *fname, int *pabort,
                               int (*cb)(DB_playItem_t *it, void *data),
                               void *user_data) {
    trace("load_pls %s\n", fname);
    ...
    while (p < end) {
        ...
        if (!strncasecmp(p, "file", 4)) {
            ...
            if (uri[0] && idx != lastidx && lastidx != -1) {
                DB_playItem_t *it = pls_insert_file(plt, after, fname, uri, pabort, cb,
                                                    user_data, title, length);
                if (it) {
                    after = it;
                }
                if (pabort && *pabort) {
...
static DB_playItem_t *m3uplug_load(ddb_playlist_t *plt, DB_playItem_t *after,
                                   const char *fname, int *pabort,
                                   int (*cb)(DB_playItem_t *it, void *data),
                                   void *user_data) {
...
int m3uplug_save_m3u(const char *fname, DB_playItem_t *first,
                     DB_playItem_t *last) {
...
int m3uplug_save_pls(const char *fname, DB_playItem_t *first,
                     DB_playItem_t *last) {
    FILE *fp = fopen(fname, "w+t");
    if (!fp) {
        return -1;
...
int m3uplug_save(ddb_playlist_t *plt, const char *fname, DB_playItem_t *first,
                 DB_playItem_t *last) {
...
DB_plugin_t *m3u_load(DB_functions_t *api) {
    deadbeef = api;
