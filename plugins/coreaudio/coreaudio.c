...
static OSStatus ca_fmtchanged(AudioObjectID inObjectID,
                              UInt32 inNumberAddresses,
                              const AudioObjectPropertyAddress *inAddresses,
                              void *inClientData);

static OSStatus ca_buffer_callback(AudioDeviceID inDevice,
...
static UInt32 GetNumberAvailableNominalSampleRateRanges() {
  UInt32 theAnswer = 0;
  ...
  OSStatus err =
      AudioObjectGetPropertyDataSize(device_id, &theAddress, 0, NULL, &theSize);
  if (err != noErr) {
    trace("AudioObjectGetPropertyDataSize "
          "kAudioDevicePropertyAvailableNominalSampleRates %x",
          err);
    return 0;
  }
...
static void get_avail_samplerates(void) {
  AudioObjectPropertyAddress theAddress = {
      kAudioDevicePropertyAvailableNominalSampleRates,
      kAudioObjectPropertyScopeGlobal, kAudioObjectPropertyElementMaster};
  ...
  if (num > 0) {
    ...
    AudioObjectGetPropertyData(device_id, &theAddress, 0, NULL, &dataSize,
                               nsrs);
...
int get_best_samplerate(int samplerate, int *all_samplerates, int count) {
  int64_t nearest = 0;
  int index = -1;

  for (int i = 0; i < count; i++) {
    // score is based on distance and modulo, with slightly more weight put on
    // distance
    int64_t dist = llabs(all_samplerates[i] - samplerate);
    int64_t mod = samplerate > all_samplerates[i]
                      ? (samplerate % all_samplerates[i])
                      : (all_samplerates[i] % samplerate);
...
static int ca_apply_format(void) {
  int res = -1;
  OSStatus err;
  UInt32 sz;
  deadbeef->mutex_lock(mutex);
  if (req_format.mSampleRate > 0) {
    req_format.mSampleRate = get_best_samplerate(
        (int)req_format.mSampleRate, avail_samplerates, num_avail_samplerates);

    // setting nominal samplerate doesn't work in most cases, and requires some
    // timing trickery
#if 0
    ...
    // NOTE: for unsupported formats, this call may cause bogus messages to
    // appear in console / debug output.
    err = AudioObjectSetPropertyData(device_id, &theAddress, 0, NULL, sz,
                                     &req_format);
    if (err != noErr) {
      err = AudioObjectSetPropertyData(device_id, &theAddress, 0, NULL, sz,
                                       &default_format);
      // ignore the result of this operation -- it may fail even when attempting
      // to change to the same format that's current right now
    }
...
OSStatus callbackFunction(AudioObjectID inObjectID, UInt32 inNumberAddresses,
                          const AudioObjectPropertyAddress inAddresses[],
                          void *inClientData) {
...
static void ca_get_deviceid(void) {
  device_id = 0;
  ...
  if (kAudioHardwareNoError != status) {
    trace("AudioObjectGetPropertyDataSize (kAudioHardwarePropertyDevices) "
          "failed: %i\n",
          status);
    return;
  }
  ...
  if (kAudioHardwareNoError != status) {
    trace("AudioObjectGetPropertyData (kAudioHardwarePropertyDevices) failed: "
          "%i\n",
          status);
    free(audioDevices);
    return;
  ...
  for (UInt32 i = 0; i < deviceCount; ++i) {
    ...
    if (kAudioHardwareNoError != status) {
      trace("AudioObjectGetPropertyData "
...
static int ca_init(void) {
  ...
  if (device_id == 0) {
    sz = sizeof(device_id);
    err = AudioObjectGetPropertyData(kAudioObjectSystemObject, &theAddress, 0,
                                     NULL, &sz, &device_id);
    if (err != noErr) {
      trace("AudioObjectGetPropertyData "
  ...
  err = AudioObjectGetPropertyData(device_id, &theAddress, 0, NULL, &sz,
                                   device_name);
  if (err != noErr) {
  ...
  err =
      AudioObjectSetPropertyData(device_id, &theAddress, 0, NULL, sz, &bufsize);
  if (err != noErr) {
    // non-critical
    trace(
        "AudioObjectSetPropertyData kAudioDevicePropertyBufferFrameSize: %x\n",
        err);
  }
  ...
  err = AudioDeviceCreateIOProcID(device_id, ca_buffer_callback, NULL,
                                  &process_id);
  if (err != noErr) {
  ...
  err = AudioObjectAddPropertyListener(device_id, &theAddress, ca_fmtchanged,
                                       NULL);
  if (err != noErr) {
    trace(
        "AudioObjectAddPropertyListener kAudioDevicePropertyStreamFormat: %x\n",
        err);
    return -1;
  }
  ...
  AudioObjectPropertyAddress outputDeviceAddress = {
      kAudioHardwarePropertyDefaultOutputDevice,
      kAudioObjectPropertyScopeGlobal, kAudioObjectPropertyElementMaster};
  AudioObjectAddPropertyListener(kAudioObjectSystemObject, &outputDeviceAddress,
...
static int ca_free(void) {
  ...
  if (device_id != 0) {
    ...
    err = AudioObjectRemovePropertyListener(device_id, &theAddress,
                                            ca_fmtchanged, NULL);
    if (err != noErr) {
      trace("AudioObjectRemovePropertyListener "
            "kAudioDevicePropertyStreamFormat: %x\n",
            err);
    }
...
static int ca_setformat(ddb_waveformat_t *fmt) {
  ...
  // audioqueue happily accepts ultra-high samplerates, but doesn't really play
  // them
  ...
  if (is_float) {
    ...
    req_format.mFormatFlags = kLinearPCMFormatFlagIsSignedInteger |
...
static void ca_prevent_sleep(void) {
  if (sleep_prevented) {
...
static void ca_allow_sleep(void) {
  if (sleep_prevented) {
...
static OSStatus ca_fmtchanged(AudioObjectID inObjectID,
  ...
  if (err != noErr) {
    ...
    //        trace ("requested %d, obtained %d\n", (int)req_format.mSampleRate,
    //        (int)device_format.mSampleRate);
...
static int ca_unpause(void) { return ca_play(); }

static ddb_playback_state_t ca_state(void) { return state; }

static void ca_enum_soundcards(void (*callback)(const char *name,
  ...
  for (UInt32 i = 0; i < deviceCount; ++i) {
    ...
    if (kAudioHardwareNoError != status) {
      trace("AudioObjectGetPropertyData (kAudioDevicePropertyDeviceUID) "
            "failed: %i\n",
            status);
      continue;
    }
...
static int ca_plugin_start(void) {
  mutex = deadbeef->mutex_create();
  return 0;
}

static int ca_plugin_stop(void) {
  deadbeef->mutex_free(mutex);
  return 0;
}

static DB_output_t plugin = {
    DDB_PLUGIN_SET_API_VERSION.plugin.version_major = 2,
...
DB_plugin_t *coreaudio_load(DB_functions_t *api) {
  deadbeef = api;
