...
#define trace(...)                                                             \
  { deadbeef->log_detailed(&plugin.misc.plugin, 0, __VA_ARGS__); }
#define trace_err(...)                                                         \
  {                                                                            \
    deadbeef->log_detailed(&plugin.misc.plugin, DDB_LOG_LAYER_DEFAULT,         \
                           __VA_ARGS__);                                       \
  }
...
ddb_encoder_preset_t *encoder_preset_alloc(void) {
...
void encoder_preset_free(ddb_encoder_preset_t *p) {
  if (p) {
    if (p->title) {
      free(p->title);
    }
...
ddb_encoder_preset_t *encoder_preset_load(const char *fname) {
  int err = 1;
...
int encoder_preset_save(ddb_encoder_preset_t *p, int overwrite) {
  ...
  mkdir(path, 0755);
  if (snprintf(path, sizeof(path), "%s/presets/encoders/%s.txt", confdir,
               p->title) < 0) {
    return -1;
  }
...
void encoder_preset_copy(ddb_encoder_preset_t *to, ddb_encoder_preset_t *from) {
...
ddb_encoder_preset_t *encoder_preset_get_list(void) { return encoder_presets; }

ddb_encoder_preset_t *encoder_preset_get_for_idx(int idx) {
...
void encoder_preset_append(ddb_encoder_preset_t *p) {
  // append
...
void encoder_preset_remove(ddb_encoder_preset_t *p) {
...
void encoder_preset_replace(ddb_encoder_preset_t *from,
                            ddb_encoder_preset_t *to) {
  ddb_encoder_preset_t *prev = encoder_presets;
...
ddb_dsp_preset_t *dsp_preset_alloc(void) {
...
void dsp_preset_free(ddb_dsp_preset_t *p) {
  if (p) {
    if (p->title) {
      free(p->title);
    }
...
void dsp_preset_copy(ddb_dsp_preset_t *to, ddb_dsp_preset_t *from) {
  to->title = strdup(from->title);
...
ddb_dsp_preset_t *dsp_preset_get_list(void) { return dsp_presets; }

ddb_dsp_preset_t *dsp_preset_load(const char *fname) {
...
int dsp_preset_save(ddb_dsp_preset_t *p, int overwrite) {
  if (!p->title || !p->title[0]) {
  ...
  mkdir(path, 0755);
  if (snprintf(path, sizeof(path), "%s/presets/dsp/%s.txt", confdir, p->title) <
      0) {
    return -1;
  }
...
int scandir_preset_filter(const struct dirent *ent) {
...
int load_encoder_presets(void) {
  // check if we need to install presets
  char ppath[PATH_MAX];
  char epath[PATH_MAX];
  snprintf(ppath, sizeof(ppath), "%s/presets",
           deadbeef->get_system_dir(DDB_SYS_DIR_CONFIG));
  snprintf(epath, sizeof(epath), "%s/encoders", ppath);

  char path[PATH_MAX];
  if (snprintf(path, sizeof(path), "%s/presets/encoders",
               deadbeef->get_system_dir(DDB_SYS_DIR_CONFIG)) < 0) {
    return -1;
  }

  char syspath[PATH_MAX];
  if (snprintf(syspath, sizeof(syspath), "%s/convpresets",
               deadbeef->get_system_dir(DDB_SYS_DIR_PLUGIN_RESOURCES)) < 0) {
    return -1;
  }

  const char *preset_dirs[] = {syspath, path, NULL};
      ...
      if (snprintf(s, sizeof(s), "%s/%s", path, namelist[i]->d_name) > 0) {
        ...
        if (p) {
          if (path == syspath) {
            ...
            for (ddb_encoder_preset_t *pr = encoder_presets; pr;
                 pr = pr->next) {
...
void free_encoder_presets(void) {
  ddb_encoder_preset_t *p = encoder_presets;
  while (p) {
...
int load_dsp_presets(void) {
  ddb_dsp_preset_t *tail = NULL;
  char path[PATH_MAX];
  if (snprintf(path, sizeof(path), "%s/presets/dsp",
               deadbeef->get_system_dir(DDB_SYS_DIR_CONFIG)) < 0) {
    return -1;
  }
  struct dirent **namelist = NULL;
...
ddb_dsp_preset_t *dsp_preset_get_for_idx(int idx) {
  ddb_dsp_preset_t *p = dsp_presets;
  while (p && idx--) {
    p = p->next;
  }
...
void dsp_preset_append(ddb_dsp_preset_t *p) {
  // append
...
void dsp_preset_remove(ddb_dsp_preset_t *p) {
...
void dsp_preset_replace(ddb_dsp_preset_t *from, ddb_dsp_preset_t *to) {
  ddb_dsp_preset_t *prev = dsp_presets;
  while (prev && prev->next != from) {
    prev = prev->next;
  }
...
static void escape_filepath(const char *path, char *out, int sz) {
...
static void get_output_field(DB_playItem_t *it, const char *field, char *out,
                             int sz) {
...
static void get_output_field2(DB_playItem_t *it, ddb_playlist_t *plt,
                              const char *field, char *out, int sz) {
  int idx = deadbeef->pl_get_idx_of(it);
  char temp[PATH_MAX];
...
static void get_output_path_int(DB_playItem_t *it, ddb_playlist_t *plt,
                                const char *outfolder_user, const char *outfile,
                                ddb_encoder_preset_t *encoder_preset,
                                int preserve_folder_structure,
                                const char *root_folder,
                                int write_to_source_folder, char *out, int sz,
                                int use_new_tf) {
  //    trace ("get_output_path: %s %s %s\n", outfolder_user, outfile,
  //    root_folder);
  ...
  if (preserve_folder_structure) {
    ...
    if (e) {
      ...
      subpath[e - s] = 0;
      snprintf(outfolder_preserve, sizeof(outfolder_preserve), "%s%s",
               outfolder_user[0] ? outfolder_user : getenv("HOME"), subpath);
    }
  }
...
void get_output_path(DB_playItem_t *it, const char *outfolder_user,
                     const char *outfile, ddb_encoder_preset_t *encoder_preset,
                     int preserve_folder_structure, const char *root_folder,
                     int write_to_source_folder, char *out, int sz) {
  get_output_path_int(it, NULL, outfolder_user, outfile, encoder_preset,
                      preserve_folder_structure, root_folder,
                      write_to_source_folder, out, sz, 0);
}

void get_output_path2(DB_playItem_t *it, ddb_playlist_t *plt,
                      const char *outfolder_user, const char *outfile,
                      ddb_encoder_preset_t *encoder_preset,
                      int preserve_folder_structure, const char *root_folder,
                      int write_to_source_folder, char *out, int sz) {
  get_output_path_int(it, plt, outfolder_user, outfile, encoder_preset,
                      preserve_folder_structure, root_folder,
                      write_to_source_folder, out, sz, 1);
}

static void get_output_path_1_0(DB_playItem_t *it, const char *outfolder,
                                const char *outfile,
                                ddb_encoder_preset_t *encoder_preset, char *out,
                                int sz) {
  trace("warning: old version of \"get_output_path\" has been called, please "
        "update your plugins which depend on converter 1.1\n");
  *out = 0;
  sz = 0;
}

static int check_dir(const char *dir, mode_t mode) {
  char *tmp = strdup(dir);
  char *slash = tmp;
...
static inline void write_int32_le(char *p, uint32_t value) {
  p[0] = value & 0xff;
  p[1] = (value >> 8) & 0xff;
  p[2] = (value >> 16) & 0xff;
  p[3] = (value >> 24) & 0xff;
...
static inline void write_int16_le(char *p, uint16_t value) {
  p[0] = value & 0xff;
  p[1] = (value >> 8) & 0xff;
}
...
static int64_t _write_wav(DB_playItem_t *it, DB_decoder_t *dec,
                          DB_fileinfo_t *fileinfo, ddb_dsp_preset_t *dsp_preset,
                          ddb_encoder_preset_t *encoder_preset, int *abort,
                          int fd, int output_bps, int output_is_float) {
  int64_t res = -1;
  char *buffer = NULL;
  char *dspbuffer = NULL;
  ...
  for (;;) {
    ...
    if (dsp_preset) {
      ...
      while (dsp) {
        frames = dsp->plugin->process(dsp, (float *)dspbuffer, frames,
                                      dspsize / (fmt.channels * 4), &fmt, NULL);
        if (frames <= 0) {
          break;
        }
        dsp = dsp->next;
      ...
      int n = deadbeef->pcm_convert(&fmt, dspbuffer, &outfmt, buffer,
                                    frames * sizeof(float) * fmt.channels);
      sz = n;
    } else if (fileinfo->fmt.bps != output_bps ||
               fileinfo->fmt.is_float != output_is_float) {
      ddb_waveformat_t outfmt;
      ...
      int frames = sz / samplesize;
      int n = deadbeef->pcm_convert(&fileinfo->fmt, buffer, &outfmt, dspbuffer,
                                    frames * samplesize);
    ...
    if (!header_written) {
      ...
      uint64_t size =
          (int64_t)(endsample - startsample) * outch * output_bps / 8;
      if (!size) {
        size = (double)deadbeef->pl_get_item_duration(it) *
               fileinfo->fmt.samplerate * outch * output_bps / 8;
      }
      ...
      uint16_t blockalign = outch * output_bps /
                            8; // block_align; bits_per_sample; cbSize; validBPS
      ...
      if (exheader) {
        ...
        write_int16_le(
            wavehdr + 36,
            cbSize); // cbSize (validBPS + channelmask + codec ID = 22 bytes)
        write_int16_le(wavehdr + 38, output_bps); // validBPS
        int32_t chMask = 3;
        write_int32_le(wavehdr + 40, chMask); // channelMask

        memcpy(wavehdr + 44,
               output_is_float ? format_id_float32 : format_id_pcm,
               16); // 16 bytes format ID
...
static int _get_encoder_cmdline(ddb_encoder_preset_t *encoder_preset, char *enc,
                                int len, const char *escaped_out,
  ...
  *o = 0;
  ...
  while (e && *e) {
    if (len <= 0) {
      trace("Failed to assemble encoder command line - buffer is not big "
            "enough, try to shorten your parameters. max allowed length is %u "
            "characters\n",
            (unsigned)sizeof(enc));
      return -1;
    }
...
static int _copy_file(const char *in, const char *out) {
...
static int _converter_write_tags(ddb_encoder_preset_t *encoder_preset,
                                 DB_playItem_t *it, const char *out) {
  int err = 0;

  DB_playItem_t *out_it = NULL;

  if (!(encoder_preset->tag_id3v2 || encoder_preset->tag_id3v1 ||
        encoder_preset->tag_apev2 || encoder_preset->tag_flac ||
        encoder_preset->tag_oggvorbis || encoder_preset->tag_mp4)) {
  ...
  if (tagflags) {
    ...
    deadbeef->junk_rewrite_tags(out_it, tagflags,
                                encoder_preset->id3v2_version + 3, "iso8859-1");
  }
...
static int convert2(ddb_converter_settings_t *settings, DB_playItem_t *it,
                    const char *out, int *pabort) {
  ...
  int bypass_conversion_on_same_format =
      settings->bypass_conversion_on_same_format;
  ...
  if (bypass_conversion_on_same_format &&
      !strcasecmp(ext, encoder_preset->ext)) {
  ...
  if (encoder_preset->method == DDB_ENCODER_METHOD_FILE ||
      encoder_preset->method == DDB_ENCODER_METHOD_PIPE) {
    deadbeef->pl_lock();
    dec = (DB_decoder_t *)deadbeef->plug_get_for_id(
        deadbeef->pl_find_meta(it, ":DECODER"));
    deadbeef->pl_unlock();
    ...
    if (dec) {
      ...
      if (fileinfo) {
        ...
        case DDB_ENCODER_METHOD_FILE: {
          ...
          snprintf(input_file_name, sizeof(input_file_name), "%s/ddbconvXXXXXX",
                   tmp);
        ...
        if (_get_encoder_cmdline(encoder_preset, enc, sizeof(enc), escaped_out,
                                 input_file_name) < 0) {
          goto error;
        }
        ...
        if (!encoder_preset->encoder[0]) {
          // write to wave file
          temp_file =
              open(out, O_LARGEFILE | O_WRONLY | O_CREAT | O_TRUNC | _O_BINARY,
                   wrmode);
          if (temp_file == -1) {
          ...
          temp_file = open(
              input_file_name,
              O_LARGEFILE | O_WRONLY | O_CREAT | O_TRUNC | _O_BINARY, wrmode);
          if (temp_file == -1) {
        ...
        if (temp_file > 0) {
          int64_t outsize =
              _write_wav(it, dec, fileinfo, dsp_preset, encoder_preset, pabort,
                         temp_file, output_bps, output_is_float);
          ...
          if (encoder_preset->method == DDB_ENCODER_METHOD_FILE) {
            if (temp_file != -1 &&
                (!enc_pipe || temp_file != fileno(enc_pipe))) {
              close(temp_file);
              temp_file = -1;
            }
          }
  ...
  if (enc[0] && (encoder_preset->method == DDB_ENCODER_METHOD_FILE ||
                 encoder_preset->method == DDB_ENCODER_METHOD_FILENAME)) {
    enc_pipe = popen(enc, "w");
  }
...
static int convert(DB_playItem_t *it, const char *out, int output_bps,
                   int output_is_float, ddb_encoder_preset_t *encoder_preset,
                   ddb_dsp_preset_t *dsp_preset, int *abort) {
...
static int convert_1_0(DB_playItem_t *it, const char *outfolder,
                       const char *outfile, int output_bps, int output_is_float,
                       int preserve_folder_structure, const char *root_folder,
                       ddb_encoder_preset_t *encoder_preset,
                       ddb_dsp_preset_t *dsp_preset, int *abort) {
  trace("An old version of \"convert\" has been called, please update your "
        "plugins which depend on converter 1.1\n");
  return -1;
}

int converter_cmd(int cmd, ...) { return -1; }

int converter_start(void) {
...
static ddb_converter_t plugin = {
    ...
    .misc.plugin.descr =
        "Converts any supported formats to other formats.\n"
...
DB_plugin_t *converter_load(DB_functions_t *api) {
  deadbeef = api;
