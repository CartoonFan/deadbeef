...
DB_plugin_t *lastfm_load(DB_functions_t *api) {
  deadbeef = api;
...
static void lfm_send_nowplaying(const char *lfm_nowplaying);

static void lfm_update_auth(void) {
  deadbeef->conf_lock();
...
static size_t lastfm_curl_res(void *ptr, size_t size, size_t nmemb,
                              void *stream) {
...
static int lfm_curl_control(void *stream, double dltotal, double dlnow,
...
static int curl_req_send(const char *req, const char *post) {
  ...
  if (deadbeef->conf_get_int("network.proxy", 0)) {
    ...
    curl_easy_setopt(curl, CURLOPT_PROXYPORT,
                     deadbeef->conf_get_int("network.proxy.port", 8080));
    const char *type =
        deadbeef->conf_get_str_fast("network.proxy.type", "HTTP");
...
static void curl_req_cleanup(void) { lfm_reply_sz = 0; }
...
static int auth(void) {
  ...
  const char *scrobbler_url =
      deadbeef->conf_get_str_fast("lastfm.scrobbler_url", SCROBBLER_URL_LFM);
#if LFM_TESTMODE
  snprintf(req, sizeof(req), "%s/?hs=true&p=1.2.1&c=tst&v=1.0&u=%s&t=%d&a=%s",
           scrobbler_url, lfm_user, (int)timestamp, token);
#else
  snprintf(req, sizeof(req), "%s/?hs=true&p=1.2.1&c=%s&v=%d.%d&u=%s&t=%d&a=%s",
           scrobbler_url, LFM_CLIENTID, plugin.plugin.version_major,
           plugin.plugin.version_minor, lfm_user, (int)timestamp, token);
#endif
  deadbeef->conf_unlock();
  // handshake
...
static int lfm_fetch_song_info(DB_playItem_t *song, float playtime, char *a,
                               char *t, char *b, float *l, char *n, char *m) {
  if (deadbeef->conf_get_int("lastfm.prefer_album_artist", 0)) {
    if (!deadbeef->pl_get_meta_with_override(song, "band", a,
                                             META_FIELD_SIZE)) {
      if (!deadbeef->pl_get_meta_with_override(song, "album artist", a,
                                               META_FIELD_SIZE)) {
        if (!deadbeef->pl_get_meta_with_override(song, "albumartist", a,
                                                 META_FIELD_SIZE)) {
          if (!deadbeef->pl_get_meta_with_override(song, "artist", a,
                                                   META_FIELD_SIZE)) {
            return -1;
          }
        }
  ...
  *l = deadbeef->pl_get_item_duration(song);
  ...
  if (!deadbeef->conf_get_int("lastfm.mbid", 0) ||
      !deadbeef->pl_get_meta_with_override(song, "musicbrainz_trackid", m,
                                           META_FIELD_SIZE)) {
    *m = 0;
  }
  return 0;
...
static int lfm_uri_encode(char *out, size_t outl, const char *str) {
  size_t l = outl;
...
// or -1 on error
static int lfm_add_keyvalue_uri_encoded(char **out, int *outl, const char *key,
                                        const char *value) {
  int ll = *outl;
...
static int lfm_format_uri(int subm, DB_playItem_t *song, char *out, int outl,
                          time_t started_timestamp, float playtime) {
  if (subm > 50) {
    trace("lastfm: it's only allowed to send up to 50 submissions at once (got "
          "idx=%d)\n",
          subm);
    return -1;
  }
  ...
  outl -= processed;
  if (subm >= 0) {
    processed = snprintf(out, outl, "i[%d]=%d&o[%d]=P&r[%d]=&", subm,
                         (int)started_timestamp, subm, subm);
    if (processed > outl) {
      //            trace ("failed to add i[%d]=%d&o[%d]=P&r[%d]=&\n", subm,
      //            (int)song->started_timestamp, subm, subm);
      return -1;
    }
    out += processed;
    outl -= processed;
...
static int lastfm_songstarted(ddb_event_track_t *ev, uintptr_t data) {
  ...
  dispatch_async(request_queue, ^{
    ...
    if (lfm_format_uri(-1, it, lfm_nowplaying, sizeof(lfm_nowplaying),
                       started_timestamp, 120) > 0) {
      // try to send nowplaying
      if (lfm_nowplaying[0] &&
          !deadbeef->conf_get_int("lastfm.disable_np", 0)) {
...
static int lastfm_songchanged(ddb_event_trackchange_t *ev, uintptr_t data) {
  if (!deadbeef->conf_get_int("lastfm.enable", 0)) {
    return 0;
  }
  ...
  if (dur < 30 && ev->playtime < 30) {
    ...
    if (!(dur > 0 && fabs(ev->playtime - dur) < 1.f &&
          deadbeef->conf_get_int("lastfm.submit_tiny_tracks", 0))) {
      trace("track duration is %f sec, playtime if %f sec. not eligible for "
            "submission\n",
            dur, ev->playtime);
      return 0;
    }
  ...
  if (!deadbeef->pl_meta_exists_with_override(ev->from, "artist") ||
    ...
    trace("lfm: not enough metadata for submission, artist=%s, title=%s, "
          "album=%s\n",
          deadbeef->pl_find_meta_with_override(ev->from, "artist"),
          deadbeef->pl_find_meta_with_override(ev->from, "title"),
          deadbeef->pl_find_meta_with_override(ev->from, "album"));
    return 0;
  }
...
static void lfm_send_nowplaying(const char *nowplaying_uri) {
  if (auth() < 0) {
    ...
    if (!status) {
      if (strncmp((char *)lfm_reply, "OK", 2)) {
        ...
        if (!strncmp((char *)lfm_reply, "BADSESSION", 7)) {
          ...
          snprintf(s, sizeof(s), "s=%s&", lfm_sess);
          snprintf(lfm_nowplaying, sizeof(lfm_nowplaying), "%s%s",
                   nowplaying_uri, s);
...
static void lfm_send_submission(ddb_playItem_t *it, time_t started_timestamp,
                                float playtime) {
...
static int lfm_message(uint32_t id, uintptr_t ctx, uint32_t p1, uint32_t p2) {
  switch (id) {
...
static int lfm_action_lookup(DB_plugin_action_t *action,
  ...
  if (!deadbeef->pl_get_meta_with_override(it, "artist", artist,
                                           sizeof(artist))) {
    goto out;
  }
...
static DB_plugin_action_t lookup_action = {.title = "Lookup On Last.fm",
                                           .name = "lfm_lookup",
...
static DB_plugin_action_t *lfm_get_actions(DB_playItem_t *it) {
  deadbeef->pl_lock();
  if (!it || !deadbeef->pl_find_meta_with_override(it, "artist") ||
...
static const char settings_dlg[] =
    ...
    "\";"
    "property \"Prefer Album Artist over Artist field\" checkbox "
    "lastfm.prefer_album_artist 0;"
    "property \"Send MusicBrainz ID\" checkbox lastfm.mbid 0;"
    "property \"Submit tracks shorter than 30 seconds (not recommended)\" "
    "checkbox lastfm.submit_tiny_tracks 0;";
...
static DB_misc_t plugin = {
    ...
    .plugin.descr = "Sends played songs information to your last.fm account, "
                    "or other service that use AudioScrobbler protocol",
    .plugin.copyright =
        "Last.fm scrobbler plugin for DeaDBeeF Player\n"
        "Copyright (C) 2009-2014 Alexey Yakovenko\n"
        "\n"
