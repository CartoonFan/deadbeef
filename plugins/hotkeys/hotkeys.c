...
#ifndef NO_XLIB_H
static void init_mapped_keycodes(Display *disp, Atom *syms, int first_kk,
                                 int last_kk, int ks_per_kk) {
  int i, ks;
...
#ifndef NO_XLIB_H
static void cmd_invoke_plugin_command(DB_plugin_action_t *action,
                                      ddb_action_context_t ctx) {
  if (action->callback) {
    if (ctx == DDB_ACTION_CTX_MAIN) {
      ...
      if ((selected_count == 1) &&
          (!(action->flags & DB_ACTION_SINGLE_TRACK))) {
...
static DB_plugin_action_t *find_action_by_name(const char *command) {
...
#ifndef NO_XLIB_H
static int get_x11_keycode(const char *name, Atom *syms, int first_kk,
                           int last_kk, int ks_per_kk) {
  int i, ks;
...
static int read_config(Display *disp) {
  int ks_per_kk;
...
static int read_config(void) {
...
#ifndef NO_XLIB_H
          cmd_entry->x11_keycode =
              get_x11_keycode(p, syms, first_kk, last_kk, ks_per_kk);
          trace("%s: kc=%d, xkc=%d\n", p, cmd_entry->keycode,
                cmd_entry->x11_keycode);
#endif
        }
  ...
  for (i = 0; i < command_count; i++) {
    ...
    for (int f = 0; f < 16; f++) {
      ...
      XGrabKey(disp, commands[i].x11_keycode, commands[i].modifier | flags,
               DefaultRootWindow(disp), False, GrabModeAsync, GrabModeAsync);
    }
  }
#endif
...
DB_plugin_t *hotkeys_load(DB_functions_t *api) {
  deadbeef = api;
...
#ifndef NO_XLIB_H
static int x_err_handler(Display *d, XErrorEvent *evt) {
#if 0
...
static void hotkeys_event_loop(void *unused) {
  int i;
#ifdef __linux__
    ...
    if (need_reset) {
      ...
      for (int i = 0; i < command_count; i++) {
        for (int f = 0; f < 16; f++) {
          ...
          XUngrabKey(disp, commands[i].x11_keycode,
                     commands[i].modifier | flags, DefaultRootWindow(disp));
        }
      }
...
static int hotkeys_connect(void) {
#ifndef NO_XLIB_H
  finished = 0;
  loop_tid = 0;
...
static int hotkeys_message(uint32_t id, uintptr_t ctx, uint32_t p1,
                           uint32_t p2) {
...
static int hotkeys_disconnect(void) {
#ifndef NO_XLIB_H
  if (loop_tid) {
    finished = 1;
...
const char *hotkeys_get_name_for_keycode(int keycode) {
  for (int i = 0; keys[i].name; i++) {
...
DB_plugin_action_t *hotkeys_get_action_for_keycombo(int key, int mods,
                                                    int isglobal, int *ctx) {
  int i;
  // find mapped keycode
  ...
  for (i = 0; i < command_count; i++) {
    trace("hotkeys: command %s keycode %x mods %x\n", commands[i].action->name,
          commands[i].keycode, commands[i].modifier);
    if (commands[i].keycode == keycode && commands[i].modifier == mods &&
        commands[i].isglobal == isglobal) {
      *ctx = commands[i].ctx;
...
int action_play_cb(struct DB_plugin_action_s *action,
                   ddb_action_context_t ctx) {
...
int action_play_pause_cb(struct DB_plugin_action_s *action,
                         ddb_action_context_t ctx) {
...
int action_seek_5p_backward_cb(struct DB_plugin_action_s *action,
                               ddb_action_context_t ctx) {
  deadbeef->pl_lock();
  DB_playItem_t *it = deadbeef->streamer_get_playing_track();
  if (it) {
...
int action_seek_1p_forward_cb(struct DB_plugin_action_s *action,
                              ddb_action_context_t ctx) {
  deadbeef->pl_lock();
  DB_playItem_t *it = deadbeef->streamer_get_playing_track();
  if (it) {
...
int action_seek_1p_backward_cb(struct DB_plugin_action_s *action,
                               ddb_action_context_t ctx) {
  deadbeef->pl_lock();
  DB_playItem_t *it = deadbeef->streamer_get_playing_track();
  if (it) {
...
int action_seek_1s_forward_cb(struct DB_plugin_action_s *action,
                              ddb_action_context_t ctx) {
  return seek_sec(1.f);
}

int action_seek_1s_backward_cb(struct DB_plugin_action_s *action,
                               ddb_action_context_t ctx) {
  return seek_sec(-1.f);
}

int action_seek_5s_forward_cb(struct DB_plugin_action_s *action,
                              ddb_action_context_t ctx) {
  return seek_sec(5.f);
}

int action_seek_5s_backward_cb(struct DB_plugin_action_s *action,
                               ddb_action_context_t ctx) {
  return seek_sec(-5.f);
}

int action_volume_up_cb(struct DB_plugin_action_s *action,
                        ddb_action_context_t ctx) {
  deadbeef->volume_set_db(deadbeef->volume_get_db() + 1);
  return 0;
}

int action_volume_down_cb(struct DB_plugin_action_s *action,
                          ddb_action_context_t ctx) {
  deadbeef->volume_set_db(deadbeef->volume_get_db() - 1);
  return 0;
}
...
static DB_plugin_action_t *hotkeys_get_actions(DB_playItem_t *it) {
...
static DB_hotkeys_plugin_t plugin = {
    ...
    .misc.plugin.descr =
        "Manages local and global hotkeys, and executes actions when the "
        "assigned key combinations are pressed\n\n"
        "This plugin has its own API, to allow 3rd party GUI plugins to reuse "
        "the code.\n"
        "Check the plugins/hotkeys/hotkeys.h in the source tree if you need "
        "this.\n\n"
        "Changes in version 1.1\n"
        "    * adaptation to new deadbeef 0.6 plugin API\n"
        "    * added local hotkeys support\n",
    .misc.plugin.copyright =
        "Copyright (C) 2012-2013 Alexey Yakovenko "
