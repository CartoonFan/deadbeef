...
- (void)updateTitleAttributes {
    ...
    if (isDarkMode == self.isDarkMode &&
            self.window.isKeyWindow == self.isKeyWindow &&
    ...
    NSColor *textColor =
        self.isDarkMode ? NSColor.controlTextColor : self.accentColor;
...
static NSString *plt_get_title_wrapper(int plt) {
    if (plt == -1) {
        return @"";
    }
...
- (void)drawTab:(int)idx area:(NSRect)area selected:(BOOL)sel {
    ...
    [line moveToPoint:NSMakePoint(tabRect.origin.x + tabRect.size.width - 0.5,
                                  tabRect.origin.y + 5)];
    [line lineToPoint:NSMakePoint(tabRect.origin.x + tabRect.size.width - 0.5,
                                  tabRect.origin.y + tabRect.size.height - 10)];
    if (sel && _dragging >= 0) {
        [line
         moveToPoint:NSMakePoint(tabRect.origin.x - 0.5, tabRect.origin.y + 5)];
        [line lineToPoint:NSMakePoint(tabRect.origin.x - 0.5,
                                      tabRect.origin.y + tabRect.size.height - 10)];
    }
    [line stroke];
    ...
    // tab title
    NSDictionary *attrs =
        sel ? self.titleAttributesSelected : self.titleAttributes;

    NSString *tab_title = plt_get_title_wrapper(idx);

    NSSize size = [tab_title sizeWithAttributes:attrs];

    [tab_title drawInRect:NSMakeRect(area.origin.x + text_left_padding,
                                     area.origin.y + area.size.height / 2 -
                                     size.height / 2,
                                     area.size.width - (text_left_padding +
                                             text_right_padding - 1),
                                     size.height)
    ...
    if (idx == _pointedTab && (_dragging == -1 || !self.dragReallyBegan)) {
        ...
        if (NSPointInRect(_lastMouseCoord, atRect)) {
            NSBezierPath *path = [NSBezierPath bezierPathWithRoundedRect:atRect
                                               xRadius:1
                                               yRadius:1];
            ...
            NSColor *closeButtonBackgroundColor =
                [NSColor.controlTextColor colorWithAlphaComponent:alpha];
        ...
        [NSBezierPath strokeLineFromPoint:NSMakePoint(from.x, to.y)
                      toPoint:NSMakePoint(to.x, from.y)];
    }
}
...
- (void)drawRect:(NSRect)dirtyRect {
    ...
    if (_dragging < 0 || _prepare || tab_selected != _dragging) {
        ...
        selectedTabRect = [self tabRectForXPos:x
                                width:w
            ...
            if (idx == _dragging) {
                ...
                if (w > 0) {
                    ...
                    if (selectedTabRect.origin.x > 0) {
                        NSRect clipLeft = NSMakeRect(0, 0, selectedTabRect.origin.x,
                                                     selectedTabRect.size.height);
                        [clipPath appendBezierPathWithRect:clipLeft];
                    }
                    int xx = selectedTabRect.origin.x + selectedTabRect.size.width;
                    NSRect clipRight = NSMakeRect(xx, 0, self.bounds.size.width - xx,
                                                  selectedTabRect.size.height);
    ...
    for (idx = 0; idx < c; idx++) {
        w = widths[idx];
        NSRect area = [self tabRectForXPos:x
                            width:w
                            height:self.bounds.size.height];
...
- (NSRect)tabCloseButtonRectForTabRect:(NSRect)tabRect {
    NSPoint from = NSMakePoint(tabRect.origin.x + close_btn_left_offs + 0.5,
                               tabRect.origin.y + tabRect.size.height / 2 - 6);
    NSRect atRect;
    atRect.origin = from;
    atRect.origin.x -= 2;
...
- (BOOL)handleClickedTabCloseRect {
    ...
    NSRect tabRect = [self tabRectForXPos:x
                           width:w
                           height:self.bounds.size.height];
...
- (void)closePlaylist:(id)sender {
    if (_tab_clicked != -1) {
        ...
        alert.informativeText = [NSString
                                 stringWithFormat:@"Do you really want to remove the playlist '%@'?",
                                 plt_get_title_wrapper(_tab_clicked)];
...
- (NSMenu *)menuForEvent:(NSEvent *)theEvent {
    ...
    if ((theEvent.type == NSEventTypeRightMouseDown ||
        ...
        [menu insertItemWithTitle:@"Add New Playlist"
              action:@selector(addNewPlaylist:)
              keyEquivalent:@""
              atIndex:0]
        .target = self;
        if (_tab_clicked != -1) {
            [menu insertItemWithTitle:@"Close Playlist"
                  action:@selector(closePlaylist:)
                  keyEquivalent:@""
                  atIndex:0]
            .target = self;

            // ignore the warning, the message is sent to 1st responder, which will be
            // the mainwincontroller in this case
            NSMenuItem *item =
                [[NSMenuItem alloc] initWithTitle:@"Rename Playlist"
                                    action:@selector(renamePlaylistAction:)
                                    keyEquivalent:@""];
            [menu insertItem:item atIndex:0];
        }

        [menu
         addActionItemsForContext:DDB_ACTION_CTX_PLAYLIST
         track:NULL
...
- (void)setupTrackingArea {
    // setup tracking area covering entire view, for managing the tab close
...
- (IBAction)renamePlaylistAction:(id)sender {
    ...
    [self.window beginSheet:self.renamePlaylistWindow
                completionHandler:^(NSModalResponse returnCode) {
                    if (returnCode == NSModalResponseOK) {
                        deadbeef->plt_set_title(
                            plt, [[self.renamePlaylistTitle stringValue] UTF8String]);
