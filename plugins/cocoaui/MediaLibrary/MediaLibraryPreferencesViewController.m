...
static NSString *const kMedialibBrowseInitialFolder =
    @"MedialibBrowseInitialFolder";

extern DB_functions_t *deadbeef;

@interface MediaLibraryPreferencesViewController () <NSTableViewDelegate,
...
- (void)initializeList {
  ...
  NSInteger count =
      (NSInteger)self.medialibPlugin->folder_count(self.medialibSource);
  for (NSInteger i = 0; i < count; i++) {
    char folder[PATH_MAX];
    self.medialibPlugin->folder_at_index(self.medialibSource, (int)i, folder,
                                         sizeof(folder));
...
- (void)addAction:(id)sender {
  ...
  NSString *initialPath = [NSUserDefaults.standardUserDefaults
      stringForKey:kMedialibBrowseInitialFolder];
  if (initialPath) {
      ...
      beginSheetModalForWindow:self.view.window
             completionHandler:^(NSInteger result) {
               [NSUserDefaults.standardUserDefaults
                   setObject:panel.directoryURL.absoluteString
                      forKey:kMedialibBrowseInitialFolder];
               if (result == NSModalResponseOK) {
                 [self.tableView beginUpdates];
                 for (NSURL *url in panel.URLs) {
                   [self.tableView
                       insertRowsAtIndexes:[NSIndexSet indexSetWithIndex:index]
...
- (void)applyChanges {
  const char **paths =
      self.folders.count ? calloc(sizeof(char *), self.folders.count) : NULL;
  for (NSUInteger i = 0; i < self.folders.count; i++) {
    paths[i] = self.folders[i].UTF8String;
  }

  self.medialibPlugin->set_folders(self.medialibSource, paths,
                                   self.folders.count);
