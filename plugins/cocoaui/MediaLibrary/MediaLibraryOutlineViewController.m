...
@interface MediaLibraryOutlineViewController () <
    TrackContextMenuDelegate, MediaLibraryFilterSelectorCellViewDelegate,
                              MediaLibrarySearchCellViewDelegate>
...
- (instancetype)initWithOutlineView:(NSOutlineView *)outlineView {
    ...
    self.medialibPlugin =
        (ddb_medialib_plugin_t *)deadbeef->plug_get_for_id("medialib");
    self.artworkPlugin =
        (ddb_artwork_plugin_t *)deadbeef->plug_get_for_id("artwork2");
    self.listenerId = self.medialibPlugin->plugin.add_listener(
                          self.medialibSource, _medialib_listener, (__bridge void *)self);
...
- (void)dealloc {
    self.medialibPlugin->plugin.remove_listener(self.medialibSource,
            self.listenerId);
    self.listenerId = -1;
    self.medialibPlugin = NULL;
}

static void _medialib_listener(ddb_mediasource_event_type_t event,
                               void *user_data) {
    MediaLibraryOutlineViewController *ctl =
        (__bridge MediaLibraryOutlineViewController *)user_data;
...
- (void)initializeTreeView:(int)index {
    ...
    if (self.outlineViewInitialized) {
        ...
        [self.outlineView
         removeItemsAtIndexes:[NSIndexSet indexSetWithIndex:itemIndex]
         inParent:nil
    ...
    if (self.medialibItemTree) {
        self.medialibPlugin->plugin.free_list(self.medialibSource,
                                              self.medialibItemTree);
        self.medialibItemTree = NULL;
    }
    self.medialibItemTree = self.medialibPlugin->plugin.create_list(
                                self.medialibSource, indexNames[index],
                                self.searchString ? self.searchString.UTF8String : NULL);
    self.medialibRootItem =
        [[MediaLibraryItem alloc] initWithItem:self.medialibItemTree];
    ...
    if (self.outlineViewInitialized) {
        [self.outlineView
         insertItemsAtIndexes:[NSIndexSet indexSetWithIndex:itemIndex]
         inParent:nil
         withAnimation:NSTableViewAnimationEffectNone];
    }
...
- (void)updateMedialibStatus {
    ...
    NSTableCellView *view = [[self.outlineView rowViewAtRow:row
                              makeIfNecessary:NO] viewAtColumn:0];
...
- (void)outlineViewDoubleAction:(NSOutlineView *)sender {
    ...
    if (deadbeef->conf_get_int("cli_add_to_specific_playlist", 1)) {
        char str[200];
        deadbeef->conf_get_str("cli_add_playlist_name", "Default", str,
                               sizeof(str));
...
- (id<NSPasteboardWriting>)outlineView:(NSOutlineView *)outlineView
    pasteboardWriterForItem:(MediaLibraryItem *)item {
    if (![item isKindOfClass:MediaLibraryItem.class]) {
        return nil;
    }
...
static void cover_get_callback(int error, ddb_cover_query_t *query,
                               ddb_cover_info_t *cover) {
    MediaLibraryCoverQueryData *data =
        (__bridge MediaLibraryCoverQueryData *)(query->user_data);
    [data.viewController coverGetCallbackWithQuery:query
                         coverInfo:cover
                         error:error];
}
...
- (NSString *)albumArtCacheKeyForTrack:(ddb_playItem_t *)track {
    const char *artist =
        deadbeef->pl_find_meta(track, "artist") ?: "Unknown Artist";
...
- (void)coverGetCallbackWithQuery:(ddb_cover_query_t *)query
    coverInfo:(ddb_cover_info_t *)cover
    error:(int)error {
    MediaLibraryCoverQueryData *data =
        (__bridge_transfer MediaLibraryCoverQueryData *)(query->user_data);
    if (!error) {
        NSImage *image;
        if (cover->image_filename) {
            image = [[NSImage alloc]
                     initByReferencingFile:[NSString stringWithUTF8String:cover->
                                            image_filename]];
        } else if (cover->blob) {
            NSData *blobData = [NSData dataWithBytes:cover->blob
                                       length:cover->blob_size];
        ...
        dispatch_async(dispatch_get_main_queue(), ^ {
            ...
            if (row >= 0) {
                ...
                [[self.outlineView rowViewAtRow:row
                  makeIfNecessary:NO] viewAtColumn:0];
                if (cellView) {
...
- (nullable NSView *)outlineView:(NSOutlineView *)outlineView
    ...
    if ([item isKindOfClass:MediaLibraryItem.class]) {
            ...
            if (it) {
                if (mlItem.coverImage) {
                    ...
                    if (image) {
                        ...
                        query->flags =
                            DDB_ARTWORK_FLAG_NO_CACHE | DDB_ARTWORK_FLAG_LOAD_BLOB;
                        query->track = it;
        ...
        view = [outlineView makeViewWithIdentifier:@"FilterSelectorCell"
                            owner:self];
        MediaLibrarySelectorCellView *selectorCellView =
            (MediaLibrarySelectorCellView *)view;
        ...
        MediaLibrarySearchCellView *searchCellView =
            (MediaLibrarySearchCellView *)view;
...
- (void)filterChanged {
    ...
    [self.outlineView expandItem:self.medialibRootItem
                      expandChildren:self.searchString != nil];
}
...
- (void)menuNeedsUpdate:(TrackContextMenu *)menu {
    ...
    if (clickedRow != -1 && [self.outlineView isRowSelected:clickedRow]) {
        [self.outlineView.selectedRowIndexes
        enumerateIndexesUsingBlock:^(NSUInteger row, BOOL *_Nonnull stop) {
...
- (NSIndexSet *)outlineView:(NSOutlineView *)outlineView
    ...
    enumerateIndexesUsingBlock:^(NSUInteger row, BOOL *_Nonnull stop) {
        ...
        if (item != self.selectorItem && item != self.medialibRootItem &&
