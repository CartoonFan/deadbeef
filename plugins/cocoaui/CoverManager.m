...
- (CoverManager *)init {
  ...
  // Each file may contain its own album art, therefore we can't really cache
  // them easily by album/artist/title. This however would duplicate the same
  // image, for every track in every album, if the custom grouping is set per
  // file.
...
static void cover_loaded_callback(int error, ddb_cover_query_t *query,
  ...
  if (!img && cover && cover->blob) {
    NSData *data =
        [NSData dataWithBytesNoCopy:cover->blob + cover->blob_image_offset
  ...
  if (!img && cover && cover->image_filename) {
    img = [[NSImage alloc]
        initWithContentsOfFile:[NSString
                                   stringWithUTF8String:cover->image_filename]];
  }
  if (!img) {
    img = [cm defaultCover];
  }
...
- (void)addCoverForTrack:(ddb_playItem_t *)track withImage:(NSImage *)img {
  ...
  NSMutableDictionary *d = [[NSMutableDictionary alloc]
      initWithObjectsAndKeys:hash, @"hash", t, @"ts", nil];
  if (img != nil) {
    d[@"img"] = img;
  }
