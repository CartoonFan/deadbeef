...
MPNowPlayingInfoPropertyPlaybackRate :
    ...
    [commandCenter.togglePlayPauseCommand
     addTarget:self
     action:@selector(togglePlayPauseCommand:)];
    ...
    [commandCenter.nextTrackCommand addTarget:self
                                    action:@selector(nextTrackCommand:)];
    commandCenter.nextTrackCommand.enabled = YES;

    [commandCenter.previousTrackCommand
     addTarget:self
     action:@selector(previousTrackCommand:)];
    commandCenter.previousTrackCommand.enabled = YES;

    [commandCenter.changePlaybackPositionCommand
     addTarget:self
     action:@selector(changePlaybackPositionCommand:)];
    ...
    block:^(NSTimer *_Nonnull timer) {
        ...
        if (it) {
            float duration =
                deadbeef->pl_get_item_duration(it);
            if (duration >= 0) {
                info[MPMediaItemPropertyPlaybackDuration] =
                    @((NSTimeInterval)duration);
                info[MPNowPlayingInfoPropertyElapsedPlaybackTime] =
                    @((NSTimeInterval)deadbeef
            ...
            // song info
            char text[1000];
            deadbeef->pl_get_meta(it, "title", text,
                                  sizeof(text));
            ...
            deadbeef->tf_eval(&ctx,
                              nowPlayable.artist_tf,
                              text, sizeof(text));
            info[MPMediaItemPropertyArtist] =
                [NSString stringWithUTF8String:text];
            deadbeef->tf_eval(&ctx, nowPlayable.album_tf,
                              text, sizeof(text));
...
- (MPRemoteCommandHandlerStatus)changePlaybackPositionCommand:
    (MPChangePlaybackPositionCommandEvent *)sender {
    deadbeef->sendmessage(DB_EV_SEEK, 0, (uint32_t)(sender.positionTime * 1000),
                          0);
    return MPRemoteCommandHandlerStatusSuccess;
}
