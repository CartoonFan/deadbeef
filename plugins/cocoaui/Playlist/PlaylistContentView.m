...
- (instancetype)initWithFrame:(NSRect)rect {
  ...
  [self
      registerForDraggedTypes:[NSArray arrayWithObjects:ddbPlaylistItemsUTIType,
                                                        ddbMedialibItemUTIType,
...
- (void)drawLineIndicator:(NSRect)dirtyRect yy:(int)yy {
  ...
  [NSBezierPath
      strokeLineFromPoint:NSMakePoint(dirtyRect.origin.x, yyline)
                  toPoint:NSMakePoint(dirtyRect.origin.x + dirtyRect.size.width,
                                      yyline)];
...
- (NSDragOperation)draggingUpdated:(id<NSDraggingInfo>)sender {
  ...
  _lastDragLocation = [self convertPoint:[sender draggingLocation]
                                fromView:nil];
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
  ...
  NSPoint draggingLocation = [self convertPoint:[sender draggingLocation]
                                       fromView:nil];
  ...
  if ([pboard.types containsObject:ddbPlaylistItemsUTIType]) {
    NSArray *classes = [[NSArray alloc]
        initWithObjects:[PlaylistLocalDragDropHolder class], nil];
    NSDictionary *options = [NSDictionary dictionary];
    NSArray<PlaylistLocalDragDropHolder *> *draggedItems =
    ...
    for (PlaylistLocalDragDropHolder *holder in draggedItems) {
      ...
      [delegate dropItems:(int)from_playlist
                   before:row
                  indices:indices
                    count:(int)length
                     copy:op == NSDragOperationCopy];
      free(indices);
    }
  }
  if ([pboard.types containsObject:ddbMedialibItemUTIType]) {
    NSArray *classes = [[NSArray alloc]
        initWithObjects:[MedialibItemDragDropHolder class], nil];
    NSDictionary *options = [NSDictionary dictionary];
    NSArray<MedialibItemDragDropHolder *> *draggedItems =
        [pboard readObjectsForClasses:classes options:options];
    ...
    if (row != [delegate invalidRow]) {
      ...
      DdbListviewRow_t lastRow =
          [delegate rowForIndex:([delegate rowCount] - 1)];
...
- (void)renderAlbumArtForGroup:(DdbListviewGroup_t *)grp
  ...
  for (DdbListviewCol_t col = [self.delegate firstColumn];
    ...
    if ([self.delegate isAlbumArtColumn:col] && x + w > clip.origin.x) {
      ...
      if (title_height > 0) {
        [delegate drawAlbumArtForGroup:grp
                            groupIndex:groupIndex
                              inColumn:col
                         isPinnedGroup:isPinnedGroup
                        nextGroupCoord:grp_next_y
                                  xPos:x
                                  yPos:y
                             viewportY:viewportY
                                 width:w
...
- (void)drawGroupTitle:(DdbListviewGroup_t *)grp
                 grp_y:(int)grp_y
...
- (void)updatePinnedGroup {
  ...
  if (self.delegate.pinGroups && grp && grp_y < scrollPos &&
      grp_y + grp->height >= scrollPos) {
    ...
    self.pinnedGroupTitleView.frame =
        NSMakeRect(0, pos, NSWidth(self.frame), self.grouptitle_height);
  } else {
...
- (void)drawListView:(NSRect)dirtyRect {
  ...
  while (grp && grp_y < clip_y + clip_h) {
    ...
    for (int i = 0, yy = grp_y + title_height;
         it && i < grp->num_items && yy < clip_y + clip_h;
         i++, yy += rowheight) {
      ii++;

      if (yy + rowheight >= clip_y) {
        // draw row
        NSColor *clr = [NSColor.controlAlternatingRowBackgroundColors
            objectAtIndex:ii % 2];
        [clr set];
        [NSBezierPath fillRect:NSMakeRect(dirtyRect.origin.x, yy,
                                          dirtyRect.size.width, rowheight)];
        ...
        for (DdbListviewCol_t col = [self.delegate firstColumn];
             ...
             col = [self.delegate nextColumn:col]) {
          int w = [self.delegate columnWidth:col];
          if (CGRectIntersectsRect(dirtyRect,
                                   NSMakeRect(x, yy, w, rowheight))) {
            [self.delegate drawCell:idx + i
                             forRow:it
                          forColumn:col
                             inRect:NSMakeRect(x, yy, w, rowheight - 1)
                            focused:focused];
          }
          x += w;
        }
        ...
        if (x < dirtyRect.size.width) {
          [self.delegate drawCell:idx + i
                           forRow:it
                        forColumn:[delegate invalidColumn]
                           inRect:NSMakeRect(x, yy, dirtyRect.size.width - x,
                                             rowheight - 1)
                          focused:focused];
        }
        ...
        if (it == cursor_it) {
          [[NSGraphicsContext currentContext] saveGraphicsState];
          NSRect rect = NSMakeRect(self.frame.origin.x + 0.5, yy + 0.5,
                                   self.frame.size.width - 1, rowheight - 1);
    ...
    // draw album art
    int grp_next_y = grp_y + grp->height;
    [self renderAlbumArtForGroup:grp
                      groupIndex:groupIndex
                   isPinnedGroup:NO
                  nextGroupCoord:grp_next_y
                            yPos:grp_y + title_height
                       viewportY:dirtyRect.origin.y
                      clipRegion:dirtyRect];
  ...
  //        self.pinnedGroupTitleView.hidden = NO;
  //    }
  //        [self drawGroupTitle:pin_grp grp_y:pinnedGrpPos
    ...
    while (y < dirtyRect.origin.y + dirtyRect.size.height) {
      if (y + rowheight >= dirtyRect.origin.y) {
        NSColor *clr = [NSColor.controlAlternatingRowBackgroundColors
            objectAtIndex:ii % 2];
        [clr set];
        [NSBezierPath fillRect:NSMakeRect(dirtyRect.origin.x, y,
                                          dirtyRect.size.width, rowheight)];
      }
      y += rowheight;
      ii++;
...
- (void)mouseDown:(NSEvent *)event {
  ...
  if (event.buttonNumber != 0 ||
    ...
    [self clickSelection:convPt
                     grp:grp
               grp_index:grp_index
                     sel:sel
                     dnd:YES
                  button:1];
  } else if (event.buttonNumber == 0 &&
    ...
    int idx = 0;
    for (DdbListviewRow_t it = [delegate firstRow]; it != [delegate invalidRow];
         idx++) {
...
- (void)mouseDragged:(NSEvent *)event {
  ...
  dragLocation = [self convertPoint:[event locationInWindow] fromView:nil];
...
- (void)listMouseDragged:(NSEvent *)event {
  ...
  if (_dragwait) {
    if (fabs(_lastpos.x - pt.x) > 3 || fabs(_lastpos.y - pt.y) > 3) {
      ...
      // Need playlist identifier and all playlist items when dragging
      // internally, this is represented with the DdbListviewLocalDragDropHolder
      // interface

      pboard = [NSPasteboard pasteboardWithName:NSDragPboard];
      ddb_playlist_t *plt = deadbeef->plt_get_curr();
      PlaylistLocalDragDropHolder *data = [[PlaylistLocalDragDropHolder alloc]
          initWithSelectedItemsOfPlaylist:plt];
      deadbeef->plt_unref(plt);
      [pboard declareTypes:[NSArray arrayWithObject:ddbPlaylistItemsUTIType]
                     owner:self];
    ...
    int grp_index;
    int sel;
    if ([self pickPoint:pt.y group:&grp groupIndex:&grp_index
                  index:&sel] == -1) {
...
#define NUM_CHANGED_ROWS_BEFORE_FULL_REDRAW 10
      ...
      if (nchanged >= NUM_CHANGED_ROWS_BEFORE_FULL_REDRAW) {
        self.needsDisplay = YES;
        [_delegate selectionChanged:it]; // that means "selection changed a lot,
                                         // redraw everything
      }
...
- (void)clickSelection:(NSPoint)pt
                   grp:(DdbListviewGroup_t *)grp
             grp_index:(int)grp_index
                   sel:(int)sel
                   dnd:(BOOL)dnd
                button:(int)button {
  ...
  if (sel == -1 && !album_art_column &&
      (!grp || (pt.y > _grouptitle_height && grp_index >= grp->num_items))) {
    // clicked empty space, deselect everything
    [self deselectAll];
  } else if ((sel != -1 && grp && grp_index == -1) ||
             (pt.y <= _grouptitle_height + vis.origin.y &&
              [_delegate pinGroups]) ||
             album_art_column) {
    ...
    if (it == [_delegate invalidRow] || ![_delegate rowSelected:it] ||
        (![_delegate hasDND] &&
         button ==
             1)) // HACK: don't reset selection by right click in search window
    {
  ...
  while (grp) {
    ...
    if (y >= grp_y - _grouptitle_height / 2 && y < grp_y + h - rowheight / 2) {
      ...
      // within group
                       ...
                       rowheight / 2) {
        return idx +
               (int)((y - _grouptitle_height + rowheight / 2) / rowheight);
      }
...
- (int)pickPoint:(CGFloat)y
           group:(DdbListviewGroup_t **)group
      groupIndex:(int *)group_idx
...
- (void)keyDown:(NSEvent *)theEvent {
  ...
  if ([theArrow length] == 1) {
    ...
    case NSPageDownFunctionKey: {
      [self
          scrollPoint:NSMakePoint(vis.origin.x,
                                  vis.origin.y + vis.size.height - rowheight)];
      break;
    }
    ...
    case NSEndFunctionKey:
      [self scrollPoint:NSMakePoint(vis.origin.x, (self.frame.size.height -
...
- (void)initGroups {
  ...
  int min_height = 0;
  for (DdbListviewCol_t c = [_delegate firstColumn];
       c != [_delegate invalidColumn]; c = [_delegate nextColumn:c]) {
    if ([_delegate columnMinHeight:c] &&
        [_delegate columnWidth:c] > min_height) {
  ...
  while (it != [_delegate invalidRow]) {
    ...
    if (!grp || (!curr && grp->num_items >= BLANK_GROUP_SUBDIVISION) ||
        (curr && [str isNotEqualTo:curr])) {
      str = curr;
...
- (int)getDrawInfo:(int)row
             group:(DdbListviewGroup_t **)pgrp
            isEven:(int *)even
            cursor:(int *)cursor
           group_y:(int *)group_y
              rect:(NSRect *)rect {
  ...
  int totalwidth = 0;
  for (DdbListviewCol_t col = [_delegate firstColumn];
       col != [_delegate invalidColumn]; col = [_delegate nextColumn:col]) {
