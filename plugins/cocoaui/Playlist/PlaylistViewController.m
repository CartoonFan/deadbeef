...
@interface PlaylistViewController () <DdbListviewDelegate,
                                      TrackContextMenuDelegate>
...
@property(nonatomic)
    GroupByCustomWindowController *groupByCustomWindowController;
...
- (void)cleanup {
  ...
  // this would cause deadbeef's track refcount checker to run before the
  // objects are really released
  @autoreleasepool {
...
- (void)menuAddColumn:(id)sender {
  if (!self.editColumnWindowController) {
    self.editColumnWindowController = [[EditColumnWindowController alloc]
        initWithWindowNibName:@"EditColumnPanel"];
  }

  [self.editColumnWindowController initAddColumnSheet];

  [self.view.window beginSheet:self.editColumnWindowController.window
...
- (void)menuEditColumn:(id)sender {
  ...
  NSColor *color = [NSColor colorWithDeviceRed:c[0] / 255.f
                                         green:c[1] / 255.f
                                          blue:c[2] / 255.f
                                         alpha:c[3] / 255.f];

  [self.editColumnWindowController
      initEditColumnSheetWithTitle:
          [NSString stringWithUTF8String:self.columns[self.menuColumn].title]
                              type:self.columns[self.menuColumn].type
                            format:[NSString
                                       stringWithUTF8String:
                                           self.columns[self.menuColumn].format]
...
- (void)menuGroupByCustom:(id)sender {
  if (!self.groupByCustomWindowController) {
    self.groupByCustomWindowController = [[GroupByCustomWindowController alloc]
        initWithWindowNibName:@"GroupByCustomWindow"];
  }

  char buf[1000];
  deadbeef->conf_get_str(self.groupByConfStr, "", buf, sizeof(buf));
  [self.groupByCustomWindowController
      initWithFormat:[NSString stringWithUTF8String:buf]];

  [self.view.window
             beginSheet:self.groupByCustomWindowController.window
      completionHandler:^(NSModalResponse returnCode) {
        if (returnCode == NSModalResponseOK) {
          [self clearGrouping];
          self.groupStr =
              self.groupByCustomWindowController.formatTextField.stringValue;
          deadbeef->conf_set_str([self groupByConfStr],
                                 self.groupStr.UTF8String);
          PlaylistView *lv = (PlaylistView *)self.view;
          [lv.contentView reloadData];
        }
      }];
...
- (void)columnsChanged {
  ...
  NSMutableArray *columns =
      [[NSMutableArray alloc] initWithCapacity:self.ncolumns];
  for (int i = 0; i < self.ncolumns; i++) {
    ...
    NSDictionary *dict = [[NSDictionary alloc]
        initWithObjectsAndKeys:
            ...
            [NSNumber numberWithInt:self.columns[i].set_text_color],
            @"set_text_color",
            [NSString stringWithFormat:@"#%02x%02x%02x%02x", col[3], col[0],
                                       col[1], col[2]],
            @"text_color", nil];
  ...
  NSError *err = nil;
  NSData *dt = [NSJSONSerialization dataWithJSONObject:columns
                                               options:0
                                                 error:&err];

  [lv.contentView updateContentFrame];

  NSString *json = [[NSString alloc] initWithData:dt
...
- (void)updateColumn:(int)idx {
  CGFloat r, g, b, a;
  NSColor *color = [self.editColumnWindowController.colorWell.color
      colorUsingColorSpace:[NSColorSpace deviceRGBColorSpace]];
  [color getRed:&r green:&g blue:&b alpha:&a];

  uint8_t rgba[] = {(uint8_t)(r * 255), (uint8_t)(g * 255), (uint8_t)(b * 255),
                    (uint8_t)(a * 255)};

  int type = -1;

  NSInteger typeIndex =
      self.editColumnWindowController.typePopUpButton.indexOfSelectedItem;
  switch (typeIndex) {
  case 0:
  ...
  case 2:
    ...
    break;
  case 3: // artist / album
    self.editColumnWindowController.formatTextField.stringValue =
        @"$if(%album artist%,%album artist%,Unknown Artist)[ - %album%]";
    break;
  case 4: // artist
    self.editColumnWindowController.formatTextField.stringValue =
  ...
  [self initColumn:idx
          withTitle:self.editColumnWindowController.titleTextField.stringValue
                        .UTF8String
             withId:(int)type
           withSize:self.columns[idx].size
         withFormat:self.editColumnWindowController.formatTextField.stringValue
                        .UTF8String
      withAlignment:(int)self.editColumnWindowController.alignmentPopUpButton
                        .indexOfSelectedItem
       withSetColor:(self.editColumnWindowController.setColorButton.state ==
                     NSOnState)
          withColor:rgba];
  [self columnsChanged];
}
...
#define DEFAULT_COLUMNS                                                        \
  "[{\"title\":\"Playing\", \"id\":\"1\", \"format\":\"%playstatus%\", "       \
  "\"size\":\"50\"}, {\"title\":\"Artist / Album\", \"format\":\"$if(%album "  \
  "artist%,%album artist%,Unknown Artist)[ - %album%]\", \"size\":\"150\"}, "  \
  "{\"title\":\"Track Nr\", \"format\":\"%track number%\", \"size\":\"50\"}, " \
  "{\"title\":\"Title / Track Artist\", \"format\":\"%title%[ // %track "      \
  "artist%]\", \"size\":\"150\"}, {\"title\":\"Length\", "                     \
  "\"format\":\"%length%\", \"size\":\"50\"}]"
...
- (void)initContent {
  ...
  self.groupTextAttrsDictionary = [NSDictionary
      dictionaryWithObjectsAndKeys:
          [NSFont
              boldSystemFontOfSize:[NSFont
                                       systemFontSizeForControlSize:rowheight]],
          NSFontAttributeName, [NSNumber numberWithFloat:0],
          NSBaselineOffsetAttributeName, NSColor.controlTextColor,
          NSForegroundColorAttributeName, textStyle,
          NSParagraphStyleAttributeName, nil];

  self.cellTextAttrsDictionary = [NSDictionary
      dictionaryWithObjectsAndKeys:
          [NSFont controlContentFontOfSize:
                      [NSFont systemFontSizeForControlSize:rowheight]],
          NSFontAttributeName, [NSNumber numberWithFloat:0],
          NSBaselineOffsetAttributeName, NSColor.controlTextColor,
          NSForegroundColorAttributeName, textStyle,
          NSParagraphStyleAttributeName, nil];

  self.cellSelectedTextAttrsDictionary = [NSDictionary
      dictionaryWithObjectsAndKeys:
          [NSFont controlContentFontOfSize:
                      [NSFont systemFontSizeForControlSize:rowheight]],
          NSFontAttributeName, [NSNumber numberWithFloat:0],
...
- (int)insertColumn:(int)beforeIdx {
  if (self.ncolumns >= self.columnsAllocated) {
    ...
    self.columns =
        realloc(self.columns, sizeof(plt_col_info_t) * self.columnsAllocated);
  }

  if (beforeIdx >= 0 && beforeIdx < self.ncolumns) {
    memmove(&self.columns[beforeIdx + 1], &self.columns[beforeIdx],
            (self.ncolumns - beforeIdx) * sizeof(plt_col_info_t));
  }
  self.ncolumns++;
...
- (void)initColumn:(int)idx
         withTitle:(const char *)title
            withId:(int)_id
          withSize:(int)size
        withFormat:(const char *)format
     withAlignment:(int)alignment
      withSetColor:(BOOL)setColor
...
- (void)removeColumnAtIndex:(int)idx {
  ...
  if (idx != self.ncolumns - 1) {
    memmove(&self.columns[idx], &self.columns[idx + 1],
            (self.ncolumns - idx) * sizeof(plt_col_info_t));
  }
  self.ncolumns--;
...
- (NSMenu *)contextMenuForColumn:(DdbListviewCol_t)col
  ...
  [menu insertItemWithTitle:@"Add Column"
                     action:@selector(menuAddColumn:)
              keyEquivalent:@""
                    atIndex:0]
      .target = self;
  if (col != -1) {
    [menu insertItemWithTitle:@"Edit Column"
                       action:@selector(menuEditColumn:)
                keyEquivalent:@""
                      atIndex:1]
        .target = self;
    [menu insertItemWithTitle:@"Remove Column"
                       action:@selector(menuRemoveColumn:)
                keyEquivalent:@""
                      atIndex:2]
        .target = self;
    NSMenuItem *item = [menu insertItemWithTitle:@"Pin Groups When Scrolling"
                                          action:@selector(menuTogglePinGroups:)
                                   keyEquivalent:@""
                                         atIndex:3];
    ...
    [groupBy insertItemWithTitle:@"None"
                          action:@selector(menuGroupByNone:)
                   keyEquivalent:@""
                         atIndex:0]
        .target = self;
    [groupBy insertItemWithTitle:@"Artist/Date/Album"
                          action:@selector(menuGroupByArtistDateAlbum:)
                   keyEquivalent:@""
                         atIndex:1]
        .target = self;
    [groupBy insertItemWithTitle:@"Artist"
                          action:@selector(menuGroupByArtist:)
                   keyEquivalent:@""
                         atIndex:2]
        .target = self;
    [groupBy insertItemWithTitle:@"Custom"
                          action:@selector(menuGroupByCustom:)
                   keyEquivalent:@""
                         atIndex:3];

    NSMenuItem *groupByItem = [[NSMenuItem alloc] initWithTitle:@"Group By"
                                                         action:nil
                                                  keyEquivalent:@""];
...
- (void)loadColumns:(NSArray *)cols {
  ...
  [cols enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
    ...
    [self initColumn:colIdx
            withTitle:title
               withId:colId
             withSize:size
           withFormat:fmt
        withAlignment:alignment
         withSetColor:setcolor
            withColor:rgba];
  }];
}
...
- (void)activate:(int)idx {
  DB_playItem_t *it =
      deadbeef->pl_get_for_idx_and_iter(idx, [self playlistIter]);
  if (it) {
...
- (DdbListviewRow_t)nextRow:(DdbListviewRow_t)row {
  return (DdbListviewRow_t)deadbeef->pl_get_next((DB_playItem_t *)row,
                                                 [self playlistIter]);
}
...
- (DdbListviewRow_t)rowForIndex:(int)idx {
  return (DdbListviewRow_t)deadbeef->pl_get_for_idx_and_iter(
      idx, [self playlistIter]);
}
...
- (NSMutableAttributedString *)
    stringWithTintAttributesFromString:(const char *)inputString
                     initialAttributes:(NSDictionary *)attributes
  ...
  numTintRanges = calculate_tint_ranges_from_string(
      inputString, tintRanges, maxTintRanges, &plainString);

  NSMutableAttributedString *str = [[NSMutableAttributedString alloc]
      initWithString:[NSString stringWithUTF8String:plainString]
  ...
  for (NSUInteger i = 0; i < numTintRanges / 2; i++) {
    ...
    NSColor *tinted =
        [backgroundColor blendedColorWithFraction:blend
                                          ofColor:foregroundColor];

    [str addAttributes:@{NSForegroundColorAttributeName : tinted}
...
- (void)drawCell:(int)idx
  ...
  if (self.columns[col].type == DB_COLUMN_PLAYING && playing_track &&
      (DB_playItem_t *)row == playing_track) {
    NSImage *img = NULL;
    ...
    NSColor *imgColor = sel ? NSColor.alternateSelectedControlTextColor
    ...
    maskRect.origin = NSMakePoint(
        rect.origin.x + rect.size.width / 2 - maskRect.size.width / 2,
        rect.origin.y + rect.size.height / 2 - maskRect.size.height / 2);

    CGImageRef maskImage =
        [img CGImageForProposedRect:&maskRect
  ...
  if (self.columns[col].bytecode) {
    ...
    if (text[0]) {
      NSDictionary *attributes = sel ? self.cellSelectedTextAttrsDictionary
                                     : self.cellTextAttrsDictionary;
      NSColor *foreground = attributes[NSForegroundColorAttributeName];

      NSMutableAttributedString *attrString =
          [self stringWithTintAttributesFromString:text
    ...
    if (ctx.update > 0) {
      ...
      self.playPosUpdateTrack =
          deadbeef->pl_get_for_idx_and_iter(ctx.idx, [self playlistIter]);

      self.playPosUpdateTimer = [NSTimer
          scheduledTimerWithTimeInterval:ctx.update / 1000.0
                                 repeats:NO
                                   block:^(NSTimer *_Nonnull timer) {
                                     ddb_playlist_t *curr =
                                         deadbeef->plt_get_curr();
                                     DB_playItem_t *trk =
                                         deadbeef->pl_get_for_idx_and_iter(
                                             ctx.idx, [self playlistIter]);

                                     if (ctx.plt == curr &&
                                         trk == self.playPosUpdateTrack) {
                                       PlaylistView *lv =
                                           (PlaylistView *)self.view;
...
- (void)drawGroupTitle:(DdbListviewRow_t)row inRect:(NSRect)rect {
  ...
  NSMutableAttributedString *attrString = [self
      stringWithTintAttributesFromString:text
                       initialAttributes:self.groupTextAttrsDictionary
                         foregroundColor:self.groupTextAttrsDictionary
  ...
  strRect.origin.x += 5;
  strRect.origin.y =
      strRect.origin.y + strRect.size.height / 2 - size.height / 2;
  ...
  if (size.width < rect.size.width - 15) {
    [NSBezierPath fillRect:NSMakeRect(size.width + 10,
                                      rect.origin.y + rect.size.height / 2,
                                      rect.size.width - size.width - 15, 1)];
  }
}
...
static void coverAvailCallback(NSImage *img, void *user_data) {
  ...
  PlaylistViewController *ctl =
      (__bridge_transfer PlaylistViewController *)info->ctl;
...
- (void)drawAlbumArtForGroup:(DdbListviewGroup_t *)grp
  ...
  inf->grp = groupIndex;
  NSImage *image =
      [[CoverManager defaultCoverManager] getCoverForTrack:it
                                     withCallbackWhenReady:coverAvailCallback
                                   withUserDataForCallback:inf];
  if (!image) {
    // FIXME: the problem here is that if the cover is not found (yet) -- it
    // won't draw anything, but the rect is already invalidated, and will come
    // out as background color
    return;
  }

  int art_width = width - ART_PADDING_HORZ * 2;
  int art_height =
      height - ART_PADDING_VERT * 2 - lv.contentView.grouptitle_height;
  ...
  CGFloat min_y = (pinned ? viewportY + lv.contentView.grouptitle_height : y) +
                  ART_PADDING_VERT;
...
- (void)selectionChanged:(DdbListviewRow_t)row {
  PlaylistView *lv = (PlaylistView *)self.view;
  deadbeef->sendmessage(DB_EV_PLAYLISTCHANGED, (uintptr_t)lv,
...
- (void)songChanged:(PlaylistView *)listview
               from:(DB_playItem_t *)from
                 to:(DB_playItem_t *)to {
  int to_idx = -1;
  if (to) {
    int cursor_follows_playback =
        deadbeef->conf_get_int("playlist.scroll.cursorfollowplayback", 1);
    int scroll_follows_playback =
...
- (void)sortColumn:(DdbListviewCol_t)column {
  ...
  deadbeef->plt_sort_v2(plt, PL_MAIN, c->type, c->format,
                        self.columns[column].sort_order);
  deadbeef->plt_unref(plt);
...
- (void)dropItems:(int)from_playlist
  ...
  if (copy) {
    deadbeef->plt_copy_items(plt, PL_MAIN, from, (DB_playItem_t *)before,
                             indices, count);
  } else {
    deadbeef->plt_move_items(plt, PL_MAIN, from, (DB_playItem_t *)before,
                             indices, count);
  }
    ...
    dispatch_async(aQueue, ^{
      ...
      for (NSUInteger i = 0; i < paths.count; i++) {
        ...
        if (path) {
          ...
          DB_playItem_t *inserted = deadbeef->plt_insert_dir2(
              0, plt, after, fname, &abort, NULL, NULL);
          if (!inserted && !abort) {
            inserted = deadbeef->plt_insert_file2(0, plt, after, fname, &abort,
                                                  NULL, NULL);
            if (!inserted && !abort) {
              inserted =
                  deadbeef->plt_load2(0, plt, after, fname, &abort, NULL, NULL);
            }
          }
...
- (void)dropPlayItems:(DdbListviewRow_t *)items
  ...
  ddb_playItem_t *after =
      before ? deadbeef->pl_get_prev((ddb_playItem_t *)before, PL_MAIN)
             : deadbeef->plt_get_tail_item(plt, PL_MAIN);
...
- (void)trackProperties {
  if (!self.trkProperties) {
    self.trkProperties = [[TrackPropertiesWindowController alloc]
        initWithWindowNibName:@"TrackProperties"];
  }
  ddb_playlist_t *plt = deadbeef->plt_get_curr();
