...
- (instancetype)init {
    ...
    self.reloadMetadataItem = [self insertItemWithTitle:@"Reload metadata"
                                    action:@selector(reloadMetadata)
                                    keyEquivalent:@""
                                    atIndex:0];
    NSMenu *rgMenu = [[NSMenu alloc] initWithTitle:@"ReplayGain"];
    rgMenu.autoenablesItems = NO;

    self.rgScanPerFileItem = [rgMenu addItemWithTitle:@"Scan Per-file Track Gain"
                                     action:@selector(rgScanTracks:)
                                     keyEquivalent:@""];
    self.rgScanPerFileItem.target = self;
    self.rgScanAsSingleAlbumItem =
        [rgMenu addItemWithTitle:@"Scan Selection As Single Album"
                action:@selector(rgScanAlbum:)
                keyEquivalent:@""];
    self.rgScanAsSingleAlbumItem.target = self;
    self.rgScanAsAlbumsItem =
        [rgMenu addItemWithTitle:@"Scan Selection As Albums (By Tags)"
                action:@selector(rgScanAlbumsAuto:)
                keyEquivalent:@""];
    self.rgScanAsAlbumsItem.target = self;
    self.rgRemoveInformationItem =
        [rgMenu addItemWithTitle:@"Remove ReplayGain Information"
                action:@selector(rgRemove:)
                keyEquivalent:@""];
    self.rgScanAsAlbumsItem.target = self;

    self.rgMenuItem = [[NSMenuItem alloc] initWithTitle:@"ReplayGain"
                                          action:nil
                                          keyEquivalent:@""];
    self.rgMenuItem.submenu = rgMenu;
    [self addItem:self.rgMenuItem];

    self.addToQueueItem = [self addItemWithTitle:@"Add To Playback Queue"
                                action:@selector(addToPlaybackQueue)
                                keyEquivalent:@""];
    self.addToQueueItem.target = self;

    self.removeFromQueueItem =
        [self addItemWithTitle:@"Remove From Playback Queue"
              action:@selector(removeFromPlaybackQueue)
              keyEquivalent:@""];
    ...
    self.convertItem = [self addItemWithTitle:@"Convert"
                             action:@selector(convertSelection)
                             keyEquivalent:@""];
    self.convertItem.target = self;

    self.deleteFromDiskItem = [self addItemWithTitle:@"Delete From Disk"
                                    action:@selector(deleteFromDisk)
                                    keyEquivalent:@""];
    ...
    self.trackPropertiesItem = [self addItemWithTitle:@"Track Properties"
                                     action:@selector(trackProperties)
                                     keyEquivalent:@""];
...
- (void)updateWithTrackList:(ddb_playItem_t **)tracks
    ...
    if (tracks) {
        _selectedTracksList = ddbUtilTrackListInitWithWithTracks(
                                  ddbUtilTrackListAlloc(), plt, DDB_ACTION_CTX_SELECTION, tracks,
                                  (unsigned)count);
    }
}
...
- (void)convertSelection {
    ...
    [ConverterWindowController runConverterWithTracks:tracks
                               count:count
...
_warningMessageForCtx(ddbDeleteFromDiskController_t ctl,
    ...
    [menu deleteFromDiskWarningMessage:ctl
          ctx:ctx
...
static int _deleteFile(ddbDeleteFromDiskController_t ctl, const char *uri) {
    ...
    if (deadbeef->conf_get_int("cocoaui.delete_use_bin", 1)) {
        [NSWorkspace.sharedWorkspace
         recycleURLs:@[ url ]
         completionHandler:^(NSDictionary<NSURL *, NSURL *> *_Nonnull newURLs,
...
- (void)deleteFromDiskWarningMessage:(ddbDeleteFromDiskController_t)ctl
    ctx:(ddb_action_context_t)ctx
    trackCount:(unsigned)trackcount
    callback:
    (ddbDeleteFromDiskControllerWarningCallback_t)
    callback {
    if (self.view != nil &&
            deadbeef->conf_get_int("cocoaui.delete_files_confirm", 1)) {
        NSString *buf;
        NSString *buf2 = deadbeef->conf_get_int("cocoaui.delete_use_bin", 1)
                         ? @" The files will be moved to Recycle Bin.\n\n(This "
                         @"dialog can be turned off in the Preferences)"
        ...
        if (ctx == DDB_ACTION_CTX_SELECTION) {
            ...
            if (selected_files == 1) {
                buf = [NSString
                       stringWithFormat:
                       @"Do you really want to delete the selected file?%@", buf2];
            } else {
                buf = [NSString
                       stringWithFormat:
                       @"Do you really want to delete all %d selected files?%@",
                       selected_files, buf2];
            }
        } else if (ctx == DDB_ACTION_CTX_PLAYLIST) {
            int files = trackcount;
            [NSString stringWithFormat:@"Do you really want to delete all %d files "
                      @"from the current playlist?%@",
                      files, buf2];
        } else if (ctx == DDB_ACTION_CTX_NOWPLAYING) {
            [NSString
             stringWithFormat:
             @"Do you really want to delete the currently playing file?%@",
             buf2];
        }
...
- (void)deleteFromDisk {
    ...
    if (self.selectedTracksList) {
        self.deleteFromDiskController =
            ddbDeleteFromDiskControllerInitWithTrackList(
                self.deleteFromDiskController, self.selectedTracksList);
    } else {
        self.deleteFromDiskController = ddbDeleteFromDiskControllerInitWithPlaylist(
                                            ddbDeleteFromDiskControllerAlloc(), plt, ctx);
    }

    ddbDeleteFromDiskControllerSetShouldSkipDeletedTracks(
        self.deleteFromDiskController,
        deadbeef->conf_get_int("cocoaui.skip_deleted_tracks", 0));
    ddbDeleteFromDiskControllerSetUserData(self.deleteFromDiskController,
                                           (__bridge void *)(self));

    ddbDeleteFromDiskControllerRunWithDelegate(self.deleteFromDiskController,
            delegate);
