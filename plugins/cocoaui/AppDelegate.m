...
- (void)configChanged {
  NSMenuItem *shuffle_items[] = {_orderLinear, _orderShuffle, _orderRandom,
                                 _orderShuffleAlbums, nil};
  ...
  NSMenuItem *repeat_items[] = {_loopAll, _loopNone, _loopSingle, nil};
  ...
  _scrollFollowsPlayback.state =
      deadbeef->conf_get_int("playlist.scroll.followplayback", 1) ? NSOnState
                                                                  : NSOffState;
  _cursorFollowsPlayback.state =
      deadbeef->conf_get_int("playlist.scroll.cursorfollowplayback", 1)
          ? NSOnState
          : NSOffState;

  _stopAfterCurrent.state =
      deadbeef->conf_get_int("playlist.stop_after_current", 0) ? NSOnState
                                                               : NSOffState;
  _stopAfterCurrentAlbum.state =
      deadbeef->conf_get_int("playlist.stop_after_album", 0) ? NSOnState
                                                             : NSOffState;

  _descendingSortMode.state =
      deadbeef->conf_get_int("cocoaui.sort_desc", 0) ? NSOnState : NSOffState;
...
static void fileadd_begin(ddb_fileadd_data_t *data, void *user_data) {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    [g_appDelegate.mainWindow.window beginSheet:g_appDelegate.addFilesWindow
...
static void fileadd_end(ddb_fileadd_data_t *data, void *user_data) {
  dispatch_async(dispatch_get_main_queue(), ^{
    [g_appDelegate.mainWindow.window endSheet:g_appDelegate.addFilesWindow
...
static int file_added(ddb_fileadd_data_t *data, void *user_data) {
  ...
  if (!_settingLabel) {
    ...
    // but we also want to avoid sending multiple labels, becuase that's
    // meaningless. So we use a basic flag, to see if the label is being set
    // already.
...
- (void)initMainWindow {
  _mainWindow =
      [[MainWindowController alloc] initWithWindowNibName:@"MainWindow"];
...
- (void)initSearchWindow {
  _searchWindow =
      [[SearchWindowController alloc] initWithWindowNibName:@"Search"];
...
- (void)initMainMenu {
  // add new actions
  [self.mainMenu
      addActionItemsForContext:DDB_ACTION_CTX_MAIN
                         track:nil
                        filter:^BOOL(DB_plugin_action_t *_Nonnull action) {
                          return (action->flags &
                                  (DB_ACTION_COMMON | DB_ACTION_ADD_MENU)) ==
                                 (DB_ACTION_COMMON | DB_ACTION_ADD_MENU);
                        }];
}
...
- (void)initEqualizerWindow {
  if (!_equalizerWindow) {
    _equalizerWindow = [[EqualizerWindowController alloc]
        initWithWindowNibName:@"EqualizerWindowController"];
...
- (NSApplicationTerminateReply)applicationShouldTerminate:
  ...
  if (deadbeef->have_background_jobs()) {
    ...
    alert.informativeText = @"DeaDBeeF is currently running background tasks. "
                            @"If you quit now, the tasks will be cancelled or "
                            @"interrupted. This may result in data loss.";
...
extern void main_cleanup_and_quit(void);
...
- (void)bind {
  [_mainWindowToggleMenuItem bind:@"state"
                         toObject:_mainWindow.window
                      withKeyPath:@"visible"
                          options:nil];
  [_logWindowToggleMenuItem bind:@"state"
                        toObject:_logWindow.window
                     withKeyPath:@"visible"
                         options:nil];
  [_equalizerWindowToggleMenuItem bind:@"state"
                              toObject:_equalizerWindow.window
                           withKeyPath:@"visible"
                               options:nil];
  [_equalizerMenuItem
             bind:@"hidden"
         toObject:self
      withKeyPath:@"equalizerAvailable"
          options:@{
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
  // high sierra would terminate the app on SIGPIPE by default, which breaks
...
- (void)application:(NSApplication *)sender openFiles:(NSArray *)filenames {
  ...
  dispatch_async(aQueue, ^{
    char str[100];
    // building single paths string for the deadbeef function, paths must be
    // separated by '\0'
        ...
        if (returnCode == NSModalResponseOK) {
          ...
          if (!deadbeef->plt_add_files_begin(plt, 0)) {
            ...
            dispatch_async(aQueue, ^{
              ...
              deadbeef->plt_insert_file2(0, plt, tail, [text UTF8String], NULL,
                                         NULL, NULL);
              if (tail) {
...
- (IBAction)addLocationCancelAction:(id)sender {
  [_mainWindow.window endSheet:_addLocationPanel
...
- (void)sortPlaylistByTF:(const char *)tf {
  ddb_playlist_t *plt = deadbeef->plt_get_curr();
  deadbeef->plt_sort_v2(plt, PL_MAIN, -1, tf,
                        _descendingSortMode.state == NSOffState
                            ? DDB_SORT_ASCENDING
                            : DDB_SORT_DESCENDING);
...
- (IBAction)sortPlaylistCustom:(id)sender {
  deadbeef->pl_lock();
  _customSortEntry.stringValue = [NSString
      stringWithUTF8String:deadbeef->conf_get_str_fast("cocoaui.custom_sort_tf",
                                                       "")];
  deadbeef->pl_unlock();
  _customSortDescending.state =
      deadbeef->conf_get_int("cocoaui.sort_desc", 0) ? NSOnState : NSOffState;
  [_mainWindow.window
             beginSheet:_customSortPanel
      completionHandler:^(NSModalResponse returnCode) {
        ...
        deadbeef->conf_set_str("cocoaui.custom_sort_tf",
                               [[_customSortEntry stringValue] UTF8String]);
...
- (IBAction)customSortCancelAction:(id)sender {
  [_mainWindow.window endSheet:_customSortPanel
                    returnCode:NSModalResponseCancel];
}
...
+ (int)ddb_message:(int)_id ctx:(uint64_t)ctx p1:(uint32_t)p1 p2:(uint32_t)p2 {
  [[DdbWidgetManager defaultWidgetManager] widgetMessage:_id
                                                     ctx:ctx
                                                      p1:p1
                                                      p2:p2];

  [g_appDelegate.mainWindow.rootViewController sendMessage:_id
                                                       ctx:ctx
                                                        p1:p1
                                                        p2:p2];
  [(DesignableViewController *)(g_appDelegate.searchWindow.viewController)
      sendMessage:_id
              ctx:ctx
               p1:p1
               p2:p2];
  ...
  if (_id == DB_EV_CONFIGCHANGED) {
    [g_appDelegate performSelectorOnMainThread:@selector(configChanged)
                                    withObject:nil
                                 waitUntilDone:NO];
  } else if (_id == DB_EV_SONGSTARTED) {
    [g_appDelegate performSelectorOnMainThread:@selector(updateDockNowPlaying)
                                    withObject:nil
                                 waitUntilDone:NO];
  } else if (_id == DB_EV_SONGFINISHED) {
    [g_appDelegate performSelectorOnMainThread:@selector(clearDockNowPlaying)
                                    withObject:nil
                                 waitUntilDone:NO];
  } else if (_id == DB_EV_SONGCHANGED || _id == DB_EV_TRACKINFOCHANGED ||
             (_id == DB_EV_PLAYLISTCHANGED &&
              p1 == DDB_PLAYLIST_CHANGE_CONTENT)) {
    [g_appDelegate performSelectorOnMainThread:@selector(updateTitleBar)
                                    withObject:nil
                                 waitUntilDone:NO];
  } else if (_id == DB_EV_VOLUMECHANGED) {
    [g_appDelegate performSelectorOnMainThread:@selector(volumeChanged)
                                    withObject:nil
                                 waitUntilDone:NO];
  } else if (_id == DB_EV_OUTPUTCHANGED) {
    [g_appDelegate performSelectorOnMainThread:@selector(outputDeviceChanged)
                                    withObject:nil
                                 waitUntilDone:NO];
  }
...
- (void)clearDockNowPlaying {
  if (_dockMenuNPHeading) {
    ...
    _dockMenuNPHeading = _dockMenuNPTitle = _dockMenuNPArtistAlbum =
        _dockMenuNPSeparator = nil;
  }
}
...
- (void)updateDockNowPlaying {
  ...
  if (!_dockMenuNPHeading) {
    _dockMenuNPHeading = [[NSMenuItem alloc] initWithTitle:@"Now playing:"
                                                    action:nil
                                             keyEquivalent:@""];
  ...
  _dockMenuNPArtistAlbum = [[NSMenuItem alloc]
      initWithTitle:[NSString stringWithUTF8String:artistAlbum]
             action:nil
      keyEquivalent:@""];
  ...
  if ([openDlg runModal] == NSModalResponseOK) {
    ...
    dispatch_async(aQueue, ^{
      ...
      if (!deadbeef->plt_add_files_begin(plt, 0)) {
        deadbeef->plt_clear(plt);
        int abort = 0;
        deadbeef->plt_load2(0, plt, NULL, [fname UTF8String], &abort, NULL,
                            NULL);
        deadbeef->plt_save_config(plt);
        deadbeef->plt_add_files_end(plt, 0);
      }
...
- (IBAction)savePlaylistAction:(id)sender {
  ...
  if ([panel runModal] == NSModalResponseOK) {
    ...
    if (fname) {
      ddb_playlist_t *plt = deadbeef->plt_get_curr();
      if (plt) {
        deadbeef->plt_save(plt, NULL, NULL, [fname UTF8String], NULL, NULL,
                           NULL);
        deadbeef->plt_unref(plt);
      }
    }
...
- (IBAction)openPrefWindow:(id)sender {
  if (!_prefWindow) {
    _prefWindow = [[PreferencesWindowController alloc]
...
- (IBAction)showHelp:(id)sender {
  if (!_helpWindow) {
    _helpWindow =
        [[HelpWindowController alloc] initWithWindowNibName:@"HelpViewer"];
  }
