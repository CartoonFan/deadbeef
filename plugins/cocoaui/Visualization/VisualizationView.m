...
- (void)updateSpectrumAnalyzer {
  ...
  for (int i = 0; i < NUM_BARS; i++) {
    ...
    int si =
        MIN(sdCount / 2 - 1, (int)(fn / samplerate * (float)(sdCount / 2)));
...
- (void)drawSaGrid:(CGContextRef)context {
  ...
  for (int i = 12; i < NUM_BARS; i += 12) {
    ...
    CGPoint points[] = {CGPointMake(x, 0),
  ...
  for (int i = 0; i < NUM_BARS; i += 12) {
    ...
    [string drawAtPoint:NSMakePoint(x, NSHeight(self.bounds) - 12)
         withAttributes:self.textAttrs];
  }
  ...
  for (int db = 10; db < lower; db += 10) {
    ...
    CGPoint points[] = {CGPointMake(0, y),
                        CGPointMake(NSWidth(self.bounds) - 1, y)};
    CGContextAddLines(context, points, 2);

    NSString *string = [NSString stringWithFormat:@"%d dB", -db];
    [string drawAtPoint:NSMakePoint(0, NSHeight(self.bounds) - y - 12)
         withAttributes:self.textAttrs];
  }
  CGContextStrokePath(context);
}
