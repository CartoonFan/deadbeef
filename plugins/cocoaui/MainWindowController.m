...
- (void)cleanup {
  ...
  // FIXME: this should not be needed, since PlaylistViewController dealloc is
  // supposed to handle everything, but for some reason some stuff remains
  // unreleased if we rely on dealloc to call this method.
...
- (void)windowDidLoad {
  ...
  PlaylistViewController *pvc =
      [[PlaylistViewController alloc] initWithNibName:nil bundle:nil];
  ...
  [view.topAnchor
      constraintEqualToAnchor:self.designableContainerView.topAnchor]
      .active = YES;
  [view.bottomAnchor
      constraintEqualToAnchor:self.designableContainerView.bottomAnchor]
      .active = YES;
  [view.leadingAnchor
      constraintEqualToAnchor:self.designableContainerView.leadingAnchor]
      .active = YES;
  [view.trailingAnchor
      constraintEqualToAnchor:self.designableContainerView.trailingAnchor]
      .active = YES;
  ...
  if (self.window.contentLayoutGuide && self.playlistWithTabsView) {
    ...
    NSLayoutConstraint *constraint =
        [self.playlistWithTabsView.topAnchor constraintEqualToAnchor:topAnchor];
  ...
  if (!self.tabStrip.superview) {
    ...
    NSTitlebarAccessoryViewController *vc =
        [NSTitlebarAccessoryViewController new];
  ...
  _updateTimer = [NSTimer timerWithTimeInterval:1.0f / 10.0f
                                        repeats:YES
                                          block:^(NSTimer *_Nonnull timer) {
                                            MainWindowController *strongself =
                                                weakself;
                                            if (strongself) {
                                              [self frameUpdate];
                                            }
...
- (void)updateSonginfo {
  ...
  if (daystotal == 0) {
    snprintf(totaltime_str, sizeof(totaltime_str), "%d:%02d:%02d", hourtotal,
             mintotal, sectotal);
  } else if (daystotal == 1) {
    snprintf(totaltime_str, sizeof(totaltime_str), _("1 day %d:%02d:%02d"),
             hourtotal, mintotal, sectotal);
  } else {
    snprintf(totaltime_str, sizeof(totaltime_str), _("%d days %d:%02d:%02d"),
             daystotal, hourtotal, mintotal, sectotal);
  }

  DB_playItem_t *track = deadbeef->streamer_get_playing_track();

  if (!output || (output->state() == DDB_PLAYBACK_STATE_STOPPED || !track)) {
    snprintf(sbtext_new, sizeof(sbtext_new),
             _("Stopped | %d tracks | %s total playtime"),
             deadbeef->pl_getcount(PL_MAIN), totaltime_str);
  } else {
    ...
    snprintf(sbtext_new, sizeof(sbtext_new),
             "%s | %d tracks | %s total playtime", buffer,
             deadbeef->pl_getcount(PL_MAIN), totaltime_str);
  }
...
- (void)updateTitleBarConfig {
  ...
  deadbeef->conf_get_str("cocoaui.titlebar_playing",
                         DEFAULT_TITLEBAR_PLAYING_VALUE, script,
                         sizeof(script));
  _titlebar_playing_script = deadbeef->tf_compile(script);

  deadbeef->conf_get_str("cocoaui.titlebar_subtitle_playing",
                         DEFAULT_TITLEBAR_SUBTITLE_PLAYING_VALUE, script,
                         sizeof(script));
  _titlebar_playing_subtitle_script = deadbeef->tf_compile(script);

  deadbeef->conf_get_str("cocoaui.titlebar_stopped",
                         DEFAULT_TITLEBAR_STOPPED_VALUE, script,
                         sizeof(script));
  _titlebar_stopped_script = deadbeef->tf_compile(script);

  deadbeef->conf_get_str("cocoaui.titlebar_stopped",
                         DEFAULT_TITLEBAR_SUBTITLE_STOPPED_VALUE, script,
                         sizeof(script));
  _titlebar_stopped_subtitle_script = deadbeef->tf_compile(script);

  _statusbar_playing_script = deadbeef->tf_compile(
      "$if2($strcmp(%ispaused%,),Paused | )$if2($upper(%codec%),-) |[ "
      "%playback_bitrate% kbps |][ %samplerate%Hz |][ %:BPS% bit |][ "
...
- (void)updateTitleBar {
  ...
  deadbeef->tf_eval(
      &ctx, ctx.it ? _titlebar_playing_script : _titlebar_stopped_script,
      titleBuffer, sizeof(titleBuffer));

  char subtitleBuffer[200];
  deadbeef->tf_eval(&ctx,
                    ctx.it ? _titlebar_playing_subtitle_script
                           : _titlebar_stopped_subtitle_script,
...
#endif
  {
    ...
    self.window.title = [NSString
        stringWithFormat:@"%@%@%@", subTitle,
                         (titleBuffer[0] && subtitleBuffer[0]) ? @" - " : @"",
                         title];
  }
}
