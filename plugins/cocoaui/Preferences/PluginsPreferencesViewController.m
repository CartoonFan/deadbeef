...
@interface PluginConfigPropertySheetDataSource
    : NSObject <PropertySheetDataSource> {
    DB_plugin_t *_plugin;
...
- (NSString *)propertySheet:(PropertySheetViewController *)vc
    configForItem:(id)item {
    return _plugin->configdialog
           ? [NSString stringWithUTF8String:_plugin->configdialog]
           : nil;
}
...
- (void)propertySheet:(PropertySheetViewController *)vc
...
@interface PluginsPreferencesViewController () <NSTableViewDataSource,
    NSTableViewDelegate>
...
@property(strong)
IBOutlet PropertySheetViewController *pluginConfViewController;
...
@property(nonatomic)
PluginConfigPropertySheetDataSource *pluginPropertySheetDataSource;
...
- (void)initPluginConfiguration:(NSInteger)idx {
    ...
    self.pluginPropertySheetDataSource =
        [[PluginConfigPropertySheetDataSource alloc] initWithPlugin:p];
    ...
    if (p->website && (!strncmp(p->website, "http://", 7) ||
...
- (void)setPluginInfo:(NSInteger)idx {
    ...
    if (idx != -1) {
        ...
        version = [NSString stringWithFormat:@"%d.%d", p[idx]->version_major,
                            p[idx]->version_minor];
        if (p[idx]->descr) {
