...
static NSString *kContentTypeMappingChangedNotification =
    @"ContentTypeMappingChanged";
...
- (void)setContentType:(NSString *)contentType {
    ...
    [NSNotificationCenter.defaultCenter
     postNotificationName:kContentTypeMappingChangedNotification
     object:self];
}
...
- (void)viewDidLoad {
    ...
    [NSNotificationCenter.defaultCenter
     addObserver:self
     selector:@selector(contentTypeMappingChanged:)
     name:kContentTypeMappingChangedNotification
     object:nil];
}

- (void)contentTypeMappingChanged:(ContentTypeMap *)sender {
    NSArray<ContentTypeMap *> *objects =
        self.contentTypeMappingArrayController.arrangedObjects;
...
- (void)initContentTypeMapping {
    ...
    char ctmap_str[2048];
    deadbeef->conf_get_str("network.ctmapping", DDB_DEFAULT_CTMAPPING, ctmap_str,
                           sizeof(ctmap_str));
            ...
            for (int i = 0; m->plugins[i]; i++) {
                ...
                plugins = [plugins
                           stringByAppendingString:[NSString
                                                    stringWithUTF8String:m->plugins[i]]];
            }

            ContentTypeMap *map = [[ContentTypeMap alloc]
                                   initWithContentType:[NSString stringWithUTF8String:m->ct]
                                   plugins:plugins];
...
- (void)setNetworkProxyAddress:(NSString *)networkProxyAddress {
    ...
    deadbeef->conf_set_str("network.proxy.address",
                           (networkProxyAddress ?: @"").UTF8String);
    deadbeef->sendmessage(DB_EV_CONFIGCHANGED, 0, 0, 0);
}
...
- (void)setNetworkProxyUserName:(NSString *)networkProxyUserName {
    ...
    deadbeef->conf_set_str("network.proxy.username",
                           (networkProxyUserName ?: @"").UTF8String);
    deadbeef->sendmessage(DB_EV_CONFIGCHANGED, 0, 0, 0);
}
...
- (void)setNetworkProxyPassword:(NSString *)networkProxyPassword {
    ...
    deadbeef->conf_set_str("network.proxy.password",
                           (networkProxyPassword ?: @"").UTF8String);
    deadbeef->sendmessage(DB_EV_CONFIGCHANGED, 0, 0, 0);
}
...
- (void)setNetworkProxyUserAgent:(NSString *)networkProxyUserAgent {
    ...
    deadbeef->conf_set_str("network.http_user_agent",
                           (networkProxyUserAgent ?: @"").UTF8String);
    deadbeef->sendmessage(DB_EV_CONFIGCHANGED, 0, 0, 0);
}
