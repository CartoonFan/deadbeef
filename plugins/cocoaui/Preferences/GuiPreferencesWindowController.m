...
- (instancetype)initWithCoder:(NSCoder *)coder {
  ...
  // gui/misc -> player
  _enableShiftJISDetection =
      deadbeef->conf_get_int("junk.enable_shift_jis_detection", 0) ? YES : NO;
  _enableCP1251Detection =
      deadbeef->conf_get_int("junk.enable_cp1251_detection", 0) ? YES : NO;
  _enableCP936Detection =
      deadbeef->conf_get_int("junk.enable_cp936_detection", 0) ? YES : NO;
  _refreshRate = deadbeef->conf_get_int("cocoaui.refresh_rate", 10);
  _titlebarPlaying = conf_get_nsstr("cocoaui.titlebar_playing",
                                    DEFAULT_TITLEBAR_PLAYING_VALUE);
  _titlebarStopped = conf_get_nsstr("cocoaui.titlebar_stopped",
                                    DEFAULT_TITLEBAR_STOPPED_VALUE);

  // gui/misc -> playlist
  _mmbDeletePlaylist =
      deadbeef->conf_get_int("cocoaui.mmb_delete_playlist", 1) ? YES : NO;
  _hideRemoveFromDisk =
      deadbeef->conf_get_int("cocoaui.hide_remove_from_disk", 0) ? YES : NO;
  _namePlaylistFromFolder =
      deadbeef->conf_get_int("cocoaui.name_playlist_from_folder", 1) ? YES : NO;

  // delete from disk
  _deleteFromDiskUseBin =
      deadbeef->conf_get_int("cocoaui.delete_use_bin", 1) ? YES : NO;
  _deleteFromDiskEnableConfirmationDialog =
      deadbeef->conf_get_int("cocoaui.delete_files_confirm", 1) ? YES : NO;
  _deleteFromDiskSkipDeletedTracks =
      deadbeef->conf_get_int("cocoaui.skip_deleted_tracks", 0) ? YES : NO;
...
- (void)setEnableShiftJISDetection:(BOOL)enableShiftJISDetection {
  ...
  deadbeef->conf_set_int("junk.enable_shift_jis_detection",
                         enableShiftJISDetection);
...
- (void)setNamePlaylistFromFolder:(BOOL)namePlaylistFromFolder {
  ...
  deadbeef->conf_set_int("cocoaui.name_playlist_from_folder",
                         namePlaylistFromFolder);
  deadbeef->sendmessage(DB_EV_CONFIGCHANGED, 0, 0, 0);
}
...
- (void)setDeleteFromDiskEnableConfirmationDialog:
    (BOOL)deleteFromDiskEnableConfirmationDialog {
  _deleteFromDiskEnableConfirmationDialog =
      deleteFromDiskEnableConfirmationDialog;
  deadbeef->conf_set_int("cocoaui.delete_files_confirm",
                         deleteFromDiskEnableConfirmationDialog);
  deadbeef->sendmessage(DB_EV_CONFIGCHANGED, 0, 0, 0);
}

- (void)setDeleteFromDiskSkipDeletedTracks:
    (BOOL)deleteFromDiskSkipDeletedTracks {
  _deleteFromDiskSkipDeletedTracks = deleteFromDiskSkipDeletedTracks;
  deadbeef->conf_set_int("cocoaui.skip_deleted_tracks",
                         deleteFromDiskSkipDeletedTracks);
  deadbeef->sendmessage(DB_EV_CONFIGCHANGED, 0, 0, 0);
}
