...
#define trace(...)                                                             \
  { deadbeef->log_detailed(&plugin.plugin, 1, __VA_ARGS__); }
#define warn(...)                                                              \
  { deadbeef->log(__VA_ARGS__); }
#define info(...)                                                              \
  { deadbeef->log_detailed(&plugin.plugin, 1, __VA_ARGS__); }

// This is my 4th rewrite of portaudio plugin. Ok, actually 5th, because idea in
// 4th rewrite didn't work. Now it's 6th...

static PaSampleFormat pa_GetSampleFormat(int bps, int is_float);

static void pa_SetDefault();

static void portaudio_stream_create(void);

static void portaudio_thread(void *context);

static int portaudio_free(void);

static int portaudio_setformat(ddb_waveformat_t *fmt);

static int portaudio_play(void);

static int portaudio_stop(void);

static int portaudio_pause(void);

static int portaudio_unpause(void);
...
static void pa_stream_finished_callback(void *uData);
...
// portaudio_stream_create opens a stream using stream_parameters and plugin.fmt
// information
static void portaudio_stream_create(void) {
  ...
  dispatch_async(stream_queue, ^{
    ...
    {
      deadbeef->conf_lock();
      const char *portaudio_soundcard_string =
          deadbeef->conf_get_str_fast("portaudio_soundcard", "default");
      if (strcmp(portaudio_soundcard_string, "default") == 0) {
        stream_parameters.device = Pa_GetDefaultOutputDevice();
        deadbeef->conf_unlock();
      } else {
        deadbeef->conf_unlock();
        stream_parameters.device =
            deadbeef->conf_get_int("portaudio_soundcard", -1);
      }
    }

    // Bufer
    // Using paFramesPerBufferUnspecified with alsa gives warnings about
    // underrun
    unsigned long buffer_size;
    int buffer_size_config =
        deadbeef->conf_get_int("portaudio.buffer", DEFAULT_BUFFER_SIZE);
...
static void portaudio_stream_destroy(void) {
  ...
  dispatch_async(stream_queue, ^{
    if (stream) {
      ...
      // modify stream parameters, so that new stream will be created in
      // setformat
      plugin.fmt.bps = 0;
      stream_totalsize = -1;
      stream_framesize = -1;
      ...
      // portaudio_free() and portaudio_setformat() combo might try to free
...
// since we can't change stream parameters, we have to abort actual stream, set
// values and start streaming again
static int portaudio_setformat(ddb_waveformat_t *fmt) {
  ...
  dispatch_async(stream_queue, ^{
    ...
    trace("portaudio_setformat %dbit %s %dch %dHz channelmask=%X\n",
          requested_fmt.bps, fmt->is_float ? "float" : "int", fmt->channels,
          fmt->samplerate, fmt->channelmask);
    ...
    if (!memcmp(&requested_fmt, &plugin.fmt, sizeof(ddb_waveformat_t)) &&
      ...
      trace("switching format: (requested->actual)\n"
            ...
            "samplerate %d -> %d\n"
            "channelmask %d -> %d\n",
            fmt->bps, plugin.fmt.bps, fmt->is_float, plugin.fmt.is_float,
            fmt->channels, plugin.fmt.channels, fmt->samplerate,
            plugin.fmt.samplerate, fmt->channelmask, plugin.fmt.channelmask);
    }
    ...
    dispatch_async(stream_queue, ^{
      ...
      stream_parameters.sampleFormat =
          pa_GetSampleFormat(plugin.fmt.bps, plugin.fmt.is_float);
      stream_parameters.suggestedLatency = 0.0;
      stream_parameters.hostApiSpecificStreamInfo = NULL;

      err =
          Pa_IsFormatSupported(NULL, &stream_parameters, plugin.fmt.samplerate);
      if (err != paNoError) {
...
static int portaudio_play(void) {
  ...
  // New plugin structure always keeps at least one stream open (unless plugin
  // not in usage)
...
static int portaudio_pause(void) {
  ...
  dispatch_async(command_queue, ^{
    ...
    dispatch_semaphore_wait(command_semaphore, DISPATCH_TIME_FOREVER);
    if (state == DDB_PLAYBACK_STATE_STOPPED) {
      // If option 'Resume previous session on startup' is enabled deadbeef will
      // call: portaudio_stop() and portaudio_pause() (no portaudio_play()). If
      // it's the case we should just set the state and do the main job later.
...
static int portaudio_configchanged(void) {
  dispatch_async(stream_queue, ^{
    ...
    int buffer_size_new =
        deadbeef->conf_get_int("portaudio.buffer", DEFAULT_BUFFER_SIZE);
    if ((stream && portaudio_soundcard != stream_parameters.device) ||
...
static void portaudio_enum_soundcards(void (*callback)(const char *name,
                                                       const char *desc,
                                                       void *),
                                      void *userdata) {
  ...
  for (i = 0; i < device_count; i++) {
    ...
    if (api_info->type == paDirectSound || api_info->type == paMME) {
      ...
      if (charset && strlen(charset)) {
        ...
        deadbeef->junk_iconv(device->name, strlen(device->name), name_converted,
                             strlen(device->name) * 4, charset,
                             "UTF-8//IGNORE");
      }
    }
#endif
...
static int portaudio_callback(const void *in, void *out,
                              unsigned long framesPerBuffer,
                              const PaStreamCallbackTimeInfo *timeInfo,
                              PaStreamCallbackFlags statusFlags, void *uData) {
...
static ddb_playback_state_t portaudio_get_state(void) { return state; }

static int portaudio_message(uint32_t id, uintptr_t ctx, uint32_t p1,
                             uint32_t p2) {
  switch (id) {
...
static const char settings_dlg[] =
    "property \"Buffer size (-1 to use optimal value chosen by portaudio)\" "
    "entry portaudio.buffer " DEFAULT_BUFFER_SIZE_STR ";\n"
#ifdef __MINGW32__
    "property \"Device name encoding\" select[3] portaudio.devenc_list 0 "
    "\"ASCII / UTF-8\" cp1250 \"Defined below\";\n"
    "property \"Custom device name encoding\" entry portaudio.devenc_custom "
    "\"\";\n"
#endif
    ;

static int p_portaudio_start(void) {
...
static int p_portaudio_stop(void) {
  PaError err;
  err = Pa_Terminate();
  if (err != paNoError) {
...
DB_plugin_t *portaudio_load(DB_functions_t *api) {
  deadbeef = api;
...
static DB_output_t plugin = {
    ...
    .plugin.descr =
        "This plugin plays audio using PortAudio library.\n"
        "\n"
        "Changes in version 1.7:\n"
        "    * Converted plugin to use libdispatch.\n"
        "Changes in version 1.6:\n"
        "    * Fixed ending of stream when option 'Stop after current "
        "track/album' is used.\n"
