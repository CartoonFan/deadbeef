...
static inline void le_int16(int16_t in, unsigned char *out) {
  out[0] = in & 0xff;
  out[1] = (in & 0xff00) >> 8;
}

// NOTE: we know that current SLDB is larger than that, but we want the code to
// go into realloc path
...
static void sldb_load() {
  ...
  deadbeef->conf_get_str("hvsc_path", "", conf_hvsc_path,
                         sizeof(conf_hvsc_path));
  if (!conf_hvsc_path[0]) {
    sldb_disable = 1;
    return;
  }
  ...
  while (fgets(str, 1024, fp) == str) {
    if (sldb_count >= sldb_allocated_size) {
      ...
      sldb = (sldb_item_t *)realloc(sldb,
                                    sldb_allocated_size * sizeof(sldb_item_t));
    }
    line++;
    if (str[0] == ';') {
    ...
    while (*p >= ' ') {
      ...
      if (sldb_lengths_count >= sldb_lengths_allocated_size) {
        ...
        sldb_lengths = (uint16_t *)realloc(
            sldb_lengths, sizeof(uint16_t) * sldb_lengths_allocated_size);
      }
...
static int sldb_find(const uint8_t *digest) {
  if (!sldb) {
...
static void csid_mute_voices(sid_info_t *info, int chip_voices) {
...
int csid_init(DB_fileinfo_t *_info, DB_playItem_t *it) {
  ...
  conf.playback =
      deadbeef->conf_get_int("sid.mono", 0) ? sid2_mono : sid2_stereo;
...
void csid_free(DB_fileinfo_t *_info) {
  sid_info_t *info = (sid_info_t *)_info;
  if (info) {
    delete info->sidplay;
    delete info->resid;
...
int csid_read(DB_fileinfo_t *_info, char *bytes, int size) {
  sid_info_t *info = (sid_info_t *)_info;
...
int csid_seek(DB_fileinfo_t *_info, float time) {
  sid_info_t *info = (sid_info_t *)_info;
  float t = time;
  if (t < _info->readpos) {
    // reinit
...
static void find_hvsc_path_from_fname(const char *fname) {
...
extern "C" DB_playItem_t *csid_insert(ddb_playlist_t *plt, DB_playItem_t *after,
                                      const char *fname) {
  trace("inserting %s\n", fname);
  ...
  if (sldb_legacy) {
    deadbeef->md5_append(&md5,
                         (const uint8_t *)tune->cache.get() + tune->fileOffset,
                         tune->getInfo().c64dataLen);
    ...
    if (tune->getInfo().clockSpeed == SIDTUNE_CLOCK_NTSC) {
      deadbeef->md5_append(&md5, &tune->getInfo().clockSpeed,
                           sizeof(tune->getInfo().clockSpeed));
    }
  } else {
    deadbeef->md5_append(&md5, (const uint8_t *)tune->cache.get(),
                         tune->getInfo().dataFileLen);
  }
  ...
  for (int s = 0; s < tunes; s++) {
    ...
    if (tune->selectSong(s + 1)) {
      DB_playItem_t *it =
          deadbeef->pl_item_alloc_init(fname, sid_plugin.plugin.id);
      ...
      if (i >= 1 && sidinfo.infoString[0] && sidinfo.infoString[0][0]) {
        ...
        deadbeef->pl_add_meta(it, meta,
                              convstr(sidinfo.infoString[0],
                                      (int)strlen(sidinfo.infoString[0]), temp,
                                      sizeof(temp)));
      }
      if (i >= 2 && sidinfo.infoString[1] && sidinfo.infoString[1][0]) {
        deadbeef->pl_add_meta(it, "artist",
                              convstr(sidinfo.infoString[1],
                                      (int)strlen(sidinfo.infoString[1]), temp,
                                      sizeof(temp)));
      }
      if (i >= 3 && sidinfo.infoString[2] && sidinfo.infoString[2][0]) {
        deadbeef->pl_add_meta(it, "copyright",
                              convstr(sidinfo.infoString[2],
                                      (int)strlen(sidinfo.infoString[2]), temp,
                                      sizeof(temp)));
      }

      for (int j = 3; j < i; j++) {
        if (sidinfo.infoString[j] && sidinfo.infoString[j][0]) {
          deadbeef->pl_add_meta(it, "info",
                                convstr(sidinfo.infoString[j],
                                        (int)strlen(sidinfo.infoString[j]),
                                        temp, sizeof(temp)));
        }
      }
      char trk[10];
...
static int sid_configchanged(void) {
...
extern "C" DB_plugin_t *sid_load(DB_functions_t *api) {
  deadbeef = api;
