...
// Params:
// seek_to_sample: -1 means to the end (scan whole file), otherwise a sample to
// seek to When seeking, the packet offset returned will be the one containing
// seek_to_sample, not accounting for delay. `totalsamples` will be set to the
// sample count at the packet start. So the caller is responsible for adding
// delay to seek_to_sample, and then adjusting skipsamples based on returned
// "totalsamples".
//
//
// returns:
...
// -1: error
//
// the caller is supposed to start decoding from info->packet_offs, and skip
// info->skipsamples samples
