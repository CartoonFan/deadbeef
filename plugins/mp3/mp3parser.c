...
#define MIN_PACKET_LENGTH                                                      \
  (MIN_PACKET_SAMPLES / 8 * MIN_BITRATE * 1000 / MAX_SAMPLERATE)
...
static int _parse_packet(mp3packet_t *restrict packet, uint8_t *restrict fb) {
  ...
  if (packet->layer == 2) {
    if ((packet->bitrate <= 56 || packet->bitrate == 80) &&
        packet->nchannels != 1) {
      //            trace ("mp3: bad frame %d: layer %d, channels %d, bitrate
  ...
  if (packet->samplerate > 0 && packet->bitrate > 0) {
    ...
    packet->packetlength =
        packet->samples_per_frame / 8 * packet->bitrate / packet->samplerate +
        padding;
...
#define READ_UINT32()                                                          \
  ({                                                                           \
    if (buffer_size < 4)                                                       \
      return -1;                                                               \
    uint32_t _temp32 = (uint32_t)buffer[3] | ((uint32_t)buffer[2] << 8) |      \
...
static int _check_xing_header(mp3info_t *info, mp3packet_t *packet,
  ...
  int havecookie =
      !memcmp(cookieXing, buffer, 4) || !memcmp(cookieInfo, buffer, 4);
  ...
  if (lame_header) {
    ...
    info->delay =
        (((uint32_t)buffer[0]) << 4) | ((((uint32_t)buffer[1]) & 0xf0) >> 4);
    info->padding =
        ((((uint32_t)buffer[1]) & 0x0f) << 8) | ((uint32_t)buffer[2]);
    buffer += 3;
    buffer_size -= 3;
...
// returns 0 to continue, 1 to stop (seek position reached, or another reason to
// stop scanning)
static int _process_packet(mp3info_t *info, mp3packet_t *packet,
                           int64_t seek_to_sample) {
...
static int _packet_same_fmt(mp3packet_t *ref_packet, mp3packet_t *packet) {
...
int mp3_parse_file(mp3info_t *info, uint32_t flags, DB_FILE *fp, int64_t fsize,
                   int startoffs, int endoffs, int64_t seek_to_sample) {
  ...
  while (fsize > 0 || fsize < 0) {
    ...
    if (res < 0 ||
        (info->npackets && !_packet_same_fmt(&info->ref_packet, &packet))) {
      ...
      if (!info->have_xing_header && !info->checked_xing_header) {
        ...
        if (deadbeef->fread(xinghdr, 1, sizeof(xinghdr), fp) !=
            sizeof(xinghdr)) {
          return -1;
        }
        info->num_reads++;
        ...
        int xingres =
            _check_xing_header(info, &packet, xinghdr, (int)sizeof(xinghdr));
        if (!xingres) {
          got_xing = 1;
          info->have_xing_header = 1;
          // trust the xing header -- even if requested to scan for precise
          // duration parameters have been discovered from xing header, no need
          // to continue
          memcpy(&info->ref_packet, &packet, sizeof(mp3packet_t));
      ...
      if (!got_xing) {
        ...
        if (seek_to_sample > 0 &&
            info->pcmsample + packet.samples_per_frame >= seek_to_sample) {
          goto end;
        }
      ...
      if (info->have_xing_nframes && seek_to_sample < 0 &&
          !(flags & MP3_PARSE_FULLSCAN) && info->npackets >= 200) {
        goto end;
      }
      // Calculate CBR duration from file size
      else if (!variable_packets && !info->have_xing_header &&
               seek_to_sample < 0 && !(flags & MP3_PARSE_FULLSCAN) &&
               info->npackets >= 200) {
        // calculate total number of packets from file size
        // 8876 - 9485
        int64_t npackets =
            ceil(datasize / (float)info->ref_packet.packetlength);
      ...
      if (seek_to_sample == -1 && fsize > 0 && offs > (startoffs + 50000) &&
          (info->is_streaming || (flags & MP3_PARSE_ESTIMATE_DURATION))) {
        if (!info->valid_packets) {
          goto error;
        }

        info->avg_packetlength =
            floor(info->avg_packetlength / info->valid_packets);
...
error:
  ...
  float elapsed = (end_tv.tv_sec - start_tv.tv_sec) +
                  (end_tv.tv_usec - start_tv.tv_usec) / 1000000.f;
  printf("mp3 stats:\nSeeks: %llu\nReads: %llu\nBytes: %llu\nTime: %f sec\n",
