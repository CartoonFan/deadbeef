...
int cmp3_seek_stream(DB_fileinfo_t *_info, int sample) {
    ...
    mp3info_t mp3info;
    int res = mp3_parse_file(&mp3info, info->mp3flags, info->file,
                             deadbeef->fgetlength(info->file), info->startoffs,
                             info->endoffs, sample);
...
static DB_fileinfo_t *cmp3_open(uint32_t hints) {
    ...
    if ((hints & DDB_DECODER_HINT_16BIT) ||
            deadbeef->conf_get_int("mp3.force16bit", 0))
#endif
    {
        info->want_16bit = 1;
    }
...
void cmp3_set_extra_properties(DB_playItem_t *it, mp3info_t *mp3info,
                               int fake) {
    char s[100];
    ...
    // set codec profile (cbr or vbr) and mp3 vbr method (guessed, or from
    // Xing/Info header)

    char codec_profile[100];
    snprintf(codec_profile, sizeof(codec_profile), "MP3 %s",
             (!mp3info->vbr_type || mp3info->vbr_type == XING_CBR ||
              mp3info->vbr_type == XING_CBR2)
             ? "CBR"
             : "VBR");
    if (mp3info->vbr_type != XING_CBR && mp3info->vbr_type != XING_CBR2 &&
            (mp3info->lamepreset & 0x7ff)) {
        const static struct {
            int v;
            const char *name;
        } presets[] = {
    ...
    snprintf(s, sizeof(s), "MPEG%s layer%d",
             versions[mp3info->ref_packet.ver - 1], mp3info->ref_packet.layer);
    deadbeef->pl_replace_meta(it, ":MPEG_VERSION", s);
    deadbeef->pl_replace_meta(it, ":XING_HEADER",
                              mp3info->have_xing_header ? "Yes" : "No");
    deadbeef->pl_replace_meta(it, fake ? "!FILETYPE" : ":FILETYPE", "MP3");
}

static int cmp3_init(DB_fileinfo_t *_info, DB_playItem_t *it) {
    mp3_info_t *info = (mp3_info_t *)_info;
    ...
    if (!info->file->vfs->is_streaming() &&
            !(info->mp3flags & MP3_PARSE_ESTIMATE_DURATION)) {
        deadbeef->junk_get_tag_offsets(info->file, &info->startoffs,
                                       &info->endoffs);
        if (info->startoffs > 0) {
            trace("mp3: skipping %d(%xH) bytes of junk\n", info->startoffs,
                  info->endoffs);
        }
        int res = mp3_parse_file(&info->mp3info, info->mp3flags, info->file,
                                 deadbeef->fgetlength(info->file), info->startoffs,
                                 info->endoffs, -1);
        if (res < 0) {
        ...
        if (endsample > 0) {
            info->startsample =
                deadbeef->pl_item_get_startsample(it) + info->mp3info.delay;
            info->endsample = endsample + info->mp3info.delay;
            // that comes from cue, don't calc duration, just seek and play
        } else {
            ddb_playlist_t *plt = deadbeef->pl_get_playlist(it);
            int64_t totalsamples = info->mp3info.totalsamples - info->mp3info.delay -
                                   info->mp3info.padding;
            deadbeef->plt_set_item_duration(
                plt, it,
                (float)((double)totalsamples / info->mp3info.ref_packet.samplerate));
            if (plt) {
        ...
        if (info->mp3info.totalsamples >= 0) {
            deadbeef->plt_set_item_duration(
                plt, it,
                (float)((double)info->mp3info.totalsamples /
                        info->mp3info.ref_packet.samplerate));
            info->endsample = info->mp3info.totalsamples - info->mp3info.padding - 1;
        } else {
    ...
    _info->fmt.channelmask =
        _info->fmt.channels == 1
        ? DDB_SPEAKER_FRONT_LEFT
        : (DDB_SPEAKER_FRONT_LEFT | DDB_SPEAKER_FRONT_RIGHT);
    trace("mp3 format: bps:%d sr:%d channels:%d\n", _info->fmt.bps,
          _info->fmt.samplerate, _info->fmt.channels);
...
static void cmp3_decode(mp3_info_t *info) {
    int eof = 0;
    while (!eof) {
...
static void cmp3_free(DB_fileinfo_t *_info) {
    mp3_info_t *info = (mp3_info_t *)_info;
    if (info->it) {
...
static int cmp3_read(DB_fileinfo_t *_info, char *bytes, int size) {
#if WRITE_DUMP
    if (!out) {
        out = fopen("out.raw", "w+b");
    }
    ...
    if (!info->file->vfs->is_streaming() &&
        ...
        if (size / samplesize + curr > info->endsample) {
            ...
            trace("\033[0;32mmp3: size truncated to %d bytes (%d samples), "
                  "cursample=%d, endsample=%d\033[37;0m\n",
    ...
    if (!info->raw_signal) {
        ...
        deadbeef->replaygain_apply(&fmt, info->want_16bit ? info->conv_buf : bytes,
                                   req_size - info->bytes_to_decode);

        // convert to 16 bit, if needed
        if (info->want_16bit) {
            int sz = req_size - info->bytes_to_decode;
            int ret =
                deadbeef->pcm_convert(&fmt, info->conv_buf, &_info->fmt, bytes, sz);
            info->bytes_to_decode = size - ret;
        }
    }

    info->currentsample += (size - info->bytes_to_decode) / samplesize;
    _info->readpos = (float)(info->currentsample - info->startsample) /
                     info->mp3info.ref_packet.samplerate;
#if WRITE_DUMP
...
static int cmp3_seek_sample(DB_fileinfo_t *_info, int sample) {
    mp3_info_t *info = (mp3_info_t *)_info;
    if (!info->file) {
        return -1;
    }
    ...
    if (info->file->vfs->is_streaming() ||
            (info->mp3flags & MP3_PARSE_ESTIMATE_DURATION)) {
        if (info->mp3info.totalsamples > 0 &&
                info->mp3info.avg_samples_per_frame > 0 &&
                info->mp3info.avg_packetlength >
                0) { // that means seekable remote stream, like podcast
            ...
            r = deadbeef->fseek(
                    info->file, frm * info->mp3info.avg_packetlength + info->startoffs,
                    SEEK_SET);
            ...
            info->skipsamples =
                (int)(sample - frm * info->mp3info.avg_samples_per_frame);
        ...
        trace(
            "seek is impossible (avg_samples_per_frame=%d, avg_packetlength=%f)!\n",
    ...
    //    int ms = (tm2.tv_sec*1000+tm2.tv_usec/1000) -
    //    (tm1.tv_sec*1000+tm1.tv_usec/1000); printf ("mp3_parse_file took %d
    //    ms\n", ms);
    trace("seeked to %d\n", info->currentsample);
    _info->readpos = (float)(info->currentsample - info->startsample) /
                     info->mp3info.ref_packet.samplerate;
...
static int cmp3_seek(DB_fileinfo_t *_info, float time) {
    mp3_info_t *info = (mp3_info_t *)_info;
    int sample = time * info->mp3info.ref_packet.samplerate;
    return cmp3_seek_sample(_info, sample);
}

static DB_playItem_t *cmp3_insert(ddb_playlist_t *plt, DB_playItem_t *after,
                                  const char *fname) {
    trace("cmp3_insert %s\n", fname);
    ...
    deadbeef->plt_set_item_duration(
        plt, it,
        (float)((double)mp3info.totalsamples / mp3info.ref_packet.samplerate));

    cmp3_set_extra_properties(it, &mp3info, 0);

    deadbeef->fclose(fp);

    DB_playItem_t *cue = deadbeef->plt_process_cue(
                             plt, after, it, mp3info.totalsamples - mp3info.delay - mp3info.padding,
                             mp3info.ref_packet.samplerate);
    if (cue) {
        deadbeef->pl_item_unref(it);
        return cue;
    }
...
int cmp3_read_metadata(DB_playItem_t *it) {
    deadbeef->pl_lock();
    const char *uri = strdupa(deadbeef->pl_find_meta(it, ":URI"));
    deadbeef->pl_unlock();
...
int cmp3_write_metadata(DB_playItem_t *it) {
    // get options
    ...
    deadbeef->conf_get_str("mp3.id3v1_encoding", "cp1252", id3v1_encoding,
                           sizeof(id3v1_encoding));
    return deadbeef->junk_rewrite_tags(it, junk_flags, id3v2_version,
                                       id3v1_encoding);
}

static const char *exts[] = {"mp1", "mp2", "mp3", "mpga", NULL};
...
DB_plugin_t *mp3_load(DB_functions_t *api) {
    deadbeef = api;
