...
static DB_fileinfo_t *vtx_open(uint32_t hints) {
  vtx_info_t *info = calloc(sizeof(vtx_info_t), 1);
  return &info->info;
}

static int vtx_init(DB_fileinfo_t *_info, DB_playItem_t *it) {
  ...
  trace("vtx: data=%p, size=%d\n", info->decoder->regdata,
        info->decoder->regdata_size);
  ...
  ayemu_set_sound_format(&info->ay, samplerate, _info->fmt.channels,
                         _info->fmt.bps);
...
static void vtx_free(DB_fileinfo_t *_info) {
 ...
 */
static int ayemu_vtx_get_next_frame(vtx_info_t *info) {
...
static int vtx_read(DB_fileinfo_t *_info, char *bytes, int size) {
...
static int vtx_seek_sample(DB_fileinfo_t *_info, int sample) {
...
static int vtx_seek(DB_fileinfo_t *_info, float time) {
...
static DB_playItem_t *vtx_insert(ddb_playlist_t *plt, DB_playItem_t *after,
                                 const char *fname) {
...
DB_plugin_t *vtx_load(DB_functions_t *api) {
  deadbeef = api;
