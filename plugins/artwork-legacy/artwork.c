...
static float scale_dimensions(int scaled_size, int width, int height,
...
#ifndef USE_IMLIB2
#ifdef USE_BICUBIC
static float cerp(float p0, float p1, float p2, float p3, float d, float d2,
                  float d3) {
  /* Cubic Hermite spline (a = -0.5, Catmull-Rom) */
  return p1 - (p0 * d3 - 3 * p1 * d3 + 3 * p2 * d3 - p3 * d3 - 2 * p0 * d2 +
               5 * p1 * d2 - 4 * p2 * d2 + p3 * d2 + p0 * d - p2 * d) /
                  2;
}

static int_fast16_t bcerp(const png_byte *row0, const png_byte *row1,
                          const png_byte *row2, const png_byte *row3,
                          const uint_fast32_t x0, const uint_fast32_t x1,
                          const uint_fast32_t x2, const uint_fast32_t x3,
                          const uint_fast8_t component, float dx, float dx2,
                          float dx3, float dy, float dy2, float dy3) {
  ...
  float p0 = cerp(row0[index0], row1[index0], row2[index0], row3[index0], dy,
                  dy2, dy3);
  float p1 = cerp(row0[index1], row1[index1], row2[index1], row3[index1], dy,
                  dy2, dy3);
  float p2 = cerp(row0[index2], row1[index2], row2[index2], row3[index2], dy,
                  dy2, dy3);
  float p3 = cerp(row0[index3], row1[index3], row2[index3], row3[index3], dy,
                  dy2, dy3);
  return cerp(p0, p1, p2, p3, dx, dx2, dx3) + 0.5;
}
#endif
static uint_fast32_t
blerp_pixel(const png_byte *row, const png_byte *next_row,
            const uint_fast32_t x_index, const uint_fast32_t next_x_index,
            const uint_fast32_t weight, const uint_fast32_t weightx,
            const uint_fast32_t weighty, const uint_fast32_t weightxy) {
  return row[x_index] * weight + row[next_x_index] * weightx +
         next_row[x_index] * weighty + next_row[next_x_index] * weightxy;
}

static uint_fast32_t *calculate_quick_dividers(float scaling_ratio) {
  ...
  uint_fast32_t *dividers =
      malloc((max_sample_size * max_sample_size + 1) * sizeof(uint_fast32_t));
  if (dividers) {
    for (uint16_t y = max_sample_size; y; y--) {
      for (uint16_t x = y; x; x--) {
        ...
        dividers[num_pixels] =
            num_pixels <= 256 || !(num_pixels & (num_pixels - 1))
                ? 256 * 256 / num_pixels
                : 0;
      }
    }
...
static int jpeg_resize(const char *fname, const char *outname,
                       int scaled_size) {
  ...
  float scaling_ratio = scale_dimensions(scaled_size, width, height,
                                         &scaled_width, &scaled_height);
  if (scaling_ratio >= 65535 || scaled_width < 1 || scaled_width > 32767 ||
      scaled_height < 1 || scaled_width > 32767) {
  ...
  if (scaling_ratio > 2) {
    ...
    sample_rows_buffer =
        malloc(max_sample_height * row_components * sizeof(JSAMPLE));
    ...
    for (uint_fast16_t scaled_y = 0; scaled_y < scaled_height; scaled_y++) {
      ...
      const uint_fast16_t num_y_pixels =
          (y_limit < height ? y_limit : height) - y;
      ...
      float x_interp = 0.5;
      for (uint_fast32_t scaled_x = 0; scaled_x < scaled_row_components;
           scaled_x += num_components) {
        ...
        /* Sum all values where the scaled pixel overlaps at least half a pixel
         * in each direction */
        const uint_fast32_t num_pixels = num_y_pixels * (x_limit - x);
        const uint_fast32_t quick_divider = quick_dividers[num_pixels];
        for (uint_fast8_t component = 0; component < num_components;
             component++) {
          uint_fast32_t value = 0;
          for (uint_fast32_t pixel_index = x_index + component;
               pixel_index < x_limit_index; pixel_index += num_components) {
            for (uint_fast16_t row_index = 0; row_index < num_y_pixels;
                 row_index++) {
              value += rows[row_index][pixel_index];
            }
          }
          out_row[scaled_x + component] =
              quick_divider ? value * quick_divider >> 16 : value / num_pixels;
        }
      }
    ...
    for (uint_fast16_t scaled_y = 0; scaled_y < scaled_height;
         scaled_y++, y_interp += scaling_ratio) {
      const uint_fast32_t y = y_interp;
      ...
      for (uint_fast32_t scaled_x = 0; scaled_x < scaled_row_components;
           scaled_x += num_components, x_interp += scaling_ratio) {
        const uint_fast32_t x = x_interp;
        const uint_fast32_t x_index = x * num_components;
        const uint_fast32_t next_x_index =
            x + 1 < width ? x_index + num_components : x_index;
        ...
        for (uint_fast8_t component = 0; component < num_components;
             component++) {
          out_line[scaled_x + component] =
              blerp_pixel(row, next_row, x_index + component,
                          next_x_index + component, weight, weightx, weighty,
                          weightxy) >>
              16;
        }
      }
      ...
      for (uint_fast32_t scaled_x = 0; scaled_x < scaled_row_components;
        ...
        for (uint_fast8_t component = 0; component < num_components;
          ...
          const int_fast16_t pixel =
              bcerp(row0, row1, row2, row3, x0, x1, x2, x3, component, dx, dx2,
                    dx3, dy, dy2, dy3);
          out_row[scaled_x + component] =
              pixel < 0 ? 0 : pixel > 255 ? 255 : pixel;
        }
      }
...
static int png_resize(const char *fname, const char *outname, int scaled_size) {
  ...
  png_set_IHDR(new_png_ptr, new_info_ptr, scaled_width, scaled_height,
  ...
  const uint8_t num_values = color_type == PNG_COLOR_TYPE_GRAY ||
  ...
  if (scaling_ratio > 2) {
    ...
    for (uint_fast16_t scaled_y = 0; scaled_y < scaled_height; scaled_y++) {
      ...
      for (uint_fast32_t scaled_x = 0; scaled_x < scaled_row_components;
        ...
        /* Sum all values where the scaled pixel overlaps at least half an
         * original pixel in each direction */
        const uint_fast32_t quick_divider = quick_dividers[num_pixels];
        if (has_alpha) {
          ...
          if (alpha_value < num_pixels) {
            ...
            out_row[scaled_x + num_values] =
                alpha_value > 254 * num_pixels
                    ? 255
                    : quick_divider ? alpha_value * quick_divider >> 16
                                    : alpha_value / num_pixels;
          ...
          /* For opaque RGB pixels, use a simple average on each colour
           * component */
          ...
          out_row[scaled_x] = quick_divider ? red_value * quick_divider >> 16
                                            : red_value / num_pixels;
          out_row[scaled_x + 1] = quick_divider
                                      ? green_value * quick_divider >> 16
                                      : green_value / num_pixels;
          out_row[scaled_x + 2] = quick_divider
                                      ? blue_value * quick_divider >> 16
                                      : blue_value / num_pixels;
        } else {
          /* For opaque grey pixels, use a simple average of the values */
          uint_fast32_t grey_value = 0;
          for (uint_fast16_t row_index = 0; row_index < num_y_pixels;
               row_index++) {
            for (uint_fast32_t pixel_index = x_index;
                 pixel_index < x_limit_index; pixel_index++) {
              grey_value += rows[row_index][pixel_index];
            }
          }
          out_row[scaled_x] = quick_divider ? grey_value * quick_divider >> 16
                                            : grey_value / num_pixels;
        }
      }
    ...
    for (uint_fast16_t scaled_y = 0; scaled_y < scaled_height;
      ...
      for (uint_fast32_t scaled_x = 0; scaled_x < scaled_row_components;
        ...
        const uint_fast32_t x_index = x * num_components;
        const uint_fast32_t next_x_index =
            x < width ? x_index + num_components : x_index;
        ...
        if (scaled_alpha == 255 << 16) {
          ...
          for (uint_fast8_t component = 0; component < num_values;
               component++) {
            out_row[scaled_x + component] =
                blerp_pixel(row, next_row, x_index + component,
                            next_x_index + component, weight, weightx, weighty,
          ...
          /* Alpha-weight partially transparent pixels to avoid background
           * colour bleeding */
          for (uint_fast8_t component = 0; component < num_values;
               component++) {
            out_row[scaled_x + component] =
                blerp_pixel(row, next_row, x_index + component,
                            next_x_index + component, alpha, alphax, alphay,
                            alphaxy) /
                scaled_alpha;
          }
        }
    ...
    for (uint_fast16_t scaled_y = 0; scaled_y < scaled_height;
      ...
      for (uint_fast32_t scaled_x = 0; scaled_x < scaled_row_components;
        ...
        if (has_alpha) {
          scaled_alpha = bcerp(row0, row1, row2, row3, x0, x1, x2, x3, 3, dx,
                               dx2, dx3, dy, dy2, dy3);
          out_row[scaled_x + 3] =
              scaled_alpha < 0 ? 0 : scaled_alpha > 255 ? 255 : scaled_alpha;
        }

        if (scaled_alpha == 255) {
          for (uint_fast8_t component = 0; component < 3; component++) {
            const int_fast16_t pixel =
                bcerp(row0, row1, row2, row3, x0, x1, x2, x3, component, dx,
                      dx2, dx3, dy, dy2, dy3);
            out_row[scaled_x + component] =
                pixel < 0 ? 0 : pixel > 255 ? 255 : pixel;
          }
...
#ifdef USE_IMLIB2
static int imlib_resize(const char *in, const char *out, int img_size) {
...
static int scale_file(const char *in, const char *out, int img_size) {
...
static int make_cache_dir_path(char *path, int size, const char *artist,
                               int img_size) {
  ...
  if (img_size == -1) {
    path_length =
        snprintf(path + strlen(path), size_left, "covers/%s/", esc_artist);
  } else {
    path_length = snprintf(path + strlen(path), size_left, "covers-%d/%s/",
                           img_size, esc_artist);
  }
...
static int make_cache_path2(char *path, int size, const char *fname,
  ...
  int max_album_chars =
      min(NAME_MAX, size - strlen(path)) - sizeof("1.jpg.part");
  ...
  const char *palbum = strlen(album) > max_album_chars
                           ? album + strlen(album) - max_album_chars
                           : album;
  size_t i = 0;
  do {
...
static void make_cache_path(char *path, int size, const char *album,
                            const char *artist, int img_size) {
...
static void clear_default_cover(void) {
...
static void clear_query(cover_query_t *query) {
  if (query->fname) {
    free(query->fname);
  }
  if (query->artist) {
...
static void cache_reset_callback(const char *fname, const char *artist,
  ...
  deadbeef->conf_set_int64("artwork.scaled.cache_reset_time",
                           scaled_cache_reset_time);
...
static cover_callback_t *new_query_callback(artwork_callback cb, void *ud) {
  if (!cb) {
    return NULL;
  }
...
static int strings_match(const char *s1, const char *s2) {
  return s1 == s2 || s1 && s2 && !strcasecmp(s1, s2);
}

static void enqueue_query(const char *fname, const char *artist,
                          const char *album, int img_size,
                          const artwork_callback cb, void *ud) {
  for (cover_query_t *q = queue; q; q = q->next) {
    if (strings_match(artist, q->artist) && strings_match(album, q->album) &&
        q->size == img_size) {
...
static int filter_custom(const struct dirent *f) {
...
static char *vfs_scan_results(struct dirent *entry, const char *container_uri) {
  /* VFS container, double check the match in case scandir didn't implement
   * filtering */
  if (filter_custom(entry)) {
    trace("found cover %s in %s\n", entry->d_name, container_uri);
    char *artwork_path =
        malloc(strlen(container_uri) + 1 + strlen(entry->d_name) + 1);
    if (artwork_path) {
...
static int scan_local_path(char *mask, const char *cache_path,
                           const char *local_path, const char *uri,
                           DB_vfs_t *vfsplug) {
  filter_custom_mask = mask;
  struct dirent **files;
  int (*custom_scandir)(
      const char *, struct dirent ***, int (*)(const struct dirent *),
      int (*)(const struct dirent **, const struct dirent **));
  custom_scandir = vfsplug ? vfsplug->scandir : scandir;
  int files_count = custom_scandir(local_path, &files, filter_custom, NULL);
  if (files_count > 0) {
    char *artwork_path = uri ? vfs_scan_results(files[0], uri)
                             : dir_scan_results(files, files_count, local_path);
...
static void extract_relative_path_from_mask(const char *mask,
...
static int local_image_file(const char *cache_path, const char *local_path,
                            const char *uri, DB_vfs_t *vfsplug) {
  if (!artwork_filemask) {
    return -1;
  }
  ...
  for (char *mask = filemask; mask < filemask_end; mask += strlen(mask) + 1) {
    ...
    if (strrchr(mask, '/')) {
      ...
      char *full_local_path =
          (char *)malloc(strlen(local_path) + strlen(mask) + 1);

      extract_relative_path_from_mask(mask, local_path, trimmed_mask,
                                      full_local_path);
      is_found = !scan_local_path(trimmed_mask, cache_path, full_local_path,
                                  uri, vfsplug);
...
static const uint8_t *id3v2_skip_str(int enc, const uint8_t *ptr,
                                     const uint8_t *end) {
  if (enc == 0 || enc == 3) {
    while (ptr < end && *ptr) {
      ptr++;
...
static const uint8_t *id3v2_artwork(const DB_id3v2_frame_t *f,
                                    int minor_version) {
  if (strcmp(f->id, "APIC")) {
    return NULL;
  }
...
#ifdef USE_METAFLAC
static size_t flac_io_read(void *ptr, size_t size, size_t nmemb,
...
static FLAC__IOCallbacks flac_iocb = {.read = flac_io_read,
                                      .write = NULL,
                                      .seek = flac_io_seek,
                                      .tell = flac_io_tell,
                                      .eof = NULL,
...
static int flac_extract_art(const char *filename, const char *outname) {
  ...
  int res = FLAC__metadata_chain_read_with_callbacks(
      chain, (FLAC__IOHandle)file, flac_iocb);
#if USE_OGG
  if (!res && FLAC__metadata_chain_status(chain) ==
                  FLAC__METADATA_SIMPLE_ITERATOR_STATUS_NOT_A_FLAC_FILE) {
    res = FLAC__metadata_chain_read_ogg_with_callbacks(
        chain, (FLAC__IOHandle)file, flac_iocb);
  }
#endif
...
static int id3_extract_art(const char *fname, const char *outname) {
  int err = -1;
...
static int apev2_extract_art(const char *fname, const char *outname) {
  int err = -1;
...
static int mp4_extract_art(const char *fname, const char *outname) {
  int ret = -1;
...
static int web_lookups(const char *artist, const char *album,
...
static int process_scaled_query(const cover_query_t *query) {
  char unscaled_path[PATH_MAX];
  make_cache_path2(unscaled_path, sizeof(unscaled_path), query->fname,
                   query->album, query->artist, -1);

  struct stat stat_buf;
  if (!stat(unscaled_path, &stat_buf) && S_ISREG(stat_buf.st_mode) &&
      stat_buf.st_size > 0) {
    char scaled_path[PATH_MAX];
    make_cache_path2(scaled_path, sizeof(scaled_path), query->fname,
                     query->album, query->artist, query->size);
    trace("artwork: scaling %s into %s (%d pixels)\n", unscaled_path,
          scaled_path, query->size);
...
static char *vfs_path(const char *fname) {
...
static DB_vfs_t *scandir_plug(const char *vfs_fname) {
  DB_vfs_t **vfsplugs = deadbeef->plug_get_vfs_list();
  for (size_t i = 0; vfsplugs[i]; i++) {
    if (vfsplugs[i]->is_container && vfsplugs[i]->is_container(vfs_fname) &&
        vfsplugs[i]->scandir) {
      return vfsplugs[i];
    }
  }
...
static int path_more_recent(const char *fname, const time_t placeholder_mtime) {
  struct stat stat_buf;
  return !stat(fname, &stat_buf) && stat_buf.st_mtime > placeholder_mtime;
}

static int recheck_missing_artwork(const char *input_fname,
                                   const time_t placeholder_mtime) {
  int res = 0;
  ...
  if (deadbeef->is_local_file(fname)) {
    ...
    /* Recheck artwork if file (track or VFS container) was modified since the
     * last check */
...
static int process_query(const cover_query_t *query) {
  if (!query->fname) {
    return 0;
  }

  char cache_path[PATH_MAX];
  make_cache_path2(cache_path, sizeof(cache_path), query->fname, query->album,
                   query->artist, -1);
  trace("artwork: query cover for %s %s to %s\n", query->album, query->artist,
        cache_path);

  /* Flood control, don't retry missing artwork for an hour unless something
   * changes */
  struct stat placeholder_stat;
  if (!stat(cache_path, &placeholder_stat) &&
      placeholder_stat.st_mtime + 60 * 60 > time(NULL)) {
    int recheck =
        recheck_missing_artwork(query->fname, placeholder_stat.st_mtime);
    if (!recheck) {
      return 0;
    }
  }
  ...
  if (artwork_enable_local && deadbeef->is_local_file(query->fname)) {
    ...
    if (fname_copy) {
      ...
      if (vfs_fname) {
        ...
        if (plugin &&
            !local_image_file(cache_path, vfs_fname, fname_copy, plugin)) {
          free(fname_copy);
          return 1;
        }
      }
      ...
      if (!local_image_file(cache_path,
                            dirname(vfs_fname ? vfs_fname : fname_copy), NULL,
                            NULL)) {
        free(fname_copy);
        return 1;
      }
...
#ifdef USE_VFS_CURL
  /* Web lookups */
  if (artwork_enable_wos && strlen(query->fname) > 3 &&
      !strcasecmp(query->fname + strlen(query->fname) - 3, ".ay")) {
...
static void send_query_callbacks(cover_callback_t *callback, const char *fname,
                                 const char *artist, const char *album) {
  if (callback) {
    trace("Make callback with data %s %s %s to %p (next=%p)\n", fname, artist,
          album, callback->ud, callback->next);
...
static cover_query_t *dequeue_query(void) {
...
static void fetcher_thread(void *none) {
#ifdef __linux__
  ...
  while (!terminate) {
    ...
    pthread_cond_wait((pthread_cond_t *)queue_cond,
    ...
    while (queue) {
      ...
      int cached_art = queue->size == -1 ? process_query(queue)
                                         : process_scaled_query(queue);
      deadbeef->mutex_lock(queue_mutex);
      cover_query_t *query = dequeue_query();
      deadbeef->mutex_unlock(queue_mutex);

      /* Make all the callbacks (and free the chain), with data if a file was
       * written */
      if (cached_art) {
        trace("artwork fetcher: cover art file cached\n");
        send_query_callbacks(query->callback, query->fname, query->artist,
                             query->album);
      } else {
...
static int check_file_age(const char *path, const time_t mtime,
                          const time_t reset_time) {
  if (mtime <= reset_time) {
...
static const char *find_image(const char *path, const time_t reset_time) {
  struct stat stat_buf;
  ...
  if (stat_buf.st_size == 0 &&
      !check_file_age(path, stat_buf.st_mtime, default_reset_time)) {
    trace("artwork: %s invalidated after default artwork reset\n", path);
    return NULL;
  }

  if (!check_file_age(path, stat_buf.st_mtime, reset_time) ||
      stat_buf.st_size == 0) {
...
static char *get_album_art(const char *fname, const char *artist,
                           const char *album, int size,
                           artwork_callback callback, void *user_data) {
  /* Check if the image is already cached */
  char cache_path[PATH_MAX];
  make_cache_path2(cache_path, sizeof(cache_path), fname, album, artist, size);
  deadbeef->mutex_lock(queue_mutex);
  const time_t reset_time =
      size == -1 ? cache_reset_time : scaled_cache_reset_time;
  deadbeef->mutex_unlock(queue_mutex);
  ...
  if (size != -1) {
    char unscaled_path[PATH_MAX];
    make_cache_path2(unscaled_path, sizeof(unscaled_path), fname, album, artist,
                     -1);
...
static void artwork_reset(int fast) {
...
static void get_fetcher_preferences(void) {
  artwork_enable_embedded =
      deadbeef->conf_get_int("artwork.enable_embedded", 1);
  artwork_enable_local =
      deadbeef->conf_get_int("artwork.enable_localfolder", 1);
  if (artwork_enable_local) {
    deadbeef->conf_lock();
    const char *new_artwork_filemask =
        deadbeef->conf_get_str_fast("artwork.filemask", NULL);
...
#endif
  scale_towards_longer =
      deadbeef->conf_get_int("artwork.scale_towards_longer", 1);
  missing_artwork = deadbeef->conf_get_int("artwork.missing_artwork", 1);
  if (missing_artwork == 2) {
    deadbeef->conf_lock();
    const char *new_nocover_path =
        deadbeef->conf_get_str_fast("artwork.nocover_path", NULL);
...
static void insert_cache_reset(void *user_data) {
  ...
  while (*last_callback) {
    if ((*last_callback)->cb == cache_reset_callback) {
      ...
      if ((*last_callback)->ud == &scaled_cache_reset_time &&
          user_data == &cache_reset_time) {
...
static void artwork_configchanged(void) {
  cache_configchanged();
  ...
  if (old_missing_artwork != missing_artwork ||
      old_nocover_path != nocover_path) {
...
static int artwork_message(uint32_t id, uintptr_t ctx, uint32_t p1,
                           uint32_t p2) {
  switch (id) {
...
static int invalidate_playitem_cache(DB_plugin_action_t *action,
...
static DB_plugin_action_t *artwork_get_actions(DB_playItem_t *it) {
...
static int artwork_plugin_stop(void) {
  if (tid) {
...
static int artwork_plugin_start(void) {
  get_fetcher_preferences();
  cache_reset_time = deadbeef->conf_get_int64("artwork.cache_reset_time", 0);
  scaled_cache_reset_time =
      deadbeef->conf_get_int64("artwork.scaled.cache_reset_time", 0);
...
static const char settings_dlg[] =
    ...
    "property \"Cache update period (in hours, 0=never)\" entry "
    "artwork.cache.period 48;\n"
    "property \"Fetch from embedded tags\" checkbox artwork.enable_embedded "
    "1;\n"
    "property box hbox[2] spacing=0 height=-1;\n"
    "property \"Fetch from local folder\" checkbox artwork.enable_localfolder "
    "1;\n"
    ...
    "property \"Fetch from MusicBrainz\" checkbox artwork.enable_musicbrainz "
    "0;\n"
    "property \"Fetch from Albumart.org\" checkbox artwork.enable_albumartorg "
    "0;\n"
    "property \"Fetch from World of Spectrum (AY files only)\" checkbox "
    "artwork.enable_wos 0;\n"
#endif
    "property box vbox[2] spacing=4 border=8;\n"
    "property box hbox[1] height=-1;"
    "property \"When no artwork is found\" select[3] artwork.missing_artwork 1 "
    "\"leave blank\" \"use DeaDBeeF default cover\" \"display custom image\";"
    "property \"Custom image path\" file artwork.nocover_path \"\";\n"
    "property \"Scale artwork towards longer side\" checkbox "
    "artwork.scale_towards_longer 1;\n";
...
static DB_artwork_plugin_t plugin = {
    ...
    .plugin.plugin.descr = "Loads album artwork from embedded tags, local "
                           "directories, or internet services",
...
DB_plugin_t *artwork_load(DB_functions_t *api) {
  deadbeef = api;
