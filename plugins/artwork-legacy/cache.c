...
void cache_lock(void) { deadbeef->mutex_lock(files_mutex); }

void cache_unlock(void) { deadbeef->mutex_unlock(files_mutex); }

int make_cache_root_path(char *path, const size_t size) {
  const char *ddb_cache = deadbeef->get_system_dir(DDB_SYS_DIR_CACHE);

  int ret = snprintf(path, size,
                     (ddb_cache[strlen(ddb_cache) - 1] == '/') ? "%s" : "%s/",
                     ddb_cache);
  if (ret >= 0 && ret > size) {
...
static int filter_scaled_dirs(const struct dirent *f) {
  return !strncasecmp(f->d_name, "covers-", 7);
}

void remove_cache_item(const char *entry_path, const char *subdir_path,
                       const char *subdir_name, const char *entry_name) {
  /* Unlink the expired file, and the artist directory if it is empty */
  cache_lock();
  unlink(entry_path);
  rmdir(subdir_path);

  /* Remove any scaled copies of this file, plus parent directories that are now
   * empty */
  char cache_root_path[PATH_MAX];
  make_cache_root_path(cache_root_path, PATH_MAX);
  struct dirent **scaled_dirs = NULL;
  int scaled_dirs_count =
      scandir(cache_root_path, &scaled_dirs, filter_scaled_dirs, NULL);
  ...
  for (int i = 0; i < scaled_dirs_count; i++) {
    ...
    if (snprintf(scaled_entry_path, PATH_MAX, "%s%s/%s/%s", cache_root_path,
                 scaled_dirs[i]->d_name, subdir_name, entry_name) < PATH_MAX) {
...
static int path_ok(const size_t dir_length, const char *entry) {
  return strcmp(entry, ".") && strcmp(entry, "..") &&
         dir_length + strlen(entry) + 1 < PATH_MAX;
}

static void cache_cleaner_thread(void *none) {
  ...
  while (!terminate) {
    ...
    while (!terminate && covers_dir && (covers_subdir = readdir(covers_dir))) {
      ...
      if (cache_secs > 0 &&
          path_ok(covers_path_length, covers_subdir->d_name)) {
        ...
        while (subdir && (entry = readdir(subdir))) {
          if (path_ok(subdir_path_length, entry->d_name)) {
            ...
            /* Test against the cache expiry time (cache invalidation resets are
             * not handled here) */
            struct stat stat_buf;
            if (!stat(entry_path, &stat_buf)) {
              if (stat_buf.st_mtime <= cache_expiry) {
                trace("%s expired from cache\n", entry_path);
                remove_cache_item(entry_path, subdir_path,
                                  covers_subdir->d_name, entry->d_name);
    ...
    if (cache_expiry_seconds > 0 && !terminate) {
      ...
      trace("Cache cleaner sleeping for %d seconds\n",
            max(60, oldest_mtime - time(NULL) + cache_expiry_seconds));
      pthread_cond_timedwait((pthread_cond_t *)thread_cond,
                             (pthread_mutex_t *)thread_mutex, &wake_time);
    }
    ...
    while (cache_expiry_seconds <= 0 && !terminate) {
      ...
      pthread_cond_wait((pthread_cond_t *)thread_cond,
...
void cache_configchanged(void) {
  const int32_t new_cache_expiry_seconds =
      deadbeef->conf_get_int("artwork.cache.period", 48) * 60 * 60;
