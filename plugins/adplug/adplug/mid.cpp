...
const int CmidPlayer::fnums[] = {0x16b, 0x181, 0x198, 0x1b0, 0x1ca, 0x1e5,
...
CPlayer *CmidPlayer::factory(Copl *newopl) { return new CmidPlayer(newopl); }
...
bool CmidPlayer::load_sierra_ins(const std::string &fname,
  ...
  for (i = 0; i < 2; i++) {
    for (k = 0; k < 48; k++) {
      ...
      myinsbank[l][0] = (ins[9] * 0x80) + (ins[10] * 0x40) + (ins[5] * 0x20) +
                        (ins[11] * 0x10) + ins[1]; // 1=ins5
      myinsbank[l][1] = (ins[22] * 0x80) + (ins[23] * 0x40) + (ins[18] * 0x20) +
...
void CmidPlayer::sierra_next_section() {
  ...
  while (i != 0xff) {
    ...
    //       track[curtrack].spos=getnext(1)+(getnext(1)<<8)+4;		//
    //       dynamite!: doesn't optimize correctly!!
...
bool CmidPlayer::load(const std::string &filename, const CFileProvider &fp) {
  ...
  case 0x84:
    ...
    break;
  default:
    if (size == fp.filesize(f) && s[4] == 'A' && s[5] == 'D')
      good = FILE_OLDLUCAS;
    break;
  }
...
void CmidPlayer::midi_fm_playnote(int voice, int note, int volume) {
  ...
  c = ((freq & 0x300) >> 8) + ((oct & 7) << 2) +
      (adlib_mode == ADLIB_MELODIC || voice < 6 ? (1 << 5) : 0);
...
bool CmidPlayer::update() {
      ...
      if (track[curtrack].on && track[curtrack].iwait == 0 &&
        ...
        case 0x90: /*note on*/
          ...
          if (ch[c].on != 0) {
            ...
            if (vel != 0 && ch[c].inum >= 0 && ch[c].inum < 128) {
              ...
              midi_fm_playnote(on, note + ch[c].nshift,
              ...
              if (adlib_mode == ADLIB_RYTHM && c >= 11) {
                // Still need to turn off the perc instrument before playing it
                // again, as not all songs send a noteoff.
        ...
        case 0xf0:
          ...
          case 0xf7: /*sysex*/
            ...
            if (datalook(pos) == 0x7d && datalook(pos + 1) == 0x10 &&
                datalook(pos + 2) < 16) {
  ...
  if (iwait != 0 && ret == 1) {
    ...
    fwait = 1.0f /
            (((float)iwait / (float)deltas) * ((float)msqtr / (float)1000000));
  } else
...
float CmidPlayer::getrefresh() { return (fwait > 0.01f ? fwait : 0.01f); }
