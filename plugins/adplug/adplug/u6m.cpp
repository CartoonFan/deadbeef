...
CPlayer *Cu6mPlayer::factory(Copl *newopl) { return new Cu6mPlayer(newopl); }
...
Cu6mPlayer::byte_pair Cu6mPlayer::expand_freq_byte(unsigned char freq_byte) {
  const byte_pair freq_table[24] = {
      {0x00, 0x00}, {0x58, 0x01}, {0x82, 0x01}, {0xB0, 0x01}, {0xCC, 0x01},
      {0x03, 0x02}, {0x41, 0x02}, {0x86, 0x02}, {0x00, 0x00}, {0x6A, 0x01},
      {0x96, 0x01}, {0xC7, 0x01}, {0xE4, 0x01}, {0x1E, 0x02}, {0x5F, 0x02},
      {0xA8, 0x02}, {0x00, 0x00}, {0x47, 0x01}, {0x6E, 0x01}, {0x9A, 0x01},
...
void Cu6mPlayer::vibrato(int channel) {
  ...
  freq_word +=
      (vb_current_value[channel] - (vb_double_amplitude[channel] >> 1)) *
