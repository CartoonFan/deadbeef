...
#else
static const Bit8u regbase2modop[22] = {0, 1, 2, 0, 1, 2, 0, 0, 3, 4, 5,
                                        3, 4, 5, 0, 0, 6, 7, 8, 6, 7, 8
                                       };
static const Bit8u regbase2op[22] = {0,  1,  2,  9, 10, 11, 0, 0, 3,  4,  5,
                                     12, 13, 14, 0, 0,  6,  7, 8, 15, 16, 17
                                    };
#endif
...
static Bit32u wavemask[8] = {WAVEPREC - 1,        WAVEPREC - 1,
                             (WAVEPREC >> 1) - 1, (WAVEPREC >> 1) - 1,
                             WAVEPREC - 1,        ((WAVEPREC * 3) >> 2) - 1,
                             WAVEPREC >> 1,       WAVEPREC - 1
                            };

// where the first entry resides
static Bit32u wavestart[8] = {0, WAVEPREC >> 1, 0, WAVEPREC >> 2, 0, 0,
                              0, WAVEPREC >> 3
                             };

// envelope generator function constants
static fltype attackconst[4] = {(fltype)(1 / 2.82624), (fltype)(1 / 2.25280),
                                (fltype)(1 / 1.88416), (fltype)(1 / 1.59744)
                               };
static fltype decrelconst[4] = {(fltype)(1 / 39.28064), (fltype)(1 / 31.41608),
                                (fltype)(1 / 26.17344), (fltype)(1 / 22.44608)
                               };
...
void operator_advance_drums(op_type *op_pt1, Bit32s vib1, op_type *op_pt2,
                            Bit32s vib2, op_type *op_pt3, Bit32s vib3) {
...
void operator_output(op_type *op_pt, Bit32s modulator, Bit32s trem) {
    if (op_pt->op_state != OF_TYPE_OFF) {
        ...
        op_pt->cval =
            (Bit32s)(op_pt->step_amp * op_pt->vol *
...
void OPLChipClass::change_decayrate(Bitu regbase, op_type *op_pt) {
    ...
    if (decayrate) {
        ...
        op_pt->decaymul = (fltype)(
                              pow(FL2, f * pow(FL2, (fltype)(decayrate + (op_pt->toff >> 2)))));
...
void OPLChipClass::change_releaserate(Bitu regbase, op_type *op_pt) {
    ...
    if (releaserate) {
        fltype f = (fltype)(-7.4493 * decrelconst[op_pt->toff & 3] * recipsamp);
        op_pt->releasemul = (fltype)(
                                pow(FL2, f * pow(FL2, (fltype)(releaserate + (op_pt->toff >> 2)))));
...
void OPLChipClass::change_frequency(Bitu chanbase, Bitu regbase,
                                    op_type *op_pt) {
    // frequency
    Bit32u frn = ((((Bit32u)adlibreg[ARC_KON_BNUM + chanbase]) & 3) << 8) +
                 (Bit32u)adlibreg[ARC_FREQ_NUM + chanbase];
    // block number/octave
...
void OPLChipClass::enable_operator(Bitu regbase, op_type *op_pt,
                                   Bit32u act_type) {
...
void OPLChipClass::adlib_init(Bit32u samplerate, Bit32u numchannels,
    ...
    // say 6.4)
    vibtab_add = static_cast<Bit32u>(VIBTAB_SIZE * FIXEDPT_LFO / 8192 * INTFREQU /
                                     int_samplerate);
    vibtab_pos = 0;
    ...
    for (i = 0; i < TREMTAB_SIZE; i++) {
        ...
        fltype trem_val2 = (fltype)((fltype)((Bit32s)(trem_table_int[i] / 4)) *
    ...
    // tremolo at 3.7hz
    tremtab_add = (Bit32u)((fltype)TREMTAB_SIZE * TREM_FREQ * FIXEDPT_LFO /
    ...
    if (!initfirstime) {
        ...
        for (i = 0; i < (WAVEPREC >> 1); i++) {
            wavtable[(i << 1) + WAVEPREC] =
                (Bit16s)(16384 * sin((fltype)((i << 1)) * PI * 2 / WAVEPREC));
            wavtable[(i << 1) + 1 + WAVEPREC] =
                (Bit16s)(16384 * sin((fltype)((i << 1) + 1) * PI * 2 / WAVEPREC));
            wavtable[i] = wavtable[(i << 1) + WAVEPREC];
            // alternative: (zero-less)
            /*			wavtable[(i<<1)  +WAVEPREC]	=
               (Bit16s)(16384*sin((fltype)((i<<2)+1)*PI/WAVEPREC));
                                wavtable[(i<<1)+1+WAVEPREC]	=
               (Bit16s)(16384*sin((fltype)((i<<2)+3)*PI/WAVEPREC)); wavtable[i]
               = wavtable[(i<<1)-1+WAVEPREC]; */
        }
    ...
    case ARC_KON_BNUM: {
        ...
        if (base < 9) {
            ...
            if (val & 32) {
                ...
                enable_operator(modbase, &op[opbase],
                                OP_ACT_NORMAL); // modulator (if 2op)
                enable_operator(modbase + 3, &op[opbase + 9],
...
#if defined(OPLTYPE_IS_OPL3)
            Bits wselbase =
                second_set ? (base + 22) : base; // for easier mapping onto wave_sel[]
            // change waveform
            if (adlibreg[0x105] & 1)
                wave_sel[wselbase] =
...
void OPLChipClass::adlib_getsample(Bit16s *sndptr, Bits numsamples) {
    ...
    for (Bits cursmp = 0; cursmp < samples_to_process; cursmp += endsamples) {
        ...
        Bit32s vib_tshift = ((adlibreg[ARC_PERC_MODE] & 0x40) == 0)
                            ? 1
        ...
        if (adlibreg[ARC_PERC_MODE] & 0x20) {
            ...
            if (op[8].op_state != OF_TYPE_OFF) {
                ...
                if (cptr[0].vibrato) {
                    vibval3 = vibval_var1;
                    for (i = 0; i < endsamples; i++)
                        vibval3[i] = (Bit32s)((vib_lut[i] * cptr[0].freq_high / 8) *
                                              FIXEDPT * VIBFAC);
                } else
            ...
            if ((op[7].op_state != OF_TYPE_OFF) || (op[16].op_state != OF_TYPE_OFF) ||
                ...
                if ((cptr[9].vibrato) && (cptr[9].op_state == OF_TYPE_OFF)) {
                    vibval4 = vibval_var2;
                    for (i = 0; i < endsamples; i++)
                        vibval4[i] = (Bit32s)((vib_lut[i] * cptr[9].freq_high / 8) *
                                              FIXEDPT * VIBFAC);
                } else
