...
CPlayer *Cd00Player::factory(Copl *newopl) { return new Cd00Player(newopl); }
...
void Cd00Player::rewind(int subsong) {
  ...
  for (i = 0; i < 9; i++) {
    if (LE_WORD(&tpoin[subsong].ptr[i])) { // track enabled
      channel[i].speed =
          LE_WORD((unsigned short *)((char *)filedata +
...
void Cd00Player::setvolume(unsigned char chan) {
  ...
  opl->write(0x43 + op, (int)(63 - ((63 - (inst[insnr].data[2] & 63)) / 63.0) *
                                       (63 - channel[chan].vol)) +
                            (inst[insnr].data[2] & 192));
  if (inst[insnr].data[10] & 1)
    opl->write(0x40 + op, (int)(63 - ((63 - channel[chan].modvol) / 63.0) *
                                         (63 - channel[chan].vol)) +
