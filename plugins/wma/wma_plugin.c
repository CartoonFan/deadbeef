...
static DB_fileinfo_t *wmaplug_open(uint32_t hints) {
    wmaplug_info_t *info = calloc(sizeof(wmaplug_info_t), 1);
    return &info->info;
}

static DB_fileinfo_t *wmaplug_open2(uint32_t hints, DB_playItem_t *it) {
...
int get_asf_metadata(DB_FILE *fd, DB_playItem_t *it, asf_waveformatex_t *wfx,
                     int64_t *first_frame_offset);

static int wmaplug_init(DB_fileinfo_t *_info, DB_playItem_t *it) {
    ...
    int res = get_asf_metadata(info->info.file, NULL, &info->wfx,
                               &info->first_frame_offset);
    if (!res) {
        return -1;
    }
...
static void wmaplug_free(DB_fileinfo_t *_info) {
    wmaplug_info_t *info = (wmaplug_info_t *)_info;
    if (info) {
#if USE_FFMPEG
...
static int wmaplug_read(DB_fileinfo_t *_info, char *bytes, int size) {
    wmaplug_info_t *info = (wmaplug_info_t *)_info;
    int samplesize = _info->fmt.channels * _info->fmt.bps / 8;
    if (!info->info.file->vfs->is_streaming() &&
            info->endsample > info->startsample) {
...
new_packet : {
                ...
                res = asf_read_packet(&audiobuf, &audiobufsize, &packetlength, &info->wfx,
                                      info->info.file);
                int endpos = deadbeef->ftell(info->info.file);
                //                trace ("[1] packet pos: %d, packet size: %d (%d), data
                //                size: %d, blockalign: %d, audiobufsize: %d\n", pos,
                //                endpos-pos, info->wfx.packet_size, packetlength,
                //                info->wfx.blockalign, audiobufsize);
            }
            if (res > 0) {
                int nb = audiobufsize / info->wfx.blockalign;
                for (int b = 0; b < nb; b++) {
                    wma_decode_superframe_init(&info->wmadec,
                                               audiobuf + b * info->wfx.blockalign,
                                               info->wfx.blockalign);
                    ...
                    for (int i = 0; i < info->wmadec.nb_frames; i++) {
                        int wmares = wma_decode_superframe_frame(
                                         &info->wmadec, audiobuf + b * info->wfx.blockalign,
                                         info->wfx.blockalign);
                        ...
                        if (wmares < 0) {
                            ...
                            if (wmares * info->wfx.channels * info->wfx.bitspersample / 8 >
                                    sizeof(info->buffer) - info->remaining) {
                            ...
                            info->remaining +=
                                wmares * info->wfx.channels * info->wfx.bitspersample / 8;
                        }
                    }
                }
            }
...
new_packet : {
            ...
            if (res > 0) {
                ...
                for (int i = 0; i < nblocks; i++) {
                    int got_frame_ptr = 0;
                    char *data;
                    int bufsize = wma_decode_superframe(
                                      &info->wmadec, &got_frame_ptr,
                                      audiobuf + i * info->wfx.blockalign, info->wfx.blockalign);
                    trace("got frame ptr: %d, bufsize: %d\n", got_frame_ptr,
                          info->wmadec.nb_samples * 4);
...
static int wmaplug_seek_sample(DB_fileinfo_t *_info, int sample) {
    wmaplug_info_t *info = (wmaplug_info_t *)_info;
...
#else
    int skip_ms;
    int res = asf_seek((int64_t)sample * 1000 / info->wfx.rate, &info->wfx,
                       info->info.file, info->first_frame_offset, &skip_ms);
    if (res < 0) {
        info->skipsamples = 0;
...
static int wmaplug_seek(DB_fileinfo_t *_info, float t) {
    return wmaplug_seek_sample(_info, t * _info->fmt.samplerate);
}

static int wmaplug_read_metadata(DB_playItem_t *it) {
...
static DB_playItem_t *wmaplug_insert(ddb_playlist_t *plt, DB_playItem_t *after,
                                     const char *fname) {
    asf_waveformatex_t wfx;
    ...
    // wfx.channels, wfx.bitspersample, wfx.rate);
    ...
    int64_t i_length =
        wfx.play_duration / 10 * i_count / wfx.numpackets - wfx.preroll * 1000;
    trace("i_length: %lld (%lld / 10 * %lld / %lld - %lld * 1000)\n", i_length,
          wfx.play_duration, i_count, wfx.numpackets, wfx.preroll);
    ...
    DB_playItem_t *cue =
        deadbeef->plt_process_cue(plt, after, it, totalsamples, wfx.rate);
    if (cue) {
        deadbeef->pl_item_unref(it);
        return cue;
    }
...
DB_plugin_t *wma_load(DB_functions_t *api) {
    deadbeef = api;
