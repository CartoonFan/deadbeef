...
static int cdumb_startrenderer(DB_fileinfo_t *_info);

static DB_fileinfo_t *cdumb_open(uint32_t hints) {
  dumb_info_t *info = calloc(sizeof(dumb_info_t), 1);
  info->can_loop = hints & DDB_DECODER_HINT_CAN_LOOP;
  return &info->info;
}

static int cdumb_init(DB_fileinfo_t *_info, DB_playItem_t *it) {
...
static int cdumb_startrenderer(DB_fileinfo_t *_info) {
  ...
  dumb_it_set_global_volume_zero_callback(itsr, &dumb_it_callback_terminate,
                                          NULL);
...
static void cdumb_free(DB_fileinfo_t *_info) {
  trace("cdumb_free %p\n", _info);
  dumb_info_t *info = (dumb_info_t *)_info;
  if (info) {
    if (info->renderer) {
...
static int cdumb_it_callback_loop_forever(void *unused) {
  (void)unused;
  return 0;
}

static int cdumb_read(DB_fileinfo_t *_info, char *bytes, int size) {
  ...
  ret = duh_render(info->renderer, _info->fmt.bps, 0, 1,
                   65536.f / _info->fmt.samplerate, length, bytes);
  _info->readpos += ret / (float)_info->fmt.samplerate;
  trace("cdumb_read %d\n", ret * samplesize);
  return (int)(ret * samplesize);
}

static int cdumb_seek(DB_fileinfo_t *_info, float time) {
  ...
  duh_sigrenderer_generate_samples(info->renderer, 0,
                                   65536.0f / _info->fmt.samplerate, pos, NULL);
  _info->readpos = time;
  return 0;
}
...
static const char *exts[] = {
    "mod", "mdz", "stk", "m15", "fst", "oct", "nt",  "s3m",  "s3z", "stm",
    "stz", "it",  "itz", "xm",  "xmz", "ptm", "ptz", "mtm",  "mtz", "669",
    "psm", "umx", "am",  "j2b", "dsm", "amf", "okt", "okta", "mo3", NULL};

int cdumb_message(uint32_t id, uintptr_t ctx, uint32_t p1, uint32_t p2) {
  switch (id) {
  case DB_EV_CONFIGCHANGED:
    conf_bps = deadbeef->conf_get_int("dumb.8bitoutput", 0) ? 8 : 16;
    conf_samplerate = deadbeef->conf_get_int("synth.samplerate", 44100);
    conf_resampling_quality =
        deadbeef->conf_get_int("dumb.resampling_quality", 4);
...
static const char *convstr(const char *str, int sz, char *out, int out_sz) {
  int i;
  for (i = 0; i < sz; i++) {
    if (str[i] != ' ') {
      break;
...
static void read_metadata_internal(DB_playItem_t *it, DUMB_IT_SIGDATA *itsd) {
  char temp[2048];
  ...
  if (itsd->name[0]) {
    ...
    if (i == tl || !itsd->name[i]) {
      ...
      deadbeef->pl_add_meta(
          it, "title",
          convstr((char *)&itsd->name, sizeof(itsd->name), temp, sizeof(temp)));
    }
  } else {
    deadbeef->pl_add_meta(it, "title", NULL);
  }
  ...
  for (i = 0; i < itsd->n_instruments; i++) {
    ...
    deadbeef->pl_add_meta(it, key,
                          convstr((char *)&itsd->instrument[i].name,
                                  sizeof(itsd->instrument[i].name), temp,
                                  sizeof(temp)));
  }
  for (i = 0; i < itsd->n_samples; i++) {
    ...
    deadbeef->pl_add_meta(it, key,
                          convstr((char *)&itsd->sample[i].name,
                                  sizeof(itsd->sample[i].name), temp,
                                  sizeof(temp)));
  }
...
static int cdumb_read_metadata(DB_playItem_t *it) {
  DUH *duh = NULL;
  int is_it;
  int is_dos;
  int is_ptcompat;
...
static DB_playItem_t *cdumb_insert(ddb_playlist_t *plt, DB_playItem_t *after,
                                   const char *fname) {
...
static long dumb_vfs_getnc(char *ptr, long n, void *f) {
  ddb_dumb_file_t *d = f;
  return deadbeef->fread(ptr, 1, n, d->file);
}

static int dumb_vfs_seek(void *f, long n) {
  ddb_dumb_file_t *d = f;
  return deadbeef->fseek(d->file, d->startoffs + n, SEEK_SET);
}

static long dumb_vfs_get_size(void *f) {
  ddb_dumb_file_t *d = f;
  return deadbeef->fgetlength(d->file) - d->startoffs;
}

static void dumb_vfs_close(void *f) {
  ddb_dumb_file_t *d = f;
  deadbeef->fclose(d->file);
  free(d);
}

static void dumb_register_db_vfs(void) {
...
static const char settings_dlg[] =
    "property \"Resampling quality\" spinbtn[0,5,1] dumb.resampling_quality "
    "4;\n"
    "property \"Internal DUMB volume\" spinbtn[0,128,16] dumb.globalvolume "
    "64;\n"
    "property \"Volume ramping\" select[3] dumb.volume_ramping 0 None \"On/Off "
    "Only\" \"Full\";\n"
...
static DB_decoder_t plugin = {
    ...
    .plugin.copyright =
        ...
        "\n"
        "Uses a fork of DUMB (Dynamic Universal Music Bibliotheque), Version "
        "0.9.3\n"
        "Copyright (C) 2001-2005 Ben Davis, Robert J Ohannessian and Julien "
        "Cugniere\n"
        "Uses code from kode54's foobar2000 plugin, "
...
DB_plugin_t *ddb_dumb_load(DB_functions_t *api) {
  deadbeef = api;
