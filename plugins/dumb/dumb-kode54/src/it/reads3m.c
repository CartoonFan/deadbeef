...
static int it_s3m_read_sample_data(IT_SAMPLE *sample, int ffi,
  ...
  sample->data =
      calloc(datasize * (sample->flags & IT_SAMPLE_16BIT ? 2 : 1), 1);
  if (!sample->data)
    return -1;
...
static DUMB_IT_SIGDATA *it_s3m_load_sigdata(DUMBFILE *f, int *cwtv) {
  ...
  if (dumbfile_error(f) || sigdata->n_orders <= 0 || sigdata->n_samples > 256 ||
  ...
  *cwtv = dumbfile_igetw(f);
  ...
  for (n = 0; n < n_components; n++) {
    ...
    case S3M_COMPONENT_SAMPLE:
      if (it_s3m_read_sample_header(&sigdata->sample[component[n].n], &offset,
                                    &sample_pack[component[n].n], *cwtv, f)) {
        free(buffer);
        free(component);
        _dumb_it_unload_sigdata(sigdata);
        return NULL;
