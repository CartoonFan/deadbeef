...
#define avformat_close_input(ctx)                                              \
  {                                                                            \
...
#define avformat_open_input(ctx, uri, fmt, options)                            \
  av_open_input_file(ctx, uri, NULL, fmt, options)
#endif
...
static DB_fileinfo_t *ffmpeg_open(uint32_t hints) {
  ffmpeg_info_t *info = calloc(sizeof(ffmpeg_info_t), 1);
  return &info->info;
}

// ensure that the buffer can contain entire frame of frame_size bytes per
// channel
static int ensure_buffer(ffmpeg_info_t *info, int frame_size) {
...
static int ffmpeg_init(DB_fileinfo_t *_info, DB_playItem_t *it) {
  // Don't allow playing network streams.
  // Even when ffmpeg has network support, it's causing too many problems this
  // way.
  ...
  if (info->ctx->sample_fmt == AV_SAMPLE_FMT_FLT ||
      info->ctx->sample_fmt == AV_SAMPLE_FMT_FLTP) {
...
static void ffmpeg_free(DB_fileinfo_t *_info) {
  trace("ffmpeg: free\n");
  ffmpeg_info_t *info = (ffmpeg_info_t *)_info;
  if (info) {
    if (info->frame) {
...
static int ffmpeg_read(DB_fileinfo_t *_info, char *bytes, int size) {
  ...
  if (info->endsample >= 0 &&
      info->currentsample + size / samplesize > info->endsample) {
  ...
  while (size > 0) {
    ...
    while (info->left_in_packet > 0 && size > 0) {
      ...
      int got_frame = 0;
      len =
          avcodec_decode_audio4(info->ctx, info->frame, &got_frame, &info->pkt);
      if (len > 0) {
        if (ensure_buffer(info,
                          info->frame->nb_samples * (_info->fmt.bps >> 3))) {
          return -1;
        }
        if (av_sample_fmt_is_planar(info->ctx->sample_fmt)) {
          ...
          for (int c = 0; c < info->ctx->channels; c++) {
            for (int i = 0; i < info->frame->nb_samples; i++) {
              if (_info->fmt.bps == 8) {
                info->buffer[i * info->ctx->channels + c] =
                    ((int8_t *)info->frame->extended_data[c])[i];
                out_size++;
              } else if (_info->fmt.bps == 16) {
                int16_t outsample =
                    ((int16_t *)info->frame->extended_data[c])[i];
                ((int16_t *)info->buffer)[i * info->ctx->channels + c] =
                    outsample;
                out_size += 2;
              } else if (_info->fmt.bps == 24) {
                memcpy(&info->buffer[(i * info->ctx->channels + c) * 3],
          ...
          out_size = info->frame->nb_samples * (_info->fmt.bps >> 3) *
                     _info->fmt.channels;
      ...
      if (ensure_buffer(
              info,
              16384)) { // FIXME: how to get the packet size in old ffmpeg?
        return -1;
      }
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(52, 25, 0)
      len = avcodec_decode_audio3(info->ctx, (int16_t *)info->buffer, &out_size,
                                  &info->pkt);
#else
      len = avcodec_decode_audio2(info->ctx, (int16_t *)info->buffer, &out_size,
                                  info->pkt.data, info->pkt.size);
#endif
#endif
    ...
    for (;;) {
      ...
      if ((ret = av_read_frame(info->fctx, &info->pkt)) < 0) {
        ...
        if (ret == AVERROR_EOF || ret == -1) {
          ...
          if (++errcount > 4) {
            trace("ffmpeg: too many errors in a row (last is %d); interrupting "
                  "stream\n",
                  ret);
            ret = -1;
            break;
      ...
      if (info->pkt.duration > 0) {
        AVRational *time_base =
            &info->fctx->streams[info->stream_id]->time_base;
...
static int ffmpeg_seek_sample(DB_fileinfo_t *_info, int sample) {
  ffmpeg_info_t *info = (ffmpeg_info_t *)_info;
...
static int ffmpeg_seek(DB_fileinfo_t *_info, float time) {
  return ffmpeg_seek_sample(_info, time * _info->fmt.samplerate);
}

static const char *map[] = {"artist",
                            "artist",
                            "title",
                            "title",
                            "album",
...
static int ff_add_disc_meta(DB_playItem_t *it, const char *disc) {
...
static int ff_add_track_meta(DB_playItem_t *it, const char *track) {
...
static int ffmpeg_read_metadata_internal(DB_playItem_t *it,
                                         AVFormatContext *fctx) {
  ...
  for (int m = 0; map[m]; m += 2) {
    ...
    do {
      tag = av_metadata_get(md, map[m], tag,
...
static DB_playItem_t *ffmpeg_insert(ddb_playlist_t *plt, DB_playItem_t *after,
                                    const char *fname) {
  ...
  //    float duration = stream->duration * stream->time_base.num /
  ...
  DB_playItem_t *cue =
      deadbeef->plt_process_cue(plt, after, it, totalsamples, samplerate);
  if (cue) {
...
static void ffmpeg_init_exts(void) {
  deadbeef->conf_lock();
  const char *new_exts =
      deadbeef->conf_get_str_fast("ffmpeg.extensions", DEFAULT_EXTS);
  ...
  if (!use_all_ext) {
    ...
    while ((ifmt = av_iformat_next(ifmt))) {
      ...
      if (ifmt->raw_codec_id > 0 &&
          (ifmt->raw_codec_id < AV_CODEC_ID_FIRST_AUDIO ||
           ifmt->raw_codec_id > AV_CODEC_ID_FIRST_SUBTITLE))
...
static int ffmpeg_message(uint32_t id, uintptr_t ctx, uint32_t p1,
                          uint32_t p2) {
  switch (id) {
...
int ffmpeg_read_metadata(DB_playItem_t *it) {
...
static const char settings_dlg[] =
    "property \"Use all extensions supported by ffmpeg\" checkbox "
    "ffmpeg.enable_all_exts 0;\n"
    "property \"File Extensions (separate with ';')\" entry ffmpeg.extensions "
    "\"" DEFAULT_EXTS "\";\n";
...
DB_plugin_t *ffmpeg_load(DB_functions_t *api) {
  deadbeef = api;
