...
static int pulse_set_spec(ddb_waveformat_t *fmt) {
  ...
  buffer_size = deadbeef->conf_get_int(CONFSTR_PULSE_BUFFERSIZE,
                                       PULSE_DEFAULT_BUFFERSIZE);
  ...
  deadbeef->conf_lock();
  int has_server2 =
      deadbeef->conf_get_str_fast(CONFSTR_PULSE_SERVERADDR, NULL) != NULL;
  ...
  if (!has_server2) {
    ...
    deadbeef->conf_get_str(CONFSTR_PULSE_SERVERADDR, "", server,
                           sizeof(server));
  }

  s = pa_simple_new(*server ? server : NULL, "Deadbeef", PA_STREAM_PLAYBACK,
                    dev, "Music", &ss, &channel_map, attr, &error);
...
static ddb_playback_state_t pulse_get_state(void) { return state; }
...
static const char settings_dlg[] =
    "property \"PulseAudio server (leave empty for default)\" "
    "entry " CONFSTR_PULSE_SERVERADDR " \"\";\n"
    "property \"Preferred buffer size\" entry " CONFSTR_PULSE_BUFFERSIZE
    " " STR(PULSE_DEFAULT_BUFFERSIZE) ";\n";

static DB_output_t plugin = {
    DDB_PLUGIN_SET_API_VERSION.plugin.version_major = 0,
    ...
    .plugin.descr =
        "At the moment of this writing, PulseAudio seems to be very unstable "
        "in many (or most) GNU/Linux distributions.\nIf you experience "
        "problems - please try switching to ALSA or OSS output.\nIf that "
        "doesn't help - please uninstall PulseAudio from your system, and try "
