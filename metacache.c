...
static uint32_t metacache_get_hash_sdbm(const char *str, size_t len) {
  uint32_t h = 0;
  int c;
...
static metacache_str_t *metacache_find_in_bucket(uint32_t h, const char *value,
                                                 size_t len) {
...
const char *metacache_add_value(const char *value, size_t len) {
  //    printf ("n_strings=%d, n_inserts=%d, n_buckets=%d\n", n_strings,
  //    n_inserts, n_buckets);
  uint32_t h = metacache_get_hash_sdbm(value, len);
  metacache_str_t *data =
      metacache_find_in_bucket(h & (HASH_SIZE - 1), value, len);
  n_inserts++;
  if (data) {
    data->refcount++;
    return data->str;
...
void metacache_remove_value(const char *value, size_t valuesize) {
  ...
  while (chain) {
    if (chain->value_length == valuesize &&
        !memcmp(chain->str, value, valuesize)) {
      chain->refcount--;
      if (chain->refcount == 0) {
        if (prev) {
...
const char *metacache_get_value(const char *value, size_t len) {
  uint32_t h = metacache_get_hash_sdbm(value, len);
  metacache_str_t *data =
      metacache_find_in_bucket(h & (HASH_SIZE - 1), value, len);
  n_inserts++;
